...
#ifndef CONFIG_NONE_FT5826_AUTO_UPGRADE
...
/*start--set compatible with ft5826 old firmware, xmwwy 20180417*/
#ifdef  CONFIG_FT5X0X_COMPATIBLE_FT5826S
	if(0x59 == fts_updateinfo_curr.CHIP_ID){
		TPD_DEBUG("---set compatible with ft5826, chip ID = 0x%x---\n",fts_updateinfo_curr.CHIP_ID);
		TOUCH_PACKET_FLAG = 4;
	}else{
		TOUCH_PACKET_FLAG = 6;
	}
	...
	TOUCH_PACKET_FLAG = 6;
#endif
/*end--set compatible with ft5826 old firmware, xmwwy 20180417*/

	for (i = 0; i < cinfo->count; i++) {
		cinfo->p[i] = (data[3 + TOUCH_PACKET_FLAG * i] >> 6) & 0x0003; /* event flag */
		cinfo->id[i] = data[3+TOUCH_PACKET_FLAG*i+2]>>4; 						// touch id

		/*get the X coordinate, 2 bytes*/
		high_byte = data[3 + TOUCH_PACKET_FLAG * i];
		high_byte <<= 8;
		high_byte &= 0x0F00;

		low_byte = data[3 + TOUCH_PACKET_FLAG * i + 1];
		low_byte &= 0x00FF;
		cinfo->x[i] = high_byte | low_byte;

		/*get the Y coordinate, 2 bytes*/
		high_byte = data[3 + TOUCH_PACKET_FLAG * i + 2];
		high_byte <<= 8;
		high_byte &= 0x0F00;

		low_byte = data[3 + TOUCH_PACKET_FLAG * i + 3];
		low_byte &= 0x00FF;
...
//switch hidi2c mode to stdi2c mode if needed, xmwwy
#ifdef CONFIG_SWITCH_FT5826_HIDI2C_TO_STDI2C
...
#ifdef CONFIG_FT5X0X_POWER_HIGH_VOLTAGE
	retval = regulator_set_voltage(tpd->reg, 3000000, 3000000); /* set vtouch 3.0v, xmwwy 2018.03.07*/
#else
