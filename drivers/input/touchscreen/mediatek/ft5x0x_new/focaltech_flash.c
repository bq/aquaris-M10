 ...
 struct fts_Upgrade_Info fts_updateinfo[] =
{
	...
	{0x59,FTS_MAX_POINTS_10,AUTO_CLB_NONEED,30, 50, 0x79, 0x15, 1, 2000},//"FT5826", added by xmwwy,20180317
#else
...
int fts_5x26_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		fts_write_reg(client, 0xfc, FTS_UPGRADE_55);
		msleep(fts_updateinfo_curr.delay_55);
//[begin--set compatible with FT5826S, added by xmwwy,20180317]
#ifdef CONFIG_FT5X0X_COMPATIBLE_FT5826S
...
#endif
#endif
//[end--set compatible with FT5826S, added by xmwwy,20180317]
...
//[set compatible with FT5826S, added by xmwwy,20180317]
#ifdef CONFIG_FT5X0X_COMPATIBLE_FT5826S
		if(fts_updateinfo_curr.CHIP_ID==0x59)
		{
			ui_sz = sizeof(CTPM_FW_02);
		}else{
			ui_sz = sizeof(CTPM_FW);
		}
#else
		...
		if (ui_sz > 2)
		{
			...
			return CTPM_FW_02[0xbff2]; //for ft5826, xmwwy, 20180317
		#endif
		    else
			return CTPM_FW[ui_sz - 2];
		}
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x58))
	{
		...
		if (tpd_dts_data.use_tpd_button) {
            		if(1 == btn_fw_flag)
			{
				...
				//when set use_tpd_button, but chip not support, stop upgrade. added  by xmwwy
				pr_err("---use_tpd_button setting is not compitable with this chip, stop upgrade!---\n");
				return -1;
			}
		}else{
			/*FW upgrade*/
...
#ifdef CONFIG_FT5X0X_COMPATIBLE_FT5826S
		pbt_buf = CTPM_FW_02;
		TPD_DMESG("[FTS] fw_upgrade, CTPM_FW_02---\n");
	   	 /*call the upgrade function*/
    		i_ret = fts_5x26_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_02));
#else
	    	/*FW upgrade*/
    		pbt_buf = CTPM_FW;
	   	 /*call the upgrade function*/
