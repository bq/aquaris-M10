...
extern int goodix_set_i2c_doze_mode(struct goodix_ts_device *dev, int enable);
			...
			struct goodix_ext_module *module)
{
	...
	/*disable doze mode*/
	if (ts_dev->ic_type == IC_TYPE_NORMANDY) {
		ts_info("tools open, disable doze mode");
		ret = goodix_set_i2c_doze_mode(ts_dev, false);
	}
			...
			struct goodix_ext_module *module)
{
	...
	if (ts_dev->ic_type == IC_TYPE_NORMANDY) {
		ts_info("tools close, enable doze mode");

		ts_cmd.cmd_reg = ts_dev->reg.command;
		ts_cmd.length = 3;
		ts_cmd.cmds[0] = 0x11;
		ts_cmd.cmds[1] = 0;
		ts_cmd.cmds[2] = 0xef;
		ts_cmd.initialized = true;
		ret = ts_dev->hw_ops->send_cmd(ts_dev, &ts_cmd);
		if (ret < 0)
			ts_err("send command 0x11 FAILED");

		ret = goodix_set_i2c_doze_mode(ts_dev, true);
		if (ret < 0)
			ts_err("set i2c_doze_mode true FAILED");
	}
