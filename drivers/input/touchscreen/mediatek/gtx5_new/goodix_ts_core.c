		...
		struct goodix_touch_data *touch_data)
{
	...
	if (touch_num >= 1 && touch_data->pen_down) {
		...
		input_report_abs(dev, ABS_MT_POSITION_X, ts_bdata->panel_max_x-touch_data->pen_coords[0].x);
	...
	for (i = 0; i < ts_bdata->panel_max_id * 2; i++) {
		if (touch_num && i == id) { /* this is a valid touch down event */
			...
			input_report_abs(dev, ABS_MT_POSITION_X,  ts_bdata->panel_max_x-coords->x);
...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
    ...
    if (ts_bdata->vdd_rst_name) {
            core_data->vdd_rst = devm_regulator_get(dev,
                     ts_bdata->vdd_rst_name);
            if (IS_ERR_OR_NULL(core_data->vdd_rst)) {
                r = PTR_ERR(core_data->vdd_rst);
                ts_err("Failed to get regulator vdd_rst:%d", r);
                core_data->vdd_rst = NULL;
                return r;
            }
        } else {
            ts_info("vdd_rst name is NULL");
        }
...
int goodix_ts_power_on(struct goodix_ts_core *core_data)
{
    ...
    if (core_data->vdd_rst) {
            r = regulator_enable(core_data->vdd_rst);
            if (!r) {
                usleep_range(13000, 15000);
            } else {
                ts_err("Failed to enable analog power:%d", r);
...
int goodix_ts_power_off(struct goodix_ts_core *core_data)
{
    ...
    if (core_data->vdd_rst) {
            r = regulator_disable(core_data->vdd_rst);
            if (r) {
                ts_err("Failed to disable analog power:%d", r);
                return r;
            }
...
int goodix_ts_esd_init(struct goodix_ts_core *core)
{
	...
	u8 data = 0xaa;
	int r;
    ts_err("xmtdf goodix_ts_esd_init");
