...
int goodix_start_cfg_bin(struct goodix_ts_core *ts_core)
{
    ...
    ts_info("xmtdf goodix_start_cfg_bin");
...
int goodix_cfg_bin_proc(void *data)
{
	...
	if (r < 0)
	{
        ts_info("xmtdf goodix_ts_hw_init failed , r=%d", r);
		goto exit;
	}
	...
	if (r < 0)
	{
        ts_info("xmtdf goodix_ts_input_dev_config failed , r=%d", r);
		goto exit;
	}

    ts_info("xmtdf goodix_ts_irq_setup");
	/* request irq line */
	r = goodix_ts_irq_setup(core_data);
	if (r < 0)
	{
        ts_info("xmtdf goodix_ts_irq_setup failed , r=%d", r);
		goto exit;
	}
...
int goodix_get_reg_and_cfg(struct goodix_ts_device *ts_dev, struct goodix_cfg_bin *cfg_bin)
{
	...
	normal_pkg = NULL;
	high_sense_pkg = NULL;
    ts_info("goodix_get_reg_and_cfg");
