...
/**
 * firmware_packet - firmware packet infomation
...
static int goodix_parse_firmware(struct firmware_data *fw_data)
{
	...
	/*realign fw_pid*/
	for(j = 0; j < 8; j++){
		if(fw_info->fw_pid[j] != 0)
			break;
	}
	ts_info("j:%d", j);
	memcpy(fw_info->fw_pid, &fw_info->fw_pid[j], 8 - j);
	for(;j < 8; j++){
		fw_info->fw_pid[j] = 0;
	}
		...
		const struct firmware_info *fw_info)
{
	...
	if (fw_ver.valid) {
		if (memcmp(fw_ver.pid, fw_info->fw_pid, dev->reg.pid_len)) {
			ts_err("Product ID is not match,pid read from IC:%s, pid from fw:%c,%c,%c,%c,%c,%c",
					fw_ver.pid, fw_info->fw_pid[0], fw_info->fw_pid[1],
					fw_info->fw_pid[2], fw_info->fw_pid[3],
					fw_info->fw_pid[4], fw_info->fw_pid[5]);
			return -EPERM;
		}
		...
		struct firmware_packet *pkt)
{
	...
	r = goodix_isp_wait_stat(dev, ISP_STAT_IDLE);
	if (r < 0) {
		ts_err("Error occured when wait ISP idle");
		return r;
	};
...
static int goodix_update_finish(struct fw_update_ctrl *fwu_ctrl)
{
	int r = 0;
	/*reset ic*/
	r = fwu_ctrl->ts_dev->hw_ops->reset(fwu_ctrl->ts_dev);
	msleep(300);
...
static struct goodix_ext_attribute goodix_fwu_attrs[] = {
	...
	__EXTMOD_ATTR(progres , S_IRUGO, goodix_sysfs_update_progress_show, NULL),
				...
				struct goodix_ext_module *module)
{
	...
	ts_info("fw_name:%s", fwu_ctrl->fw_name);
