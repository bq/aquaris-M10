...
#ifdef CONFIG_CUSTOM_KERNEL_KPD_DHALL
void kpd_hallkey_report(unsigned long pressed)
{
	kpd_print(" DHALL Key generate, pressed=%ld \n", pressed);
	if(!kpd_input_dev){
		kpd_print("KPD input device not ready\n");
		return;
	}
	...
	switch(pressed){
		case 0:{
			input_report_key(kpd_input_dev, KEY_DIRECTION_1, 1);
			input_sync(kpd_input_dev);
			input_report_key(kpd_input_dev, KEY_DIRECTION_1, 0);
			input_sync(kpd_input_dev);
		break;
		}
		case 1:{
			input_report_key(kpd_input_dev, KEY_DIRECTION_2, 1);
			input_sync(kpd_input_dev);
			input_report_key(kpd_input_dev, KEY_DIRECTION_2, 0);
			input_sync(kpd_input_dev);
		break;
		}
		case 2:{
			input_report_key(kpd_input_dev, KEY_DIRECTION_3, 1);
			input_sync(kpd_input_dev);
			input_report_key(kpd_input_dev, KEY_DIRECTION_3, 0);
			input_sync(kpd_input_dev);
		break;
		}
		case 3:{
			input_report_key(kpd_input_dev, KEY_DIRECTION_4, 1);
			input_sync(kpd_input_dev);
			input_report_key(kpd_input_dev, KEY_DIRECTION_4, 0);
			input_sync(kpd_input_dev);
		break;
		}
	default:break;
...
#ifdef CONFIG_POTENTIOMETER_SUPPORT
void kpd_volume_report(int volume_control)
{
	if(!kpd_input_dev){
		kpd_print("KPD input device not ready\n");
		return;
...
#if defined(CONFIG_CUSTOM_KERNEL_KPD_DHALL)
	__set_bit(KEY_DIRECTION_1, kpd_input_dev->keybit);
	__set_bit(KEY_DIRECTION_2, kpd_input_dev->keybit);
	__set_bit(KEY_DIRECTION_3, kpd_input_dev->keybit);
	__set_bit(KEY_DIRECTION_4, kpd_input_dev->keybit);
#endif
