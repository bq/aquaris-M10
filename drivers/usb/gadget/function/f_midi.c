...
static void f_midi_req_put(struct f_midi *midi,
		...
		struct usb_ep *ep)
{
	...
	spin_lock_irqsave(&midi->mreq[i].req_lock, flags);
	list_add_tail(&req->list, head);
	spin_unlock_irqrestore(&midi->mreq[i].req_lock, flags);
}

static struct usb_request *f_midi_req_get(struct f_midi *midi,
		struct list_head *head,
		struct usb_ep *ep)
{
	unsigned long flags;
...
static int f_midi_req_find(struct f_midi *midi,
		struct list_head *head,
		struct usb_request *req,
		struct usb_ep *ep)
{
...
static inline struct usb_request *midi_alloc_ep_req(struct f_midi *midi,
						    struct usb_ep *ep,
						    unsigned length)
{
	struct usb_request *req;
	...
	req = alloc_ep_req(ep, length, length);

	f_midi_req_put(midi, &midi->mreq[i].req_list, req, ep);
...
static void f_midi_free_ep_req(struct f_midi *midi,
		struct usb_ep *ep,
		struct usb_request *req)
{
	int i = 0;
	...
	if (!f_midi_req_find(midi, &midi->mreq[i].req_list, req, ep))
		return;
	pr_err("midi free [%p]\n", req);
	kfree(req->buf);
...
f_midi_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case -ESHUTDOWN:	/* disconnect from host */
		...
		f_midi_free_ep_req(midi, ep, req);
		return;
...
static void f_midi_free_ep(struct f_midi *midi,
					struct usb_ep *ep)
{
	struct usb_request *req;
	int i = 0;
	...
	while ((req = f_midi_req_get(midi, &midi->mreq[i].req_list, ep)) && req) {
		pr_err("midi free1 [%p]\n", req);
...
static int f_midi_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (midi->out_ep->driver_data)
		f_midi_free_ep(midi, midi->out_ep);
	...
	for (i = 0; i < midi->qlen && err == 0; i++) {
		struct usb_request *req =
			midi_alloc_ep_req(midi, midi->out_ep,
...
static void f_midi_disable(struct usb_function *f)
{
	 ...
	 */
	f_midi_free_ep(midi, midi->in_ep);
	f_midi_free_ep(midi, midi->out_ep);
}
...
static void f_midi_transmit(struct f_midi *midi, struct usb_request *req)
{
	...
	if (!req)
		req = midi_alloc_ep_req(midi, ep, midi->buflen);
...
static struct usb_function *f_midi_alloc(struct usb_function_instance *fi)
{
	...
	/* 0 is out lock, 1 is in lock*/
	spin_lock_init(&midi->mreq[0].req_lock);
	spin_lock_init(&midi->mreq[1].req_lock);
	INIT_LIST_HEAD(&midi->mreq[0].req_list);
	INIT_LIST_HEAD(&midi->mreq[1].req_list);
...
void f_midi_unbind_config(struct usb_configuration *c)
{
	int i;
	...
	f_midi_disable(&gmidi->func);

	if (gmidi->card)
		snd_card_free(gmidi->card);
	gmidi->card = NULL;

	i = gmidi->in_ports;
	for (--i; i >= 0; i--)
		kfree(gmidi->in_port[i]);
	kfree(gmidi->id);
	kfree(gmidi);
	gmidi = NULL;
}
