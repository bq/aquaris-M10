...
/* For fake cpufreq_hotplug.c functions */
void __attribute__ ((weak)) hp_limited_cpu_num(int num)
{
}

void __attribute__ ((weak)) dbs_freq_thermal_limited(unsigned int limited, unsigned int freq)
{
}
...
int g_ramp_down_count = 0;

bool mt_cpufreq_debug = false;	/* /proc/cpufreq/cpufreq_debug */
bool mt_cpufreq_ready = false;
bool mt_cpufreq_pause = false;	/* /proc/cpufreq/cpufreq_state */
bool mt_cpufreq_ptpod_disable = false;
int	g_dvfs_disable_count = 0;
...
unsigned int g_cpufreq_get_ptp_level = 0;

/* for Thermal */
unsigned int g_thermal_protect_limited_power = 0;
unsigned int g_cpu_power_table_num = 0;
struct mtk_cpu_power_info *mt_cpu_power = NULL;

/* ptpod */
unsigned int g_max_freq_by_ptp = DVFS_F0;	/* default 1.3GHz */

bool mt_cpufreq_freq_table_allocated = false;
struct mt_cpu_freq_info *mt_cpu_freqs = NULL;
...
bool mt_cpufreq_limit_max_freq_early_suspend = false;
...
static unsigned int mt_cpufreq_limit_max_freq_by_early_suspend(void)
{
	...
	pr_info("mt_cpufreq limited max freq by early suspend %d\n", DVFS_F2);
...
unsigned int mt_cpufreq_voltage_set_by_ptpod(unsigned int pmic_volt[], unsigned int array_size)
{
	...
	unsigned long flags;
	#if 0
	unsigned int PMIC_WRAP_DVFS_WDATA_array[8] = {PMIC_WRAP_DVFS_WDATA0, PMIC_WRAP_DVFS_WDATA1,
			PMIC_WRAP_DVFS_WDATA2, PMIC_WRAP_DVFS_WDATA3, PMIC_WRAP_DVFS_WDATA4,
			PMIC_WRAP_DVFS_WDATA5, PMIC_WRAP_DVFS_WDATA6, PMIC_WRAP_DVFS_WDATA7};
	#else
	void __iomem *PMIC_WRAP_DVFS_WDATA_array[8] = {PMIC_WRAP_DVFS_WDATA0, PMIC_WRAP_DVFS_WDATA1,
			PMIC_WRAP_DVFS_WDATA2, PMIC_WRAP_DVFS_WDATA3, PMIC_WRAP_DVFS_WDATA4,
			PMIC_WRAP_DVFS_WDATA5, PMIC_WRAP_DVFS_WDATA6, PMIC_WRAP_DVFS_WDATA7};
	#endif

	if (array_size > (sizeof(mt_cpufreq_pmic_volt)/4))
		pr_info("mt_cpufreq_voltage_set_by_ptpod: ERROR!array_size is invalid, array_size = %u\n", array_size);

	cpufreq_lock(flags);
	/* spin_lock_irqsave(&mt_cpufreq_lock, flags); */

	/* for requlator framework */
	if ((g_cpufreq_get_ptp_level >= 0) && (g_cpufreq_get_ptp_level <= 4)) {
		if (mt_cpu_freqs_num != 5) {
			cpufreq_err("%s[%d], g_cpufreq_get_ptp_level = %u, mt_cpu_freqs_num == %u\n",
				__func__, __LINE__, g_cpufreq_get_ptp_level, mt_cpu_freqs_num);
			BUG_ON(1);
		}
	...
	for (i = 0; i < mt_cpu_freqs_num; i++)
		mt_cpu_freqs[i].cpufreq_volt = PMIC_VAL_TO_VOLT(pmic_volt[i]);
	*/
	...
	for (i = 0; i < array_size; i++) {
		...
		pr_info("mt_cpufreq_pmic_volt[%d] = %x\n", i, mt_cpufreq_pmic_volt[i]);
	}
	...
	pr_info("mt_cpufreq_voltage_set_by_ptpod: Set voltage directly by PTP-OD request!\n");
...
void mt_cpufreq_return_default_DVS_by_ptpod(void)
{
	if ((g_cpufreq_get_ptp_level >= 0) && (g_cpufreq_get_ptp_level <= 4)) {
		...
		mt_cpu_freqs[0].cpufreq_volt = PMIC_VAL_TO_VOLT(mt_cpufreq_pmic_volt[0]);
		mt_cpu_freqs[1].cpufreq_volt = PMIC_VAL_TO_VOLT(mt_cpufreq_pmic_volt[1]);
		mt_cpu_freqs[2].cpufreq_volt = PMIC_VAL_TO_VOLT(mt_cpufreq_pmic_volt[2]);
		mt_cpu_freqs[3].cpufreq_volt = PMIC_VAL_TO_VOLT(mt_cpufreq_pmic_volt[3]);
	} else if ((g_cpufreq_get_ptp_level >= 5) && (g_cpufreq_get_ptp_level <= 6)) {
	...
	pr_info("mt_cpufreq return default DVS by ptpod\n");
}
...
unsigned int mt_cpufreq_max_frequency_by_DVS(unsigned int num)
{
	...
	if (num == voltage_change_num) {
		pr_info("PTPOD0:num = %d, frequency= %d\n", num, mt_cpu_freqs[0].cpufreq_khz);
	...
	for (i = 1; i < mt_cpu_freqs_num; i++) {
		...
		if (num == voltage_change_num) {
			pr_info("PTPOD1:num = %d, frequency= %d\n", num, mt_cpu_freqs[i].cpufreq_khz);
			return mt_cpu_freqs[i].cpufreq_khz;
		}
	}

	pr_info("PTPOD2:num = %d, NOT found! return 0!\n", num);
	return 0;
}
EXPORT_SYMBOL(mt_cpufreq_max_frequency_by_DVS);

/************************************************
* DVFS check API for PTPOD
*************************************************/
int is_cpufreq_disabled_by_ptpod(void)
{
	return mt_cpufreq_ptpod_disable;
}
EXPORT_SYMBOL(is_cpufreq_disabled_by_ptpod);
...
void mt_cpufreq_enable_by_ptpod(void)
{
	...
	regulator_set_mode(reg_vproc, REGULATOR_MODE_NORMAL);
	if (regulator_get_mode(reg_vproc) != REGULATOR_MODE_NORMAL)
		cpufreq_err("Vproc should be REGULATOR_MODE_NORMAL(%d), but mode = %d\n",
						REGULATOR_MODE_NORMAL, regulator_get_mode(reg_vproc));

	pr_info("mt_cpufreq enabled by ptpod\n");
}
...
unsigned int mt_cpufreq_disable_by_ptpod(void)
{
	...
	regulator_set_mode(reg_vproc, REGULATOR_MODE_FAST);
	if (regulator_get_mode(reg_vproc) != REGULATOR_MODE_FAST)
		cpufreq_err("Vproc should be REGULATOR_MODE_FAST(%d), but mode = %d\n",
						REGULATOR_MODE_FAST, regulator_get_mode(reg_vproc));
	...
	pr_info("mt_cpufreq disabled by ptpod, limited freq. at %d\n", DVFS_F2);
...
/************************************************
* DVFS get cpu voltage API for PTPOD
*************************************************/
unsigned int mt_cpufreq_cur_vproc(void)
{
	return regulator_get_voltage(reg_vproc) / 10; /* mv * 100 == 10 uV */
}
			     ...
			     unsigned int relation)
{
    ...
    *************************************************/
	...
	if ((num_possible_cpus() == 4) && (num_online_cpus() < num_possible_cpus())
	    && (freqs.new > DVFS_F2)) {
		if (isp_power_on_off() == true || vdec_power_on_off() == true) {
			cpufreq_dbg
			    ("Limited frequency, because num_online_cpus() = %d, freqs.new = %d\n",
...
static int __init mt_cpufreq_pdrv_init(void)
{
	...
	struct deviceNodeName {
		...
		{ &infracfg_ao_base, "mediatek,mt8127-infracfg"},
		{ &apmixed_base, "mediatek,mt8127-apmixedsys"},
		{ &clk_cksys_base, "mediatek,mt8127-topckgen"},
		{ &pwrap_base, "mediatek,mt8127-pwrap"},
	};
