...
#define INFRA_PDN_STA		INFRA_REG(0x048)
#define PERI_PDN_STA0		PERI_REG(0x018)
#define PERI_PDN_STA1		PERI_REG(0x01c)
#define AUDIO_TOP_CON0		AUDIO_REG(0x0)
...
static void get_all_clock_state(u32 clks[NR_GRPS])
{
	...
	if (is_valid_reg(perisys_base)) {
		clks[CG_PERI0] = ~idle_readl(PERI_PDN_STA0);
		clks[CG_PERI1] = ~idle_readl(PERI_PDN_STA1);
	}

	if (is_valid_reg(infrasys_base))
		clks[CG_INFRA] = ~idle_readl(INFRA_PDN_STA);

	if (is_valid_reg(topckgen_base))
		clks[CG_TOPCK] = ~idle_readl(CLK_CFG_4);

	if (sys_is_on(SYS_DIS) && is_valid_reg(mmsys_base)) {
		clks[CG_DISP0] = ~idle_readl(DISP_CG_CON0);
		clks[CG_DISP1] = ~idle_readl(DISP_CG_CON1);
	}

	if (sys_is_on(SYS_ISP) && is_valid_reg(imgsys_base))
		clks[CG_IMAGE] = ~idle_readl(IMG_CG_CON);

	if (sys_is_on(SYS_MFG) && is_valid_reg(mfgsys_base))
		clks[CG_MFG] = ~idle_readl(MFG_CG_CON);
...
static int only_one_cpu_online(void)
{
	...
	return (ps == CA7_CPU0_PWR_STA_MASK) || (ps == CA7_CPU1_PWR_STA_MASK) ||
		(ps == CA7_CPU2_PWR_STA_MASK) || (ps == CA7_CPU3_PWR_STA_MASK);
#else /* !CONFIG_SMP */
	return true;
#endif /* CONFIG_SMP */
}
...
static u32 clk_cfg_3;

static void backup_audintbus_26MHz(void)
{
	if (clk_cfg_3 || !is_valid_reg(topckgen_base))
		return;
...
{
	get_base_from_node("mediatek,mt8127-topckgen", &topckgen_base, 0);
	get_base_from_node("mediatek,mt8127-infracfg", &infrasys_base, 0);
	get_base_from_node("mediatek,mt8127-pericfg", &perisys_base, 0);
	get_base_from_node("mediatek,mt8127-soc-machine", &audiosys_base, 0);
	get_base_from_node("mediatek,mt8127-mfg", &mfgsys_base, 0);
	get_base_from_node("mediatek,mt8127-mmsys", &mmsys_base, 0);
	get_base_from_node("mediatek,mt8127-imgsys", &imgsys_base, 0);
	get_base_from_node("mediatek,mt8127-vdecsys", &vdecsys_base, 0);
	get_base_from_node("mediatek,mt8127-scpsys", &scpsys_base, 0);
}
