...
static int ptp_volt_thread_handler(void *data)
{
	...
	do {
		...
		case PTP_CTRL_BIG:
			...
			ptp_status_temp = PTP_CPU_BIG_IS_SET_VOLT;
			break;
		case PTP_CTRL_GPU:
			aee_rr_rec_ptp_status(aee_rr_curr_ptp_status() |
				(1 << PTP_GPU_IS_SET_VOLT));
			ptp_status_temp = PTP_GPU_IS_SET_VOLT;
			break;
		default:
...
static void ptp_set_ptp_volt(struct ptp_det *det)
{
	...
	case PTP_CTRL_BIG:
		...
		temp_long = (unsigned long long) (cur_temp / 1000);
		if (temp_long != 0) {
			aee_rr_rec_ptp_temp(temp_long << (8 * PTP_CPU_BIG_IS_SET_VOLT) |
				(temp_cur & ~((unsigned long long)0xFF <<
					(8 * PTP_CPU_BIG_IS_SET_VOLT))));
		}
		...
		for (i = 0; i < NR_FREQ; i++) {
			aee_rr_rec_ptp_cpu_big_volt(
				((unsigned long long)(det->volt_tbl_pmic[i])
					<< (8 * i)) | (aee_rr_curr_ptp_cpu_big_volt()
						& ~((unsigned long long)(0xFF) << (8 * i))));
		}
		break;
	case PTP_CTRL_GPU:
		...
		temp_long = (unsigned long long) (cur_temp / 1000);
		if (temp_long != 0) {
			aee_rr_rec_ptp_temp(temp_long << (8 * PTP_GPU_IS_SET_VOLT) |
				(temp_cur & ~((unsigned long long)0xFF <<
					(8 * PTP_GPU_IS_SET_VOLT))));
		}
...
void ptp_init01(void)
{
	...
	for_each_det_ctrl(det, ctrl) {
		...
		if (atomic_read(&ctrl->in_init)) {
			...
			ptp_debug("@%s():%d, wait_for_completion(%s) in\n",
						__func__, __LINE__, det->name);
			wait_for_completion(&ctrl->init_done);
			ptp_debug("@%s():%d, wait_for_completion(%s) out\n",
						__func__, __LINE__, det->name);
		}
	}
...
void get_devinfo(struct ptp_devinfo *p)
{
...
#if 1				/* FIXME: ned get_devinfo_with_index() ready */
	{
		...
		M_HW_RES[4] = get_devinfo_with_index(15);	/* M_HW_RES4 */
		M_HW_RES[5] = get_devinfo_with_index(16);	/* M_HW_RES5 */

		/* search table (temp) */
		#ifdef DEVINFO
		for (i = 0; i < ARRAY_SIZE(devinfo); i++) {
			if (M_HW_RES[4] == devinfo[i].M_HW_RES4 &&
				M_HW_RES[5] == devinfo[i].M_HW_RES5) {
				ptp_debug("get array\n");
				M_HW_RES[0] = devinfo[i].M_HW_RES0;
				M_HW_RES[1] = devinfo[i].M_HW_RES1;
				M_HW_RES[2] = get_devinfo_with_index(9);
				M_HW_RES[3] = devinfo[i].M_HW_RES3;
				M_HW_RES[6] = devinfo[i].M_HW_RES6;
				M_HW_RES[7] = devinfo[i].M_HW_RES7;
				M_HW_RES[8] = devinfo[i].M_HW_RES8;
				M_HW_RES[9] = devinfo[i].M_HW_RES9;
		...
		if (i >= ARRAY_SIZE(devinfo)) {
			ptp_debug("get efuse\n");
			M_HW_RES[0] = get_devinfo_with_index(7);
			M_HW_RES[1] = get_devinfo_with_index(8);
			M_HW_RES[2] = get_devinfo_with_index(9);
			M_HW_RES[3] = get_devinfo_with_index(14);
			M_HW_RES[4] = get_devinfo_with_index(15);
			M_HW_RES[5] = get_devinfo_with_index(16);
			M_HW_RES[6] = get_devinfo_with_index(17);
			M_HW_RES[7] = get_devinfo_with_index(18);
			M_HW_RES[8] = get_devinfo_with_index(19);
			M_HW_RES[9] = get_devinfo_with_index(21);
		}
		#else
		ptp_debug("get efuse\n");
		M_HW_RES[0] = get_devinfo_with_index(7);
		...
		if (0 == ptp_devinfo.PTPINITEN) {
			ptp_error("hard code\n");
			M_HW_RES[0] = 0x0F0F0F0F;
			M_HW_RES[1] = 0xBABAB9B6;
			M_HW_RES[2] = 0x00000000;
			M_HW_RES[3] = 0x00000000;
			M_HW_RES[4] = 0x00000003;
			M_HW_RES[5] = 0x00000000;
			M_HW_RES[6] = 0x00000000;
			M_HW_RES[7] = 0x32271E37;
			M_HW_RES[8] = 0x252B1912;
			M_HW_RES[9] = 0x434C854B;
		}
		#endif
#ifdef CONFIG_MTK_RAM_CONSOLE
		aee_rr_rec_ptp_60((unsigned int)M_HW_RES[0]);
		aee_rr_rec_ptp_64((unsigned int)M_HW_RES[1]);
		aee_rr_rec_ptp_68((unsigned int)M_HW_RES[2]);
		aee_rr_rec_ptp_6C((unsigned int)M_HW_RES[3]);
		aee_rr_rec_ptp_78((unsigned int)M_HW_RES[4]);
		aee_rr_rec_ptp_7C((unsigned int)M_HW_RES[5]);
		aee_rr_rec_ptp_80((unsigned int)M_HW_RES[6]);
		aee_rr_rec_ptp_84((unsigned int)M_HW_RES[7]);
		aee_rr_rec_ptp_88((unsigned int)M_HW_RES[8]);
		aee_rr_rec_ptp_8C((unsigned int)M_HW_RES[9]);
#endif
...
static void _mt_ptp_aee_init(void)
{
	...
	aee_rr_rec_ptp_gpu_volt(0x0);
	aee_rr_rec_ptp_cpu_little_volt(0x0);
	aee_rr_rec_ptp_cpu_big_volt(0x0);
...
static int ptp_probe(struct platform_device *pdev)
{
	...
	ptp_debug("PTPOD starts to initialize\n");
	...
	ptp_debug("clk_axi_sel = %luHz\n", __clk_get_rate(clk_axi_sel));
	ptp_debug("clk_therm = %luHz\n", __clk_get_rate(clk_therm));
	...
	ptp_debug("PTPOD initiailization done\n");
...
void mt_ptp_opp_status(enum ptp_det_id id, unsigned int *temp,
						unsigned int *volt)
{
	struct ptp_det *det = id_to_ptp_det(id);
	int i = 0;
		...
		const char __user *buffer, size_t count, loff_t *pos)
{
	...
	if (!kstrtoint(buf, 10, &offset)) {
		...
		ret = -EINVAL;
		ptp_error("bad argument_1!! argument should be positive integer\n");
	}
...
void ptp_efuse_calibration(struct ptp_devinfo *ptp_dev)
{
	...
	if (mt_get_chip_sw_ver() != CHIP_SW_VER_04) {
		ptp_debug("Not E4 IC, No need to do ptp-efuse-calibration\n");
		return;
	}

	if (ptp_dev->CA15L_DCBDET >= 128) {
		ptp_debug("CA15L_DCBDET = %d which is correct\n", ptp_dev->CA15L_DCBDET);
		return;
	}

	ptp_debug("origianl CA15L_DCBDET = %d\n", ptp_dev->CA15L_DCBDET);
...
static int __init ptp_init(void)
{
	...
	ptp_debug("ptpod_irq_number = 0x%x\n", ptpod_irq_number);
	ptp_debug("ptpod_base = 0x%16p\n", (void *)ptpod_base);
