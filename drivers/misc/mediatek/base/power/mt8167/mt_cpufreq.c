...
static unsigned int _mt_cpufreq_get_cpu_level(void)
{
...
#if 1 /* mt8167, use efuse to decide segment, no HW bonding */
	...
	case 8:
			#if 0 /* mt8167-TBD turn on turbo mode */
...
#ifdef CONFIG_OF
	{
		...
		cpufreq_info("CPU clock-frequency from DT = %d MHz\n", cpu_speed);
			...
			/* mt8167 limit by both efuse HW bonding & DTS */
		if (cpu_speed >= 1500)
			lv = CPU_LEVEL_0;   /* 1.5G */
...
#else
	p_leakage = 155; /* mt8167-TBD Define power */
#endif
...
static int setup_power_table(struct mt_cpu_dvfs *p)
{
	#if 0 /* mt8167-TBD pwr_tbl_cgf NOT USED */
...
static int __init _mt_cpufreq_pdrv_init(void)
{
	...
	apmix_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8167-apmixedsys");
	if (apmix_node) {
		/* Setup IO addresses */
	...
	infra_ao_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8167-infracfg");
	if (infra_ao_node) {
		/* Setup IO addresses */
	...
	topckgen_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8167-topckgen");
	if (topckgen_node) {
		/* Setup IO addresses */
