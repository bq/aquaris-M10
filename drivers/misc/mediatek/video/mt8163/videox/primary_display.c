...
static int _config_ovl_input(disp_session_input_config *session_input)
{
	...
	if (session_input->config_layer_num == 0 || session_input->config_layer_num > OVL_LAYER_NUM) {
		DISPERR("_config_ovl_input, config_layer_num invalid = %d!\n",
 ...
 /*=== create new data_config for ovl input ===*/
	...
	for (i = 0; i < session_input->config_layer_num; i++) {
		...
		if ((layer < 0) || (layer > OVL_LAYER_NUM - 1))
			continue;
...
#ifdef MTK_DISP_IDLE_LP
	unsigned int skipIdleDetect_tmp = gSkipIdleDetect;
...
out:
	...
	gSkipIdleDetect = skipIdleDetect_tmp;
