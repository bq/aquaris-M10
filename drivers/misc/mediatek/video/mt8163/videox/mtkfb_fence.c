				...
				)
{
	...
	list_for_each_entry(buf, &layer_info->buf_list, list) {
		if (buf->idx == idx
			...
			if (isSecure) {
				TZ_RESULT res =
					KREE_ReferenceSecurechunkmem(primary_display_secure_memory_session_handle(), secure_handle);
		...
		list_for_each_entry_safe(buf, n, &layer_info->buf_list, list) {
			if (buf->idx <= fence) {
				...
				if (buf->isScure && buf->secure_handle) {
					TZ_RESULT res = KREE_UnreferenceSecurechunkmem(primary_display_secure_memory_session_handle(), buf->secure_handle);
					if (res != TZ_RESULT_SUCCESS)
