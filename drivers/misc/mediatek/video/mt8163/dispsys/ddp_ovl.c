				...
				DISP_BUFFER_TYPE sec, unsigned int is_engine_sec, void *handle)
{
...
#ifdef CONFIG_MTK_LCM_PHYSICAL_ROTATION_HW
	if (module == DISP_MODULE_OVL1 && (ovl1_in_hdmi_path(module) || ovl2mem_is_alive()))
		value = value | (REG_FLD_VAL((L_CON_FLD_VIRTICAL_FLIP), 0) |
			 REG_FLD_VAL((L_CON_FLD_HORI_FLIP), 0));
	else
	...
	if (module == DISP_MODULE_OVL1 && (ovl1_in_hdmi_path(module) || ovl2mem_is_alive())) {
		...
		if (bg_h == 0 && module == DISP_MODULE_OVL1 &&
...
static bool ovl1_in_hdmi_path(DISP_MODULE_ENUM module)
{
	...
	if (module == DISP_MODULE_OVL1 && is_hdmi_active() &&
			(DISP_REG_GET(DISP_REG_OVL_DATAPATH_CON) & 0x4) == 0 && !(get_boot_mode() == FACTORY_BOOT))
		in = true;
#endif
	return in;
}
