...
int rdma_config(DISP_MODULE_ENUM module, enum RDMA_MODE mode,
	...
	DISP_BUFFER_TYPE sec, unsigned int buf_offset, void *handle)	/* ourput setting */
{
...
static int do_rdma_config_l(DISP_MODULE_ENUM module, disp_ddp_path_config *pConfig, void *handle)
{
	...
	rdma_config(module, mode, (mode == RDMA_MODE_DIRECT_LINK) ? 0 : r_config->address,	/* address */
		    ...
		    width, height, lcm_param->dsi.ufoe_enable, r_config->security, r_config->buf_offset, handle);
