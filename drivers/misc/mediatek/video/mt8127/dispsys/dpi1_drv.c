...
#ifdef OUTREG32
#undef OUTREG32
#define OUTREG32(x, y) mt_reg_sync_writel(y, (volatile unsigned int *)(x))
...
static PDPI_REGS DPI1_REG_VA;
static PDSI_PHY_REGS DSI_PHY_REG_DPI1;
static unsigned long APMIXEDSYS_REG_BASE_VA;
static unsigned long TVDPLL_CON0;
static unsigned long TVDPLL_CON1;
static unsigned long TVDPLL_PWR_CON0;
...
const uint32_t BACKUP_DPI1_REG_OFFSETS[] = {
...
static void _BackupDPIRegisters(void)
{
	...
	for (i = 0; i < ARY_SIZE(BACKUP_DPI1_REG_OFFSETS); ++i) {
		OUTREG32(REG_ADDR(reg, BACKUP_DPI1_REG_OFFSETS[i]),
			 AS_UINT32(REG_ADDR(DPI1_REG_VA, BACKUP_DPI1_REG_OFFSETS[i])));
	}
}
...
static void _RestoreDPIRegisters(void)
{
	...
	for (i = 0; i < ARY_SIZE(BACKUP_DPI1_REG_OFFSETS); ++i) {
		OUTREG32(REG_ADDR(DPI1_REG_VA, BACKUP_DPI1_REG_OFFSETS[i]),
...
{
	DPI_REG_INTERRUPT enInt = DPI1_REG_VA->INT_ENABLE;

	enInt.VSYNC = 0;
	OUTREG32(&DPI1_REG_VA->INT_ENABLE, AS_UINT32(&enInt));
}
...
{
	static int counter;
	DPI_REG_INTERRUPT status = DPI1_REG_VA->INT_STATUS;
...
/* hrtimer_try_to_cancel(&hrtimer_vsync_dpi); */
	...
	OUTREG32(&DPI1_REG_VA->INT_STATUS, 0);
	return IRQ_HANDLED;
}
#endif
...
void DPI1_InitRegbase(void)
{
	...
	DPI1_REG_VA = (PDPI_REGS) DDP_REG_BASE_DPI1;
	DSI_PHY_REG_DPI1 = (PDSI_PHY_REGS) (DDP_REG_BASE_MIPI + 0x800);
	/* printk("DPI1_REG_VA is %x\n",(unsigned int) DPI1_REG_VA); */

	np = of_find_compatible_node(NULL, NULL, "mediatek,mt8127-apmixedsys");
	if (np != NULL) {
		of_property_read_u32_index(np, "reg", 0, &reg_value);
		APMIXEDSYS_REG_BASE_VA = (unsigned long)of_iomap(np, 0);
		pr_info("DT DPI1 PLL_Ref| reg base: 0x%x --> map:0x%lx\n",
			    reg_value, APMIXEDSYS_REG_BASE_VA);
		TVDPLL_CON0 = APMIXEDSYS_REG_BASE_VA + 0x0260;
		TVDPLL_CON1 = APMIXEDSYS_REG_BASE_VA + 0x0264;
		TVDPLL_PWR_CON0 = APMIXEDSYS_REG_BASE_VA + 0x026C;
	} else
...
DPI_STATUS DPI1_Init(bool isDpiPoweredOn)
{
	...
	DPI_REG_EN en = DPI1_REG_VA->DPI_EN;
	...
	OUTREG32(&DPI1_REG_VA->DPI_EN, AS_UINT32(&en));
	OUTREG32(&DPI1_REG_VA->Y_LIMIT, 0xfff0000);
	OUTREG32(&DPI1_REG_VA->C_LIMIT, 0xfff0000);

#if 0
	OUTREG32(DPI1_REG_VA + 0x64, 0x400);	/*  */
	OUTREG32(DPI1_REG_VA + 0x6C, 0x400);	/*  */
	OUTREG32(DPI1_REG_VA + 0x74, 0x400);	/*  */
	OUTREG32(DPI1_REG_VA + 0x8C, 0x0FFF0000);	/*  */
	OUTREG32(DPI1_REG_VA + 0x90, 0x0FFF0000);	/*  */
	MASKREG32(DISPSYS_BASE + 0x60, 0x1, 0x1);	/* [1]: DPI0_I2X_EN */
	/* 0: DPI0 IO is single edge mode */
	/* 1: DPI0 IO is dual edge mode */
#endif

	MASKREG32(DDP_REG_BASE_MMSYS_CONFIG + 0x60, 0xb03, 0xb03);	/* DISPSYS_BASE to  DDP_REG_BASE_MMSYS_CONFIG*/

	/* OUTREG32(DPI1_REG_VA+ 0x10, 0x000001A0);//DPI_CON */
	/* OUTREG32(DPI1_REG_VA+ 0x14, 0x00000101);//DPI_CLKCON */
	/* OUTREG32(DPI1_REG_VA+ 0x18, 0x02d00500);//DPI_SIZE 720x1280 */
	/* OUTREG32(DPI1_REG_VA+ 0x1c, 0x00000028);//DPI_TGEN_HWIDTH 40 */
	/* OUTREG32(DPI1_REG_VA+ 0x20, 0x01b800dc);//DPI_TGEN_HPORCH BACK:220 FRONT:440 */
	/* OUTREG32(DPI1_REG_VA+ 0x24, 0x00000005);//DPI_TGEN_VWIDTH_LODD 5 */
	/* OUTREG32(DPI1_REG_VA+ 0x28, 0x00050014);//DPI_TGEN_VPORCH_LODD BACK:20 FRONT:5 */
	/* OUTREG32(DPI1_REG_VA+ 0x44, 0x00001e00);//DPI_ESAV_VTIM_L WIDTH:30 lines */
	/* DPI1_ESAVVTimingControlLeft(0, 0x1E, 0, 0); */

	/* OUTREG32(DPI1_REG_VA+ 0x64, 0x1ead1f53);//DPI_MATRIX_COEFF_SET0 */
	/* OUTREG32(DPI1_REG_VA+ 0x68, 0x01320200);//DPI_MATRIX_COEFF_SET1 */
	/* OUTREG32(DPI1_REG_VA+ 0x6c, 0x00750259);//DPI_MATRIX_COEFF_SET2 */
	/* OUTREG32(DPI1_REG_VA+ 0x70, 0x1e530200);//DPI_MATRIX_COEFF_SET3 */
	/* OUTREG32(DPI1_REG_VA+ 0x74, 0x00001fa0);//DPI_MATRIX_COEFF_SET4 */
	/* DPI1_MatrixCoef(0x1F53, 0x1EAD, 0x0200, 0x0132, 0x0259, 0x0075, 0x0200, 0x1E53, 0x1FA0); */

	/* OUTREG32(DPI1_REG_VA+ 0x78, 0x00000000);//DPI_MATRIX_PREADD_SET0 */
	/* OUTREG32(DPI1_REG_VA+ 0x7c, 0x00000000);//DPI_MATRIX_PREADD_SET1 */
	/* DPI1_MatrixPreOffset(0, 0, 0); */

	/* OUTREG32(DPI1_REG_VA+ 0x80, 0x00000800);//DPI_MATRIX_POSTADD_SET0 */
	/* OUTREG32(DPI1_REG_VA+ 0x84, 0x00000800);//DPI_MATRIX_POSTADD_SET1 */
	/* DPI1_MatrixPostOffset(0x0800, 0, 0x0800); */

	/* OUTREG32(DPI1_REG_VA+ 0x88, 0x00000000);//DPI_CLPF_SETTING */
	/* DPI1_CLPFSetting(0, FALSE); */

	/* OUTREG32(DPI1_REG_VA+ 0x8c, 0x0f000100);//DPI_Y_LIMIT 256 - 3840 */
	/* OUTREG32(DPI1_REG_VA+ 0x90, 0x0f000100);//DPI_C_LIMIT 256 - 3840 */
	/* DPI1_SetChannelLimit(0x0100, 0x0F00, 0x0100, 0x0F00); */

	/* OUTREG32(DPI1_REG_VA+ 0x9c, 0x00000007);//DPI_EMBSYNC_SETTING */
	/* DPI1_EmbeddedSyncSetting(TRUE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE); */

	/* OUTREG32(DPI1_REG_VA+ 0xa8, 0x00000600);//DPI_OUTPUT_SETTING OUT_YC_MAP */
	/* DPI1_OutputSetting(DPI_OUTPUT_BIT_NUM_8BITS, FALSE, DPI_OUTPUT_CHANNEL_SWAP_RGB, DPI_OUTPUT_YC_MAP_CY); */

	/* OUTREG32(DPI1_REG_VA+ 0xb4, 0x11223341);//DPI_PATTERN */
	/* DPI1_EnableColorBar(); */

	/* OUTREG32(DPI1_REG_VA + 0x0,  0x00000001);// */
	/* DPI1_EnableClk(); */

#if 0
	cntl = DPI1_REG_VA->CNTL;
	cntl.EMBSYNC_EN = 1;
	OUTREG32(&DPI1_REG_VA->CNTL, AS_UINT32(&cntl));

	embsync = DPI1_REG_VA->EMBSYNC_SETTING;
	embsync.ESAV_CODE_MAN = 0;
	embsync.EMBVSYNC_G_Y = 1;
	embsync.EMBVSYNC_R_CR = 1;
	embsync.EMBVSYNC_B_CB = 1;
	OUTREG32(&DPI1_REG_VA->EMBSYNC_SETTING, AS_UINT32(&embsync));
#endif
...
{
	if (DSI_PHY_REG_DPI1 == 0)
		DPI1_InitRegbase();

	/* DPI1 PLL */
	SETREG32(TVDPLL_PWR_CON0, (0x1 << 0));	/* PLL_PWR_ON */
	udelay(2);
	CLRREG32(TVDPLL_PWR_CON0, (0x1 << 1));	/* PLL_ISO_EN */
	...
	default:
		{
			pr_err("[DPI1] not supported format, %s, %d, format = %d\n", __func__,
			       __LINE__, resolution);
			break;
		}
	}
...
{
	DISP_DDP("[DPI1] %s,isDpiPowerOn %d\n", __func__, s_isDpiPowerOn);
	if (!s_isDpiPowerOn) {
#ifdef DDP_USE_CLOCK_API
		int ret = 0;

		clk_prepare_enable(disp_dev.clk_map[DISP_REG_DPI1][0]);
		clk_prepare_enable(disp_dev.clk_map[DISP_REG_DPI1][1]);
		clk_prepare_enable(disp_dev.clk_map[DISP_REG_HDMI][0]);
		clk_prepare_enable(disp_dev.clk_map[DISP_REG_HDMI][1]);
		clk_prepare_enable(disp_dev.clk_map[DISP_REG_HDMI][2]);
		clk_prepare_enable(disp_dev.clk_map[DISP_REG_HDMI][3]);
		clk_prepare_enable(disp_dev.clk_map[DISP_REG_HDMI][4]);
		clk_prepare_enable(disp_dev.clk_map[DISP_REG_HDMI][5]);
		if (1 == ret) {
			DISP_LOG_PRINT(ANDROID_LOG_ERROR, "DPI",
...
DPI_STATUS DPI1_PowerOff(void)
{
	...
	if (s_isDpiPowerOn) {
		/* int ret = TRUE; */
		_BackupDPIRegisters();
		clk_disable_unprepare(disp_dev.clk_map[DISP_REG_DPI1][0]);
		clk_disable_unprepare(disp_dev.clk_map[DISP_REG_DPI1][1]);
		clk_disable_unprepare(disp_dev.clk_map[DISP_REG_HDMI][0]);
		clk_disable_unprepare(disp_dev.clk_map[DISP_REG_HDMI][1]);
		clk_disable_unprepare(disp_dev.clk_map[DISP_REG_HDMI][2]);
		clk_disable_unprepare(disp_dev.clk_map[DISP_REG_HDMI][3]);
		/*clk_disable_unprepare(disp_dev.clk_map[DISP_REG_HDMI][4]);
		clk_disable_unprepare(disp_dev.clk_map[DISP_REG_HDMI][5]);*/
		if (1 == ret) {
			DISP_LOG_PRINT(ANDROID_LOG_ERROR, "DPI",
				       "power manager API return FALSE\n");
		}
...
{
	DPI_REG_OUTPUT_SETTING ctrl = DPI1_REG_VA->OUTPUT_SETTING;
	DPI_REG_CLKCNTL clkctrl = DPI1_REG_VA->DPI_CLKCON;
/*
    ASSERT(divisor >= 2);
...
*/
	...
	OUTREG32(&DPI1_REG_VA->OUTPUT_SETTING, AS_UINT32(&ctrl));
	OUTREG32(&DPI1_REG_VA->DPI_CLKCON, AS_UINT32(&clkctrl));
...
{
	DPI_REG_CNTL ctrl = DPI1_REG_VA->CNTL;

	ctrl.YUV422_EN = 1;
	ctrl.RGB2YUV_EN = 1;
	ctrl.EMBSYNC_EN = 1;
	OUTREG32(&DPI1_REG_VA->CNTL, AS_UINT32(&ctrl));
...
{
	DPI_REG_CNTL ctrl = DPI1_REG_VA->CNTL;
	DPI_REG_OUTPUT_SETTING output = DPI1_REG_VA->OUTPUT_SETTING;

	if ((YCBCR_444 == ColorSpace) || (YCBCR_444_FULL == ColorSpace)) {
		/* YUV444 */
		ctrl.YUV422_EN = 0;
		ctrl.RGB2YUV_EN = 1;
		ctrl.IN_RB_SWAP = 0;
		output.CH_SWAP = 5;
	} else if ((YCBCR_422 == ColorSpace) || (YCBCR_422_FULL == ColorSpace)) {
		/* YUV422 */
		ctrl.YUV422_EN = 1;
		ctrl.RGB2YUV_EN = 1;
		ctrl.IN_RB_SWAP = 1;
	...
	OUTREG32(&DPI1_REG_VA->OUTPUT_SETTING, AS_UINT32(&output));
	OUTREG32(&DPI1_REG_VA->CNTL, AS_UINT32(&ctrl));
...
DPI_STATUS DPI1_ConfigDataEnable(DPI_POLARITY polarity)
{
	...
	DPI_REG_OUTPUT_SETTING pol = DPI1_REG_VA->OUTPUT_SETTING;

	pol.DE_POL = (DPI_POLARITY_FALLING == polarity) ? 1 : 0;
	OUTREG32(&DPI1_REG_VA->OUTPUT_SETTING, AS_UINT32(&pol));

	return DPI_STATUS_OK;
}
EXPORT_SYMBOL(DPI1_ConfigDataEnable);

DPI_STATUS DPI1_ConfigVsync(DPI_POLARITY polarity, uint32_t pulseWidth, uint32_t backPorch,
			    uint32_t frontPorch)
{
	DPI_REG_TGEN_VWIDTH_LODD vwidth_lodd = DPI1_REG_VA->TGEN_VWIDTH_LODD;
	DPI_REG_TGEN_VPORCH_LODD vporch_lodd = DPI1_REG_VA->TGEN_VPORCH_LODD;
	DPI_REG_OUTPUT_SETTING pol = DPI1_REG_VA->OUTPUT_SETTING;
	...
	OUTREG32(&DPI1_REG_VA->OUTPUT_SETTING, AS_UINT32(&pol));
	OUTREG32(&DPI1_REG_VA->TGEN_VWIDTH_LODD, AS_UINT32(&vwidth_lodd));
	OUTREG32(&DPI1_REG_VA->TGEN_VPORCH_LODD, AS_UINT32(&vporch_lodd));
...
DPI_STATUS DPI1_ConfigVsync_LEVEN(uint32_t pulseWidth, uint32_t backPorch, uint32_t frontPorch,
				  bool fgInterlace)
{
	DPI_REG_TGEN_VWIDTH_LEVEN vwidth_leven = DPI1_REG_VA->TGEN_VWIDTH_LEVEN;
	DPI_REG_TGEN_VPORCH_LEVEN vporch_leven = DPI1_REG_VA->TGEN_VPORCH_LEVEN;
	...
	OUTREG32(&DPI1_REG_VA->TGEN_VWIDTH_LEVEN, AS_UINT32(&vwidth_leven));
	OUTREG32(&DPI1_REG_VA->TGEN_VPORCH_LEVEN, AS_UINT32(&vporch_leven));
...
{
	DPI_REG_TGEN_VWIDTH_RODD vwidth_rodd = DPI1_REG_VA->TGEN_VWIDTH_RODD;
	DPI_REG_TGEN_VPORCH_RODD vporch_rodd = DPI1_REG_VA->TGEN_VPORCH_RODD;

	vwidth_rodd.VPW_RODD = pulseWidth;
	vporch_rodd.VBP_RODD = backPorch;
	vporch_rodd.VFP_RODD = frontPorch;

	OUTREG32(&DPI1_REG_VA->TGEN_VWIDTH_RODD, AS_UINT32(&vwidth_rodd));
	OUTREG32(&DPI1_REG_VA->TGEN_VPORCH_RODD, AS_UINT32(&vporch_rodd));
...
DPI_STATUS DPI1_ConfigVsync_REVEN(uint32_t pulseWidth, uint32_t backPorch, uint32_t frontPorch,
				  bool fgInterlace)
{
	DPI_REG_TGEN_VWIDTH_REVEN vwidth_reven = DPI1_REG_VA->TGEN_VWIDTH_REVEN;
	DPI_REG_TGEN_VPORCH_REVEN vporch_reven = DPI1_REG_VA->TGEN_VPORCH_REVEN;
	...
	OUTREG32(&DPI1_REG_VA->TGEN_VWIDTH_REVEN, AS_UINT32(&vwidth_reven));
	OUTREG32(&DPI1_REG_VA->TGEN_VPORCH_REVEN, AS_UINT32(&vporch_reven));
...
{
	OUTREG32(&DPI1_REG_VA->DPI_RST, Reset);

	return DPI_STATUS_OK;
}
EXPORT_SYMBOL(DPI1_SW_Reset);

DPI_STATUS DPI1_ConfigHsync(DPI_POLARITY polarity, uint32_t pulseWidth, uint32_t backPorch,
			    uint32_t frontPorch)
{
	DPI_REG_TGEN_HPORCH hporch = DPI1_REG_VA->TGEN_HPORCH;
	DPI_REG_OUTPUT_SETTING pol = DPI1_REG_VA->OUTPUT_SETTING;

	pol.HSYNC_POL = (DPI_POLARITY_FALLING == polarity) ? 1 : 0;
	/* DPI1_REG_VA->TGEN_HWIDTH = pulseWidth; */
	OUTREG32(&DPI1_REG_VA->TGEN_HWIDTH, pulseWidth);
	hporch.HBP = backPorch;
	hporch.HFP = frontPorch;

	OUTREG32(&DPI1_REG_VA->OUTPUT_SETTING, AS_UINT32(&pol));
	OUTREG32(&DPI1_REG_VA->TGEN_HPORCH, AS_UINT32(&hporch));
...
{
/* return DPI1_REG_VA->CNTL.DSI_MODE ? TRUE : FALSE; */
	return FALSE;
}
...
DPI_STATUS DPI1_FBSetSize(uint32_t width, uint32_t height)
{
	...
	OUTREG32(&DPI1_REG_VA->SIZE, AS_UINT32(&size));
...
DPI_STATUS DPI1_DumpRegisters(void)
{
	...
	for (i = 0; i < sizeof(DPI_REGS); i += 4) {
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI1", "addr=0x%x (DPI1+%04x) : 0x%08x\n",
			(unsigned int)(DPI1_REG_VA + i), i, DISP_REG_GET(DPI1_REG_VA + i));
	}
...
uint32_t DPI1_GetCurrentFB(void)
{
	return 0;
}
...
DPI_STATUS DPI1_Capture_Framebuffer(unsigned int pvbuf, unsigned int bpp)
{
	...
	if (dpi_needPowerOff)
		fbv = (unsigned char *)ioremap_cached((unsigned int)DPI1_REG_VA->FB[0].ADDR, fbsize);
	else
		fbv =
		    (unsigned char *)ioremap_cached((unsigned int)DPI1_REG_VA->FB[DPI1_GetCurrentFB()].
						    ADDR, fbsize);
...
DPI_STATUS DPI1_EnableInterrupt(DISP_INTERRUPT_EVENTS eventID)
{
	...
	case DISP_DPI_VSYNC_INT:
		/* DPI1_REG_VA->INT_ENABLE.VSYNC = 1; */
		OUTREGBIT(DPI_REG_INTERRUPT, DPI1_REG_VA->INT_ENABLE, VSYNC, 1);
		break;
	default:
		return DPI_STATUS_ERROR;
	}
...
{
/*
	DPI_REG_PATTERN pattern = DPI1_REG_VA->PATTERN;

       pattern.PAT_EN = 1;
	pattern.PAT_SEL = 4; // Color Bar

    OUTREG32(&DPI1_REG_VA->PATTERN, AS_UINT32(&pattern));
*/
	OUTREG32(DPI1_REG_VA + 0xf00, 0x41);
...
DPI_STATUS DPI1_MatrixCoef(uint16_t c00, uint16_t c01, uint16_t c02,
			   uint16_t c10, uint16_t c11, uint16_t c12, uint16_t c20, uint16_t c21, uint16_t c22)
{

/*
    DPI_REG_MATRIX_COEFF_SET0 set0 = DPI1_REG_VA->MATRIX_COEFF_SET0;
    DPI_REG_MATRIX_COEFF_SET1 set1 = DPI1_REG_VA->MATRIX_COEFF_SET1;
    DPI_REG_MATRIX_COEFF_SET2 set2 = DPI1_REG_VA->MATRIX_COEFF_SET2;
    DPI_REG_MATRIX_COEFF_SET3 set3 = DPI1_REG_VA->MATRIX_COEFF_SET3;
    DPI_REG_MATRIX_COEFF_SET4 set4 = DPI1_REG_VA->MATRIX_COEFF_SET4;
    ...
    OUTREG32(&DPI1_REG_VA->MATRIX_COEFF_SET0, AS_UINT32(&set0));
    OUTREG32(&DPI1_REG_VA->MATRIX_COEFF_SET1, AS_UINT32(&set1));
    OUTREG32(&DPI1_REG_VA->MATRIX_COEFF_SET2, AS_UINT32(&set2));
    OUTREG32(&DPI1_REG_VA->MATRIX_COEFF_SET3, AS_UINT32(&set3));
    OUTREG32(&DPI1_REG_VA->MATRIX_COEFF_SET4, AS_UINT32(&set4));
*/
	return DPI_STATUS_OK;
}
...
{
/*
    DPI_REG_MATRIX_PREADD_SET0 set0 = DPI1_REG_VA->MATRIX_PREADD_SET0;
    DPI_REG_MATRIX_PREADD_SET1 set1 = DPI1_REG_VA->MATRIX_PREADD_SET1;

    set0.MATRIX_PRE_ADD_0 = preAdd0; set0.MATRIX_PRE_ADD_1 = preAdd1;
    set1.MATRIX_PRE_ADD_2 = preAdd2;

    OUTREG32(&DPI1_REG_VA->MATRIX_PREADD_SET0, AS_UINT32(&set0));
    OUTREG32(&DPI1_REG_VA->MATRIX_PREADD_SET1, AS_UINT32(&set1));
*/
	return DPI_STATUS_OK;
}
...
{
/*
    DPI_REG_MATRIX_POSTADD_SET0 set0 = DPI1_REG_VA->MATRIX_POSTADD_SET0;
    DPI_REG_MATRIX_POSTADD_SET1 set1 = DPI1_REG_VA->MATRIX_POSTADD_SET1;

    set0.MATRIX_POST_ADD_0 = postAdd0; set0.MATRIX_POST_ADD_1 = postAdd1;
    set1.MATRIX_POST_ADD_2 = postAdd2;

    OUTREG32(&DPI1_REG_VA->MATRIX_POSTADD_SET0, AS_UINT32(&set0));
    OUTREG32(&DPI1_REG_VA->MATRIX_POSTADD_SET1, AS_UINT32(&set1));
*/
	return DPI_STATUS_OK;
}
...
{
	DPI_REG_Y_LIMIT y = DPI1_REG_VA->Y_LIMIT;
	DPI_REG_C_LIMIT c = DPI1_REG_VA->C_LIMIT;
	...
	OUTREG32(&DPI1_REG_VA->Y_LIMIT, AS_UINT32(&y));
	OUTREG32(&DPI1_REG_VA->C_LIMIT, AS_UINT32(&c));
...
{
	DPI_REG_CLPF_SETTING setting = DPI1_REG_VA->CLPF_SETTING;

	setting.CLPF_TYPE = clpfType;
	setting.ROUND_EN = roundingEnable ? 1 : 0;

	OUTREG32(&DPI1_REG_VA->CLPF_SETTING, AS_UINT32(&setting));
...
{
	DPI_REG_EMBSYNC_SETTING setting = DPI1_REG_VA->EMBSYNC_SETTING;
	...
	OUTREG32(&DPI1_REG_VA->EMBSYNC_SETTING, AS_UINT32(&setting));
...
{
	DPI_REG_OUTPUT_SETTING setting = DPI1_REG_VA->OUTPUT_SETTING;
	...
	OUTREG32(&DPI1_REG_VA->OUTPUT_SETTING, AS_UINT32(&setting));
...
DPI_STATUS DPI1_ESAVVTimingControlLeft(uint16_t offsetOdd, uint16_t widthOdd, uint16_t offsetEven,
				       uint16_t widthEven)
{
	DPI_REG_ESAV_VTIM_LODD setting1 = DPI1_REG_VA->ESAV_VTIM_LODD;
	DPI_REG_ESAV_VTIM_LEVEN setting2 = DPI1_REG_VA->ESAV_VTIM_LEVEN;
	...
	OUTREG32(&DPI1_REG_VA->ESAV_VTIM_LODD, AS_UINT32(&setting1));
	OUTREG32(&DPI1_REG_VA->ESAV_VTIM_LEVEN, AS_UINT32(&setting2));

	return DPI_STATUS_OK;
}
EXPORT_SYMBOL(DPI1_ESAVVTimingControlLeft);

DPI_STATUS DPI1_ESAVVTimingControlRight(uint16_t offsetOdd, uint16_t widthOdd, uint16_t offsetEven,
					uint16_t widthEven)
{
	DPI_REG_ESAV_VTIM_RODD setting1 = DPI1_REG_VA->ESAV_VTIM_RODD;
	DPI_REG_ESAV_VTIM_REVEN setting2 = DPI1_REG_VA->ESAV_VTIM_REVEN;
	...
	OUTREG32(&DPI1_REG_VA->ESAV_VTIM_RODD, AS_UINT32(&setting1));
	OUTREG32(&DPI1_REG_VA->ESAV_VTIM_REVEN, AS_UINT32(&setting2));
