...
#else
#define BLS_MSG(string, args...) do { if (bls_dbg_log) pr_err("[BLS]"string, ##args); } while (0)
#endif
/* default on, err msg */
...
#else
	DISP_REG_SET(DISP_REG_BLS_SRC_SIZE, (srcHeight << 16) | srcWidth);
	DISP_REG_SET(DISP_REG_BLS_PWM_DUTY, 1023);
	DISP_REG_SET(DISP_REG_BLS_PWM_CON, 0x0 | (gPWMDiv << 16));
...
int disp_bls_set_backlight(unsigned int level)
{
	...
	BLS_MSG("AAL : disp_bls_set_backlight: %d\n", level);
