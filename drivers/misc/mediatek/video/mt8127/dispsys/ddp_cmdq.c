...
static DEFINE_MUTEX(gCmdqThreadLock);	/* ensure atomic access thread info */
...
/* 0 for FB, 1 for Bitblt, 2 for HDMI, 3 for BLS */
static int mutex_used[ENGINE_MUTEX_NUM] = {1, 1, 1, 1, 0, 0, 0, 0};
...
static long cmdq_proc_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DISP_IOCTL_GET_TDSHPINDEX:
		...
		if (copy_to_user((void *)arg, tdshp_index, sizeof(DISPLAY_TDSHP_T))) {
			CMDQ_LOG("disp driver : DISP_IOCTL_GET_TDSHPINDEX Copy to user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_PQPARAM:
		...
		if (copy_to_user((void *)arg, pq_param, sizeof(DISP_PQ_PARAM))) {
			CMDQ_LOG("disp driver : DISP_IOCTL_GET_PQPARAM Copy to user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_PQ_CAM_PARAM:
		...
		if (copy_to_user((void *)arg, pq_param, sizeof(DISP_PQ_PARAM))) {
			CMDQ_LOG("disp driver : DISP_IOCTL_GET_PQ_CAM_PARAM Copy to user failed\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_PQ_GAL_PARAM:
		...
		if (copy_to_user((void *)arg, pq_param, sizeof(DISP_PQ_PARAM))) {
			CMDQ_LOG("disp driver : DISP_IOCTL_GET_PQ_GAL_PARAM Copy to user failed\n");
			return -EFAULT;
		}
		...
		for (index = 0 ; index < ENGINE_MUTEX_NUM ; index += 1) {
			if ((1 << index) & pNode->u4LockedMutex) {
				...
				DISP_DBG("unlock index = %d, mutex_used[%d %d %d %d ]\n",
					index, mutex_used[0], mutex_used[1],
					mutex_used[2], mutex_used[3]);
			}
		}
...
static const struct file_operations cmdq_proc_fops = {
	.owner	  = THIS_MODULE,
	.open		= cmdq_proc_open,
	.read	   = cmdq_proc_read,
								...
								loff_t *ppos)
{
	...
	mutex_lock(&gCmdqThreadLock);
	smp_mb();

	length += sprintf(&pBuffer[length], "====== Clock Status =======\n");
	length += sprintf(&pBuffer[length],
		"MT_CG_DISP0_MM_CMDQ: %d, MT_CG_DISP0_MUTEX: %d\n",
		cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_CMDQ][0]), /* clock_is_on(MT_CG_DISP0_MM_CMDQ), */
		cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MUTEX][0]) /* clock_is_on(MT_CG_DISP0_MUTEX) */);

	length += sprintf(&pBuffer[length], "====== Engine Usage =======\n");
	length += sprintf(&pBuffer[length],
		"IMGI: count %d, owner %d, fail: %d, reset: %d\n",
		pEngine[tIMGI].userCount, pEngine[tIMGI].currOwner,
		pEngine[tIMGI].failCount, pEngine[tIMGI].resetCount);
	length += sprintf(&pBuffer[length],
		"RDMA0: count %d, owner %d, fail: %d, reset: %d\n",
		pEngine[tRDMA0].userCount, pEngine[tRDMA0].currOwner,
		pEngine[tRDMA0].failCount, pEngine[tRDMA0].resetCount);
	length += sprintf(&pBuffer[length],
		"RSZ0: count %d, owner %d, fail: %d, reset: %d\n",
		pEngine[tSCL0].userCount, pEngine[tSCL0].currOwner,
		pEngine[tSCL0].failCount, pEngine[tSCL0].resetCount);
	length += sprintf(&pBuffer[length],
		"RSZ1: count %d, owner %d, fail: %d, reset: %d\n",
		pEngine[tSCL1].userCount, pEngine[tSCL1].currOwner,
		pEngine[tSCL1].failCount, pEngine[tSCL1].resetCount);
	length += sprintf(&pBuffer[length],
		"TDSHP: count %d, owner %d, fail: %d, reset: %d\n",
		pEngine[tTDSHP].userCount, pEngine[tTDSHP].currOwner,
		pEngine[tTDSHP].failCount, pEngine[tTDSHP].resetCount);
	length += sprintf(&pBuffer[length],
		"tWDMA1: count %d, owner %d, fail: %d, reset: %d\n",
		pEngine[tWDMA1].userCount, pEngine[tWDMA1].currOwner,
		pEngine[tWDMA1].failCount, pEngine[tWDMA1].resetCount);
	length += sprintf(&pBuffer[length],
		"tWROT: count %d, owner %d, fail: %d, reset: %d\n",
		pEngine[tWROT].userCount, pEngine[tWROT].currOwner,
		pEngine[tWROT].failCount, pEngine[tWROT].resetCount);
	...
	for (index = 0; ((index < CMDQ_MAX_THREAD_COUNT) && (0 == isFull)); index++) {
		...
		if (pThread->taskCount > 0) {
			...
			for (inner = 0; ((inner < CMDQ_MAX_TASK_COUNT) && (0 == isFull)); inner++) {
				...
				if (NULL != pTask) {
					length += sprintf(&pBuffer[length],
						"Slot: %d, Task: 0x%08x, MVABase: 0x%08x, Size: %d, Last Command 0x%08x, 0x%08x\n",
						inner, (uint32_t)pTask, pTask->MVABase,
						pTask->blockSize, pTask->pCMDEnd[-1],
						pTask->pCMDEnd[0]);
	...
	for (index = 0; ((index < count) && (0 == isFull)); index++) {
		...
		if (pTask->engineFlag & (0x01 << tRDMA0)) {
			for (inner = 0; inner < 1024; inner++) {
				length += sprintf(&pPage[length],
					"0x%08x: 0x%08x\n",
					(DISP_REG_MDP_RDMA_EN + (inner << 2)),
					pError->regVal[tRDMA0][inner]);
		...
		if (pTask->engineFlag & (0x01 << tWROT)) {
			for (inner = 0; inner < 1024; inner++) {
				length += sprintf(&pPage[length],
					"0x%08x: 0x%08x\n",
					(DISP_REG_MDP_WROT_VIDO_CTRL + (inner << 2)),
					pError->regVal[tWROT][inner]);
		...
		if (pTask->engineFlag & (0x01 << tWDMA1)) {
			for (inner = 0; inner < 1024; inner++) {
				length += sprintf(&pPage[length],
					"0x%08x: 0x%08x\n",
					(DISP_REG_MDP_WDMA_INT_EN + (inner << 2)),
					pError->regVal[tWDMA1][inner]);
		...
		for (pCMD  = pTask->pVABase; size > 0; pCMD += 2, size -= 8, MVABase += 8) {
			...
			case CMDQ_CODE_WRITE:
				...
				length += sprintf(&pPage[length],
					"MVA: 0x%08x, WRITE Addr: 0x%08x, Value: 0x%08x\n",
					MVABase, addr, pCMD[0]);
				break;
			case CMDQ_CODE_POLL:
				value = (pCMD[1] >> 22) & 0x03;
				...
				length += sprintf(&pPage[length],
					"MVA: 0x%08x, POLL Addr: 0x%08x, Value: 0x%08x\n",
					MVABase, addr, pCMD[0]);
				break;
			case CMDQ_CODE_JUMP:
				length += sprintf(&pPage[length],
					"MVA: 0x%08x, JUMP 0x%08x\n",
					MVABase, pCMD[0]);
				break;
			case CMDQ_CODE_WFE:
				value = pCMD[1] & 0x00FFFFFF;
				length += sprintf(&pPage[length],
					"MVA: 0x%08x, WFE 0x%08x\n",
					MVABase, value);
				break;
			case CMDQ_CODE_EOC:
				length += sprintf(&pPage[length],
					"MVA: 0x%08x, EOC 0x%08x\n",
					MVABase, pCMD[0]);
				break;
			default:
				length += sprintf(&pPage[length],
					"MVA: 0x%08x, 0x%08x:0x%08x\n",
									 ...
									 loff_t *ppos)
{
	...
	textBuf[len] = '\0';
	/* value = (int)strtod(textBuf, textBuf + len); */

	if (kstrtoul(textBuf, 10, &value)) {
		CMDQ_ERR("read write_debug_level fail[%d]\n", __LINE__);
		return len;
	}
									 ...
									 loff_t *ppos)
{
	...
	ret = sscanf(textBuf, "%d %d %d", &swTimeout, &predumpStartTime, &predumpDuration);
	if (0 == ret) {
		CMDQ_ERR("read swTimeout fail[%d]\n", __LINE__);
		return len;
	}
...
void cmdqInitialize(struct device *dev)
{
	...
	pVABase = dma_alloc_coherent(dev, CMDQ_MAX_DMA_BUF_SIZE, &MVABase, GFP_KERNEL | __GFP_NO_KSWAPD);
	CMDQ_LOG("initialize cmdq pVaBase[0x%p] MVABase[0x%08x]\n",
			pVABase, MVABase);
	if (NULL == pVABase) {
	...
	/* proc init for cmdq debug/internal usage */
	cmdq_init_procfs();
...
int32_t cmdqResumeTask(void)
{
	...
	/* note the tokens' value become random number after disable and enable clock
...
static TaskStruct *cmdq_find_free_task(uint32_t blockSize)
{
	...
	if (list_empty(&gCmdqFreeTask) ||
		...
		spin_unlock_irqrestore(&gCmdqTaskLock, flags);
		pTask = kmalloc(sizeof(TaskStruct), GFP_KERNEL);
...
TaskStruct *cmdq_acquire_task(cmdqCommandStruct *pCommandDesc)
{
	...
	do {
		...
		/* copy command buffer */
		if (0 > cmdq_insert_command_buffer(pTask->pVABase,
		...
		if ((0 < pCommandDesc->secData.addrListLength) &&
		   (0 > cmdq_insert_command_buffer(pTask->secData.addrList,
							pCommandDesc->secData.addrList,
							secAddrListLength_byte,
							copyCmdFromUserSpace))) {
			status = -3030;
			break;
		}
		...
		if ((0 < pCommandDesc->secData.portListLength) &&
		   (0 > cmdq_insert_command_buffer(pTask->secData.portList,
							pCommandDesc->secData.portList,
							secPortListLength_byte,
							copyCmdFromUserSpace))) {
			status = -3040;
			break;
		}
	...
	if (0 > status && pTask) {
		...
		CMDQ_ERR("Source: 0x%08x, target: 0x%08x, size: %d\n",
			...
			pCommandDesc->blockSize);
		CMDQ_ERR("Source: 0x%08x, target: 0x%08x, size: %d\n",
			(uint32_t)pCommandDesc->secData.addrList,
			(uint32_t)pTask->secData.addrList,
			secAddrListLength_byte);
		CMDQ_ERR("Source: 0x%08x, target: 0x%08x, size: %d\n",
			(uint32_t)pCommandDesc->secData.portList,
			(uint32_t)pTask->secData.portList,
			secPortListLength_byte);
...
void cmdq_enable_clock(uint32_t engineFlag, int32_t  thread)
{
	...
	pEngine = &(gCmdqContext.engine[0]);
	/*
		insead of engines' taskcount(which depends on engineFlag,
		but engineFlag is 0x0 when access GCE HW only),
	...
	if (0 == atomic_read(&gCmdqThreadUsage)) {
		...
		cmdq_core_enable_ccf_clk(DISP_REG_CMDQ, 0);
		cmdq_core_enable_ccf_clk(DISP_REG_MUTEX, 1);
#endif
	}
	atomic_inc(&gCmdqThreadUsage);

	if (engineFlag & (0x1 << tIMGI)) {
		if (!cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_RDMA][1])) { /* MT_CG_DISP0_CAM_MDP */
#if defined(DDP_USE_MTK_CLKMGR)
...
#else
/*
**	ISP has already manage their clocks themselves
**	CMDQ do not need to open or close their clocks
*/

	#if 1
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_RDMA, 2);	/* MT_CG_IMAGE_CAM_SMI */
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_RDMA, 3);	/* MT_CG_IMAGE_CAM_CAM */
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_WDMA, 1);	/* MT_CG_IMAGE_SEN_TG */
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_WDMA, 2);	/* MT_CG_IMAGE_SEN_CAM */
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_WDMA, 3);	/* MT_CG_IMAGE_LARB2_SMI */
	#endif
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_RDMA, 1);	/* CAM_MDP */
#endif
	...
	if (engineFlag & (0x1 << tRDMA0)) {
		if (!cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_RDMA][0])) {
#if defined(DDP_USE_MTK_CLKMGR)
			enable_clock(MT_CG_DISP0_MDP_RDMA, "MDP_RDMA");
#else
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_RDMA, 0);
#endif
	...
	if (engineFlag & (0x1 << tSCL0)) {
		if (!cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_RSZ0][0])) {
#if defined(DDP_USE_MTK_CLKMGR)
			enable_clock(MT_CG_DISP0_MDP_RSZ0, "MDP_RSZ0");
#else
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_RSZ0, 0);
#endif
	...
	if (engineFlag & (0x1 << tSCL1)) {
		if (!cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_RSZ1][0])) {
#if defined(DDP_USE_MTK_CLKMGR)
			enable_clock(MT_CG_DISP0_MDP_RSZ1, "MDP_RSZ1");
#else
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_RSZ1, 0);
#endif
	...
	if (engineFlag & (0x1 << tTDSHP)) {
		if (!cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_TDSHP][0])) {
#if defined(DDP_USE_MTK_CLKMGR)
			enable_clock(MT_CG_DISP0_MDP_TDSHP, "MDP_TDSHP");
#else
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_TDSHP, 0);
#endif
	...
	if (engineFlag & (0x1 << tWROT)) {
		if (!cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_WROT][0])) {
#if defined(DDP_USE_MTK_CLKMGR)
			enable_clock(MT_CG_DISP0_MDP_WROT, "MDP_WROT");
#else
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_WROT, 0);
#endif
	...
	if (engineFlag & (0x1 << tWDMA1)) {
		if (!cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_WDMA][0])) {
#if defined(DDP_USE_MTK_CLKMGR)
			enable_clock(MT_CG_DISP0_MDP_WDMA, "MDP_WDMA");
#else
			cmdq_core_enable_ccf_clk(DISP_REG_MDP_WDMA, 0);
#endif
...
/*		spin_lock_irqsave(&gCmdqThreadLock, flags); */
		...
		for (index = 0; ((index < tTotal) && (engine != 0)); index++) {
			if (engine & (0x1 << index)) {
				if (CMDQ_INVALID_THREAD == pEngine[index].currOwner) {
					...
					CMDQ_LOG("THREAD: try locate on THR %d but engine %d also occupied by THR %d,",
							thread, index, pEngine[index].currOwner);
					CMDQ_LOG(" engineFlag: 0x%08x\n", engineFlag);
					thread = CMDQ_INVALID_THREAD;
		...
		if (!isSecureTask && CMDQ_MIN_SECURE_THREAD_ID == thread) {
			...
			const int startIndex = (isSecureTask) ?
				(CMDQ_MIN_SECURE_THREAD_ID) : (CMDQ_MAX_SECURE_THREAD_COUNT);
			const int endIndex   = (isSecureTask) ?
				(CMDQ_MAX_SECURE_THREAD_COUNT) : (CMDQ_MAX_THREAD_COUNT);
...
int32_t cmdq_acquire_thread(const TaskStruct *pTask)
{
	...
	do {
		...
		if (0 == waitQ) {
			...
			CMDQ_ERR("<--THREAD: engine: 0x%08x, waitQ: %d,",
				pTask->engineFlag, waitQ);
			CMDQ_ERR(" pTask %p(sec:%d) acquires THR: %d, pid(%d:%d)\n",
				pTask, pTask->secData.isSecure, thread,
				current->tgid, current->pid);
		} else {
			CMDQ_MSG("<--THREAD: engine: 0x%08x, waitQ: %d,",
				pTask->engineFlag, waitQ);
			CMDQ_MSG(" pTask %p(sec:%d) acquires THR: %d, pid(%d:%d)\n",
				pTask, pTask->secData.isSecure, thread,
				current->tgid, current->pid);
		}
	} while (0);
...
int32_t cmdq_disable_clock(uint32_t engineFlag)
{
	...
	if (engineFlag & (0x1 << tWDMA1)) {
		...
		if ((pEngine[tWDMA1].userCount <= 0) &&
			cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_WDMA][0])) {
			DISP_REG_SET(DISP_REG_MDP_WDMA_RESET, 0x1);

			loopCount = 0;
			while (loopCount < CMDQ_MAX_LOOP_COUNT) {
				if (0x1 == (DISP_REG_GET(DISP_REG_MDP_WDMA_FLOW_CTRL_DBG) & 0x3FF))
					break;
			...
			DISP_REG_SET(DISP_REG_MDP_WDMA_RESET, 0x0);
#if defined(DDP_USE_MTK_CLKMGR)
			disable_clock(MT_CG_DISP0_MDP_WDMA, "MDP_WDMA");
#else
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_WDMA, 0);
#endif
	...
	if (engineFlag & (0x1 << tWROT)) {
		...
		if ((pEngine[tWROT].userCount <= 0) &&
			cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_WROT][0])) {
			DISP_REG_SET(DISP_REG_MDP_WROT_VIDO_SOFT_RST, 0x1);

			loopCount = 0;
			while (loopCount < CMDQ_MAX_LOOP_COUNT) {
				if (0x0 == (DISP_REG_GET(DISP_REG_MDP_WROT_VIDO_SOFT_RST_STATUS) & 0x1))
					break;
			...
			DISP_REG_SET(DISP_REG_MDP_WROT_VIDO_SOFT_RST, 0x0);

#if defined(DDP_USE_MTK_CLKMGR)
			disable_clock(MT_CG_DISP0_MDP_WROT, "MDP_WROT");
#else
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_WROT, 0);
#endif
	...
	if (engineFlag & (0x1 << tTDSHP)) {
		...
		if ((pEngine[tTDSHP].userCount <= 0) &&
			cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_TDSHP][0])) {
			DISP_REG_SET(DISP_REG_MDP_TDSHP_CTRL, 0x0);
			DISP_REG_SET(DISP_REG_MDP_TDSHP_CTRL, 0x2);
			DISP_REG_SET(DISP_REG_MDP_TDSHP_CTRL, 0x0);

#if defined(DDP_USE_MTK_CLKMGR)
			disable_clock(MT_CG_DISP0_MDP_TDSHP, "MDP_TDSHP");
#else
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_TDSHP, 0);
#endif
	...
	if (engineFlag & (0x1 << tSCL1)) {
		...
		if ((pEngine[tSCL1].userCount <= 0) &&
			cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_RSZ1][0])) {
			DISP_REG_SET(DISP_REG_MDP_RSZ1_EN, 0x0);
			DISP_REG_SET(DISP_REG_MDP_RSZ1_EN, 0x10000);
			DISP_REG_SET(DISP_REG_MDP_RSZ1_EN, 0x0);

#if defined(DDP_USE_MTK_CLKMGR)
			disable_clock(MT_CG_DISP0_MDP_RSZ1, "MDP_RSZ1");
#else
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_RSZ1, 0);
#endif
	...
	if (engineFlag & (0x1 << tSCL0)) {
		...
		if ((pEngine[tSCL0].userCount <= 0) &&
			cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_RSZ0][0])) {
			DISP_REG_SET(DISP_REG_MDP_RSZ0_EN, 0x0);
			DISP_REG_SET(DISP_REG_MDP_RSZ0_EN, 0x10000);
			DISP_REG_SET(DISP_REG_MDP_RSZ0_EN, 0x0);

#if defined(DDP_USE_MTK_CLKMGR)
			disable_clock(MT_CG_DISP0_MDP_RSZ0, "MDP_RSZ0");
#else
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_RSZ0, 0);
#endif
	...
	if (engineFlag & (0x1 << tRDMA0)) {
		...
		if ((pEngine[tRDMA0].userCount <= 0) &&
			cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_RDMA][0])) {
			DISP_REG_SET(DISP_REG_MDP_RDMA_RESET, 0x1);

			loopCount = 0;
			while (loopCount < CMDQ_MAX_LOOP_COUNT) {
				if (0x100 == (DISP_REG_GET(DISP_REG_MDP_RDMA_MON_STA_1) & 0x7FF00))
					break;
			...
			DISP_REG_SET(DISP_REG_MDP_RDMA_RESET, 0x0);

#if defined(DDP_USE_MTK_CLKMGR)
			disable_clock(MT_CG_DISP0_MDP_RDMA, "MDP_RDMA");
#else
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_RDMA, 0);
#endif
...
#else
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_RDMA, 1);	/* CLK_MM_CAM_MDP */
...
**	CMDQ do not need to open or close their clocks
*/
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_RDMA, 2);	/* CLK_IMG_CAM_SMI */
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_RDMA, 3);	/* CLK_IMG_CAM_CAM */
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_WDMA, 1);	/* CLK_IMG_SEN_TG */
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_WDMA, 2);	/* CLK_IMG_SEN_CAM */
			cmdq_core_disable_ccf_clk(DISP_REG_MDP_WDMA, 3);	/* CLK_IMG_LARB2_SMI */
	#endif
#endif
...
#else
		cmdq_core_disable_ccf_clk(DISP_REG_MUTEX, 1);
		cmdq_core_disable_ccf_clk(DISP_REG_CMDQ, 0);
	...
	if (engineFlag & (0x1 << tWROT)) {
		if (cmdq_core_clock_is_on(disp_dev.clk_map[DISP_REG_MDP_WROT][0])) {
			...
			if (loopCount >= CMDQ_MAX_LOOP_COUNT) {
				...
				regValue = DISP_REG_GET(DISP_REG_CONFIG_MMSYS_SW_RST_B);
				regValue &= 0xFFFFFBFF;
				DISP_REG_SET(DISP_REG_CONFIG_MMSYS_SW_RST_B, regValue);
...
void cmdq_dump_mmsys_config(void)
{
	...
	CMDQ_ERR("MMSYS_DL_VALID_0: 0x%08x, MMSYS_DL_VALID_1: 0x%08x,",
		DISP_REG_GET(DISP_REG_CONFIG_VALID),
		DISP_REG_GET(DISP_REG_CONFIG_VALID_1));
	CMDQ_ERR(" MMSYS_DL_READY0: 0x%08x, MMSYS_DL_READY1: 0x%08x\n",
		DISP_REG_GET(DISP_REG_CONFIG_READY),
		DISP_REG_GET(DISP_REG_CONFIG_READY_1));
}
								   ...
								   int32_t	thread)
{
	...
	*/
	...
	if (NULL != pTask) {
		...
		CMDQ_ERR("Task: 0x%08x, Scenario: %d, State: %d,",
			(uint32_t)pTask, pTask->scenario, pTask->taskState);
		CMDQ_ERR(" Priority: %d, Reorder: %d, Engine: 0x%08x, irgFlag: 0x%08x\n",
			pTask->priority, pTask->reorder, pTask->engineFlag, pTask->irqFlag);

		CMDQ_ERR("VABase: 0x%08x, CMDEnd: 0x%08x, MVABase: 0x%08x, Size: %d,",
			(uint32_t)pTask->pVABase, (uint32_t)pTask->pCMDEnd, pTask->MVABase, pTask->blockSize);
		CMDQ_ERR(" Last Inst: 0x%08x:0x%08x, 0x%08x:0x%08x\n",
			pTask->pCMDEnd[-3], pTask->pCMDEnd[-2], pTask->pCMDEnd[-1], pTask->pCMDEnd[0]);

		CMDQ_ERR("Trigger: %d:%d, Got IRQ: %d:%d, Finish: %d:%d\n",
			(uint32_t)pTask->trigger.tv_sec,
			(uint32_t)pTask->trigger.tv_usec,
			(uint32_t)pTask->gotIRQ.tv_sec,
			(uint32_t)pTask->gotIRQ.tv_usec,
			(uint32_t)pTask->wakedUp.tv_sec,
			(uint32_t)pTask->wakedUp.tv_usec);
	}

	CMDQ_ERR("=============== [CMDQ] Mutex Status ===============\n");
	value[0] = DISP_REG_GET(DISP_REG_CONFIG_MUTEX_INTSTA);
	value[1] = DISP_REG_GET(DISP_REG_CONFIG_REG_COMMIT);
	...
	value[0] = DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0);
	value[1] = DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON1);
	...
	*/
	...
	if (NULL != pTask) {
		...
		/* CMDQ_MAX_ERROR_COUNT is 1 now,
			replace the old one instead of using gCmdqContext.
			errNum as the index init value
		...
		for (index = 0; index < CMDQ_MAX_TASK_COUNT; index++) {
			if (NULL != pThread->pCurTask[index]) {
				CMDQ_ERR("Slot %d, Task: 0x%08x, VABase: 0x%08x, MVABase 0x%08x, Size: %d,",
					index,
					(uint32_t)(pThread->pCurTask[index]),
					(uint32_t)(pThread->pCurTask[index]->pVABase),
					pThread->pCurTask[index]->MVABase,
					pThread->pCurTask[index]->blockSize);
				CMDQ_ERR(" Last Inst 0x%08x:0x%08x, 0x%08x:0x%08x\n",
					pThread->pCurTask[index]->pCMDEnd[-3],
					pThread->pCurTask[index]->pCMDEnd[-2],
					pThread->pCurTask[index]->pCMDEnd[-1],
					pThread->pCurTask[index]->pCMDEnd[0]);
			}
		}
		...
		mt_irq_dump_status(disp_dev.irq[DISP_REG_CMDQ]);
#endif
		...
		if (hwPC && pTask && hwPC >= pTask->pVABase) {
			...
			print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 16, 4,
						   pTask->pVABase,
						   (2 + hwPC - pTask->pVABase) * sizeof(uint32_t), true);
		} else {
			...
			struct timeval gotIRQ)
{
	...
	do {
		CMDQ_ERR("warning task[%d][0x%p], THR[%d], taskCount[%d], CurrCookie[%d], irqFlag[%d], gotIRQ[%d:%d]",
			index, pTask, thread, taskCountInThread, cookie, irqFlag,
			(uint32_t)(gotIRQ.tv_sec), (uint32_t)(gotIRQ.tv_usec));
			...
			uint32_t predump_retry_count)
{
	...
	waitQ = wait_event_interruptible_timeout(gCmdWaitQueue[thread],
	...
	for (i = 0; i < predump_max_retry_count; i++) {
		...
		snprintf(predumpTag, sizeof(predumpTag), "PREDUMP %3d on Thread %d", i, thread);
			...
			uint32_t predump_retry_count)
{
	...
	waitQ = cmdq_core_wait_task_done_with_interruptible_timeout(pTask,
		thread, predump_start_time_ms, predump_duration_ms, predump_retry_count);
	...
	if (TASK_STATE_DONE == pTask->taskState) {
		...
		/* CMDQ err
			except thread's taskCount, HW/SW cookie count,
			GCE/HW reset haved been fixed, so print err only */
		status = -EFAULT;
		...
		CMDQ_ERR("should not go here. waitQ[%d], thread[%d],",
			waitQ, thread);
		CMDQ_ERR(" taskCount[%d], pTask[0x%p], pTask->MVABase[0x%08x]\n",
			pThread->taskCount, pTask, pTask->MVABase);
		...
		do {
			...
			CMDQ_ERR("timeout return status[0x%08x] PC[0x%08x] END_ADDR[0x%08x]\n",
				DISP_REG_GET(DISP_REG_CMDQ_THRx_STATUS(thread)),
				DISP_REG_GET(DISP_REG_CMDQ_THRx_PC(thread)),
				DISP_REG_GET(DISP_REG_CMDQ_THRx_END_ADDR(thread)));
			...
			if (TASK_STATE_ERROR == pTask->taskState) {
				/* CMDQ err
					...
					GCE/HW reset haved been fixed, and previous task are done
					so print log only, continuous to other handle */
				CMDQ_ERR("execute error, thread[%d], pTask[0x%p], irqFlag[%x]\n",
					thread, pTask, pTask->irqFlag);
			}
			...
			if ((currPC >= pTask->MVABase) && (currPC <= (pTask->MVABase + pTask->blockSize))) {
				...
				if (-EFAULT == status) {
					...
					spin_unlock_irqrestore(&gCmdqExecLock, flags);
					CMDQ_AEE("MDP",
						"reset hardware engine 0x%08x after sw timeout, thread:%d, task:0x%p\n",
								   ...
								   int32_t	thread)
{
	...
	if (pThread->taskCount <= 0) {
		...
		/*
			...
			in order to prevent double err handle
			(the side effect is inconsistent cookie/thread's task count, PC etc),
			...
			in order to prevent infinite INVALID_INSTN ISR,
			which is casued by resuming thread before correct HW thread PC from invalid instn,
			treat INVALID INSTN err as one type of SW timeout
		*/
		#if 1
		DISP_REG_SET(DISP_REG_CMDQ_THRx_IRQ_FLAG_EN(thread),
			CMDQ_THR_IRQ_FALG_EXEC_CMD |
			CMDQ_THR_IRQ_FALG_INVALID_INSTN);
		#else
		DISP_REG_SET(DISP_REG_CMDQ_THRx_IRQ_FLAG_EN(thread),
			CMDQ_THR_IRQ_FALG_EXEC_CMD |
			CMDQ_THR_IRQ_FALG_INSTN_TIMEOUT |
			CMDQ_THR_IRQ_FALG_INVALID_INSTN);
		#endif
		/* HW thread config */
		...
		CMDQ_ERR("reuse thread[%d] status[0x%08x]\n",
				thread,
				DISP_REG_GET(DISP_REG_CMDQ_THRx_STATUS(thread)));
		if (0 > cmdq_suspend_HW_thread(thread)) {
			spin_unlock_irqrestore(&gCmdqExecLock, flags);
		...
		if ((DISP_REG_GET(DISP_REG_CMDQ_THRx_PC(thread)) ==
			(DISP_REG_GET(DISP_REG_CMDQ_THRx_END_ADDR(thread)) - 8)) ||
		   (DISP_REG_GET(DISP_REG_CMDQ_THRx_PC(thread)) ==
		   (DISP_REG_GET(DISP_REG_CMDQ_THRx_END_ADDR(thread)) - 0))) {
...
void cmdqHandleIRQ(int32_t thread, const uint32_t value)
{
	...
	const uint32_t validIrqFlag = (CMDQ_THR_IRQ_FALG_INVALID_INSTN |
			CMDQ_THR_IRQ_FALG_INSTN_TIMEOUT |
			CMDQ_THR_IRQ_FALG_EXEC_CMD);
	...
	if (false == isAlreadySuspended || isInvalidInstruction) {
		CMDQ_LOG("harry >>> IRQ: thread %d status: %x, start to suspend\n",
	...
	/*
		...
		so do not resume if it is invalid instruction.
