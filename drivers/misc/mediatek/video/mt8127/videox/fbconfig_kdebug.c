...
static long fbconfig_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	int layer_size, layer_enable;
	int layer_dp_size, layer_dp_enable;
	unsigned int kva = 0;
	unsigned int mapped_size = 0;
	unsigned int mva =
	    fbconfig_get_layer_vaddr(global_layer_id, &layer_dp_size, &layer_dp_enable);
	int cc_enable;
	ESD_PARA esd_para;
	int i = 0;
	...
	case TE_SET_ENABLE:
		{
			if (copy_from_user(&te_enable, (void __user *)argp, sizeof(te_enable))) {
			...
			if (fbconfig_if_drv->set_te_enable)
				fbconfig_if_drv->set_te_enable(te_enable);
	...
	case FB_LAYER_GET_SIZE:
		{
			...
			fbconfig_get_layer_vaddr(global_layer_id, &layer_size, &layer_enable);
			if ((layer_size == 0) || (layer_enable == 0))
				return -2;
			else
	...
	case FB_LAYER_DUMP:
		{
			if ((layer_dp_size != 0) && (layer_dp_enable != 0)) {
				pr_info("sxk==>FB_LAYER_DUMP==>layer_size is %d   mva is 0x%x\n",
					layer_dp_size, mva);

				m4u_mva_map_kernel(mva, layer_dp_size, 0, &kva, &mapped_size);
				pr_info("sxk==> addr from user space is 0x%p\n", argp);
	...
	case MIPI_SET_CC:
		{
			if (copy_from_user(&cc_enable, (void __user *)argp, sizeof(cc_enable))) {
			...
			if (fbconfig_if_drv->set_continuous_clock)
				fbconfig_if_drv->set_continuous_clock(cc_enable);
...
const FBCONFIG_DISP_IF *disphal_fbconfig_get_def_if(void)
{
