...
static long __disp_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	return disp_unlocked_ioctl(file, cmd, arg);
}

#ifdef CONFIG_COMPAT
static long __disp_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
static const struct file_operations disp_fops = {
	.owner = THIS_MODULE,
	.unlocked_ioctl = __disp_unlocked_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl = __disp_compat_ioctl,
#endif
	.open = disp_open,
...
static int __init disp_init(void)
{
	...
	pr_info("Register the disp driver\n");
...
MODULE_AUTHOR("Weiqing Kong <weiqing.kong@mediatek.com>");
