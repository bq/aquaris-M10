...
static VAL_INT32_T MT8127Driver_Open_Count;	/* mutex : DriverOpenCountLock */
...
#if defined(MT8127_VENC_USE_L2C)
/*extern int config_L2(int option); */
#endif

#ifdef CONFIG_OF
static struct clk *clk_venc_clk;
/* static struct clk *clk_venc_pwr, *clk_venc_pwr2; */
static struct clk *clk_mm_sel;
#endif
static int venc_enableIRQ(VAL_HW_LOCK_T *prHWLock);
static int venc_disableIRQ(VAL_HW_LOCK_T *prHWLock);
bool vdec_power_on_off(void)
{
	return (bool)gu4VdecPWRCounter;
}
EXPORT_SYMBOL(vdec_power_on_off);
...
void vdec_power_on(void)
{
    ...
    clk_prepare(clk_mm_sel);
    clk_enable(clk_mm_sel);
	MODULE_MFV_LOGD("vdec_power_on D -\n");
#else
...
void vdec_power_off(void)
{
	...
	clk_disable(clk_mm_sel);
	clk_unprepare(clk_mm_sel);
	MODULE_MFV_LOGD("vdec_power_off D -\n");
#else
...
void dec_isr(void)
{
	...
	MODULE_MFV_LOGD("[VCODEC_DEBUG] %s,line =%d\n", __func__, __LINE__);
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#endif
	MODULE_MFV_LOGD("%s,line:%d\n", __func__, __LINE__);
	switch (cmd) {
	case VCODEC_SET_THREAD_ID:
		MODULE_MFV_LOGE("[8127] VCODEC_SET_THREAD_ID [EMPTY] + tid = %d\n", current->pid);
	...
	case VCODEC_ALLOC_NON_CACHE_BUFFER:
		{
			MODULE_MFV_LOGE("[8127][M4U]! %s,line:%d VCODEC_ALLOC_NON_CACHE_BUFFER + tid = %d\n",
			...
			if (ret) {
				MODULE_MFV_LOGE
				    ("[ERROR] %s,line:%d VCODEC_ALLOC_NON_CACHE_BUFFER, copy_from_user failed: %lu\n",
			...
			MODULE_MFV_LOGD("%s,line:%d, kernel va = 0x%lx, kernel pa = 0x%lx, memory size = %lu\n", __func__, __LINE__,
			...
			MODULE_MFV_LOGE("[8127][M4U]! VCODEC_ALLOC_NON_CACHE_BUFFER - tid = %d\n",
				 current->pid);
		}
		break;
	...
	case VCODEC_FREE_NON_CACHE_BUFFER:
		{
			MODULE_MFV_LOGD("[8127][M4U]! VCODEC_FREE_NON_CACHE_BUFFER + tid = %d\n",
				 current->pid);
			...
			MODULE_MFV_LOGE("[8127][M4U]! VCODEC_FREE_NON_CACHE_BUFFER - tid = %d\n",
				 current->pid);
		}
		break;

	case VCODEC_INC_DEC_EMI_USER:
		MODULE_MFV_LOGD("[8127] %s,line:%d, VCODEC_INC_DEC_EMI_USER + tid = %d\n",
			__func__, __LINE__, current->pid);
	...
	case VCODEC_DEC_DEC_EMI_USER:
		MODULE_MFV_LOGD("[8127] VCODEC_DEC_DEC_EMI_USER + tid = %d\n", current->pid);
	...
	case VCODEC_INC_ENC_EMI_USER:
		MODULE_MFV_LOGD("[8127] VCODEC_INC_ENC_EMI_USER + tid = %d\n", current->pid);
	...
	case VCODEC_DEC_ENC_EMI_USER:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_DEC_ENC_EMI_USER + tid = %d\n", current->pid);
	...
	case VCODEC_LOCKHW:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_LOCKHW + tid = %d\n", current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
			...
			if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
				...
				while (bLockedHW == VAL_FALSE) {
					...
					if (grVcodecDecHWLock.pvHandle == 0) {	/* No one holds dec hw lock now */
						...
						MODULE_MFV_LOGD("[NOT ERROR][VCODEC_LOCKHW] E\n");
	...
	case VCODEC_UNLOCKHW:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_UNLOCKHW + tid = %d\n", current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
			ret = copy_from_user(&rHWLock, user_data_addr, sizeof(VAL_HW_LOCK_T));
			if (ret) {
	...
	case VCODEC_INC_PWR_USER:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_INC_PWR_USER + tid = %d\n", current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
	...
	case VCODEC_DEC_PWR_USER:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_DEC_PWR_USER + tid = %d\n", current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
			ret = copy_from_user(&rPowerParam, user_data_addr, sizeof(VAL_POWER_T));
			if (ret) {
				MODULE_MFV_LOGE
	...
	case VCODEC_WAITISR:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_WAITISR + tid = %d\n", current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
	...
	case VCODEC_INITHWLOCK:
		{
			MODULE_MFV_LOGE("[8127] VCODEC_INITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
	...
	case VCODEC_DEINITHWLOCK:
		{
			MODULE_MFV_LOGE("[8127] VCODEC_DEINITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
	...
	case VCODEC_GET_CPU_LOADING_INFO:
		{
			...
			MODULE_MFV_LOGD("[8127] VCODEC_GET_CPU_LOADING_INFO +\n");
			user_data_addr = (VAL_UINT8_T *) arg;
			/* TODO: */
	...
	case VCODEC_GET_CORE_LOADING:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_GET_CORE_LOADING + - tid = %d\n", current->pid);
	...
	case VCODEC_GET_CORE_NUMBER:
		{
			MODULE_MFV_LOGD("[8127] VCODEC_GET_CORE_NUMBER + - tid = %d\n", current->pid);
	...
	case VCODEC_SET_CPU_OPP_LIMIT:
		{
			MODULE_MFV_LOGE("[8127] VCODEC_SET_CPU_OPP_LIMIT [EMPTY] + - tid = %d\n",
				 current->pid);
			user_data_addr = (VAL_UINT8_T *) arg;
			ret =
...
static int vcodec_open(struct inode *inode, struct file *file)
{
	...
	MODULE_MFV_LOGE("vcodec_open pid = %d, MT8127Driver_Open_Count %d\n", current->pid,
		 MT8127Driver_Open_Count);
...
static int vcodec_flush(struct file *file, fl_owner_t id)
{
	...
	MODULE_MFV_LOGE("vcodec_flush pid = %d, MT8127Driver_Open_Count %d\n",
...
/*In the futuer may be we need this feature*/
static int vdec_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data);
static struct notifier_block vdec_fb_notifier = {
	.notifier_call = vdec_fb_notifier_callback,
};

static void vcodec_early_suspend(void)
{
	mutex_lock(&PWRLock);
...
static void vcodec_late_resume(void)
{
	mutex_lock(&PWRLock);
...
static int vdec_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	static struct fb_event *evdata = data;
	static int blank;

	blank = *(int *)evdata->data;
	MODULE_MFV_LOGE("vdec_fb_notifier_callback %ld, %d\n", event , FB_EVENT_BLANK);
...
#ifdef CONFIG_OF
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt8127-venc");
	clk_venc_clk = of_clk_get_by_name(node, "venc");
	if (IS_ERR(clk_venc_clk))
		MODULE_MFV_LOGE("+vcodec_probe get VENC clk error!\n");
    clk_mm_sel = devm_clk_get(&pdev->dev, "mm_sel");
    if (IS_ERR(clk_mm_sel)) {
	    dev_err(&pdev->dev, "get mm_sel clock fail: %ld\n",
			PTR_ERR(clk_mm_sel));
	    return PTR_ERR(clk_mm_sel);
	}
}
#endif
...
static const struct of_device_id vcodec_of_ids[] = {
	{.compatible = "mediatek,mt8127-vdecsys",},
	{}
};
...
static int __init vcodec_driver_init(void)
{
	...
	KVA_VENC_SW_HRST_N = KVA_VENC_BASE + VENC_SW_HRST_N;
	{
		...
		node = of_find_compatible_node(NULL, NULL, "mediatek,mt8127-vdec");
	...
	}
	{
		...
		node = of_find_compatible_node(NULL, NULL, "mediatek,mt8127-vdecsys");
		KVA_VDEC_GCON_BASE = (VAL_ULONG_T) of_iomap(node, 0);
		MODULE_MFV_LOGD
		    ("[DeviceTree] %s,line:%d KVA_VENC_BASE(0x%lx), KVA_VDEC_BASE(0x%lx), KVA_VDEC_GCON_BASE(0x%lx)",
		      __func__, __LINE__, KVA_VENC_BASE, KVA_VDEC_BASE, KVA_VDEC_GCON_BASE);
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	if (fb_register_client(&vdec_fb_notifier))
		MODULE_MFV_LOGE("@%s: line:%d,register FB client failed!\n", __func__, __LINE__);
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	fb_unregister_client(&vdec_fb_notifier);
#endif
...
MODULE_DESCRIPTION("8127 Vcodec Driver");
MODULE_LICENSE("GPL");
