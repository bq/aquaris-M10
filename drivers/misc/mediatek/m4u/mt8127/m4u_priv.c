...
#define M4ULOG(string, args...)	pr_warn("M4U[pid=%d]"string, current->tgid, ##args)
...
/* temporary disable the TEE in m4u since to walk around translation fault problem*/
...
#define M4U_MVA_DBUG(string, args...)	pr_debug("[M4U_K][MVA]"string, ##args)
...
static unsigned int m4u_index_of_larb[SMI_LARB_NR + 1] = { 0, 0, 0, 0 };
static unsigned int smi_port0_in_larbx[SMI_LARB_NR + 1] = { 0, 10, 17, 34 };
static unsigned int m4u_port0_in_larbx[SMI_LARB_NR + 1] = { 0, 10, 17, 34 };
...
typedef enum {
	...
	/*
	 * stricker verification ,may use M4UERR instead M4UMSG sometimes,
	...
	if (L2_en) {
		if (isInvAll) {
			...
			while (!
				...
				if (i++ >= 10000) {
					pr_err_ratelimited("m4u: invalid_tlb_all could not get the F_L2_GDC_ST_EVENT, break\n");
					break;
				}
			...
			while (!
				...
				if (i++ >= 10000) {
					pr_err_ratelimited("m4u: invalid_tlb ould not get the RANGE_INV_DONE, break\n");
					break;
				}
	...
	while (pListHead != &(pNode->mvaList)) {
		...
		if (pList->mvaStart != 0) {
			...
			if (!(pList->flags & MVA_REGION_REGISTER)) {
				M4UERR
				    ("warning: in garbage reclaim: mva==0, but MVA_REGION_REGISTER is not set!! ");
				M4UERR("flag=0x%x\n", pList->flags);
			}
		}
	}
...
static long MTK_M4U_ioctl(struct file *a_pstFile, unsigned int a_Command, unsigned long a_Param)
{
	...
	case MTK_M4U_T_DEALLOC_MVA_SEC:
		{
			...
			M4UDBG
			    ("MTK_M4U_T_DEALLOC_MVA_SEC, eModuleID:%d, VABuf:0x%x, Length:%d, MVAStart=0x%x \r\n",
			     m4u_module.eModuleID, m4u_module.BufAddr, m4u_module.BufSize,
			     m4u_module.MVAStart);
	...
	case MTK_M4U_T_DUMP_PAGETABLE:
		do {
			...
			M4UMSG("=========  compare these automatically =======>\n");
			for (i = 0; i < page_num; i++) {
				...
				/* Fix build error. self-comparison always evaluates to false */
				#if 0
...
static irqreturn_t MTK_M4U_isr(int irq, void *dev_id)
{
	...
	/* now we only have one m4u in mt8127, do not need to calc the m4u index. */
	m4u_index = 0;
...

		{
			...
			m4u_aee_print
			    ("\nCRDISPATCH_KEY:%s\ntranslation fault: larb=%d,module=%s,port=%s,mva=0x%x\n",
			     m4u_get_port_name(m4u_port), larbID,
		...
		/* search invalid main tlb */
		{
			...
			for (i = 0; i < M4U_MAIN_TLB_NR; i++) {
				...
				if ((mva & (F_MAIN_TLB_VALID_BIT | F_MAIN_TLB_INV_DES_BIT))
					...
					M4UINFO(KERN_INFO "%d:0x%x, 0x%x", i,
		...
		/* search invalid pfh tlb */
		{
			...
			for (i = 0; i < M4U_MAIN_TLB_NR; i++) {
				...
				if ((tag_valid & des_invalid) != 0) {
					M4UINFO("%d:0x%x, 0x%x 0x%x 0x%x 0x%x", i, tag,
...
int m4u_parse_dt(struct platform_device *pdev, struct m4u_device *m4udev)
{
	...
	gM4uDev->bclk = devm_clk_get(&pdev->dev, "bclk");
	/*
	 * async: asynchronous clock.
	 * It help transform the smi clock into the emi clock domain.
	 */
	gM4uDev->async = devm_clk_get(&pdev->dev, "async");
	if (IS_ERR(gM4uDev->bclk) || IS_ERR(gM4uDev->async)) {
...
static int m4u_probe(struct platform_device *pdev)
{
	...
	/* Set IRQ */
	if (request_irq(gM4uDev->irq_num[0], MTK_M4U_isr, IRQF_TRIGGER_NONE, M4U_DEVNAME, NULL)) {
...
static int m4u_clock_on(void)
{
	...
	ret = clk_prepare_enable(gM4uDev->bclk); /*never close it*/
	if (ret)
		M4UERR("clk_enable bclk error\n");

	ret = clk_prepare_enable(gM4uDev->async); /*never close it*/
	if (ret)
		M4UERR("clk_enable aync error\n");


	M4UMSG("clk_enable all success\n");
	return 0;
}
...
#endif
#endif
	clk_disable_unprepare(gM4uDev->bclk);
	clk_disable_unprepare(gM4uDev->async);
				   ...
				   unsigned int mvaRegionAddr, struct sg_table *sg_table)
{
	...
	{
		/* get offset in the page table */
		pteStart = (unsigned int)mva_pteAddr_nonsec(mvaRegionAddr);
		pteNr =
			      ...
			      struct sg_table *sg_table)
{
	...
	for (i = 0; i < page_num; i++) {
		...
		if ((pa & (PAGE_SIZE - 1)) && ((pa & (PAGE_SIZE - 1)) != 0x20)) {
			pa_invalid = 1;
			mutex_unlock(&gM4uMutex);
			vfree(pPhys);
			return 0;
...
int __m4u_alloc_mva(mva_info_t *pMvaInfo, struct sg_table *sg_table)
{
	...
	if (page_num == 0) {
	 ...
	 if (pa_invalid) {
		pa_invalid = 0;
		M4UMSG("pa invalid, alloc_mva error: id=%s, addr=0x%08x, size=%d, sec=%d, mva = 0x%x\n",
			m4u_get_module_name(eModuleID), BufAddr, BufSize, security, mvaStart);
		goto error_alloc_mva;
	}
	/*
	 * for m4u bug in mt6589: we need our entry_start and entry_end align at
	 * 128 bits (pfh tlb is 128bits align)
	 * this aims to ensure that no invalid tlb will be fetched into tlb
	 * TODO: now prefetch_distance is 1 for most engine. if prefetch is not 1,
		      ...
		      const unsigned int BufSize, const unsigned int MVA, struct sg_table *sg_table)
{
	...
	/* if(eModuleID!=M4U_CLNTMOD_RDMA_GENERAL && eModuleID!=M4U_CLNTMOD_ROT_GENERAL) */
	{
		if (m4u_invalid_seq_range_by_mva
			...
			M4UMSG("warning: dealloc mva without invalid tlb range!! ");
			M4UMSG("id=%s,add=0x%x,size=0x%x,mva=0x%x\n",
	...
	for (i = m4u_index_offset; i < m4u_index_offset + SEQ_RANGE_NUM; i++) {
		if (1 == pRangeDes[i].Enabled) {
			if (MVAEnd < pRangeDes[i].MVAStart || MVAStart > pRangeDes[i].MVAEnd) {	/* no overlap */
				...
				M4UMSG
				    ("error: insert tlb range overlapped, current process=%s,!\n",
				     current->comm);
...
/* ======================================== */
	...
	if (RangeReg_ID == M4U_INVALID_ID) {
		...
		mutex_unlock(&gM4uMutex);
		/* do not need return erro, nothing will happen even insert tlb range fails */
		return 0;
	}
/* ====================================================== */
...
retry:
		...
		if ((start_pa & (PAGE_SIZE - 1)) != (BufAddr & (PAGE_SIZE - 1))) {
			vma = find_vma(current->mm, BufAddr);
			handle_mm_fault(current->mm, vma, BufAddr,
					(vma->vm_flags & VM_WRITE) ? FAULT_FLAG_WRITE : 0);
			M4UMSG("m4u user v2p failed, handle mm fault\n");
			if (retry_count++ < 3000)
				goto retry;
		}
		/*
		 ...
		 */
			...
			if ((vma->vm_flags) & VM_PFNMAP) {
				...
				if (bufEnd > vma->vm_end) {
					M4UMSG
					    ("error: page_num=%d,module=%s, va=0x%x, size=0x%x, vm_flag=0x%lx\n",
				...
				M4UINFO
				    ("alloc_mva VM_PFNMAP module=%s, va=0x%x, size=0x%x, vm_flag=0x%lx\n",
				     m4u_get_module_name(eModuleID), BufAddr, BufSize,
				     vma->vm_flags);
				up_read(&current->mm->mmap_sem);
			} else {
				...
				ret =
				    m4u_get_user_pages(eModuleID, current, current->mm, BufAddr,
						       page_num, write_mode,
						       0,	/* force */
				...
				if (ret < page_num) {
					...
					if (unlikely(fatal_signal_pending(current))) {
						M4UMSG
						    ("error: receive sigkill during get_user_pages(),  ");
						M4UMSG
						    ("page_num=%d, return=%d, module=%s, current_process:%s\n",
						     page_num, ret, m4u_get_module_name(eModuleID),
						     current->comm);
						return -EFAULT;
					}
					if (ret > 0) {
						/* smaller than expected, trigger red screen */
						M4UMSG
						    ("epage_num=%d, ret=%d, module=%s, process:%s\n",
						     page_num, ret,
						     m4u_get_module_name(eModuleID),
						     current->comm);
						M4UMSG
						    ("allocated VA size smaller than size of m4u_alloc_mva()!");
						/* maybe the buffer is not exist, return error */
					} else {

						M4UMSG
						    ("page_num=%d, ret=%d, module=%s, process:%s\n",
						     page_num, ret,
						     m4u_get_module_name(eModuleID),
						     current->comm);
						M4UMSG
						    ("VA is deallocated or no VA has be allocated!");
					}
					m4u_dump_maps(BufAddr);
			     ...
			     unsigned int BufSize, unsigned int MVA, struct sg_table *sg_table)
{
	...
	if (BufAddr < PAGE_OFFSET && sg_table == NULL) {	/* from user space */
		...
		for (i = 0; i < page_num; i++) {
			...
			if ((start_pa & 0x02) == 0)
				...
				if (page_count(page) > 0) {
					...
					tmp = m4u_put_unlock_page(page);
					if (tmp) {
						M4UINFO("warning: put_unlock_page fail ");
						M4UINFO("(page is unlocked before put page)");
						M4UINFO("module=%s, va=0x%x, size=0x%x,mva=0x%x\n",
						 m4u_get_module_name(eModuleID), BufAddr,
						 BufSize, MVA);
						M4UINFO("i=%d (%d)\n", i, page_num);
					}
				} else {
					M4UMSG("page_count is 0: pfn=%ld\n", page_to_pfn(page));
					/* dump_page(page, "page_count is 0"); */
				}
		...
		if (put_page_err == M4U_ERR_PAGE_UNLOCKED) {
			M4UMSG("warning: in m4u_release_page: ");
			M4UMSG
			    ("module=%s, va=0x%x, size=0x%x,mva=0x%x (page is unlocked before put page)\n",
			     m4u_get_module_name(eModuleID), BufAddr, BufSize, MVA);
		}
	}
...
RELEASE_FINISH:
	...
	if (pmodule_current_size[eModuleID] < BufSize) {
		...
		M4UMSG("error pmodule_current_size is less than BufSize, ");
		M4UMSG("module=%s, current_size=%d, BufSize=%d\n",
...
int m4u_do_dma_cache_maint(M4U_MODULE_ID_ENUM eModuleID, const void *va, size_t size, int direction)
{
	...
	MMProfileLogEx(M4U_MMP_Events[PROFILE_DMA_MAINT_ALL], MMProfileFlagStart, (unsigned int)va,
		       size);
	...
	M4ULOG("cache_sync: module=%s, addr=0x%p, size=0x%x\n", m4u_get_module_name(eModuleID),
	       va, (unsigned int)size);
...
struct page *m4u_alloc_pages()
{
	...
	/* frame buffer pa is > 0xf0000000 */
	/*
	 * somehow the pa will be in the range of 1~4G in when boot, so keep the pagetables.
...
void m4u_dma_cache_by_range(unsigned int BufAddr, unsigned int BufSize, unsigned int mvaRegionAddr)
{
	...
	pteNr =
	    ((BufSize + (BufAddr & 0xfff)) / DEFAULT_PAGE_SIZE) + (((BufAddr + BufSize) & 0xfff) !=
								   0);
	pteNr += align_page_num;
...
/* ======= alloc pagetable done======================= */
	...
	pProtectVA =
	    kmalloc(TF_PROTECT_BUFFER_SIZE * TOTAL_M4U_NUM + M4U_PROTECT_BUF_OFFSET,
...
/* =============================== */

	{
		...
		for (i = 0; i < SMI_LARB_NR; i++) {
			...
			unsigned int larb_base = 0;

			larb_base = mtk_smi_larb_get_base(i);
...
#endif
			if (regval)
				M4UMSG("%s(%d), ", m4u_get_port_name(i), i);
		}
		M4UMSG("\n");
	}
#endif
