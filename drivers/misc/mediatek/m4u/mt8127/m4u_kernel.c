...
#define MTK_M4U_MSG
#ifdef MTK_M4U_MSG
#define M4UMSG(string, args...)	pr_debug("M4U_K[pid=%d]"string, current->tgid, ##args)
		...
		do {
			...
			while (!page) {
				...
				if (ret & VM_FAULT_ERROR) {
					if (ret & VM_FAULT_OOM) {
						...
						M4UMSG
						    ("aaddr:0x%08lx (%d pages are allocated), module=%d\n",
						     start, i, eModuleID);
						/* m4u_dump_maps(start); */
						return i ? i : -ENOMEM;
					}
					...
					if (ret & (VM_FAULT_HWPOISON | VM_FAULT_SIGBUS)) {
						M4UMSG
						    ("handle_mm_fault() error: invalid memory address, ");
						M4UMSG
						    ("vaddr:0x%lx (%d pages are allocated), module=%d\n",
						     start, i, eModuleID);
						/* m4u_dump_maps(start); */
						return i ? i : -EFAULT;
					}
			...
			if (pages) {
				...
				do {
					...
					if (trycnt == 30000)
						M4UMSG
						    ("cannot handle_mm_fault to get pte: va=0x%lx\n",
						     start);
					break;
