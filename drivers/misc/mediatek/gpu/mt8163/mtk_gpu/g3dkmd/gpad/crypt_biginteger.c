...
#ifdef __KERNEL__
#define _DEBUGPRINT(level, fmt, args...) printk(level fmt, ##args)
#define DEBUGPRINT(fmt, args...) _DEBUGPRINT(KERN_ERR, fmt, ##args)
#else
...
VOID BigInteger_Init(PBIG_INTEGER *pBI)
{
	...
	*pBI = (PBIG_INTEGER) kmalloc(sizeof(BIG_INTEGER), GFP_ATOMIC);
	if (*pBI == NULL) {
...
VOID BigInteger_AllocSize(PBIG_INTEGER *pBI, UINT Length)
{
	...
	if ((*pBI)->pIntegerArray == NULL) {
		(*pBI)->pIntegerArray = kmalloc(sizeof(UINT32)*ArrayLength, GFP_ATOMIC);
		if ((*pBI)->pIntegerArray == NULL) {
...
VOID BigInteger_ClearHighBits(PBIG_INTEGER pBI)
{
	...
	if ((pBI == NULL) || (pBI->pIntegerArray == NULL) || (pBI->ArrayLength < 0))
		return;
...
VOID BigInteger_BitsOfBI(PBIG_INTEGER pBI, UINT *Bits_Of_P)
{
	...
	*Bits_Of_P = (pBI->ArrayLength*sizeof(UINT32)*8) - Index;
	...
	if (pFirstOperand->IntegerLength == pSecondOperand->IntegerLength) {
		for (BIArrayIndex = (pFirstOperand->ArrayLength - 1); BIArrayIndex >= 0; BIArrayIndex--) {
			if (pFirstOperand->pIntegerArray[BIArrayIndex] > pSecondOperand->pIntegerArray[BIArrayIndex])
				return 1;
			else if (pFirstOperand->pIntegerArray[BIArrayIndex] <
...
VOID BigInteger_Sub(PBIG_INTEGER pFirstOperand, PBIG_INTEGER pSecondOperand, PBIG_INTEGER *pBI_Result)
{
	...
	if ((pFirstOperand->Signed * pSecondOperand->Signed) > 0) {
		...
		for (BIArrayIndex = 0; BIArrayIndex < (*pBI_Result)->ArrayLength; BIArrayIndex++) {
			if (BIArrayIndex < pTempBI2->ArrayLength) {
				if ((*pBI_Result)->pIntegerArray[BIArrayIndex] >=
						(pTempBI2->pIntegerArray[BIArrayIndex] - Carry)) {
					...
					(*pBI_Result)->pIntegerArray[BIArrayIndex] =
						0xffffffffUL - pTempBI2->pIntegerArray[BIArrayIndex] - Carry
						+ (*pBI_Result)->pIntegerArray[BIArrayIndex] + 1;
					Carry = 1;
				} /* End of if */
			} else {
				if ((*pBI_Result)->pIntegerArray[BIArrayIndex] >= Carry) {
					...
					(*pBI_Result)->pIntegerArray[BIArrayIndex] = 0xffffffffUL - Carry - 1;
					Carry = 1;
				} /* End of if */
			} /* End of if */
		} /* End of for */
		...
		if  (((pFirstOperand->Signed == 1) & (pSecondOperand->Signed == 1) & (CompareResult == -1))
				|| ((pFirstOperand->Signed == -1) & (pSecondOperand->Signed == -1)
					& (CompareResult == 1)))
			(*pBI_Result)->Signed = -1;
		...
		if (FirstValue == 0) {
			...
			for (BISecondIndex = 0; BISecondIndex < pSecondOperand->ArrayLength; BISecondIndex++) {
				...
				Sum = (UINT64) ((*pBI_Result)->pIntegerArray[BIFirstIndex + BISecondIndex]
						+ SecondValue + Carry);
				Carry = Sum >> 32;
				(*pBI_Result)->pIntegerArray[BIFirstIndex + BISecondIndex] =
...
VOID BigInteger_Div(PBIG_INTEGER pFirstOperand, PBIG_INTEGER pSecondOperand, PBIG_INTEGER *pBI_Result, PBIG_INTEGER *pBI_Remainder)
{
	...
	for (Index = (INT) ComputeSize; Index >= 0; Index--) {
		...
		if (((pSecondOperand->IntegerLength + Index) - (*pBI_Remainder)->IntegerLength) <= 1) {
			...
			ShiftIndex--;
			MulStart = 0;
			if (Index != ComputeSize) {
				MulStart = (BigInteger_GetByteValue((*pBI_Remainder), pFirstOperand->IntegerLength + Index
							- ComputeSize + 1) & 0xFF) << 8;
			}
			...
			MulStart = MulStart / (UINT32) (SecondHighByte+1);
			...
			for (MulIndex = (INT) MulStart; MulIndex <= 0x101; MulIndex++) { /* 0xFFFF / 0xFF = 0x101 */
				...
				if (CompareResult < 0 && MulIndex == 0x100 && ShiftIndex == 3)
					pMulBI->pIntegerArray[ArrayIndex + 1] = 0;

				if (CompareResult < 1 && MulIndex > 1) {
					if (CompareResult != 0)
						pMulBI->pIntegerArray[ArrayIndex] =
							((UINT) (MulIndex - 1) << (8*ShiftIndex));
...
VOID BigInteger_Montgomery_Reduction(PBIG_INTEGER pBI_A, PBIG_INTEGER pBI_P, PBIG_INTEGER pBI_R, PBIG_INTEGER *pBI_Result)
{
	...
	BigInteger_BitsOfBI(pBI_R, &Bits_Of_R);
...
VOID BigInteger_Montgomery_ExpMod(PBIG_INTEGER pBI_G, PBIG_INTEGER pBI_E, PBIG_INTEGER pBI_P, PBIG_INTEGER *pBI_Result)
{
	...
	if (NdisCmpMemory(pBI_P->pIntegerArray, pBI_Temp1->pIntegerArray, pBI_P->IntegerLength) == 0) {
		...
		pRValue = kmalloc(sizeof(UINT8)*AllocLength, GFP_ATOMIC);
		if (pRValue == NULL) {
...
{
	INT    Bits_Of_E, i;
	UINT32  AllocLength, E;
	PBIG_INTEGER pBI_Temp1 = NULL, pBI_Temp2 = NULL;

	AllocLength = (pBI_G->IntegerLength + pBI_P->IntegerLength + 300);
	BigInteger_AllocSize(&pBI_Temp1, AllocLength);
	BigInteger_AllocSize(&pBI_Temp2, AllocLength);

	if (pBI_E->IntegerLength > 4) {
		DEBUGPRINT("BigInteger_ExpMod_32: Invalid E\n");
		goto memory_free;
	} /* End of if */

	E = pBI_E->pIntegerArray[0];

	for (Bits_Of_E = 31; Bits_Of_E >= 0; Bits_Of_E--)
		if (0 != (E & (0x1 << Bits_Of_E)))
			break;


	BigInteger_Div(pBI_G, pBI_P, &pBI_Temp2, pBI_Result);
	for (i = Bits_Of_E-1; i >= 0; i--) {
		BigInteger_Square(*pBI_Result, &pBI_Temp1);
		BigInteger_Div(pBI_Temp1, pBI_P, &pBI_Temp2, pBI_Result);

		if (0 != (E & (0x1 << i))) {
			BigInteger_Mul(*pBI_Result, pBI_G, &pBI_Temp1);
			BigInteger_Div(pBI_Temp1, pBI_P, &pBI_Temp2, pBI_Result);
		}
	}
