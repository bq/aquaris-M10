...
#if 0 /*TEMP disable checking */
	
	u32 mali_mfg_id = readl(clk_mfgcfg_base + 0x20);
	pr_warn("Mali id %#x, mfg id %#x\n", regdump.gpu_id, mali_mfg_id);
	if(regdump.gpu_id == 0) {
		if(mali_mfg_id == 0x2) {
			pr_warn(" Mali GPU id is 0 and mfg id is 2.. Check clk\n");
		} else {
			pr_warn(" Mali GPU id is 0 and mfg id is %#x, HW is inhouse GPU ???\n", mali_mfg_id);
		}
	}
#endif
