...
static int parser_standard_timing(int hsize, int vsize, int vrefresh_rate)
{
	int stdtiming = -1;

	HDMI_EDID_LOG("h=%d,v=%d,vf=%d\n", hsize, vsize, vrefresh_rate);
	/* mtk hdmi supports aspect ratio in 16:9 only */
	if ((vsize != (hsize * 9) / 16) || (vrefresh_rate > 60))
		return stdtiming;
	...
	case 720: /* 720P */
		if (vrefresh_rate == 50)
			stdtiming = SINK_720P50;
		else if (vrefresh_rate == 60)
			stdtiming = SINK_720P60;
		break;
	case 1080: /* 1080P */
		if (vrefresh_rate == 24)
			stdtiming = SINK_1080P24;
		else if (vrefresh_rate == 25)
			stdtiming = SINK_1080P25;
		else if (vrefresh_rate == 30)
			stdtiming = SINK_1080P30;
		else if (vrefresh_rate == 50)
			stdtiming = SINK_1080P50;
		else if (vrefresh_rate == 60)
			stdtiming = SINK_1080P60;
		break;
	default:
		break;
	}
...
static int add_standard_timings(unsigned char *prbData)
{
	int i, ret, hsize, vsize, vrefresh_rate;
	unsigned aspect_ratio, vfreq;
	unsigned int stdtiming = 0;
	unsigned char *std_base = prbData + 0x26;

	HDMI_EDID_FUNC();

	for (i = 0; i < 8; i++) {
		aspect_ratio = ((*(std_base + 1)) & (0x03 << 6)) >> 6;
		vfreq = ((*(std_base + 1)) & 0x3f);

		if (((*std_base) == 0x00 && (*(std_base + 1)) == 0x00) ||
			((*std_base) == 0x01 && (*(std_base + 1)) == 0x01) ||
			((*std_base) == 0x20 && (*(std_base + 1)) == 0x20)) {
			std_base = std_base + 2;
			continue;
		}

		/* According to the EDID spec, the hdisplay = hsize * 8 + 248 */
		hsize = (*std_base) * 8 + 248;
		...
		ret = parser_standard_timing(hsize, vsize, vrefresh_rate);
		if (ret >= 0)
			stdtiming |= ret;
		std_base = std_base + 2;
	}

	_HdmiSinkAvCap.ui4_sink_dtd_ntsc_resolution |= stdtiming;
	_HdmiSinkAvCap.ui4_sink_dtd_pal_resolution |= stdtiming;
	_HdmiSinkAvCap.ui4_sink_1st_dtd_ntsc_resolution |= stdtiming;
	_HdmiSinkAvCap.ui4_sink_1st_dtd_pal_resolution |= stdtiming;
	return 0;
}

static int add_standard_timings_dvi(unsigned char *prbData)
{
	HDMI_EDID_FUNC();

	/* add standard timing for dvi device only */
	if (*(prbData + 0x7e) != 0)
		return 0;

	add_standard_timings(prbData);
	return 0;
}
...
unsigned char fgParserEDID(unsigned char *prbData)
{
	...
	add_standard_timings_dvi(prbData);
