...
const char *hdmi_use_module_name_spy(enum HDMI_REF_MODULE_ENUM module, const char *compatible)
{
	if (!strncmp(compatible, "mediatek,mt8127-disp-hdmi", strlen("mediatek,mt8127-disp-hdmi"))) {
		switch (module) {
		case AP_CCIF0:
	...
	/*	case PERISYS_REG:
		...
		switch (module) {
		case AP_CCIF0:
			return "mediatek,mt7623-apmixedsys";	/* TVD//PLL */
		case TOPCK_GEN:
			return "mediatek,mt7623-topckgen";
		case INFRA_SYS:
			return "mediatek,mt7623-infracfg";
		case MMSYS_CONFIG:
			return "mediatek,mt7623-mmsys";
		case GPIO_REG:
			return "mediatek,mt7623-pinctrl";
		case DPI1_REG:
			return "mediatek,mt8127-disp-dpi1";
	/*	case EFUSE_REG:
			return "mediatek,mt8127-disp_config2"; */
		case GIC_REG:
			return "mediatek,mt7623-mcucfg";
	/*	case PERISYS_REG:
			return "mediatek,PERICFG"; */
...
int hdmi_internal_power_on(void)
{
	...
	vWriteHdmiSYSMsk(HDMI_SYS_CFG110, ENABLE_SPDIFCLK, DISABLE_SPDIFCLK);
#if defined(MHL_BRIDGE_SUPPORT)
...
#endif
	vWriteHdmiSYSMsk(HDMI_SYS_CFG110, DISABLE_SPDIFCLK, DISABLE_SPDIFCLK);
...
void hdmi_timer_impl(void)
{
	if (hdmi_hdmiinit == 0) {
		...
		vInitAvInfoVar();
#if 0
		if (request_irq(MT_CEC_IRQ_ID, hdmi_irq_handler, IRQF_TRIGGER_LOW, "mtkhdmi", NULL)
		    < 0) {
			RETNULL(1);
		}
#endif
...
static struct of_device_id hdmi_match_types[] = {
	{
		.compatible = "mediatek,mt7623-disp-hdmi",
	}, {
...
int hdmi_internal_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < HDMI_REF_REG_NUM; i++) {
		np = of_find_compatible_node(NULL, NULL, hdmi_use_module_name_spy(i, match->compatible));
		if (np == NULL)
			continue;
