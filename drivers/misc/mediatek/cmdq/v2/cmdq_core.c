					...
					const TaskStruct **pOutNGTask)
{
	...
	/*  */
	CMDQ_ERR("================= [CMDQ] Begin of Error %d trigger(%lld)================\n",
		 gCmdqContext.errNum,
		pTask->trigger);
	...
	CMDQ_ERR("================= [CMDQ] End of Error %d trigger(%lld)================\n",
		 gCmdqContext.errNum,
		pTask->trigger);
	gCmdqContext.errNum++;
...
static int32_t cmdq_core_wait_task_done_with_timeout_impl(TaskStruct *pTask, int32_t thread)
{
	...
	while (0 == waitQ && retryCount < CMDQ_PREDUMP_RETRY_COUNT) {
		CMDQ_LOG("=============== [CMDQ] SW timeout trigger(%lld) Pre-dump(%d)===============\n",
			pTask->trigger,
			 retryCount);
	...
	if (pTask) {
		...
		if (finishCallback) {
			...
			if (0 > finishCallback(userData)) {
				...
				CMDQ_LOG("======TASK 0x%p trigger(%lld) command (%d) START\n", pTask,
					pTask->trigger,
					 commandSize);
				if (pCmd != NULL)
