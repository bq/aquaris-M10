...
#include <linux/clk.h>
struct clk *usbmain_clk;
struct clk *usbmcu_clk;
struct clk *usbslv_clk;
#endif
#include "usb20.h"
...
static int mt_usb_dts_probe(struct platform_device *pdev)
{
...
#ifndef CONFIG_MTK_CLKMGR
	usbmain_clk = devm_clk_get(&pdev->dev, "usb0_main");
	if (IS_ERR(usbmain_clk)) {
		DBG(0, KERN_WARNING "cannot get usb0_main clock\n");
		return PTR_ERR(usbmain_clk);
	}
	DBG(0, KERN_WARNING "get usbmcu clock ok, prepare it\n");
	retval = clk_prepare(usbmain_clk);
	if (retval == 0) {
	...
	usbmcu_clk = devm_clk_get(&pdev->dev, "usb0_mcu");
	if (IS_ERR(usbmcu_clk)) {
		DBG(0, KERN_WARNING "cannot get usbmcu_clk clock\n");
	...
	usbslv_clk = devm_clk_get(&pdev->dev, "usb_slv");
	if (IS_ERR(usbslv_clk)) {
		DBG(0, KERN_WARNING "cannot get usbslv_clk clock\n");
		return PTR_ERR(usbslv_clk);
	}
	DBG(0, KERN_WARNING "get usbslv_clk clock ok, prepare it\n");
	retval = clk_prepare(usbslv_clk);
	if (retval == 0) {
		DBG(0, KERN_WARNING "prepare done\n");
	} else {
		DBG(0, KERN_WARNING "prepare fail\n");
