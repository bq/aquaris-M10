...
#include "../../base/power/mt8127/mt_cpufreq.h"
...
#define THERMAL_DEVICE_NAME	"mediatek,mt8127-thermal"
...
static irqreturn_t thermal_interrupt_handler(int irq, void *dev_id)
{
	...
	/* tscpu_dprintk("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n"); */
	tscpu_dprintk("thermal_interrupt_handler,ret=0x%08x\n", ret);
	/* tscpu_dprintk("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n"); */
...
static void thermal_reset_and_initial(void)
{
	...
	tscpu_dprintk("[Reset and init thermal controller]\n");
	...
	*/
	...
	while (cnt < 30) {
		...
		tscpu_dprintk("TEMPMSRCTL1 = 0x%x\n", temp2);
		/*
...
static void thermal_cal_prepare(void)
{
	...
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] temp0=0x%x, temp1=0x%x\n", temp0, temp1);
	...
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_adc_ge_t      = 0x%x\n", g_adc_ge_t);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_adc_oe_t      = 0x%x\n", g_adc_oe_t);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_degc_cali     = 0x%x\n", g_degc_cali);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_adc_cali_en_t = 0x%x\n", g_adc_cali_en_t);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_o_slope       = 0x%x\n", g_o_slope);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_o_slope_sign  = 0x%x\n", g_o_slope_sign);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_id            = 0x%x\n", g_id);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_o_vtsmcu1     = 0x%x\n", g_o_vtsmcu1);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_o_vtsmcu2     = 0x%x\n", g_o_vtsmcu2);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_o_vtsabb      = 0x%x\n", g_o_vtsabb);
}
...
static void thermal_cal_prepare_2(U32 ret)
{
	...
	tscpu_dprintk("thermal_cal_prepare_2\n");
	...
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_ge       = 0x%x\n", g_ge);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_gain     = 0x%x\n", g_gain);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_gain     = 0x%x\n", g_gain);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_x_roomt1 = 0x%x\n", g_x_roomt1);
	tscpu_dprintk("[Power/CPU_Thermal] [Thermal calibration] g_x_roomt2 = 0x%x\n", g_x_roomt2);
}
...
static kal_int32 temperature_to_raw_abb(kal_uint32 ret)
{
	...
	tscpu_dprintk("[temperature_to_raw_abb]\n");
	...
	tscpu_dprintk("[temperature_to_raw_abb] temperature=%d, raw=%d", ret, format_4);
	return format_4;
}
...
static kal_int32 raw_to_temperature_MCU1(kal_uint32 ret)
{
	...
	tscpu_dprintk("raw_to_temperature_MCU1\n");
...
static kal_int32 raw_to_temperature_MCU2(kal_uint32 ret)
{
	...
	tscpu_dprintk("raw_to_temperature_MCU2\n");
	if (ret == 0)
		return 0;
...
int get_immediate_temp2_wrap(void)
{
	...
	tscpu_dprintk("[get_immediate_temp2_wrap] curr_raw=%d\n", curr_raw);
	return curr_raw;
}
...
static int tscpu_get_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
	...
	tscpu_dprintk(" mtktscpu_get_temp  CPU T1=%d\n", curr_temp);
...
int tscpu_get_cpu_temp(void)
{
	...
	tscpu_dprintk(" mtktscpu_get_cpu_temp  CPU T1=%d\n", curr_temp);
		...
		loff_t *data)
{
	...
	tscpu_dprintk("mtktscpu_write_log bad argument\n");
...
int thermal_fast_init(void)
{
	...
	tscpu_dprintk("thermal_fast_init\n");
