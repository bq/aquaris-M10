...
//	else if (batt_cust_data.high_battery_4150_voltage_support)
//		bq24196_set_vreg(0x29); /*VREG 4.144V*/
	else
...
//			cv_value = 4160000;
	...
	battery_log(BAT_LOG_FULL, "charging_set_cv_voltage cv_value = %d\r\n", cv_value);
...
static unsigned int charging_get_current(void *data)
{
	...
	/*start-xmyyq-20161201-fix charging_get_current parsing error*/
	unsigned int ret_value = 0;
	/*end-xmyyq-20161201-fix charging_get_current parsing error*/
	...
	/*start-xmyyq-20161201-fix charging_get_current parsing error*/
	ret_value = (ret_val*64) + 512;

	if (ret_force_20pct == 0)
		*(unsigned int *)data = ret_value;
	else
		*(unsigned int *)data = (int)(ret_value<<1)/10;
	/*end-xmyyq-20161201-fix charging_get_current parsing error*/
...
static unsigned int charging_set_power_off(void *data)
{
	...
	/*g1015 fix bug 0061577 by xmcdl*/
	//kernel_power_off();
	mt_power_off();
	/*g1015 fix bug 0061577 by xmcdl over*/
	return status;
}
...
static unsigned int charging_get_diso_state(void *data)
{
	...
	/*start-xmyyq-2016.11.22-fix when plug in otg device, show dc charger*/
//	if ((((diso_state >> 1) & 0x3) != 0x0))
	if ((((diso_state >> 1) & 0x3) != 0x0) || (diso_state == 0x1)) {
	/*end-xmyyq-2016.11.22-fix when plug in otg device, show dc charger*/
		switch (diso_state) {
		case USB_ONLY:
			set_vdc_auxadc_irq(DISO_IRQ_DISABLE, 0);
			set_vusb_auxadc_irq(DISO_IRQ_DISABLE, 0);
		...
		default: /*OTG only also can trigger vcdt IRQ*/
			...
			/*start-xmyyq-20161121-modify usb otg power by gpio config*/
			bq24196_set_en_hiz(0x1);	/* disable power path */
			g_otg_only = 1;
			/*end-xmyyq-20161121-modify usb otg power by gpio config*/
	...
	/*start-xmyyq-20161121-modify usb otg power by gpio config*/
	if((g_otg_only == 1) && (diso_state != 0x1)){
		bq24196_set_en_hiz(0x0);	/* enable power path */
		g_otg_only = 0;
	}
	/*end-xmyyq-20161121-modify usb otg power by gpio config*/
