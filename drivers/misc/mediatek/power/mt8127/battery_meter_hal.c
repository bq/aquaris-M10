...
#else
	/*pr_err("[Chip_Trim] need check reg number\n");*/
#endif

	switch (chip_diff_trim_value_4_0) {
	case 0:
		chip_diff_trim_value = 1000;
		/*pr_err("[Chip_Trim] chip_diff_trim_value = 1000\n");*/
		break;
	case 1:
...
signed int use_chip_trim_value(signed int not_trim_val)
{
	...
	/*pr_err("[use_chip_trim_value] %d -> %d\n", not_trim_val, ret_val);*/
...
int get_hw_ocv(void)
{
	...
	/*pr_err("[oam] get_hw_ocv (swchr) : adc_result_reg=%d, adc_result=%d\n", adc_result_reg, adc_result);*/
	#else
	adc_result_reg = upmu_get_rg_adc_out_wakeup_pchr();
	adc_result = (adc_result_reg*r_val_temp*VOLTAGE_FULL_RANGE)/ADC_PRECISE;
	/*pr_err("[oam] get_hw_ocv (pchr) : adc_result_reg=%d, adc_result=%d\n", adc_result_reg, adc_result);*/
	#endif
...
static signed int read_adc_v_bat_temp(void *data)
{
	...
	*(signed int *)(data) = 0;
		...
		while (i--) {
			...
			ret += ret_temp;
			/*pr_err("[get_tbat_volt] ret_temp=%d\n", ret_temp);*/
		}

		ret = ret*1500/4096;
		ret = ret/times;
		/*pr_err("[get_tbat_volt] Battery output mV = %d\n", ret);*/

		*(signed int *)(data) = ret;

	/*pr_err("[read_adc_v_charger] return PMIC_IMM_GetOneChannelValue(4,times,1);\n");*/
	#else
