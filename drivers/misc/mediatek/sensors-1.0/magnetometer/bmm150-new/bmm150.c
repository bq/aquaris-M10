...
static long bmm150_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:		//used by MTK ftm
			...
			//modified by xmwwy, fixed bug, for ATA test
			...
			*/
			...
			sprintf(buff, "%x %x %x %x %x", sensor_data[8],  sensor_data[9], sensor_data[10], sensor_data[11], CONVERT_O_DIV); 
			mutex_unlock(&sensor_data_mutex);
...
/*----------------------------------------------------------------------------*/
static int ECS_ConvertCoordinate(int data[3]){	
	int	pat, tmp;	
	struct i2c_client *client = this_client;	
	struct bmm150_i2c_data *i2c_data = i2c_get_clientdata(client);	
...
/*----------------------------------------------------------------------------*/
static int bmm150_get_data(int *x, int *y, int *z, int *status)
{
 
	int data[3];
    mutex_lock(&sensor_data_mutex);
    ECS_GetRawData(data);
	ECS_ConvertCoordinate(data);
	
		*x = data[0] * CONVERT_M_DIV * 5;
		*y = data[1] * CONVERT_M_DIV *(-5);
		*z = data[2] * CONVERT_M_DIV * 4;
...
int bmm150_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ctl.enable = bsx_algo_m_enable;
	ctl.set_delay = bsx_algo_m_set_delay;
	ctl.open_report_data = bsx_algo_m_open_report_data;
	...
	mag_data.get_data = bmm150_get_data;
...
static const struct dev_pm_ops bmm150_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(bmm150_suspend, bmm150_resume)
};
#endif
