...
UINT32 HI704YUVSetVideoMode(UINT32 u2FrameRate)
{
	spin_lock(&hi704_yuv_drv_lock);
...
void HI704YUVSetMaxMinFps(UINT32 u2MinFrameRate, UINT32 u2MaxFrameRate)
{
	SENSORDB("HI704_YUV_SetMaxMinFps+ :FrameRate= %d %d\n", u2MinFrameRate, u2MaxFrameRate);

	HI704YUVSetVideoMode(u2MaxFrameRate);
}
			   ...
			   UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_SET_MIN_MAX_FPS:
		/*SENSORDB("SENSOR_FEATURE_SET_MIN_MAX_FPS:[%llu,%llu]\n",*pFeatureData,*(pFeatureData+1));*/
		HI704YUVSetMaxMinFps((UINT32)*pFeatureData, (UINT32)*(pFeatureData+1));
		break;
