...
#define DEBUG	1
#if DEBUG
#define LOG_DBG(format, args...)	pr_debug(PFX "[%s] " format "\n", __func__, ##args)
#define LOG_ERR(format, args...)	pr_err(PFX "[%s] " format "\n", __func__, ##args)
#define LOG_DBG_IF(loggable, format, args...)	\
	do { if (loggable) pr_debug(PFX "[%s] " format "\n", __func__, ##args); } while (0)
#else
...
u8 SP2518_Initialize_from_T_Flash(void)
{
	/* u8 *data_buff = NULL; */
	u8 *curr_ptr = NULL;
	u32 file_size = 0;
	...
	if (IS_ERR(fp)) {
		LOG_ERR("create file error 0\n");
		fp = filp_open("/storage/sdcard0/sp2518_sd.dat", O_RDONLY, 0);
		if (IS_ERR(fp)) {
			LOG_ERR("create file error 1\n");
			return 2;	/* -1; */
		}
	} else
		LOG_DBG("SP2518_Initialize_from_T_Flash Open File Success\n");
	fs = get_fs();
	set_fs(KERNEL_DS);
	...
	while (curr_ptr < (data_buff + file_size)) {
		...
		if (strcmp((const char *)func_ind, "REG") == 0) {	/* REG */
			...
			SP2518_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 10);
		}
		i++;
	...
	for (j = 0; j < i; j++) {
		if (SP2518_Init_Reg[j].op_code == SP2518_OP_CODE_END) {	/* End of the setting. */
			...
			SP2518_write_cmos_sensor((kal_uint8) SP2518_Init_Reg[j].init_reg,
						 (kal_uint8) SP2518_Init_Reg[j].init_val);
		} else {
			LOG_DBG("REG ERROR!\n");
		}
	}
...
{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0, 0, 1600, 1200, 160, 240, 1280, 720}
};				/* slim video */
...
static void SP2518_ae_enable(kal_bool enable)
{
	...
	LOG_DBG("Ronlus SP2518_ae_enable\r\n");
...
{
	/* kal_uint16 temp_AWB_reg = 0; */
	LOG_DBG("Ronlus SP2518_awb_enable\r\n");
}
...
/* kal_uint32 temp_reg, banding_step; */
	LOG_DBG("Ronlus SP2518_set_hb_shutter\r\n");
}				/* SP2518_set_dummy */
#endif
...
void SP2518_night_mode(kal_bool bEnable)
{
	...
	/* \810\886\810\893\810\884\810\870\810\865\810\896\810\856\810\870\810\867\810\858sp2518
	   a8-10 \810\857\810\857\810\862\810\887\810\880\810\8A7810 3p
	*/
	LOG_DBG("bEnable:%d", bEnable);
	...
	if (bEnable) {		/*night mode settings */
		...
		if (SP2518_MPEG4_encode_mode == KAL_TRUE) {
			if (SP2518_CAM_BANDING_50HZ == KAL_TRUE) {	/*video normal mode 50hz */
				LOG_DBG("video normal mode 50hz");
				...
				LOG_DBG("video normal mode 60hz");
...
{
	SP2518_write_cmos_sensor(0xfd, 0x00);
	SP2518_write_cmos_sensor(0x1b, 0x0a);	/* maximum drv ability//0x1a for low clock */
	SP2518_write_cmos_sensor(0x0e, 0x01);
	...
	/* Gr\810\843\810\844Gb \810\881\A1\A7\810\858\810\868
	   \810\872\810\893\810\855\810\867\810\874\810\865\810\863\810\851
	*/
	...
	SP2518_write_cmos_sensor(0xad, 0x40);	/* raw_rb_fac_low */
	/* Gr\810\843\810\844Gb \810\881\A1\A7\810\858\810\868\810
	   \872\810\893\810\855\810\867\810\874\810\865\810\863\810\851
	*/
...
{
	LOG_DBG("Ronlus SP2518_PV_setting\r\n");
...
static void SP2518_CAP_setting(void)
{
	...
	LOG_DBG("Ronlus SP2518_CAP_setting\r\n");
}				/* SP2518_CAP_setting */
...
UINT32 SP2518Open(void)
{
	...
	/* mdelay(50); */
	if (sensor_id != SP2518_YUV_SENSOR_ID) {
		LOG_ERR("Sensor id read failed, ID = %x\n", sensor_id);
	...
	LOG_DBG("SP2518 Open File Start\n");
	fp = filp_open("/storage/sdcard1/sp2518_sd.dat", O_RDONLY, 0);
	if (IS_ERR(fp)) {
		LOG_ERR("open file error 0\n");
		fp = filp_open("/storage/sdcard0/sp2518_sd.dat", O_RDONLY, 0);
		if (IS_ERR(fp)) {
			fromsd = 0;
			LOG_ERR("open file error 1\n");
		} else {
			LOG_DBG("open file success 1\n");
			fromsd = 1;
			filp_close(fp, NULL);
		}
	} else {
		LOG_DBG("open file success 0\n");
		fromsd = 1;
		LOG_DBG("open file ok\n");
	...
	if (fromsd == 1) {	/* 是否\u017d覵D读取//gepeiwei   120903 */
		LOG_DBG("________________from t!\n");
...
/* Sensor_SetMode(sensor_snapshot_mode); */
	...
	/* LOG_DBG("tangzibo sp2518 tmp = %x\n",tmp); */
	/* LOG_DBG("tangzibo sp2518 tmp1 = %x\n",tmp1); */
	/* LOG_DBG("tangzibo sp2518 tmp2 = %x\n",tmp2); */
	/* LOG_DBG("tangzibo sp2518 tmp3 = %x\n",tmp3); */
	...
	/* LOG_DBG("tangzibo sp2518 tmp end = %x\n",tmp); */
	/* LOG_DBG("tangzibo sp2518 tmp1 end= %x\n",tmp1); */
	/* LOG_DBG("tangzibo sp2518 tmp2 end= %x\n",tmp2); */
	/* LOG_DBG("tangzibo sp2518 tmp3 end= %x\n",tmp3); */
		     ...
		     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	*/
	...
	if (sensor_config_data->SensorOperationMode == MSDK_SENSOR_OPERATION_MODE_VIDEO) {	/* MPEG4 Encode Mode */
		LOG_DBG("video preview");
		SP2518_MPEG4_encode_mode = KAL_TRUE;
	} else {
		LOG_DBG("normal preview");
		     ...
		     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if ((image_window->ImageTargetWidth <= 640 - 16)
	 && (image_window->ImageTargetHeight <= 480 - 12)) {	/* Less than PV Mode */
...
UINT32 SP2518GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	pSensorResolution->SensorVideoWidth = IMAGE_SENSOR_PV_WIDTH;	/* 1600-64;  //800-16-8; */
	pSensorResolution->SensorVideoHeight = IMAGE_SENSOR_PV_HEIGHT;	/* 1200-48; //600-12-6; */
...
UINT32 SP2518Control(MSDK_SCENARIO_ID_ENUM ScenarioId,
		     MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow,
		     MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	LOG_DBG("ScenarioId:%d", ScenarioId);
...
UINT32 SP2518YUVSensorSetting(FEATURE_ID iCmd, UINT16 iPara)
{
	...
	return TRUE;
#endif
	LOG_DBG_IF(0, "iCmd:%d, iPara:%d, nightmode:%d", iCmd, iPara, SP2518_CAM_Nightmode);
	switch (iCmd) {
	case FID_SCENE_MODE:
...
UINT32 sp2518_get_sensor_id(UINT32 *sensorID)
{
	...
	LOG_DBG("GetSensorID=%#x", sensor_id);

	if (sensor_id != SP2518_YUV_SENSOR_ID) {
		LOG_ERR(">>>>>>>>>>SP2518 read ID error<<<<<<<<<<<");
		*sensorID = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
/* UINT32 i; */
	...
	case SENSOR_FEATURE_GET_CONFIG_PARA:
		memcpy(pSensorConfigData, &SP2518SensorConfigData,
	...
	case SENSOR_FEATURE_SET_TEST_PATTERN:
		SP2518_SetTestPatternMode((BOOL) *pFeatureData16);
		break;
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		...
		/* For pointer address in user space is 32-bit,
		   need use compat_ptr to convert it to 64-bit in kernel space
		*/
