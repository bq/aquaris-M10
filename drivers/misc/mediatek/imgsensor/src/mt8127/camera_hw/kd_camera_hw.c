...
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	if (IS_ERR(cam_ldo0_h)) {
		...
		pr_debug("%s : pinctrl warning, cam_ldo0_h\n", __func__);
	}
	...
	if (IS_ERR(cam_ldo0_l)) {
		...
		pr_debug("%s : pinctrl warning, cam_ldo0_l\n", __func__);
	}
	return ret;
}
...
#define VOL_1000 1000000
	...
	if (On) {
		...
		if ((pinSetIdx == 0) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5647_RAW, currSensorName))) {
			...
			mdelay(5);
		} else if (pinSetIdx == 1 && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV2659_YUV))) {
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
			mdelay(5);
		...
		ISP_MCLK1_EN(0);
		if (pinSetIdx == 0 && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV5647_RAW))) {
			...
			if (TRUE != _hwPowerDownCnt(pinSetIdx, VCAMAF, mode_name)) {
				PK_DBG("[CAMERA SENSOR] Fail to OFF analog power (VCAM_AF), power id = %d\n", VCAMAF);
				goto _kdCISModulePowerOn_exit_;
			}
