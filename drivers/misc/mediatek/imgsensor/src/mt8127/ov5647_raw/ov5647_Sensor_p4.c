...
static UINT32 OV5647SetMaxFrameRate(UINT16 u2FrameRate);
...
UINT32 OV5647SetMaxFrameRate(UINT16 u2FrameRate)
{
	...
	/* } */
	SENSORDB
	    ("FrameHeight = %d, dummy_line = %d, OV5647_PV_PERIOD_LINE_NUMS = %d, OV5647_FULL_PERIOD_LINE_NUMS = %d\n",
...
/*No Need to implement this function*/
	...
	if (!OV5647_sensor.video_mode)
		return;
	max_exposure_lines =
	    OV5647_sensor.pclk * OV5647_FPS(1) / (pv_min_fps * OV5647_sensor.line_length);
	if (max_exposure_lines > OV5647_sensor.frame_height)
		...
		static void OV5647_camera_para_to_sensor(void)
{
		...
		for (i = 0; 0xFFFFFFFF != OV5647_sensor.eng.reg[i].Addr; i++) {
			OV5647_write_cmos_sensor(OV5647_sensor.eng.reg[i].Addr,
						 OV5647_sensor.eng.reg[i].Para);
		}
		for (i = OV5647_FACTORY_START_ADDR; 0xFFFFFFFF != OV5647_sensor.eng.reg[i].Addr;
		     i++) {
			OV5647_write_cmos_sensor(OV5647_sensor.eng.reg[i].Addr,
						 OV5647_sensor.eng.reg[i].Para);
		}
...
/* ------------------------ Engineer mode ------------------------ */
	static inline void OV5647_get_sensor_group_count(kal_int32 * sensor_count_ptr)
{
#ifdef OV5647_DRIVER_TRACE
	...
	static inline void OV5647_get_sensor_group_info(MSDK_SENSOR_GROUP_INFO_STRUCT * para)
{
#ifdef OV5647_DRIVER_TRACE
...
static inline void OV5647_get_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{

		static const kal_char * cct_item_name[] = {
		    "XXXXXXXXXXXXXXX", "XXXXXXXXX", "XXXXXXXXXX", "XXXXXXXXXX", "XXXXXXXXX" };
		...
		case OV5647_PRE_GAIN:
			...
			sprintf(para->ItemNamePtr,
				cct_item_name[para->ItemIdx - OV5647_SENSOR_BASEGAIN]);
			para->ItemValue =
			    OV5647_sensor.eng.cct[para->ItemIdx].Para * 1000 / BASEGAIN;
		...
		case OV5647_REGISTER_EDITOR:
			...
			case 1:
				...
				para->ItemValue = 0;
				para->IsTrueFalse = para->IsReadOnly = para->IsNeedRestart =
				    KAL_FALSE;
				para->Min = 0;
...
static inline kal_bool OV5647_set_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{
		...
		case OV5647_PRE_GAIN:
			...
			case OV5647_PRE_GAIN_B_INDEX:
				spin_lock(&ov5647_drv_lock);
				OV5647_sensor.eng.cct[para->ItemIdx].Para =
...
static void OV5647_Sensor_1M(void)
{
		...
		/* Bit8Div = 1, 1, 4, 5 for 0x300F[5:4] = 0~3, respectively */
		/* SenDiv = 1, 2 for 0x3010[4] = 0 or 1 respectively */
		/* CLKDiv = (0x3011[5:0] + 1) */
		...
		OV5647_write_cmos_sensor(0x4005, 0x18);	/* mtk70677,Update BLC when gain changed */
...
/* Houston 20131003 decrease capture frame rate due to pclk can't reach to 80MHz */
...
UINT32 OV5647Preview(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		...
		OV5647_sensor.Prv_line_length = OV5647_sensor.line_length;
		/* For Capture Function to calculate capture shutter */
...
/*    kal_uint16 dummy_pixel;*/
		...
		/* if(OV5647_sensor.pv_mode == KAL_TRUE) */
		{
			...
			/* shutter = shutter * (((kal_uint32)(pv_line_length * 806))/
			   ((kal_uint32)(OV5647_sensor.line_length * 481))); */
			/* shutter = shutter * (((kal_uint32)(pv_line_length * 806 ))/
			   ((kal_uint32)(OV5647_sensor.line_length * 481))); */
			/* shutter = (shutter * (((kal_uint32)(pv_line_length * 806 * 1000))/
			   ((kal_uint32)(OV5647_sensor.line_length * 481))))/1000 - 3; */
			/* shutter = shutter * (((kal_uint32)(pv_line_length * 806000))/
			   ((kal_uint32)(OV5647_sensor.line_length * 563333))); */
			/* shutter = shutter * (((kal_uint32)(pv_line_length * 806000))/
			   ((kal_uint32)(OV5647_sensor.line_length * 481000))); */
#if defined(__OV5647_48M__)
			shutter =
			    (shutter *
			     (((kal_uint32) (pv_line_length * 806 * 1000)) /
			      ((kal_uint32) (OV5647_sensor.line_length * 481)))) / 1000;
#elif defined(__OV5647_52M__)
			shutter =
			    (shutter *
			     (((kal_uint32) (pv_line_length * 806 * 1000)) /
			      ((kal_uint32) (OV5647_sensor.line_length * 528667))));
#elif defined(__OV5647_54M__)
			shutter =
			    (shutter *
			     (((kal_uint32) (pv_line_length * 806 * 1000)) /
			      ((kal_uint32) (OV5647_sensor.line_length * 546)))) / 1000;
#elif defined(__OV5647_56M__)
			shutter =
			    (shutter *
			     (((kal_uint32) (pv_line_length * 806 * 1000)) /
			      ((kal_uint32) (OV5647_sensor.line_length * 563)))) / 1000;
#endif
			...
			dummy_pixel =
			    OV5647_sensor.pclk * OV5647_FPS(1) / (OV5647_FULL_PERIOD_LINE_NUMS *
								  cap_fps);
			dummy_pixel =
			    dummy_pixel <
			    OV5647_FULL_PERIOD_PIXEL_NUMS ? 0 : dummy_pixel -
...
UINT32 OV5647GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
		...
		SENSORDB("[%s] ===> Full(%d x %d) , Preview (%d x %d) , Video (%d x %d)\n",
			 __func__, pSensorResolution->SensorFullWidth,
			 pSensorResolution->SensorFullHeight, pSensorResolution->SensorPreviewWidth,
...
UINT32 OV5647GetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId,
...
#ifdef OV5647_DRIVER_TRACE
		char scenario_str[10][20] = {
		    "XXXXXXX", "XXXXXXXXXXXX", "XXXXXXXXXXXXX", "XXXXXXXXXXXXXXXX", "XXX",
			"3D PREVIEW", "3D CAPTURE", "3D VIDEO", "TV OUT", "MAX" };
		SENSORDB("[%s] ===> ScenarioId(%d):%s , SensorType: %d\n", __func__, ScenarioId,
			 scenario_str[ScenarioId], pSensorInfo->SensroInterfaceType);
#endif
		...
		/* 1;  Houston 20130917 increase pv delay frame to fix pclk phase shift */
		pSensorInfo->VideoDelayFrame = 1;

		pSensorInfo->SensorMasterClockSwitch = 0;
		pSensorInfo->SensorDrivingCurrent = ISP_DRIVING_2MA;
		/* ISP_DRIVING_4MA;//ISP_DRIVING_6MA;//ISP_DRIVING_8MA;  */
		/*Houston 20130917 increase pv delay frame to fix pclk phase shift */
		...
		memcpy(pSensorConfigData, &OV5647SensorConfigData,
...
UINT32 OV5647Control(MSDK_SCENARIO_ID_ENUM ScenarioId,
...
UINT32 OV5647SetAutoFlickerMode(kal_bool bEnable, UINT16 u2FrameRate)
{
		...
		if (bEnable) {
			...
			if ((OV5647_sensor.FixedFps == 30)
			    && (OV5647_sensor.video_mode == KAL_TRUE))
				OV5647SetMaxFrameRate(296);
			if ((OV5647_sensor.FixedFps == 15)
			    && (OV5647_sensor.video_mode == KAL_TRUE))
				OV5647SetMaxFrameRate(148);
...
UINT32 OV5647SetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate)
{
		kal_uint32 pclk;
		kal_int16 dummyLine;
		kal_uint16 lineLength, frameHeight;

		/* LOG_DBG("scenarioId = %d, frame rate = %d\n", scenarioId, frameRate); */

		switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			pclk = OV5647_PREVIEW_CLK;
			lineLength = OV5647_PV_PERIOD_PIXEL_NUMS;
			frameHeight = (10 * pclk) / frameRate / lineLength;
			dummyLine = frameHeight - OV5647_PV_PERIOD_LINE_NUMS;
			OV5647_Set_Dummy(0, dummyLine);
			break;
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			pclk = OV5647_PREVIEW_CLK;
			lineLength = OV5647_PV_PERIOD_PIXEL_NUMS;
		...
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_CAMERA_ZSD:
			pclk = OV5647_CAPTURE_CLK;
			lineLength = OV5647_FULL_PERIOD_PIXEL_NUMS;
			frameHeight = (10 * pclk) / frameRate / lineLength;
			dummyLine = frameHeight - OV5647_FULL_PERIOD_LINE_NUMS;
			OV5647_Set_Dummy(0, dummyLine);
			break;
		case MSDK_SCENARIO_ID_CAMERA_3D_PREVIEW:	/* todo if possible */
			break;
		case MSDK_SCENARIO_ID_CAMERA_3D_VIDEO:
			break;
		case MSDK_SCENARIO_ID_CAMERA_3D_CAPTURE:	/* todo if possible */
			break;
		default:
			break;
		}
...
UINT32 OV5647GetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId,
						   MUINT32 *pframeRate) {
...
/* UINT32 i; */
		...
		case SENSOR_FEATURE_GET_CCT_REGISTER:	/* 12 */
			if (*pFeatureParaLen >= sizeof(OV5647_sensor.eng.cct) + sizeof(kal_uint32)) {
				*((kal_uint32 *) pFeaturePara++) = sizeof(OV5647_sensor.eng.cct);
				memcpy(pFeaturePara, &OV5647_sensor.eng.cct,
				       sizeof(OV5647_sensor.eng.cct));
			}
			break;
		...
		case SENSOR_FEATURE_GET_ENG_REGISTER:	/* 14 */
			if (*pFeatureParaLen >= sizeof(OV5647_sensor.eng.reg) + sizeof(kal_uint32)) {
				*((kal_uint32 *) pFeaturePara++) = sizeof(OV5647_sensor.eng.reg);
				memcpy(pFeaturePara, &OV5647_sensor.eng.reg,
				       sizeof(OV5647_sensor.eng.reg));
			}
		case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
			...
			memcpy(((PNVRAM_SENSOR_DATA_STRUCT) pFeaturePara)->SensorEngReg,
			       &OV5647_sensor.eng.reg, sizeof(OV5647_sensor.eng.reg));
			memcpy(((PNVRAM_SENSOR_DATA_STRUCT) pFeaturePara)->SensorCCTReg,
			       &OV5647_sensor.eng.cct, sizeof(OV5647_sensor.eng.cct));
			*pFeatureParaLen = sizeof(NVRAM_SENSOR_DATA_STRUCT);
			break;
		case SENSOR_FEATURE_GET_CONFIG_PARA:
			memcpy(pFeaturePara, &OV5647_sensor.cfg_data,
			       sizeof(OV5647_sensor.cfg_data));
		...
		case SENSOR_FEATURE_GET_GROUP_COUNT:
			...
			*pFeatureParaLen = 4;
			break;
			OV5647_get_sensor_group_info((MSDK_SENSOR_GROUP_INFO_STRUCT *)
						     pFeaturePara);
		...
		case SENSOR_FEATURE_GET_ENG_INFO:
			memcpy(pFeaturePara, &OV5647_sensor.eng_info,
			       sizeof(OV5647_sensor.eng_info));
		...
		case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
			OV5647SetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
							*(pFeatureData32 + 1));
			break;
		case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
			OV5647GetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *
							    pFeatureData32,
							    (MUINT32 *)(uintptr_t)(*(pFeatureData32 + 1)));
			break;
