...
#define OV2659_TEST_PATTERN_CHECKSUM (0xfd0d79ae)
...
static kal_uint32 OV2659_GetSensorID(kal_uint32 *sensorID)
{
	...
	for (i = 0; i < 3; i++) {
		sensor_id =
		    (OV2659_read_cmos_sensor(0x300A) << 8) | OV2659_read_cmos_sensor(0x300B);
		     ...
		     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	mDELAY(200);		/* origin: 300, it will affect the first preview issue: eg, splash black. */
		     ...
		     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (SENSOR_MODE_PREVIEW == OV2659Sensor.SensorMode) {
		...
		/* this wil affect the capture issue, eg. flash black screen, abnormal showing.. */
		mDELAY(100);	/* origin: 300 ;  100: capture ok  */
...
UINT32 OV2659YUVSensorSetting(FEATURE_ID iCmd, UINT32 iPara)
{
	...
	mDELAY(15);		/* originally, delay 30. if commented, capture may splash light green blocks */
	return TRUE;
}
...
{
	UINT16 *pFeatureReturnPara16 = (UINT16 *) pFeaturePara, *pFeatureData16 =
	    (UINT16 *) pFeaturePara;
	UINT32 *pFeatureReturnPara32 = (UINT32 *) pFeaturePara, *pFeatureData32 =
	    (UINT32 *) pFeaturePara;
	...
	case SENSOR_FEATURE_GET_AE_AWB_LOCK_INFO:
		/*OV2659_get_AEAWB_lock((UINT32 *) (*pFeatureData32),
