...
#define PK_DBG_FUNC(fmt, arg...)    pr_debug(PFX "[%s] " fmt "\n", __func__, ##arg)
#define PK_INFO(fmt, arg...)    pr_info(PFX " [%s] " fmt "\n", __func__, ##arg)

#define PK_DBG_IF(enable, fmt, arg...)   if (enable) pr_debug(PFX "[%s] " fmt "\n", __func__, ##arg)

	/*#undef DEBUG_CAMERA_HW_K*/
#define DEBUG_CAMERA_HW_K
#ifdef DEBUG_CAMERA_HW_K
#define PK_DBG PK_DBG_FUNC
#define PK_ERR(fmt, arg...)         pr_err(PFX "[%s] " fmt "\n", __func__, ##arg)
...
inline static int adopt_CAMERA_HW_GetResolution(void *pBuf)
{
	...
	pRes[0] = kzalloc(sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT), GFP_KERNEL);
	if (pRes[0] == NULL) {
		PK_ERR(" ioctl allocate mem failed\n");
		return -ENOMEM;
	}
	pRes[1] = kzalloc(sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT), GFP_KERNEL);
	if (pRes[1] == NULL) {
		kfree(pRes[0]);
		PK_ERR(" ioctl allocate mem failed\n");
		return -ENOMEM;
...
bool CAMERA_Regulator_PowerOnOFF(struct regulator *pwrreg, BOOL IsOn, int vol)
{
	...
	PK_DBG_IF(0, "IsOn:%d , vol:%d", IsOn, vol);
	...
	if (IsOn) {		/* Power ON */
		...
		*/
		...
		PK_DBG_IF(0, "high_bound_voltage = %d", high_bound_voltage);
		...
		*/
		...
		PK_DBG_IF(0, "low_bound_voltage = %d", low_bound_voltage);
...
bool _hwPowerOn(int PinIdx, KD_REGULATOR_TYPE_T PwrType, int Voltage)
{
	...
	/*
		Not like 8173/8163, 8127 doesn't support PIP function,
		Therefore Main & sub share the same device node, so it MUST be idx: 0,
	*/
	PinIdx = 0;
