...
#define PREFIX	    "[ov5648]"

/* If don't want to trace more logs, set 0, else, set 1*/
#define OV5648MIPI_DRIVER_TRACE  0

#define OV5648MIPI_DEBUG  1

#if OV5648MIPI_DEBUG
#define LOG_DBG(fmt, arg...) pr_debug(PREFIX "%s: " fmt "\n", __func__ , ##arg)
#define LOG_DBG_IF(loggable, fmt, arg...)  \
		do { \
			if (loggable) \
				pr_debug(PREFIX "%s: " fmt "\n", __func__ , ##arg); \
		} while (0)

#define LOG_ERR(fmt, arg...) pr_err(PREFIX "%s: " fmt "\n", __func__ , ##arg)
#else
...
kal_uint16 OV5648MIPI_read_otp_wb(kal_uint16 index, struct OV5648MIPI_otp_struct *otp_ptr)
{
	...
	LOG_DBG("Read wb otp data success!");
	LOG_DBG("rg_ratio = %d, bg_ratio = %d, gb_gr_ratio = %d", (*otp_ptr).rg_ratio,
...
{
	LOG_DBG("R_gain[%x] G_gain[%x] B_gain[%x]", R_gain, G_gain, B_gain);
...
#if OV5648MIPI_DRIVER_TRACE
	LOG_DBG("iShutter =  %d", iShutter);
#endif
...
static void OV5648MIPI_Set_Dummy(const kal_uint16 iDummyPixels, const kal_uint16 iDummyLines)
{
	...
	LOG_DBG("iDummyPixels = %d, iDummyLines = %d ", iDummyPixels, iDummyLines);
...
void OV5648MIPISetMaxFrameRate(UINT16 u2FrameRate)
{
	...
	LOG_DBG("u2FrameRate = %d ", u2FrameRate);
...
#if OV5648MIPI_DRIVER_TRACE
	LOG_DBG("iGain = %d , iRegGain = 0x%x ", iGain, iRegGain);
#endif
...
/* ------------------------ Engineer mode ------------------------ */
static inline void OV5648MIPI_get_sensor_group_count(kal_int32 *sensor_count_ptr)
{
...
static inline void OV5648MIPI_get_sensor_group_info(MSDK_SENSOR_GROUP_INFO_STRUCT *para)
{
...
static inline void OV5648MIPI_get_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{
...
static inline kal_bool OV5648MIPI_set_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{
...
static void OV5648MIPI_Sensor_Init(void)
{
   ...
   /*****************************************************************************
	...
	SC_CMMN_PLL_MULTIPLIER[7:0] PLL_multiplier(4~252) This can be any integer
	  during 4~127 and only even integer during 128 ~ 252
...
UINT32 OV5648MIPIOpen(void)
{
	...
	if (sensor_id != OV5648MIPI_SENSOR_ID) {
		LOG_ERR("Read Sensor Id Error = 0x%x", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}

	LOG_DBG("sensor_id = 0x%x", sensor_id);
...
{
	LOG_DBG("bEnable = %d, u2FrameRate = %d ", bEnable, u2FrameRate);
