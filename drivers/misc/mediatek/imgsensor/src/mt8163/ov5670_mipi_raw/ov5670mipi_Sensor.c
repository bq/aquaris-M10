...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.linelength = 1680,//1676,				//record different mode's linelength
	...
	.pre_delay_frame = 6, 		//enter preview delay frame num//2 6
...
static void OV5670ofilm_ApplyWB(kal_uint16 rg,kal_uint16 bg,kal_uint16 RG_Ratio_Typical,kal_uint16 BG_Ratio_Typical)
{ 
...
void OV5670Ofilm_otpupdate(void)
{
	...
	OV5670ofilm_read_all(0x7011+5*(valueGroup-1),0x7010+5*(valueGroup),tempbuf);
     module_integrator_id = tempbuf[0];
	...
	if(module_integrator_id ==0x07)
		...
		{
			LOG_INF("The module is  from Q-tech\n");
	  }
	else 
		{
	...
	LOG_INF("OV5670 otp basic info module_integrator_id= %d\n",module_integrator_id);

	//AWB update
	
	OV5670ofilm_read_all(0x7020,0x7020,tempWB); 
	...
	OV5670ofilm_read_all(0x7021+3*(valueGroup-1),0x7020+3*(valueGroup),tempWB);
	
        rg = (tempWB[0]<<2) +( (tempWB[2]&0xc0)>>6);
	 bg = (tempWB[1]<<2) +( (tempWB[2]&0x30)>>4);
	
	 RG_Ratio_Typical = 0x147;
	 BG_Ratio_Typical = 0x163;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x5785, 0x01); //01n
	...
	write_cmos_sensor(0x5791, 0x06); //01n
	...
	write_cmos_sensor(0x3503, 0x30); //; real gain [2]   gain no delay, shutter no delay
	//;added					 
//	write_cmos_sensor(0x3d85, 0x17); 
//	write_cmos_sensor(0x3655, 0x20); 
	write_cmos_sensor(0x5045, 0x05); //;[2] enable MWB manual bias
	write_cmos_sensor(0x5048, 0x10); //; MWB manual bias; need to be the same with 0x4003 BLC target.
