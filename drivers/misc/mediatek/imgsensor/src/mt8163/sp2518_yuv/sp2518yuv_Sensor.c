...
kal_uint8 fromsd = 0;
static kal_uint16 SP2518_write_cmos_sensor(kal_uint8 addr, kal_uint8 para);
static kal_uint16 SP2518_read_cmos_sensor(kal_uint8 addr);
...
struct SP2518_initial_set_struct {
	u16 init_reg;
	u16 init_val;		/* Save the register value and delay tick */
	u8 op_code;		/* 0 - Initial value, 1 - Register, 2 - Delay, 3 - End of setting. */
};

struct SP2518_initial_set_struct SP2518_Init_Reg[1000];
...
u8 SP2518_Initialize_from_T_Flash(void)
{
	...
	if (IS_ERR(fp)) {
		SENSORPK("create file error 0\n");
		fp = filp_open("/storage/sdcard0/sp2518_sd.dat", O_RDONLY, 0);
		if (IS_ERR(fp)) {
			SENSORPK("create file error 1\n");
			return 2;	/* -1; */
		}
	} else
		SENSORPK("SP2518_Initialize_from_T_Flash Open File Success\n");
	fs = get_fs();
	set_fs(KERNEL_DS);
...
#define SENSORDB(fmt, args...)  pr_debug(PFX "%s: " fmt, __func__ , ##args)
#define SENSORPK(fmt, args...)  pr_err(PFX "%s: " fmt, __func__ , ##args)
#else
#define SENSORDB(x, ...)
#define SENSORPK(fmt, args...)  pr_err(PFX "%s: " fmt, __func__ , ##args)
#endif
...
UINT32 SP2518Open(void)
{
	...
	SENSORPK("SP2518 Open File Start\n");
	fp = filp_open("/storage/sdcard1/sp2518_sd.dat", O_RDONLY, 0);
	if (IS_ERR(fp)) {
		SENSORPK("open file error 0\n");
		fp = filp_open("/storage/sdcard0/sp2518_sd.dat", O_RDONLY, 0);
		if (IS_ERR(fp)) {
			fromsd = 0;
			SENSORPK("open file error 1\n");
		} else {
			SENSORPK("open file success 1\n");
			fromsd = 1;
			filp_close(fp, NULL);
		}
	} else {
		SENSORPK("open file success 0\n");
		fromsd = 1;
		SENSORPK("open file ok\n");
	...
	if (fromsd == 1) {
		SENSORPK("________________from t!\n");
		SP2518_Initialize_from_T_Flash();
	} else {
		SP2518_Sensor_Init();
...
/* SP2518_WriteReg(0x1b,0x0f); */
	...
	/* SP2518_write_cmos_sensor(0x30 , 0x04);//PLL 2¡À??¦Ì */
	SP2518_write_cmos_sensor(0x30, 0x04);	/* PLL 1.5¡À??¦Ì //130802 */
	/* SP2518_write_cmos_sensor(0x31 , 0x10);//PLL 1.5¡À??¦Ì */
#if 1
...
/* Sensor_SetMode(sensor_snapshot_mode); */
	...
	/* SENSORPK("tangzibo sp2518 tmp = %x\n",tmp); */
	/* SENSORPK("tangzibo sp2518 tmp1 = %x\n",tmp1); */
	/* SENSORPK("tangzibo sp2518 tmp2 = %x\n",tmp2); */
	/* SENSORPK("tangzibo sp2518 tmp3 = %x\n",tmp3); */
	...
	/* SP2518_WriteReg(0x30 , 0x04);//PLL 2¡À??¦Ì */
	/* SENSORPK("tangzibo sp2518 tmp end = %x\n",tmp); */
	/* SENSORPK("tangzibo sp2518 tmp1 end= %x\n",tmp1); */
	/* SENSORPK("tangzibo sp2518 tmp2 end= %x\n",tmp2); */
	/* SENSORPK("tangzibo sp2518 tmp3 end= %x\n",tmp3); */
	SP2518_write_cmos_sensor(0x30, 0x20);	/* PLL 1.5¡À??¦Ì */
#if 1
/* SP2518_WriteReg(0xe7 , 0x03); */
/* SP2518_WriteReg(0xe7 , 0x00); */
	SP2518_write_cmos_sensor(0xfd, 0x01);
		     ...
		     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	SENSORPK("@@@SP:Ronlus SP2518Preview fun start\r\n");
	...
	if (sensor_config_data->SensorOperationMode == MSDK_SENSOR_OPERATION_MODE_VIDEO) {
		SENSORPK("@@@SP:Ronlus video preview\r\n");
		SP2518_MPEG4_encode_mode = KAL_TRUE;
	} else {
		SENSORPK("@@@SP:Ronlus capture preview\r\n");
	...
	SENSORPK("@@@SP:Ronlus SP2518Preview fun end\r\n");
	/* Sleep(300); */
	SENSORPK("@@@SP:555555\r\n");
	return ERROR_NONE;
}				/* SP2518Preview */
		     ...
		     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if ((image_window->ImageTargetWidth <= 640 - 16) && (image_window->ImageTargetHeight <= 480 - 12)) {
...
UINT32 SP2518GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	/* 1600-64; 800-16-8;IMAGE_SENSOR_SVGA_WIDTH;(user this for SVGA) */
	pSensorResolution->SensorPreviewHeight = IMAGE_SENSOR_PV_HEIGHT;
	/* 1200-48; 600-12-6;IMAGE_SIMAGE_SENSOR_SVGA_HEIGHT;(use this for SVGA) */
		     ...
		     MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	/* 1600-64; 800-16-8;IMAGE_SENSOR_PV_WIDTH(use this for SVGA); */
	pSensorInfo->SensorPreviewResolutionY = IMAGE_SENSOR_PV_HEIGHT;
	/* 1200-48; 600-12-6;IMAGE_SENSOR_PV_HEIGHT(user this for SVGA); */
	...
	/* ISP_DRIVING_2MA; modify by sp_yjp,20130813 */
...
UINT32 sp2518_get_sensor_id(UINT32 *sensorID)
{
	...
	SENSORPK("@@@SP:xieyang SP2518GetSensorID ");
	...
	SENSORPK("@@@SP:SP2518GetSensorID=%#x\r\n ", sensor_id);

	while (0) {	/* add by sp_yjp for test,20130813 */
		mdelay(50);
		sensor_id = SP2518_read_cmos_sensor(0x02);
		SENSORPK("@@@SP:DeadReadSP2518ID=%#x\r\n ", sensor_id);
		if (sensor_id == SP2518_YUV_SENSOR_ID)
			break;
	}

	if (sensor_id != SP2518_YUV_SENSOR_ID) {
		SENSORPK("@@@SP:>>>>>>>>>>SP2518 read ID error<<<<<<<<<<<");
		*sensorID = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
/* UINT32 i; */
	...
	case SENSOR_FEATURE_SET_YUV_CMD:
		SP2518YUVSensorSetting((FEATURE_ID) * pFeatureData, (FEATURE_ID) * (pFeatureData + 1));
		break;
