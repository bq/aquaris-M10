...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	printk("xmhhs ov8858 otp flag is %d \n",(*otp_ptr).flag);
	return (*otp_ptr).flag;
}
// return value:
// bit[7]: 0 no otp info, 1 valid otp info
...
{
	int RG_Ratio_Typical = 324, BG_Ratio_Typical = 302;
	int rg, bg, R_gain, G_gain, B_gain, Base_gain, temp, i;
	// apply OTP WB Calibration
	if ((*otp_ptr).flag & 0x40) {
		rg = (*otp_ptr).rg_ratio;
		bg = (*otp_ptr).bg_ratio;
		printk("xmhhs read rg type is %d \n",RG_Ratio_Typical);
		printk("xmhhs read bg type is %d \n",BG_Ratio_Typical);
		printk("xmhhs read rg is %d \n",rg);
		printk("xmhhs read bg is %d \n",bg);
		//calculate G gain
