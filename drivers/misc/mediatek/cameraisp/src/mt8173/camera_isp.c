...
#define	LOG_VRB(format,	args...)    pr_info(MyTag "[%s]" format, __func__, ##args)
	...
	if (En) {
		...
		*MMSYS_CG_CLR0 = 0x00000003;*CLK_CFG_7 = *CLK_CFG_7 | 0x02000000;*CAM_CTL_CLK_EN = 0x00000009; */
		...
		if (G_u4EnableClockCount == 0) {
			LOG_INF("Now: G_u4EnableClockCount = 0, just return!");
			return;
		}
...
EXIT:
	if (Ret != 0)
		LOG_DBG("Fail, Cmd(%d),	Pid(%d), (process, pid,	tgid)=(%s, %d, %d)", Cmd,
...
static MINT32 ISP_open(struct inode *pInode, struct file *pFile)
{
	...
	/*      */
#ifdef KERNEL_LOG
	IspInfo.DebugMask = 0; /*(ISP_DBG_INT | ISP_DBG_BUF_CTRL);*/
#endif
	/*      */
EXIT:
	if (Ret < 0) {
...
static MINT32 ISP_suspend(struct platform_device *pDev, pm_message_t Mesg)
{
	...
	if (G_u4EnableClockCount > 0) {
		...
		for (i = 0; i < G_u4EnableClockCount; ++i)
			ISP_EnableClock(MFALSE);
	}
#endif
