...
unsigned int g_initialize = 0;
unsigned int g_clk_en = 0;
...
static void mt_fh_hal_init(void)
{
...
#ifdef CONFIG_OF
	fh_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8127-fhctl");
	if (fh_node) {
		/* Setup IO addresses */
	...
	apmix_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8127-apmixedsys");
	if (apmix_node) {
		/* Setup IO addresses */
