...
#ifdef CONFIG_LED_USE_GPIO
struct cust_mt65xx_led * led_cust;
#define GPIO_OUT_ONE    1
#define GPIO_OUT_ZERO   0
static unsigned int GPIO_LED_R;
static unsigned int GPIO_LED_G;
static unsigned int GPIO_LED_B;
static void leds_set_gpio_output(unsigned int GPIO, unsigned int output)
{
	gpio_direction_output(GPIO, output);
}

static void led_get_gpio_info(void)
{
	struct device_node *led_node = NULL;
	led_node = of_find_compatible_node(NULL,NULL,"mediatek,led-gpio-config");
	if(led_node) {
            GPIO_LED_R = of_get_named_gpio(led_node, "led_r_gpio", 0);
            if (gpio_is_valid(GPIO_LED_R))
                gpio_request(GPIO_LED_R, "GPIO_LED_R");
            else
                pr_warn("can not get valid gpio for red\n");

            GPIO_LED_G = of_get_named_gpio(led_node, "led_g_gpio", 0);
            if (gpio_is_valid(GPIO_LED_G))
                gpio_request(GPIO_LED_G, "GPIO_LED_G");
            else
                pr_warn("can not get valid gpio for green\n");

            GPIO_LED_B = of_get_named_gpio(led_node, "led_b_gpio", 0);
            if (gpio_is_valid(GPIO_LED_B))
                gpio_request(GPIO_LED_B, "GPIO_LED_B");
            else
                pr_warn("can not get valid gpio for blue\n");
	}
	else
            LEDS_DEBUG("can not find led-gpio-config device node\n");
}

static int mt65xx_leds_gpio_control(unsigned int level)
{
        unsigned int gpio = 0;
        LEDS_DEBUG("%s enter , level is %d\n",__func__,level);

        if(strcmp(led_cust->name, "red") == 0)
                gpio = GPIO_LED_R;
        else if(strcmp(led_cust->name, "green") == 0)
                gpio = GPIO_LED_G;
        else if(strcmp(led_cust->name, "blue") == 0)
                gpio = GPIO_LED_B;
        else {
                LEDS_DEBUG("LED(%s) is not controled by gpio",led_cust->name);
                return -1;
        }

        if(level)
                leds_set_gpio_output(gpio,GPIO_OUT_ONE);
        else
                leds_set_gpio_output(gpio,GPIO_OUT_ZERO);
		...
		for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
			...
			if (!led_node) {
				...
				case MT65XX_LED_MODE_GPIO:
					pled_dtsi[i].data =
					    (long)mt65xx_leds_gpio_control;
					LEDS_DEBUG
						("kernel:the led(%s) is controled by gpio\n",pled_dtsi[i].name);
					break;
				#endif
				default:
					break;
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	if (pmic_type == MT65XX_LED_PMIC_BUTTON) {
...
#ifdef PMIC_MT6325
		upmu_set_isinks_ch3_mode(ISINK_PWM_MODE);
		upmu_set_isinks_ch3_step(0x3);	/* 4mA */

		upmu_set_isink_dim3_duty(15);
		upmu_set_isink_dim3_fsel(11);	/* 6320 0.25KHz */
#else
		upmu_set_rg_drv_32k_ck_pdn(0x0); // Disable power down
		upmu_set_rg_isink3_ck_pdn(0);
		upmu_set_rg_isink3_ck_sel(0);
		upmu_set_isink_ch3_mode(ISINK_PWM_MODE);
		upmu_set_isink_ch3_step(ISINK_1);//8mA
		//hwPWMsetting(PMIC_PWM_2, 15, 8);
		upmu_set_isink_dim3_duty(level >> 3);
		...
		if (level) {
			...
			upmu_set_isink_rsv2_isink3(0x1);
			upmu_set_isinks_ch3_en(0x01);
#else
			upmu_set_isink_ch3_en(0x01);
#endif
		} else {
#ifdef PMIC_MT6325
			upmu_set_isinks_ch3_en(0x00);
			upmu_set_isink_rsv2_isink3(0x00);
#else
		...
		upmu_set_isink_dim1_duty(15);
		upmu_set_isink_dim1_fsel(11);	/* 6320 0.25KHz */
#else	//add for bta_j2401 backlight,Jason Zhao
		upmu_set_rg_drv_32k_ck_pdn(0x0); // Disable power down
		...
		upmu_set_isink_ch0_mode(ISINK_PWM_MODE);
		upmu_set_isink_dim0_duty(15);
		upmu_set_isink_dim0_fsel(0);	/* 6320 1KHz */

		upmu_set_rg_isink1_ck_pdn(0);
		upmu_set_rg_isink1_ck_sel(0);
		upmu_set_isink_ch1_mode(ISINK_PWM_MODE);
		upmu_set_isink_dim1_duty(15);
		upmu_set_isink_dim1_fsel(0);	/* 6320 1KHz */
		...
		if (level > 0 && level < 40) {
                            ...
                            upmu_set_isink_ch1_step(0x1);	/* 8mA */
		} else if (level >= 80 && level < 120) {
                            upmu_set_isink_ch0_step(0x2);	/* 12mA */
                            upmu_set_isink_ch1_step(0x2);	/* 12mA */
                            ...
                            upmu_set_isink_ch1_step(0x4);	/* 20mA */
		} else if (level >= 200 && level < 256) {
