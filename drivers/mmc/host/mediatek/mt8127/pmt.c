...
#define pmt_err(fmt, args...) do {     \
	pr_err(TAG);          \
	pr_err(fmt, ##args); \
} while (0)
#define pmt_info(fmt, args...) do {    \
	pr_info(TAG);       \
	pr_info(fmt, ##args); \
} while (0)
...
static unsigned long long emmc_total_size;
static unsigned long long emmc_user_size;

static struct pt_resident *last_part;
...
struct pmt_region_ops {
	...
	int (*pack)(struct pmt_region *region, void *buf, struct pt_resident *part);
	int (*unpack)(struct pmt_region *region, void *buf, struct pt_resident *part);
};
...
static int pmt_region_pack(struct pmt_region *region, void *buf, struct pt_resident *part)
{
	void *ptr;
	...
	*(int *)sig = region->sig;
	...
	/* partition info */
	ptr += PMT_VER_SIZE;
	memcpy(ptr, part, PART_NUM * sizeof(struct pt_resident));
...
static int pmt_region_unpack(struct pmt_region *region, void *buf, struct pt_resident *part)
{
	int err;
	...
	if (!memcmp(ptr, PMT_VER_V1P1, PMT_VER_SIZE)) {
		/* partition info */
		ptr += PMT_VER_SIZE;
		memcpy(part, ptr, PART_MAX_COUNT * sizeof(struct pt_resident));
		err = DM_ERR_OK;
		pmt_info("find %s at 0x%llx, ver %s\n",
			 region->name, region->base_addr, PMT_VER_V1P1);
	} else {
		pmt_err("invalid pt version 0x%x\n", *((u32 *) ptr));
		err = ERR_NO_EXIST;
		goto out;
	}
...
static inline int pmt_region_pack(struct pmt_region *region, void *buf, struct pt_resident *part)
{
	return -EBUSY;
}

static inline int pmt_region_unpack(struct pmt_region *region, void *buf, struct pt_resident *part)
{
	return -EBUSY;
}
#endif
...
static int __load_pmt(struct pmt_region *region, struct pt_resident *part)
{
	int err;
	void *buf;
...
static int __store_pmt(struct pmt_region *region, struct pt_resident *new_part)
{
	int err;
	void *buf;
...
{
	pmt_info("read_pmt\n");
	if (copy_to_user(arg, last_part, sizeof(struct pt_resident) * PART_NUM))
		return -EFAULT;
	return 0;
}
...
{
	int err = 0;
	struct pt_resident *new_part;

	new_part = kcalloc(PART_NUM, sizeof(struct pt_resident), GFP_KERNEL);
	if (!new_part) {
		err = -ENOMEM;
	...
	if (copy_from_user(new_part, arg, PART_NUM * sizeof(struct pt_resident))) {
		err = -EFAULT;
		goto out;
	}
	...
	for (xbr_idx = 0; xbr_idx < xbr_loops; xbr_idx++) {
		...
		for (slot = 0; slot < 4; slot++) {
			...
			for (pmt_idx = 0; pmt_idx < PART_NUM; pmt_idx++) {
				...
				if (last_part[pmt_idx].offset == off2mbr[xbr_idx][slot]) {
...
int init_pmt(void)
{
	...
	last_part = kcalloc(PART_MAX_COUNT, sizeof(struct pt_resident), GFP_KERNEL);
	if (!last_part) {
		err = -ENOMEM;
	...
	while (i < PART_MAX_COUNT && last_part[i].size) {
		...
		PartInfo[i].name = kzalloc(len + 1, GFP_KERNEL);
		for (j = 0; j < len; j++)
...
static int unpack_and_verify(struct DM_PARTITION_INFO_PACKET_x *packet,
			     struct pt_resident *new_part, int *changed, int *pt_change_tb)
{
	int err = 0;
	int i;
...
static void update_part_size(struct pt_resident *new_part, int *pt_change_tb)
{
	int i;
	struct storage_info s_info = { 0 };
...
static int update_msdos_partition(struct pt_resident *new_part, int *pt_change_tb)
{
	int err = 0;
	int i;
	int px;
...
static int update_disk_info(struct pt_resident *new_part, int *pt_change_tb)
{
	int err = 0;
	int i;
	int px;
...
static int pmt_upgrade_handler(struct DM_PARTITION_INFO_PACKET_x *packet)
{
	...
	memcpy(last_part, new_part, PART_NUM * sizeof(struct pt_resident));
...
static const struct file_operations pmt_cdev_ops = {
	.owner = THIS_MODULE,
