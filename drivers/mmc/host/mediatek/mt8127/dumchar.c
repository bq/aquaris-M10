...
#ifdef CONFIG_MTK_EMMC_SUPPORT
int eMMC_rw_x(loff_t addr, u32 *buffer, int host_num, int iswrite,
	      u32 totalsize, int transtype, enum Region part)
{
...
static ssize_t dumchar_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_err("[dumchar_read] It is forbidded to access %s with dumchar(/dev/%s)",
			dev->dumname, dev->dumname);
		pr_err("the %s partition is managed by filesystem!\n", dev->dumname);
...
ssize_t dumchar_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_info("[dumchar_write] It is forbidded to access %s with dumchar(/dev/%s)",
			dev->dumname, dev->dumname);
		pr_err("the %s partition is managed by filesystem!\n", dev->dumname);
...
static long dumchar_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_err("[dumchar_ioctl] It is forbidded to access %s with dumchar(/dev/%s)",
			dev->dumname, dev->dumname);
		pr_err("the %s partition is managed by filesystem!\n", dev->dumname);
...
loff_t dumchar_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_info("[dumchar_llseek] It is forbidded to access %s with dumchar(/dev/%s)",
			dev->dumname, dev->dumname);
		pr_err("the %s partition is managed by filesystem!\n", dev->dumname);
...
int dumchar_open(struct inode *inode, struct file *filp)
{
	...
	if (!strcmp(dev->dumname, "usrdata") || !strcmp(dev->dumname, "cache") ||
		...
		pr_err("[dumchar_open] It is forbidded to access %s with dumchar(/dev/%s)",
			dev->dumname, dev->dumname);
		pr_err("the %s partition is managed by filesystem!\n", dev->dumname);
		pr_info("[dumchar_open] show_stack*************************************\n");
		show_stack(NULL, NULL);
...
int dumchar_release(struct inode *inode, struct file *filp)
{
	...
	if (!strcmp(dev->dumname, "usrdata") || !strcmp(dev->dumname, "cache") ||
	    !strcmp(dev->dumname, "android") || !strcmp(dev->dumname, "fat")) {
		pr_err("[dumchar_release] It is forbidded to access %s with dumchar(/dev/%s)",
			dev->dumname, dev->dumname);
		pr_err("the %s partition is managed by filesystem!\n", dev->dumname);
	} else {
		filp_close(fo->act_filp, NULL);
...
static const struct file_operations dumchar_fops = {
	.owner = THIS_MODULE,
...
int dumchar_probe(struct platform_device *dev)
{
	...
	pr_info("[Dumchar_probe]There are 3 address in eMMC Project:Linear Address, Logical Address, Physical Address\n");
	pr_info("[Dumchar_probe]Linear Address: Used in scatter file, uboot, preloader,flash tool etc.\n");
	pr_info("[Dumchar_probe]Linear Address: MBR linear address is fixed in eMMCComo.mk,that is same for all chips in the project\n");
	pr_info("[Dumchar_probe]Logical Address: Used in /proc/dumchar_info, mmcblk0 etc. MBR logical address is 0\n");
	pr_info("[Dumchar_probe]Physical Address: Used in eMMC driver,MBR Physical Address = MBR Linear Address - (BOOT1 size + BOOT2 Size + RPMB Size)\n");
	pr_info
	...
	if (IS_ERR(dumchar_class)) {
		pr_info("DumChar: fail in class create");
...
static int dumchar_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < PART_NUM; i++) {
		if (dumchar_devices[i].type == NAND) {
			...
			default:
				strcpy(region, "ERROR");
				pr_info("ERROR, No such emmc region\n");
				break;
			}
...
static int __init dumchar_init(void)
{
	...
	if (result) {
		pr_err("DUMCHAR: Failed to register device!\n");
		goto fail_driver_register;
	}
...
late_initcall(dumchar_init); /* wait mmc block init done */
