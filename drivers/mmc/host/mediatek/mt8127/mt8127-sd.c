...
static const struct mt81xx_mmc_compatible mt8127_compat = {
	.clk_div_bits = 8,
};
...
static const struct of_device_id msdc_of_ids[] = {
	{.compatible = "mediatek,mt8127-mmc", .data = &mt8127_compat},
...
struct msdc_host *mtk_msdc_host[3] = { NULL, NULL, NULL };
...
struct msdc_host *msdc_get_host(int host_function, bool boot, bool secondary)
{
	...
	for (; host_index < 3; ++host_index) {
		if (!mtk_msdc_host[host_index])
			continue;
		if (host_function == mtk_msdc_host[host_index]->host_function &&
		    boot == mtk_msdc_host[host_index]->boot) {
	...
	if (host == NULL) {
		pr_err("Could not get host with host_function: %d, boot: %d\n",
		       host_function, boot);
	}
...
int msdc_get_reserve(void)
{
	...
	pr_info("l_otp_reserve = 0x%x blocks,l_mtk_reserve = 0x%x blocks\n",
		l_otp_reserve, l_mtk_reserve);

	return (l_otp_reserve + l_mtk_reserve);
}
...
u64 msdc_get_capacity(void)
{
	...
	for (index = 0; index < 3; ++index) {
		if ((mtk_msdc_host[index] != NULL) && (mtk_msdc_host[index]->boot)) {
			  ...
			  struct mmc_request *mrq, struct mmc_command *cmd)
{
	...
	if (cmd->error && cmd->opcode != MMC_SEND_TUNING_BLOCK &&
	    cmd->opcode != MMC_SEND_TUNING_BLOCK_HS200)
		if (!(host->host_id == 1 && (cmd->opcode == MMC_APP_CMD
			|| cmd->opcode == SD_IO_RW_DIRECT
			|| cmd->opcode == SD_IO_SEND_OP_COND
			|| cmd->opcode == MMC_SEND_OP_COND)))
			pr_err("msdc%d cmd=%d cmd_error=%d\n",
				host->host_id, cmd->opcode, cmd->error);
		...
		struct mmc_request *mrq, struct mmc_command *cmd)
{
	...
	if ((cmd->opcode == MMC_READ_SINGLE_BLOCK ||
	     cmd->opcode == MMC_READ_MULTIPLE_BLOCK) && cmd->arg == 0)
		complete(&host->card_init_done);
#endif
}
	...
	if (check_data || (stop && stop->error)) {
		...
		if ((events & MSDC_INT_XFER_COMPL) && (!stop || !stop->error)) {
			...
			if (events & MSDC_INT_DATTMO) {
				...
				data->error = -EILSEQ;
				if (data->flags & MMC_DATA_WRITE) {
					sdr_get_field(host->base + MSDC_PAD_TUNE,
						      MSDC_PAD_TUNE_DATWRDLY,
						      &host->hs200_write_int_delay);
					host->hs200_write_int_delay =
					(host->hs200_write_int_delay + 8) % 32;
				}
			}
...
static struct msdc_delay_phase get_best_delay(struct msdc_host *host, u32 delay)
{
	...
	pr_err("msdc%d [map:%x] [maxlen:%d] [final:%d]\n",
		 host->host_id, delay, len_final, final_phase);
...
skip_fall:
	...
	pr_err("msdc%d Final internal delay: 0x%x\n", host->host_id, internal_delay);
	internal_delay_phase = get_best_delay(host, internal_delay);
	sdr_set_field(host->base + tune_reg, MSDC_PAD_TUNE_CMDRRDLY,
				internal_delay_phase.final_phase);

	pr_err("msdc%d Final cmd pad delay: %x\n", host->host_id, final_delay);
...
skip_fall:
	...
	if (mmc->ios.timing == MMC_TIMING_MMC_HS200 ||
		...
		sdr_set_field(host->base + tune_reg, MSDC_PAD_TUNE_DATWRDLY,
			      host->hs200_write_int_delay);
	pr_err("msdc%d Final data pad delay: %x\n", host->host_id, final_delay);
	return final_delay == 0xff ? -EIO : 0;
}

static int msdc_send_status(struct mmc_card *card, u32 *status,
				    bool ignore_crc)
{
	int err;
	struct mmc_command cmd = {0};
...
static void msdc_send_stop(struct mmc_host *host)
{
	struct mmc_command cmd = {0};

	cmd.opcode = MMC_STOP_TRANSMISSION;
	cmd.flags = MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
	mmc_wait_for_cmd(host, &cmd, 5);
}
...
static int msdc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	pm_runtime_get_sync(host->dev);
	/*
	 * before tune, need ensure card is in transfer state.
	 * this case is only for CRC error form dumchar/pmt code
	 */
	if (mmc->card) {
		ret = msdc_send_status(mmc->card, &status, false);
		if (ret)
			dev_err(host->dev, "failed to send CMD13!\n");
	}
	if (ret || R1_CURRENT_STATE(status) == R1_STATE_DATA ||
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (ret)
		goto host_free;
#ifdef PORTING_FOR_MT8127_TURNKEY
	if (of_property_read_u32(pdev->dev.of_node, "host_function", &host->host_function)) {
		dev_err(&pdev->dev, "Please add host_function at DTS!\n");
		ret = -EINVAL;
		goto host_free;
	} else {
		if (host->host_function != MSDC_EMMC &&
		    host->host_function != MSDC_SD && host->host_function != MSDC_SDIO) {
			dev_err(&pdev->dev, "Invalid host_function!\n");
			ret = -EINVAL;
			goto host_free;
		}
	}
	...
	if (of_property_read_u32(pdev->dev.of_node, "host_id", &host->host_id)) {
		...
		ret = -EINVAL;
		goto host_free;
	} else if (host->host_id >= 3) {
		dev_err(&pdev->dev, "Invalid host_id!\n");
		ret = -EINVAL;
		goto host_free;
	} else {
		mtk_msdc_host[host->host_id] = host;
	}

	if (of_property_read_u32(pdev->dev.of_node, "boot", &host->boot)) {
		dev_err(&pdev->dev, "Please add boot at DTS!\n");
		ret = -EINVAL;
		goto host_free;
	}
