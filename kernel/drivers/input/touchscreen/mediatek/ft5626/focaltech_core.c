...
kal_bool get_charger_status(void)
{
...
#ifdef TPD_HAVE_BUTTON
	if((x >= (TPD_BUTTON_X - TPD_BUTTON_HEIGH/2))&&(x <= (TPD_BUTTON_X + TPD_BUTTON_HEIGH/2)))
	{
		tpd_button(x, y, 1);
		return;
	}
#endif
	if(x > TPD_RES_X || y > TPD_RES_Y)
	{
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	for(i = 0; i < point_num; i++)  
	{
		...
		#ifdef TP_F1005_HD
		cinfo->y[i] = (cinfo->y[i] * 2)/3;
		#endif
	}

#ifdef CHANGE_THRESHOLD_IN_CHARGE
	ret = fts_read_reg(fts_i2c_client, 0x8B, &threshold_value);
	if ( ret < 0)
	{
		return true;
	}
	else
	{
		...
		if(1 == get_charger_status())
		{
			...
			{
				fts_write_reg(fts_i2c_client,0x8B,1);
			}
		}
		else  /*Not DC in */
		{
			...
			{
				fts_write_reg(fts_i2c_client,0x8B,0);
			}
		}
	}
#endif
...
***********************************************************************/
 ...
 {	 
	...
	if((fts_read_reg(fts_i2c_client, 0x00, &data))< 0)
	{
		tpd_load_status = 0;
		printk("[FTS]tpd_probe I2C transfer error, line: %d\n", __LINE__);
	...
	*/
	...
	fts_get_upgrade_array();
		...
		if (NORMAL_BOOT == get_boot_mode() || SW_REBOOT == get_boot_mode()){
