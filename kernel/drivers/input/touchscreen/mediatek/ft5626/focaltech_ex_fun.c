...
static int fts_debug_write(struct file *filp, const char __user *buff, unsigned long count, void *data)
{
	...
	case PROC_WRITE_DATA:
		writelen = count - 1;
		//printk("xkpeng4 count = %lx value1 = %x value2 = %x\n",count,writebuf[1],writebuf[2]);
		if(writelen > 0){
			ret = fts_i2c_write(fts_i2c_client, writebuf + 1, writelen);
			if (ret < 0) {
				dev_err(&fts_i2c_client->dev, "%s:write iic error\n", __func__);
...
***********************************************************************/
static int fts_debug_read(struct file *filp, char __user *buff, unsigned long count, void *data)
{
	int ret = 0;
	unsigned long num_read_chars = 0;
