...
//static variable
g3dExecuteInfo gExecInfo;
static int g3dRefCount = 0;

#ifdef PATTERN_DUMP
void* gfAllocInfoKernal = NULL;
#endif

__DEFINE_SEM_LOCK(g3dLock);
__DEFINE_SPIN_LOCK(g3dPwrStateLock);
...
G3DKMD_APICALL int G3DAPIENTRY g3dKmdInitialize(void)
{
    ...
    if (g3dRefCount == 0)
    {
        ...
        // 1. reset list info
        {
            memset((void *)&gExecInfo, 0, sizeof(g3dExecuteInfo));
        ...
        if ( value )
        {
            ...
            {
                KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_API, "allocatePhysicalMemory for memPadding fail\n");
                YL_KMD_ASSERT(0);
                return 0;
            }
        }
      ...
      // Non polling wait queues initialization
        ...
        init_wait(&gExecInfo.s_waitqueue_cl);
#endif
...
static unsigned int _g3dKmdAssignExeInst(G3DKMD_DEV_REG_PARAM* param, pG3dTask task)
{
    int taskIdx;
    ...
    for (taskIdx = 0; taskIdx < gExecInfo.taskInfoCount; taskIdx++)
    {
        ...
        {
            KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_API,"found same pid, pid 0x%x, inst_idx %d, (old task_idx %d, cur task_idx %d)\n",
    ...
    if (instIdx == EXE_INST_NONE)
    {
...
#endif
        {
            ...
            if (instIdx == EXE_INST_NONE)
            {
                ...
                KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_API,"g3dKmdExeInstCreate : out of memory\n");
                __SEM_UNLOCK(g3dLock);
                return G3DKMD_FALSE;
            }
...
#ifdef ANDROID_UNIQUE_QUEUE
            {
                pG3dExeInst pInst = gExecInfo.exeList[instIdx];
...
G3DKMD_APICALL int G3DAPIENTRY g3dKmdRegisterContext(G3DKMD_DEV_REG_PARAM* param)
{
    ...
    pG3dTask task = g3dKmdCreateTask(param->cmdBufStart, param->cmdBufSize, param->flag);
    ...
    if (gExecInfo.exeuteMode == eG3dExecMode_single_queue)
    {
        ...
        {
            // set addr for dram
            pG3dExeInst pInst = gExecInfo.exeList[0];
            ...
            pInfo->reg_mx_secure1_top_addr.s.mx_secure1_top_addr   = task->frameCmd_base + task->frameCmd_size;
        }
    }
#endif
...
G3DKMD_APICALL void G3DAPIENTRY g3dKmdReleaseContext(int taskID, int abnormal)
{
    ...
    if (task)
    {
...
#ifndef ANDROID_PREF_NDEBUG
        {
            ...
            KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_API, "task[%03d] average queued HT (%d.%02d)(%d,%d) when flush cmd notify\n", taskID, div, rem,
                  task->accFlushHTCmdCnt,task->htCmdCnt);
            rem = 0;
        }
        ...
        KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_API, "g3dKmdReleaseContext done: taskID %d, instIdx %d\n", taskID, instIdx);
    }
...
{
   pG3dTask task = g3dKmdGetTask(taskID);
...
G3DKMD_APICALL void G3DAPIENTRY g3dKmdLockCheckDone(unsigned int num_task, struct ionGrallocLockInfo *data)
{
    ...
    for (i=0;i<num_task && ptr != NULL;i++)
    {
        ...
        {
            pG3dTask taskj = g3dKmdGetTask(j);
            
            if (taskj && j == data[i].taskID && taskj->pid == (pid_t)data[i].pid)
            {
#ifdef G3D_SUPPORT_FLUSHTRIGGER
                if (G3D_STAMP_NOT_FLUSH(data[i].write_stamp, taskj->lastFlushStamp, taskj->lastFlushStamp) ||
                    G3D_STAMP_NOT_FLUSH(data[i].read_stamp, taskj->lastFlushStamp, taskj->lastFlushStamp))
                {
                    unsigned int value = 1;
                    g3dKmdTaskFlushTrigger(1, j, &value);
                    KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_API, "flushtrigger taskID[%d,%d] by stamp (%d, %d)\n",
    ...
    for (i=0;i<num_task;i++)
    {
        ...
        for (j = 0; j < gExecInfo.taskInfoCount; j++)
        {
            ...
            if (taskj && j == data[i].taskID && taskj->pid == (pid_t)data[i].pid)
            {
                ...
                if (G3D_STAMP_NOT_FLUSH(data[i].read_stamp, taskj->lastFlushStamp, taskj->lastFlushStamp))
                    all_result = result = 0;
...
{
    pG3dTask curtask, objtask, tmptask;
    int *sortingBuffer = NULL;
    int i;
    int index;


    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_API,"g3dKmdWaitIdStamp[%03d] : need wait task[%03d](stamp = 0x%08x)\n",taskID,objIndex,objstamp);
    ...
    sortingBuffer = (int *)G3DKMDMALLOC(sizeof(pG3dTask)*gExecInfo.taskInfoCount);
    ...
    for(i=1;i<index;i++)
    {
        ...
        if((objtask->state == G3DTASKSTATE_RUNNING))
        {
            ...
            if((objtask->htInfo) && (objtask->htInfo->rdPtr == objtask->htInfo->wtPtr)) //remove it by handling hw interrupt
            {
            ...
            if((objtask->htInfo) && (objtask->htInfo->rdPtr != objtask->htInfo->wtPtr))
            {
                ...
                for(i=0; i< MAX_HT_COMMAND; i++)
                {
                    ...
                    if( index == objtask->htInfo->wtPtr )
                        break;
                    g3dKmdCopyCmd(curtask->htInfo,(pG3dHtCommand)&objtask->htInfo->htCommand[index]);

                    //g3dKmdAddList(curtask->htInfo,objtask->htInfo->shortInfo[index].index, objtask->htInfo->shortInfo[index].wptr);
                }
            }
        }
 ...
 */
#if defined(ENABLE_NON_POLLING_FLUSHCMD)
//static int _checkIfEnoughHtSlots(pG3dExeInst pInst)
...
//    __SEM_UNLOCK(g3dLock);
    ...
    return g3dkmd_wait_event_interruptible_timeout(
...
G3DKMD_APICALL G3DKMDRETVAL G3DAPIENTRY g3dKmdFlushCommand(int taskID, void *head, void *tail,
                                                           ...
                                                           unsigned int flushDCache)
{
    ...
    if(task == NULL)
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_API, "g3dKmdFlushCommand task in gExecInfo is NULL\n");
#if defined(CONFIG_HAS_EARLYSUSPEND) && defined(G3DKMD_POWER_MANAGEMENT)
        gpu_may_power_off();
#endif
        __SEM_UNLOCK(g3dLock);
    ...
    if((pInst == NULL) || (pInst->pHtDataForSwWrite->htInfo == NULL) || (pInst->pHtDataForSwWrite->htInfo->htCommand == NULL))
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_API, "g3dKmdFlushCommand task pHtDataForSwWrite->htInfo or it's htCommand is NULL\n");
#if defined(CONFIG_HAS_EARLYSUSPEND) && defined(G3DKMD_POWER_MANAGEMENT)
        gpu_may_power_off();
#endif
        __SEM_UNLOCK(g3dLock);
    ...
    __SEM_UNLOCK(g3dLock);
    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_API, "g3dKmdFlushCommand wait-queue in, taskID %d, Inst State %d\n", taskID, pInst->state);
    ...
    case -ERESTARTSYS: /* signal happened, go back to usermode immediately, and retry */
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_API, "g3dKmdFlushCommand wait-queue out for system interrupted, taskID %d, Inst State %d\n", taskID, pInst->state);
        tester_wq_singaled(taskID);
        gpu_decrement_flush_count();
        return G3DKMDRET_FAIL;

    case 0:            /* timeout */
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_API, "g3dKmdFlushCommand wait-queue out for timeout, taskID %d, Inst State %d\n", taskID, pInst->state);
    ...
    default:           /* the condition is true( there are enough Ht slots) */
        ...
        KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_API, "g3dKmdFlushCommand wait-queue out for default, taskID %d, Inst State %d\n", taskID, pInst->state);
    ...
    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_API, "g3dKmdFlushCommand in, taskID %d, Inst State %d\n", taskID, pInst->state);
    KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_FENCE, "g3dKmdFlushCommand, taskID %d, lastFlushStamp 0x%x\n", taskID, lastFlushStamp);
    //KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_API,"g3dKmdFlushCommand SW[%03d] : (head,tail) = (0x%08x,0x%08x); htCommand %p, htBuffer 0x%x\n", taskID, head, tail, pInst->pHtDataForSwWrite->htInfo->htCommand, pInst->htBuffer.hwAddr0);

    if((unsigned)(g3dKmdCheckFreeHtSlots(pInst->hwChannel, pInst->pHtDataForSwWrite->htInfo)) < HT_BUF_LOW_THD) //always 2
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_API, "g3dKmdFlushCommand available free slot is not enought\n");
#if defined(CONFIG_HAS_EARLYSUSPEND) && defined(G3DKMD_POWER_MANAGEMENT)
        gpu_may_power_off();
#endif
        __SEM_UNLOCK(g3dLock);
...
#if defined(G3D_HW) && !defined(ANDROID_PREF_NDEBUG)
{
    int queued_slots = g3dKmdGetQueuedHtSlots(pInst->hwChannel, pInst->pHtDataForHwRead->htInfo); // Louis: don't know if this is for HwRead or SwWrite.

    // HtData for HwRead and SwWrite could be different htInstance.
    if (pInst->pHtDataForHwRead != pInst->pHtDataForSwWrite)  // if pointing to differnt htInstance, add slots in HtForSwWrite.
    {
        int slots = g3dKmdGetQueuedHtSlots(pInst->hwChannel, pInst->pHtDataForSwWrite->htInfo);
        queue_slots = (queue_slots>0 && slots>0)? queue_slots : queue_slots+slots;
    }
    ...
    g3dKmdAddHTCmd(pInst, head, tail, task->task_idx);
    //g3dKmdAddHTCmd(pInst->pHtDataForSwWrite->htInfo, head, tail, task->task_idx);

    pInst->dataSynced = G3DKMD_FALSE;

#ifdef G3DKMD_MET_CTX_SWITCH
    met_tag_oneshot(0x8163, "g3dKmdFlushCommand", task->inst_idx);
    met_tag_oneshot(0x8163, "HT_CMD_NS", (HT_BUF_SIZE - g3dKmdCheckFreeHtSlots(pInst->hwChannel, pInst->pHtDataForSwWrite->htInfo))/HT_CMD_SIZE);
#endif
    ...
    if (pInst->state == G3DEXESTATE_RUNNING)
    {
        ...
        if (g3dKmdCfgGet(CFG_GCE_ENABLE))
        {
            ...
            {
                KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_API, "g3dKmdGceSendCommand available free slot is not enought\n");
#if defined(CONFIG_HAS_EARLYSUSPEND) && defined(G3DKMD_POWER_MANAGEMENT)
                gpu_may_power_off();
#endif
                __SEM_UNLOCK(g3dLock);
...
#ifdef YL_HW_DEBUG_DUMP_TEMP
            if ( lastFlushStamp - 1 >= g3dKmdHwDbgGetBeginFrameID() && lastFlushStamp - 1 <= g3dKmdHwDbgGetEndFrameID() )
            {
                unsigned int i;
    ...
    if (gExecInfo.currentExecInst && g3dKmdIsHtInfoEmpty(gExecInfo.currentExecInst->hwChannel, gExecInfo.currentExecInst->pHtDataForHwRead->htInfo))
    {
        KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_API, "g3dKmdFlushCommand triggers context switch because active task empty\n");
#ifdef G3DKMD_MET_CTX_SWITCH
    ...
    else if (pInst->isUrgentPrio && (pInst->state != G3DEXESTATE_RUNNING))
    {
        ...
        KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_API, "g3dKmdFlushCommand triggers context switch for urgent task\n");
#ifdef G3DKMD_MET_CTX_SWITCH
    ...
    if (0) //try to snapshot pattern from current states //@@
    {
        ...
        if (!dump)
        {
            if (g3dKmdSanpshotPattern(pInst, SSP_DUMP_PREVIOUS) == G3DKMDRET_SUCCESS)
                dump = 1;
        }
    }
#endif
...
void g3dKmdRegRestart(int taskID, int mode)
{
    ...
    if (!hasDumpSSP)
    {
        ...
        g3dKmdSanpshotPattern(pInst, SSP_DUMP_CURRENT);
    }
#endif
    ...
    {
        KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_API, "[%s] Reset instance info, %d\n", __FUNCTION__, g3dKmdExeInstGetIdx(pInst), pInst->hwChannel);
        gExecInfo.hwStatus[pInst->hwChannel].inst_idx   = EXE_INST_NONE;
        gExecInfo.hwStatus[pInst->hwChannel].state      = G3DKMD_HWSTATE_IDLE;
        pInst->hwChannel    = G3DKMD_HWCHANNEL_NONE;
        pInst->time_slice   = 0;
...
// not let user space trigger hw reset if reset by irq
#ifndef G3DKMD_SUPPORT_AUTO_RECOVERY
        {
            ...
            {
                g3dKmdRegRestart(param, G3DKMD_RECOVERY_RETRY);
        ...
        {
            pG3dTask task = g3dKmdGetTask(param);
            ret = (task == NULL) ? 0 : task->hw_reset_cnt;
            break;
        }
#endif
#if defined(ANDROID) && !defined(linux_user_mode)
        case G3DKMD_ACTION_QUERY_ION_MIRROR:
            ret = ion_mirror_check((unsigned long)param);
            break;
#endif
...
G3DKMD_APICALL void G3DAPIENTRY g3dKmdSeviceISR(void)
{
    ...
    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_API,"g3dKmdSeviceISR  HW[%03d] task[%03d] readptr = 0x%08x \n", hwChannel, index, hwReadPtr);
    ...
    if(task) //task == null ==> error ??
    {
        ...
        if(task->htInfo)
        {
            task->htInfo->rdPtr = g3dKmdCaculateIndex(task->htInfo,(unsigned int)hwReadPtr,task->htInfo->rdPtr);
...
G3DKMD_APICALL void G3DAPIENTRY g3dKmdDumpCommandBegin(int taskID, unsigned int dumpCount)
{
    ...
    if ((g3dKmdCfgGet(CFG_DUMP_CE_MODE) != G3DKMD_DUMP_CE_MODE_DRIVER) || (dumpCount == g3dKmdCfgGet(CFG_DUMP_START_FRM)))
    {   // in g3dKmdInitialize, the first riu file is not opened
        ...
        if (g3dKmdCfgGet(CFG_GCE_ENABLE))
        {
            ...
            g3dKmdMmcqSetJumpTarget(pInst->mmcq_handle, g3dKmdMmcqGetAddr(pInst->mmcq_handle, MMCQ_ADDR_WRITE_PTR));
        }
        else
    #endif
        {
...
G3DKMD_APICALL unsigned int G3DAPIENTRY g3dKmdGetDrvInfo(unsigned int cmd, unsigned int arg, void* buf, unsigned int bufSize, void *fd)
{
    unsigned int size = 0;
    UNUSED(fd);
...
G3DKMD_APICALL unsigned int G3DAPIENTRY g3dKmdSetDrvInfo(unsigned int cmd, unsigned int arg, void* buf, unsigned int bufSize, void* fd)
{
...
G3DKMD_APICALL void G3DAPIENTRY g3dKmdPerfModeEnable(void)
{
    ...
    if (gExecInfo.kmif_ops.notify_g3d_performance_mode)
        ...
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_API, "notify_g3d_performance_mode() is not found!\n");
}
...
G3DKMD_APICALL unsigned int G3DAPIENTRY g3dKmdSSPControl(int bIsQuery, int cmd, void *pBuf, unsigned int bufSize)
{
    ...
    switch (cmd)
    {
        case G3DKMD_DRV_SSPCMD_DUMP:
        {
            ...
            KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_API, "[SSP]%s taskID = %d, mode = %d\n", __FUNCTION__, param->taskID, param->mode);

            pInst = (param->taskID < 0) ? gExecInfo.currentExecInst : g3dKmdTaskGetInstPtr(param->taskID);

            retval = g3dKmdSanpshotPattern(pInst, param->mode);
