...
// for semaphore Lock
#if (defined(_WIN32) || (defined(linux) && defined(linux_user_mode)))
#define __DEFINE_SEM_LOCK(lock)         static int lock = 0;
/// Simple implementation.... but not strictly correct
#define __SEM_LOCK(lock)                do{while(lock) {}; lock=1;}while(0);
#define __SEM_UNLOCK(lock)              do{lock=0;}while(0);

#elif defined(linux) 
#include <linux/semaphore.h> 
#include <linux/uaccess.h>
    #ifdef DEFINE_SEMAPHORE // 3.0.13 
#define __DEFINE_SEM_LOCK(lock)         static DEFINE_SEMAPHORE(lock);
    #else // 2.6.29 
#define __DEFINE_SEM_LOCK(lock)         static DECLARE_MUTEX(lock);
    #endif
#define __SEM_LOCK(lock)                do{ down(&lock); }while(0);
#define __SEM_UNLOCK(lock)              do{ up(&lock); }while(0);
#endif
...
#define __DEFINE_SPIN_LOCK(lock)        static DEFINE_SPINLOCK(lock); 
#define __SPIN_LOCK(lock)               do{ spin_lock (&lock); }while(0);
#define __SPIN_UNLOCK(lock)             do{ spin_unlock (&lock); }while(0);
#define __SPIN_LOCK_BH(lock)            do{ spin_lock_bh (&lock); }while(0);
#define __SPIN_UNLOCK_BH(lock)          do{ spin_unlock_bh (&lock); }while(0);
#define __SPIN_LOCK2_DECLARE(flag)      unsigned long flag;
#define __SPIN_LOCK2(lockname, flag)    do{ spin_lock_irqsave(&lockname, flag); }while(0);
#define __SPIN_UNLOCK2(lockname, flag)  do{ spin_unlock_irqrestore(&lockname, flag); }while(0);
#endif
...
#define G3DKMDMALLOC(_SIZE)             g3dkmdmalloc(_SIZE, GFP_KERNEL, __FILE__, __FUNCTION__, __LINE__)
#define G3DKMDMALLOC_ATOMIC(_SIZE)      g3dkmdmalloc(_SIZE, GFP_ATOMIC, __FILE__, __FUNCTION__, __LINE__)
#define G3DKMDMALLOCFLAGS(_SIZE, flags) g3dkmdmalloc(_SIZE, flags, __FILE__, __FUNCTION__, __LINE__)
#define G3DKMDFREE(_PTR)        g3dkmdfree(_PTR, __FILE__, __FUNCTION__, __LINE__)
#define G3DKMDVMALLOC(_SIZE)    g3dkmdvmalloc(_SIZE, __FILE__, __FUNCTION__, __LINE__)
#define G3DKMDVFREE(_PTR)       g3dkmdvfree(_PTR, __FILE__, __FUNCTION__, __LINE__)
#else
