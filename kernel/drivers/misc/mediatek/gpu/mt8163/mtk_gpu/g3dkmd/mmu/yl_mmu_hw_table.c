...
//YL_BOOL
//PageTable_Initialize( FirstPageTable pageTable, FirstMetaTable metaTable )
//{
//    yl_mmu_zeromem( pageTable, sizeof(FirstPageTableRec) );
//
//    //*metaTable = (FirstMetaTable) yl_mmu_alloc( sizeof( FirstMetaTableRec ) );
//    //if ( *metaTable == NULL ) return 0;
//    //yl_mmu_zeromem( *metaTable, sizeof(FirstMetaTableRec) );
//
//    yl_mmu_zeromem( metaTable, sizeof(FirstMetaTableRec) );
//
//    return 1;
...
void 
PageTable_Release( FirstPageTable master, FirstMetaTable metaTable )
{
    {
        unsigned int i;
...
// Return 0 if failed
YL_BOOL 
Section_register( Mapper m, unsigned short index0, pa_t pa, Trunk trunk, va_t va, unsigned int flag )
{
    G3dMemory * page_table   = & m->Page_mem;
    FirstMetaTable metaTable = m->Meta_space;

#if MMU_MIRROR
    G3dMemory * mirror_table = & m->Mirror_mem;
    FirstPageTable mirror = (FirstPageTable)mirror_table->data;
#else
    FirstPageTable mirror = NULL;
#endif
    ...
    // Page Table
    FirstPageTable master = (FirstPageTable)page_table->data;
    FirstPageEntryRec   prepared_entry;
    ...
    {
        FirstPageTable lookback = (FirstPageTable)lookback_mem->data;
...
// Return 0 if failed
YL_BOOL 
Small_register( Mapper m, PageIndexRec index, pa_t pa, Trunk trunk, va_t va, unsigned int flag )
{
    ...
    FirstPageTable mirror  =  (FirstPageTable)mirror_mem->data;
#else
    FirstPageTable mirror = NULL;
#endif
    ...
    FirstPageTable master = (FirstPageTable)page_mem->data;
    FirstPageEntryRec   prepared_entry      = master->entries[index.s.index0]; 
    SecondPageEntryRec  prepared_entry2;
    SecondPageTable     start_address2      = NULL;
    ...
    // Note that the second level cleanup occured when (a) module initializing (b) when 2nd level page is released
    ...
    YL_MMU_ASSERT( ((unsigned int)(uintptr_t)start_address2 & SMALL_PAGE_LEVEL0_BASE_ADDRESS_MASK) == (unsigned int)(uintptr_t)start_address2 );
    ...
    {
        // META
        SecondMetaTable    meta_start_address2 = NULL;
    ...
    if (mirror)
    {
        ...
        start_address2_pa = (pa_t)g3dbaseVaToContinuousPa( mirror_mem, (void*)start_address2 );
        ...
        prepared_entry2.all             = ((unsigned int)index.s.index0 << G3D_VA_LEVEL0_INDEX_START_BIT) + 
    ...
    if (lookback_mem->data)
    {
        ...
        start_address2_pa = (pa_t)g3dbaseVaToContinuousPa( lookback_mem, (void*)start_address2 );

        YL_MMU_ASSERT(start_address2->entries[index.s.index1].all == 0);
        YL_MMU_ASSERT( ((unsigned int)(uintptr_t)start_address2 & SMALL_PAGE_LEVEL0_BASE_ADDRESS_MASK) == (unsigned int)(uintptr_t)start_address2 );
        YL_MMU_ASSERT( ((unsigned int)va & SMALL_PAGE_LEVEL1_BASE_ADDRESS_MASK) == (unsigned int)va );
...
// Return 0 if failed
YL_BOOL 
Section_unregister( Mapper m, unsigned short index0 )
{
    G3dMemory * page_mem     = & m->Page_mem;
    FirstMetaTable metaTable = m->Meta_space;
...
// Return 0 if failed
YL_BOOL 
Small_unregister( Mapper m, PageIndexRec index )
{
    G3dMemory * page_mem     = & m->Page_mem;
    FirstMetaTable metaTable = m->Meta_space;
    ...
    SecondPageTable    start_address2;
    SecondMetaTable    meta_start_address2;
