...
{
	{.name =   "disable", .init_func =   gpad_ipem_dvfs_disable_init, .exit_func =   gpad_ipem_dvfs_disable_exit, .do_func =   gpad_ipem_dvfs_disable_do, .update_func =   gpad_ipem_dvfs_disable_update},
	{.name =    "lookup", .init_func =    gpad_ipem_dvfs_lookup_init, .exit_func =    gpad_ipem_dvfs_lookup_exit, .do_func =    gpad_ipem_dvfs_lookup_do, .update_func =	gpad_ipem_dvfs_lookup_update},
	{.name =   "thermal", .init_func =   gpad_ipem_dvfs_thermal_init, .exit_func =   gpad_ipem_dvfs_thermal_exit, .do_func =   gpad_ipem_dvfs_thermal_do, .update_func =   gpad_ipem_dvfs_thermal_update},
	{.name = "lookahead", .init_func = gpad_ipem_dvfs_lookahead_init, .exit_func = gpad_ipem_dvfs_lookahead_exit, .do_func = gpad_ipem_dvfs_lookahead_do, .update_func = gpad_ipem_dvfs_lookahead_update},
	{.name =    "qlearn", .init_func =    gpad_ipem_dvfs_qlearn_init, .exit_func =    gpad_ipem_dvfs_qlearn_exit, .do_func =    gpad_ipem_dvfs_qlearn_do, .update_func =	gpad_ipem_dvfs_qlearn_update},
	{.name =     "sarsa", .init_func =     gpad_ipem_dvfs_sarsa_init, .exit_func =     gpad_ipem_dvfs_sarsa_exit, .do_func =     gpad_ipem_dvfs_sarsa_do, .update_func =	 gpad_ipem_dvfs_sarsa_update},
	{.name =     "type1", .init_func =     gpad_ipem_dvfs_type1_init, .exit_func =     gpad_ipem_dvfs_type1_exit, .do_func =     gpad_ipem_dvfs_type1_do, .update_func =	 gpad_ipem_dvfs_type1_update},
	{.name =     "type2", .init_func =     gpad_ipem_dvfs_type2_init, .exit_func =     gpad_ipem_dvfs_type2_exit, .do_func =     gpad_ipem_dvfs_type2_do, .update_func =	 gpad_ipem_dvfs_type2_update},
...
static unsigned long g_touch_finish_timestamp = 0;
...
static void gpad_ipem_dvfs_sarsa_update(struct gpad_ipem_work_info *work_info)
{
	...
	GPAD_LOG_INFO("[IPEM] %s: state_prev = %u, action_prev = %u, stateaction_prev = %u, q_prev = %s\n", __FUNCTION__, state_prev, action_prev, stateaction_prev, gpad_ipem_fixedpt_cstr(q_prev));
	GPAD_LOG_INFO("[IPEM] %s: state_next = %u, action_next = %u, stateaction_next = %u, q_next = %s\n", __FUNCTION__, state_next, action_next, stateaction_next, gpad_ipem_fixedpt_cstr(q_next));
	GPAD_LOG_INFO("[IPEM] %s: reward = %s\n", __FUNCTION__, gpad_ipem_fixedpt_cstr(reward));
	GPAD_LOG_INFO("[IPEM] %s: q_delta = %s\n", __FUNCTION__, gpad_ipem_fixedpt_cstr(q_delta));
	GPAD_LOG_INFO("[IPEM] %s: q = q_prev + q_delta = %s\n", __FUNCTION__, gpad_ipem_fixedpt_cstr(q));
}
...
void gpad_ipem_dvfs_policy_set(struct gpad_device *dev, unsigned int policy_id)
{
	...
	if (policy_id != ipem_info->dvfs_info.policy_id) {
		...
		GPAD_LOG_INFO("[IPEM] %s: policy_id = %u, name = %s, init_func = %p, exit_func = %p, do_func = %p, update_func = %p\n", __FUNCTION__, policy_id, ipem_info->dvfs_info.policies[policy_id].name, (void *)ipem_info->dvfs_info.policies[policy_id].init_func, (void *)ipem_info->dvfs_info.policies[policy_id].exit_func, (void *)ipem_info->dvfs_info.policies[policy_id].do_func, (void *)ipem_info->dvfs_info.policies[policy_id].update_func);
	}
    UNLOCK_IPEM();
}
