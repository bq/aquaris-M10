...
#if defined(SEMU_QEMU) || defined(QEMUL64)
#undef CONFIG_OF
#else
...
#ifdef USING_MISC_DEVICE 
static struct platform_device *g_g3d_pdev = NULL;
...
static int _g3d_NC_Page_count = 0;
...
inline struct page* _g3dkmd_NC_allocpages(gfp_t gfp_mask)
{
    ...
    if (page != 0) {
        _g3d_NC_Page_count++;
        KMDDPF_ENG(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_IOCTL, "[NON_CACHE_MEMORY]: page:%p (%d)\n", page, _g3d_NC_Page_count);
        ...
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "[NON_CACHE_MEMORY]: alloc page fail. (%d)\n", _g3d_NC_Page_count);
    }
    return NULL;
}
...
__DEFINE_SEM_LOCK(memCleanLock);

static struct kmem_cache* g3d_memlist_cachep = NULL;
...
void g3d_addMemList(g3dkmd_MemListNode **head, uintptr_t index, uintptr_t data0, unsigned int data1)
{
    ...
    (g3dkmd_MemListNode*)kmem_cache_alloc(g3d_memlist_cachep, GFP_KERNEL);
    if (pNode == NULL) {
         return;
    }
...
#define ADD_TASK_LIST(private_data, idx)  \
        if (private_data) { g3d_addMemList(&((g3d_private_data*)private_data)->gTaskhead, idx , 0, 0); }
#define DEL_TASK_LIST(private_data, idx)  \
        if (private_data) { g3d_delMemList(&((g3d_private_data*)private_data)->gTaskhead, idx); }
#define ADD_PHYMEM_LIST(private_data, idx)  \
        if (private_data) { g3d_addMemList(&((g3d_private_data*)private_data)->gPhyMemhead, idx , 0, 0); }
#define DEL_PHYMEM_LIST(private_data, idx)  \
        if (private_data) { g3d_delMemList(&((g3d_private_data*)private_data)->gPhyMemhead, idx); }
#define ADD_VIRTMEM_LIST(private_data, idx ,data) \
        if (private_data) { g3d_addMemList(&((g3d_private_data*)private_data)->gVirtMemhead, idx ,data, 0); }
#define DEL_VIRTMEM_LIST(private_data, idx) \
        if (private_data) { g3d_delMemList(&((g3d_private_data*)private_data)->gVirtMemhead, idx); }
#define ADD_G3DMMU_LIST(private_data, idx ,data0, data1) \
        if (private_data) { g3d_addMemList(&((g3d_private_data*)private_data)->gG3DMMUhead, idx, (uintptr_t)data0, (unsigned int)data1); }
#define DEL_G3DMMU_LIST(private_data, idx) \
        if (private_data) { g3d_delMemList(&((g3d_private_data*)private_data)->gG3DMMUhead, idx); }


#define ADD_VMA_PAGE_LIST(private_data, idx)  \
        if (&private_data) { g3d_addMemList((g3dkmd_MemListNode**)(&private_data), idx , 0, 0); }
#define DEL_VMA_PAGE_LIST(private_data, idx)  \
        if (&private_data) { g3d_delMemList((g3dkmd_MemListNode**)(&private_data), idx); }



__DEFINE_SEM_LOCK(backtraceLock);
        ...
        case G3D_IOCTL_FLUSHCOMMAND:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_flushCmd)) == 0)
                {
                    ...
                    cmd.result = (uint32_t)g3dKmdFlushCommand(cmd.dispatchIndex,(void*)(uintptr_t)cmd.head,(void*)(uintptr_t)cmd.tail, cmd.lastFlushStamp, cmd.carry, cmd.lastFlushPtr, cmd.flushDCache);
                    #else
                    cmd.result = (uint32_t)g3dKmdFlushCommand(cmd.dispatchIndex,(void*)(uintptr_t)cmd.head,(void*)(uintptr_t)cmd.tail, cmd.lastFlushStamp, cmd.carry, cmd.flushDCache);
                    #endif

                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_flushCmd)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_G3DVATOPA_CHUNK:
            {
                ...
                {
                    if (cmd.mapper == 0) return -EFAULT;
                    if (cmd.g3d_va == 0) return -EFAULT;
                    if (cmd.size == 0) return -EFAULT;

#if defined(PHY_ADDR_FROM_VIRT_ADDR)
                    cmd.chunk_pa   = cmd.g3d_va;
                    cmd.chunk_size = cmd.size;
#else
                    g3dKmdMmuG3dVaToPaChunk((void*)(uintptr_t)cmd.mapper, cmd.g3d_va, cmd.size, &cmd.chunk_pa, &cmd.chunk_size);
#endif
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_MMUCmdChunk)) != 0)
                        return -EFAULT;
                }
            }
            break;
        ...
        case G3D_IOCTL_MMUMAP:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_MMUCmd)) == 0)
                {
                    ...
                    cmd.g3d_va = cmd.va;
#else
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_MMUMAP(%p %p %u %d)\n", (void*)(uintptr_t)cmd.mapper, (void*)(uintptr_t)cmd.va, cmd.size, cmd.could_be_section));
                    cmd.g3d_va = g3dKmdMmuMap((void*)(uintptr_t)cmd.mapper, (void*)(uintptr_t)cmd.va, cmd.size, cmd.could_be_section, cmd.flag);
                    if(cmd.g3d_va != 0) { ADD_G3DMMU_LIST(filp->private_data, cmd.g3d_va, cmd.mapper, cmd.size); }
#endif
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_MMUCmd)) != 0)
                        return -EFAULT;
                }
        ...
        case G3D_IOCTL_MMUUNMAP:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_MMUCmd)) == 0)
                {
                    ...
                    if(cmd.g3d_va != 0) { DEL_G3DMMU_LIST(filp->private_data, cmd.g3d_va); }
                    #endif
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_MMUCmd)) != 0)
                        return -EFAULT;
                }
        ...
        case G3D_IOCTL_MMU_RANGE_SYNC:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_MMUCmd)) == 0)
                {
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_MMU_RANGE_SYNC (%p %u %u %u %d)\n", cmd.mapper, cmd.va, cmd.size, cmd.g3d_va, cmd.cache_action);
                    if (!g3dKmdMmuRangeSync((void*)(uintptr_t)cmd.mapper, (unsigned long)cmd.va, (unsigned int)cmd.size, cmd.g3d_va, cmd.cache_action))
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_MAKE_VA_LOOKUP:
            {
                ...
                {
                    if (cmd.mapper == 0) return -EFAULT;
                    cmd.hwAddr = ((Mapper)(uintptr_t)cmd.mapper)->lookback_mem.hwAddr;
                    cmd.size = ((Mapper)(uintptr_t)cmd.mapper)->lookback_mem.size;
                    if (cmd.hwAddr == 0) return -EFAULT;
                    cmd.G3d_va_base = (unsigned int)((Mapper)(uintptr_t)cmd.mapper)->G3d_va_base;
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_MAKE_VA_LOOKUP(%p) = 0x%x\n", (void*)(uintptr_t)cmd.mapper, cmd.hwAddr));
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_LookbackCmd)) != 0)
                        return -EFAULT;
                }
                else
#endif
        ...
        case G3D_IOCTL_PHY_ALLOC:
            {
                ...
                {
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_PHY_ALLOC(%u %u)=%0x%lx\n", cmd.resType, cmd.size, cmd.phyAddr));
// [Louis: Remove because this is just hack for some version. (from Brady)], I want to remove resType from memory alloc function.
//#ifdef G3DKMD_HACK_FRAME_CMD_DMA
//                    if (cmd.resType == G3D_RES_CMDQ)
//                    {
//                        // step 1: alloc a DMA memory
//                        G3dMemory mem;
//                        if (! allocatePhysicalMemory(&mem, cmd.size, 0, G3DKMD_MMU_ENABLE | G3DKMD_MMU_SHARE | G3DKMD_MMU_DMA))
//                        {
//                            YL_KMD_ASSERT(0);
//                            return;
//                        }
//                        gExecInfo.DMAFrameCmdMem.phyAddr = mem.phyAddr;
//                        cmd.phyAddr = mem.phyAddr;
//                    }
//                    else
//                    {
//                        cmd.phyAddr = g3dKmdPhyAlloc(cmd.resType, cmd.size);
//                        KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_PHY_ALLOC(%u %u)=%p\n", cmd.resType, cmd.size, cmd.phyAddr);
//                        ADD_PHYMEM_LIST(filp->private_data, cmd.phyAddr);
//                    }
//#else // ! G3DKMD_HACK_FRAME_CMD_DMA
...
//#endif // G3DKMD_HACK_FRAME_CMD_DMA
                    ...
                    if (cmd.phyAddr == 0) return -EFAULT;

#ifndef G3DKMD_HACK_FRAME_CMD_DMA
                    ADD_PHYMEM_LIST(filp->private_data, cmd.phyAddr);
#endif

                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_PhyMemCmd)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_HW_ALLOC:
            {
                ...
                {
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_HW_ALLOC(phyAddr %llu, size %u)\n", cmd.phyAddr, cmd.size));
                    cmd.hwAddr = g3dKmdAllocHw(cmd.phyAddr, cmd.size);
                    if (cmd.hwAddr == 0) return -EFAULT;
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_HWAllocCmd)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_HW_FREE:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_HWAllocCmd)) == 0)
                {
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_HW_FREE(hwAddr %u, size %u)\n", cmd.hwAddr, cmd.size));
        ...
        case G3D_IOCTL_GET_MSG:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_DataCmd)) == 0)
                {
                    ...
                    size = cmd.arg[1];
                    if (g3dKmdMsgQReceive(&type, &size, (void __user *)(uintptr_t)cmd.addr) == G3DKMD_MSGQ_RTN_EMPTY)
                    {
                        return -EFAULT;
                    }
                    ...
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_DataCmd)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_USERPAGE_GET:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_VirtMemCmd)) == 0)
                {
                    ...
                    if (cmd.virtAddr == 0) return -EFAULT;
                    ret = g3dKmdUserPageGet((unsigned long)cmd.virtAddr, cmd.size, (uintptr_t*)&cmd.pageArray, &cmd.nrPages);
                    if (ret <= 0) return (ret == 0)? -EFAULT : ret;
                    ADD_VIRTMEM_LIST(filp->private_data, (uintptr_t)cmd.pageArray, cmd.nrPages);
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_VirtMemCmd)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_USERPAGE_FREE:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_VirtMemCmd)) == 0)
                {
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_USERPAGE_FREE(0x%x 0x%x 0x%x)\n", cmd.virtAddr, cmd.size, cmd.nrPages));
        ...
        case G3D_IOCTL_CALLBACK_TEST:
            {
                ...
                {
                    pid_t g3dKmdGetTaskProcess(int);
       ...
       case G3D_IOCTL_VIRT_ALLOC:
            {
                ...
                {
                    cmd.virtAddr = (uint64_t)(uintptr_t)yl_mmu_alloc(cmd.size);//g3dKmdPhyAlloc(cmd.resType, cmd.size);
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_VIRT_ALLOC vaddr %p, size 0x%x, pfn 0x%lx\n",
                            (void*)(uintptr_t)cmd.virtAddr, cmd.size, vmalloc_to_pfn((void*)(uintptr_t)cmd.virtAddr));
                    if (cmd.virtAddr == 0) return -EFAULT;
                    ADD_VIRTMEM_LIST(filp->private_data, (uintptr_t)cmd.virtAddr, cmd.virtAddr);
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_Virt2MemCmd)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_VIRT_FREE:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_Virt2MemCmd)) == 0)
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_VIRT_FREE(%p)\n", (void*)(uintptr_t)cmd.virtAddr);
        ...
        case G3D_IOCTL_SWRDBACK_ALLOC:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_GetSwRdbackMemCmd)) == 0)
                {
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_SWRDBACK_ALLOC(%u)\n", cmd.taskID));
                    cmd.hwAddr = 0;
                    cmd.phyAddr = g3dKmdSwRdbackAlloc(cmd.taskID, &cmd.hwAddr);
                    if (cmd.phyAddr == 0) return -EFAULT;
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_GetSwRdbackMemCmd)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_FLUSHTRIGGER:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_FlushTriggerCmd)) == 0)
                {
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_FLUSHTRIGGER(%u, %u, %u)\n", cmd.set, cmd.taskID, cmd.value));
                    g3dKmdFlushTrigger(cmd.set, (int)cmd.taskID, &cmd.value);
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_FlushTriggerCmd)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_CHECK_HWSTATUS:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(G3DKMD_IOCTL_3_ARG)) == 0)
                {
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_CHECK_HWSTATUS(0x%x 0x%x)\n", cmd.auiArg[0], cmd.auiArg[1]));
                    cmd.auiArg[2] = g3dKmdCheckHWStatus(cmd.auiArg[0], cmd.auiArg[1]);
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(G3DKMD_IOCTL_3_ARG)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_DUMP_BEGIN:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(G3DKMD_IOCTL_2_ARG)) == 0)
                {
                ...
                {
                    KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "G3D_IOCTL_DUMP_BEGIN copy_from_user fail\n");
                    return -EFAULT;
                }
            }
            break;
        ...
        case G3D_IOCTL_DUMP_END:
            {
                ...
                {
                    KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "G3D_IOCTL_DUMP_END copy_from_user fail\n");
                    return -EFAULT;
                }
            }
            break;
	...
	case G3D_IOCTL_BUFFERFILE_DUMP_BEGIN:
            {
                ...
                KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_BUFFERFILE_DUMP_BEGIN(mmap pointer: 0x%lX, length: %u)\n",
                       (unsigned long int) cmd.offset, (unsigned int)cmd.length);
                if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(G3DKMD_IOCTL_BF_DUMP_BEGIN_ARG)) != 0) {
                    return -EFAULT;
                }
            }
	    break;
	case G3D_IOCTL_BUFFERFILE_DUMP_END:
	    {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(G3DKMD_IOCTL_1_ARG)) == 0)
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_BUFFERFILE_DUMP_END, isFreeClosedData: %s\n", cmd.auiArg? "true":"false");
        ...
        case G3D_IOCTL_CHECK_DONE:
            {
                ...
                if( copy_from_user((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_LockCheckCmd)) == 0)
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_CHECK_DONE(0x%x 0x%x)\n", cmd.num_tasks, cmd.data);
                    if (cmd.data == 0)
                        return -EFAULT;
                    data = (struct ionGrallocLockInfo*)G3DKMDVMALLOC(sizeof(struct ionGrallocLockInfo)*cmd.num_tasks);
                    if( copy_from_user((void *)data, (const void __user *)(uintptr_t)cmd.data, sizeof(struct ionGrallocLockInfo)*cmd.num_tasks) == 0)
                    {
                        g3dKmdLockCheckDone(cmd.num_tasks, data);
                        if( copy_to_user((void __user *)(uintptr_t)cmd.data, (void *)data, sizeof(struct ionGrallocLockInfo)*cmd.num_tasks) != 0)
                        {
                            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "G3D_IOCTL_CHECK_DONE copy_to_user fail\n");
                            kfree(data);
                            return -EFAULT;
                        }
                        G3DKMDVFREE(data);
        ...
        case G3D_IOCTL_CHECK_FLUSH:
            {
                ...
                if( copy_from_user((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_LockCheckCmd)) == 0)
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_CHECK_FLUSH(0x%x 0x%x)\n", cmd.num_tasks, cmd.data);
                    if (cmd.data == 0)
                        return -EFAULT;
                    data = (struct ionGrallocLockInfo*)G3DKMDVMALLOC(sizeof(struct ionGrallocLockInfo)*cmd.num_tasks);
                    if( copy_from_user((void *)data, (const void __user *)(uintptr_t)cmd.data, sizeof(struct ionGrallocLockInfo)*cmd.num_tasks) == 0)
                    {
                        ...
                        G3DKMDVFREE(data);
                        if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_LockCheckCmd)) != 0)
                            return -EFAULT;
                    }else
                    {
                        G3DKMDVFREE(data);
        ...
        case G3D_IOCTL_FENCE_CREATE:
...
#ifdef G3DKMD_SUPPORT_FENCE
                g3dkmd_FenceCmd cmd;
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_FenceCmd)) == 0)
                {
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_FENCE_CREATE(taskID %u, stamp %u)\n", cmd.taskID, cmd.stamp));

                    g3dKmdTaskCreateFence(cmd.taskID, &cmd.fd, cmd.stamp);
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_FenceCmd)) != 0)
                        return -EFAULT;
                }
                else
#endif
            ...
            break;
        case G3D_IOCTL_FENCE_STATUS:
            {
#ifdef G3DKMD_SUPPORT_FENCE
                g3dkmd_FenceCmd cmd;
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_FenceCmd)) == 0)
                {
                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_FENCE_STATUS(fd %d)\n", cmd.fd));
                    cmd.status = g3dkmdFenceStatus(cmd.fd);
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_FenceCmd)) != 0)
                        return -EFAULT;
                }
                else
        ...
        case G3D_IOCTL_ION_MIRROR_CREATE:
            {
                ...
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_CREATE[%d] 0x%llx %d\n", current->tgid, cmd.addr, cmd.fd);
        ...
        case G3D_IOCTL_ION_MIRROR_DESTROY:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(ion_mirror_data)) == 0)
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_DESTROY[%d] 0x%llx\n", current->tgid, cmd.addr);
        ...
        case G3D_IOCTL_ION_MIRROR_STAMP_SET:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(ion_mirror_stamp_set_data)) == 0)
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_STAMP_SET[%d] 0x%llx 0x%x %u %d %u\n", current->tgid, cmd.addr, cmd.stamp, cmd.readonly, cmd.pid, cmd.taskID);
                    ret = ion_mirror_stamp_set(cmd.addr, cmd.stamp, cmd.readonly, cmd.pid, cmd.taskID);
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_STAMP_SET[%d] %d\n", current->tgid, ret);
                    if (ret < 0)
                        return ret;
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(ion_mirror_stamp_set_data)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_ION_MIRROR_STAMP_GET:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(ion_mirror_stamp_get_data)) == 0)
                {
                    if ((void __user *)(uintptr_t)cmd.data != NULL)
                        info = kmalloc(sizeof(struct ionGrallocLockInfo)*cmd.num_task, GFP_KERNEL);
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_STAMP_GET[%d] 0x%llx %p\n", current->tgid, cmd.addr, info);
                    ret = ion_mirror_stamp_get(cmd.addr, &cmd.num_task, info);
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_STAMP_GET[%d] %u %d\n", current->tgid, cmd.num_task, ret);
                    if (ret >=0 && info)
                    {
                        if (copy_to_user((void __user *)(uintptr_t)cmd.data, info, sizeof(struct ionGrallocLockInfo)*cmd.num_task))
                            ret = -EFAULT;
                    }
                    if (info)
                        kfree(info);
                    if (ret < 0)
                        return ret;
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(ion_mirror_stamp_get_data)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
        ...
        case G3D_IOCTL_ION_MIRROR_LOCK_UPD:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(ion_mirror_lock_update_data)) == 0)
                {
                    if ((void __user *)(uintptr_t)cmd.data != NULL)
                    {
                        info = kmalloc(sizeof(struct ionGrallocLockInfo)*cmd.num_task, GFP_KERNEL);
                        if (copy_from_user(info, (void __user *)(uintptr_t)cmd.data, sizeof(struct ionGrallocLockInfo)*cmd.num_task))
                            ret = -EFAULT;
                    }
                    if (ret >=0)
                    {
                        KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_LOCK_UPD[%d] 0x%llx %u %u %p\n", current->tgid, cmd.addr, cmd.num_task, cmd.readonly, info);
                        ret = ion_mirror_lock_update(cmd.addr, cmd.num_task, cmd.readonly, info, &cmd.lock_status);
                        KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_LOCK_UPD[%d] %u %d\n", current->tgid, cmd.lock_status, ret);
                    }
                    if (info)
                        kfree(info);
                    if (ret < 0)
                        return ret;
                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(ion_mirror_lock_update_data)) != 0)
                        return -EFAULT;
                }
                else
                    return -EFAULT;
         ...
         case G3D_IOCTL_ION_MIRROR_UNLOCK_UPD:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(ion_mirror_data)) == 0)
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_UNLOCK_UPD[%d] 0x%llx\n", current->tgid, cmd.addr);
                    ret = ion_mirror_unlock_update(cmd.addr);
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_UNLOCK_UPD[%d] %d\n", current->tgid, ret);
                    if (ret < 0)
                        return ret;
                }
                else
         ...
         case G3D_IOCTL_ION_MIRROR_SET_ROTATE_INFO:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(ion_mirror_rotate_data)) == 0)
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_SET_ROTATE_INFO[%d] 0x%llx %d %d %d\n", current->tgid, cmd.addr, cmd.rotate, cmd.rot_dx, cmd.rot_dy);
                    ret = ion_mirror_set_rotate_info(cmd.addr, cmd.rotate, cmd.rot_dx, cmd.rot_dy);
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_SET_ROTATE_INFO[%d] %d\n", current->tgid, ret);
                    if (ret < 0)
                        return ret;
                }
                else
         ...
         case G3D_IOCTL_ION_MIRROR_GET_ROTATE_INFO:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(ion_mirror_rotate_data)) == 0)
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_GET_ROTATE_INFO[%d] 0x%llx %d\n", current->tgid, cmd.addr, ret);
                    ret = ion_mirror_get_rotate_info(cmd.addr, &(cmd.rotate), &(cmd.rot_dx), &(cmd.rot_dy));
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_ION_MIRROR_GET_ROTATE_INFO[%d] 0x%llx %d %d %d %d\n", current->tgid, cmd.addr, cmd.rotate, cmd.rot_dx, cmd.rot_dy, ret);
                    if (ret < 0)
                        return ret;
                }
                else
                ...
                if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(ion_mirror_rotate_data)) != 0)
                    return -EFAULT;
            }
            break;
#endif
        ...
        case G3D_IOCTL_PA_PAIR_INFO:
            {
                ...
                if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_PaPairInfoCmd)) == 0)
                {
                    if (cmd.mapper == 0) return -EFAULT;
                    if (cmd.g3d_va == 0) return -EFAULT;
                    if (cmd.size == 0) return -EFAULT;

                    g3dKmdMmuPaPairBufInfo( (void*) cmd.mapper, cmd.g3d_va, cmd.size, &cmd.buffer_size, &cmd.unit_size, &cmd.offset_mask);

                    //KMDDPF((G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_MAKE_VA_LOOKUP(%p) = 0x%x\n", (void*)(uintptr_t)cmd.mapper, cmd.hwAddr));

                    if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_PaPairInfoCmd)) != 0)
                        return -EFAULT;
                }
            }
            break;
        ...
        case G3D_IOCTL_GET_DRV_INFO:
            {
                ...
                if (copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_DataCmd)) == 0)
                {
                    ...
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_GET_DRV_INFO: drv_cmd:%u arg: %u dataSize:%u\n", drv_cmd, arg, dataSize);
                    ...
                    if (copy_to_user ((void __user *)(uintptr_t)cmd.addr, (const void *)vdata, size) != 0)
                    {
                        G3DKMDVFREE(vdata);
                        return -EFAULT;
                    }
        ...
        case G3D_IOCTL_SET_DRV_INFO:
            {
                ...
                if (copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_DataCmd)) == 0)
                {
                    ...
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_SET_DRV_INFO: drv_cmd:%u arg: %u dataSize:%u\n", drv_cmd, arg, dataSize);
                    ...
                    if (dataSize > 0)
                    {
                        ...
                        if (copy_from_user (vdata, (const void __user *)((uintptr_t)cmd.addr), dataSize) != 0)
                        {
                            G3DKMDVFREE(vdata);
                            return -EFAULT;
                        }
        ...
        case G3D_IOCTL_CL_GET_STAMP:
        {
            ...
            if( copy_from_user ((void *)&cl_printf_info, (const void __user *)args, sizeof(g3dkmd_IOCTL_CL_PRINTF_INFO)) == 0)
            {
                pG3dTask task;
                ...
                while(cl_printf_info.isrTrigger == 0)
                {
                    //init_waitqueue_head(&gExecInfo.s_waitqueue_head_cl);
                    //prepare_to_wait(&gExecInfo.s_waitqueue_head_cl, &gExecInfo.s_waitqueue_cl,TASK_INTERRUPTIBLE);
                    //schedule_timeout(1*HZ); // 1 sec.
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "[G3DKMD_SLEEP_CL_PRINT] Sleep...\n");
                }
                #endif //G3DKMD_SLEEP_CL_PRINT

                if (copy_to_user ((void __user *)args , (const void *)&cl_printf_info, sizeof(g3dkmd_IOCTL_CL_PRINTF_INFO)) != 0)
                {
                    KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "G3D_IOCTL_CL_GET_STAMP copy_to_user fail\n");
                    return -EFAULT;
                }
                else
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_CL_GET_STAMP copy_to_user success\n");
                }
            }
            else
            {
                KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "G3D_IOCTL_CL_GET_STAMP copy_from_user fail\n");
                return -EFAULT;
            }
        }
        break;
        ...
        case G3D_IOCTL_CL_UPDATE_READ_STAMP:
        {
            ...
            {
                KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_CL_RESUME_DEV resumes hw\n");
            }
#else // ! G3D_SIGNAL_CL_PRINT
                KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_SIGNAL_CL_PRINT is undefined\n");
        ...
        {
            unsigned iter_task_idx = 0;
            // set hw register to resume the hw
            KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_CL_RESUME_DEV resumes hw\n");

            // clear global data -> seems to remove to resume function
            for (iter_task_idx = 0; iter_task_idx < gExecInfo.taskInfoCount; ++iter_task_idx)
            {
                pG3dTask task = g3dKmdGetTask(iter_task_idx);
                if (NULL != task)
                {
            ...
            // step 5 clear wave to release HW
#ifdef G3D_HW
            g3dKmdRegWrite(G3DKMD_REG_UX_DWP, 0x0, 0x1, 0xFFFF, 0);
#endif
        }
        break;
        ...
        case G3D_IOCTL_HT_READ_WRITE_EQUAL:
        {
            ...
            if (copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(G3DKMD_BOOL)) == 0)
            {
                ...
                if (pInst)
                {
                    ...
                    wptr = pInst->pHtDataForHwRead->htInfo->wtPtr;     // Louis not sure it should be HwRead or SwWrite for Ht
                    cmd = (wptr == rptr)? G3DKMD_TRUE : G3DKMD_FALSE;
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "wptr 0x%x, rptr 0x%x\n", wptr, rptr);
                }
                else
                {
                    cmd = G3DKMD_TRUE;
                    KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "G3D_IOCTL_HT_READ_WRITE_EQUAL: pG3dExeInst is NULL\n");
                }

                if (copy_to_user ((void __user *)args , (const void *)&cmd, sizeof(G3DKMD_BOOL)) != 0)
                {
                    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "G3D_IOCTL_CL_GET_STAMP copy_to_user success\n");
                    return -EFAULT;
                }
        ...
        case G3D_IOCTL_MPD_CONTROL:
        {
            ...
            if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(g3dkmd_IOCTL_MPD_Control)) == 0)
            {
                KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_MPD_CONTROL, op: %d\n", cmd.op);

                CMMPDControl(&cmd);

                if( copy_to_user ( (void __user *)args,(const void *)&cmd, sizeof(g3dkmd_IOCTL_MPD_Control)) != 0)
                {
                    return -EFAULT;
                }
            }
        ...
        case G3D_IOCTL_UPDATE_RINGBUFFER_PROFILING_DATA:
        {
            ...
            if( copy_from_user ((void *)&cmd, (const void __user *)args, sizeof(G3DKMD_IOCTL_6_ARG)) == 0)
            {
                ...
                KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, " G3D_IOCTL_UPDATE_RINGBUFFER_PROFILING_DATA,"
                                         "pid: %u, usage: %u, pollCnt: %u, askSize: %u, availableSize: %u, stamp: %u\n",
...
void g3d_non_cached_vma_open(struct vm_area_struct *vma)
{
    ...
    KMDDPF_ENG(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_IOCTL, "[NON_CACHE_MEMORY]: vma:%p vma->private_data:%p (_g3d_NC_Page_count: %d)\n", vma, vma->vm_private_data,  _g3d_NC_Page_count);

    if (NULL == vma->vm_private_data) {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "vma is not belong to g3d_non_cached_vma.\n");
        return ;
    }

    {
        struct G3d_non_cached_private_data* pdata = (struct G3d_non_cached_private_data*) vma->vm_private_data;
...
void g3d_non_cached_vma_close(struct vm_area_struct *vma)
{
    ...
    KMDDPF_ENG(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_IOCTL, "[NON_CACHE_MEMORY]: vma:%p (_g3d_NC_Page_count: %d), in\n", vma, _g3d_NC_Page_count);
...
int _g3d_release_pages_non_cached_pages(struct vm_area_struct *vma)
{
    ...
    KMDDPF_ENG(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_IOCTL, "[NON_CACHE_MEMORY] vma: %p privat_data:%p  range: %x~%x \n", vma, vma->vm_private_data,vma->vm_start, vma->vm_end );
    ...
    }
    {
        ...
        KMDDPF_ENG(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_IOCTL, "[NON_CACHE_MEMORY]vma->vm_private_data->nRef: %d\n", atomic_read(&(pdata->nRef)));
  ...
  #define _setVmaNoncached(vma_page_prot) (pgprot_dmacoherent(vma_page_prot))
#else // linux
...
int _g3d_mmap_non_cached_pages(struct file *filp, struct vm_area_struct *vma)
{
    ...
    KMDDPF_ENG(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_IOCTL, "[NON_CACHE_MEMORY] vma: 0x%p addr: 0x%p size:%lu\n", vma, addr, size );
    ...
    while (size > 0)
    {
        ...
        if (!page) // if page alloc fail
        {
            ...
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "[NON_CACHE_MEMORY] Non Cached Page: alloc_pages fail: gfp_mask: 0x%x  level: 0\n", gfp_mask );
            _g3d_release_pages_non_cached_pages(vma);
            return -EAGAIN;
        }
        {
        ...
        if ( vm_insert_page(vma, addr, page)) // insert fail error
        {
            ...
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "[NON_CACHE_MEMORY] Non Cached Page: vm_insert_page fail. vma: 0x%p addr: %x page: 0x%p\n", vma, addr, page );
            _g3d_release_pages_non_cached_pages(vma);
            return -EAGAIN;
        }
        addr += PAGE_SIZE;
    ...
    KMDDPF_ENG(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_IOCTL, "[NON_CACHE_MEMORY] Successed. vma: 0x%p addr: %x page: 0x%p\n", vma, addr, page );
...
{
     // KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "g3d VMA open, virt %lx %lx, phys %lx\n", vma->vm_start, vma->vm_end, vma->vm_pgoff << PAGE_SHIFT);
}

void g3d_vma_close(struct vm_area_struct *vma)
{
    // KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "g3d VMA close, virt %lx %lx, phys %lx\n", vma->vm_start, vma->vm_end, vma->vm_pgoff << PAGE_SHIFT);
}
...
int g3d_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    if (vaddr == NON_CACHED_PAGE_MMAP_OFFSET) {   // non cached memory alloc
        ...
        KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_IOCTL, "map non cache buffer from usr space\n");
        if (ret == -EAGAIN) {
            return ret;
        }
    ...
    else if (vaddr >= VMALLOC_START)
    {
        ...
        while (size > 0)
        {
            ...
            if ((ret = remap_pfn_range(vma, addr, pfn, PAGE_SIZE, vma->vm_page_prot)) < 0) // handle error
            {
                return -EAGAIN;
            }
            addr += PAGE_SIZE;
    ...
    else
    {
        ...
        {
            vma->vm_page_prot = pgprot_noncached(vma->vm_page_prot);
            //printk("[G3DKMD_HACK_FRAME_CMD_DMA] You should appear only ONCE!\n");
...
struct file_operations g3d_fops = {
    .owner   = THIS_MODULE,
    .open    = g3d_open,
    .release = g3d_release,
    .mmap    = g3d_mmap,
...
#endif
#if ((USE_MMU == MMU_USING_GPU) || (USE_MMU == MMU_USING_M4U) || (USE_MMU == MMU_USING_WCP))	// workaround for SSP to access iommu registers in MMU_USING_WCP
...
#error "currently mt6752/mt6592/mt8163 FPGA are supported only"
	...
	__yl_base = (unsigned long)of_iomap(node, 0);
        KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_IOCTL, "of_find_compatible_node \"%s\" to get reg address = 0x%lx\n", g3d_dts_name, __yl_base);
#ifdef G3DKMD_SUPPORT_ISR
	/* get IRQ ID */
        __yl_irq = irq_of_parse_and_map(node, 0);
        KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_IOCTL, "of_find_compatible_node \"%s\" to get irq num = %ld\n", g3d_dts_name, __yl_irq);
#endif
    }else
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL, "of_find_compatible_node \"%s\" fail\n", g3d_dts_name);
    }
...
{  
    ...
    if (0 != err)
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_IOCTL,"Failed to register misc device, misc_register() returned %d\n", err);
    }
}
#else
    dev_t dev;
...
#ifdef __G3D_64__
    KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_IOCTL, "ioctl size1 = %d %d %d %d %d %d %d %d %d %d\n",
           sizeof(g3dkmd_context), sizeof(g3dkmd_flushCmd), sizeof(g3dkmd_MMUCmd), sizeof(g3dkmd_MMUCmdChunk), sizeof(g3dkmd_LookbackCmd),
           sizeof(g3dkmd_PhyMemCmd), sizeof(g3dkmd_VirtMemCmd), sizeof(g3dkmd_Virt2MemCmd), sizeof(g3dkmd_HWAllocCmd), sizeof(g3dkmd_GetSwRdbackMemCmd));
    KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_IOCTL, "ioctl size2 = %d %d %d %d %d %d %d %d %d %d\n",
           sizeof(g3dkmd_FlushTriggerCmd), sizeof(g3dkmd_DataCmd), sizeof(g3dkmd_PaPairInfoCmd), sizeof(G3DKMD_BOOL), sizeof(G3DKMD_PATTERN_CFG),
           sizeof(G3DKMD_IOCTL_1_ARG), sizeof(G3DKMD_IOCTL_2_ARG), sizeof(G3DKMD_IOCTL_3_ARG), sizeof(G3DKMD_IOCTL_4_ARG), sizeof(G3DKMD_IOCTL_BF_DUMP_BEGIN_ARG));           
    KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_IOCTL, "ioctl size3 = %d %d %d %d %d %d %d %d %d %d\n", 
           sizeof(g3dkmd_LockCheckCmd), sizeof(g3dkmd_FenceCmd), sizeof(g3dkmd_IOCTL_MPD_Control), sizeof(ion_mirror_stamp_set_data), sizeof(ion_mirror_stamp_get_data),
           sizeof(ion_mirror_lock_update_data), sizeof(ion_mirror_data), sizeof(ion_mirror_rotate_data), sizeof(g3dkmd_IOCTL_CL_PRINTF_INFO), sizeof(g3dkmd_IOCTL_DRV_INFO_MEMLIST));
    KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_IOCTL, "ioctl size4 = %d %d %d %d\n", 
           sizeof(uintptr_t), sizeof(G3dMemory), sizeof(struct ionGrallocLockInfo), sizeof(size_t));
#endif
...
{
    KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_IOCTL, "[%s]yolig3d_suspend %d\n", pdev->name, state.event); /* 2 = PM_EVENT_SUSPEND*/
    gpu_suspend();
    return 0;
}
