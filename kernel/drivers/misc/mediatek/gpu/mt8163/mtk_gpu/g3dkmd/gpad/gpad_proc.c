...
static struct proc_dir_entry *gpad_proc_dir_s = NULL;
static struct proc_dir_entry *gpad_pm_proc_file_s = NULL;
static struct proc_dir_entry *gpad_sdbg_proc_file_s = NULL;

#if defined(GPAD_SUPPORT_DVFS)
static struct proc_dir_entry *gpad_ipem_proc_file_s = NULL;
static struct proc_dir_entry *gpad_ipem_log_proc_file_s = NULL;
static struct proc_dir_entry *gpad_ipem_sbqs_proc_file_s = NULL;
static struct proc_dir_entry *gpad_ipem_sbeps_proc_file_s = NULL;
#endif /* GPAD_SUPPORT_DVFS */

static struct proc_dir_entry *gpad_dfd_areg_proc_file_s = NULL;
static struct proc_dir_entry *gpad_dfd_freg_proc_file_s = NULL;
static struct proc_dir_entry *gpad_dfd_ff_proc_file_s = NULL;
static struct proc_dir_entry *gpad_dfd_cmn_proc_file_s = NULL;
static struct proc_dir_entry *gpad_dfd_dwp_proc_file_s = NULL;

#if defined(GPAD_SUPPORT_DFD)
static struct proc_dir_entry *gpad_dfd_sram_proc_file_s = NULL;
#endif /* GPAD_SUPPORT_DFD */
...
static int gpad_proc_show(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "Ring HA   : 0x%08X, VA: %p, Index: %d/%d\n", 
                                desc->hw_addr, desc->vaddr, pm_info->hw_buffer_idx, 
                                pm_info->samples_alloc);
    seq_printf(m, "timestamp : 0x%08X, CPU: %llx\n", gpu_ts, cpu_ts);
    seq_printf(m, "PM sample : %d\n", stat->read_samples);
    seq_printf(m, "busy time : %d, total time: %d, GPU loading: %d\n", 
                                stat->busy_time, stat->total_time, 
                                ((stat->total_time/100 > 0) ? ((stat->busy_time) / (stat->total_time / 100)) : 0));
    ...
    {
        static unsigned int            last_gpu_ts = 0;
        static unsigned long long      last_cpu_ts = 0;
        int                            diff_gpu_ts;
...
static int gpad_ipem_log_proc_show(struct seq_file *m, void *v)
{
    ...
    do {
        ...
        seq_printf(m, "%lu\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%p\t%p\t%u\t%u\n", 
            stat->timestamp, 
            table[action].freq_khz,
            table[action].volt == 0 ? 0 : table[action].volt == GPU_POWER_VGPU_1_15V ? 1150 : table[action].volt == GPU_POWER_VGPU_1_25V ? 1250 : -1,
            record->workid, 
            stat->event, 
            stat->action_llimit, 
            stat->action_ulimit, 
...
void gpad_ipem_proc_init(void)
{
    static const struct file_operations fops = {
        .owner   = THIS_MODULE,
        .write   = gpad_ipem_proc_write,
        .open    = gpad_ipem_proc_open,
        .read    = seq_read,
        .llseek  = seq_lseek,
...
void gpad_ipem_sbqs_proc_init(void)
{
    static const struct file_operations fops = {
        .owner   = THIS_MODULE,
        .write   = gpad_ipem_sbqs_proc_write,
        .open    = gpad_ipem_sbqs_proc_open,
        .read    = seq_read,
        .llseek  = seq_lseek,
        .release = single_release,
    };
...
void gpad_ipem_sbeps_proc_init(void)
{
    static const struct file_operations fops = {
        .owner   = THIS_MODULE,
        .write   = gpad_ipem_sbeps_proc_write,
        .open    = gpad_ipem_sbeps_proc_open,
        .read    = seq_read,
        .llseek  = seq_lseek,
        .release = single_release,
    };
