...
YL_BOOL _Mapper_is_continuous_section( Mapper m, PaSectionLookupTable lookup_table, va_t va )
{
    ...
    *out_pa     = pre_pa;
    ...
    while ( va < end_va )
    {
        ...
        pa         = va_to_pa( m, va );
        *out_pa    = pa;
        if ( pa != pre_pa + MMU_SMALL_SIZE ) is_continuous = 0;
...
YL_BOOL 
_Mapper_contain_continuous_section( Mapper m, PaSectionLookupTable lookup_table, va_t va, size_t occupied_size )
{
...

    {
        ...
        while( s >= MMU_SECTION_SIZE )
        {
            ...
            if ( _Mapper_is_continuous_section( m, lookup_table, va_aligned ) ) return 1;
...
YL_BOOL register_table(Mapper m, Trunk t, va_t va, YL_BOOL section_detected, unsigned int flag)
{
    ...
    //FirstMetaTable meta                 = m->Meta_space;
    ...
    //FirstPageTable mirror               = (FirstPageTable)m->Mirror_mem.data;
...

    {
        ...
        while ( succ && ((index.s.index0 < end_index0) || ( (index.s.index0 == end_index0) && ( index.s.index1 < end_index1 ) )) )
        {
            ...
            {
                PaSectionLookupTable_set_invalid( & m->Pa_section_lookup_table );   // disable lookup table 
                section_mode = 0;
            }
            ...
            // P => Q == !P || Q
            //YL_MMU_ASSERT( !section_mode || ( index1 == 0 && _Mapper_is_section_aligned( va_base ) ) );
            ...
            if ( section_mode && ! _Mapper_is_continuous_section( m, & m->Pa_section_lookup_table, va_base ) )
            {
                section_mode = 0;
            }
            ...
            if ( section_mode )
            {
                pa = PaSectionLookupTable_use_if_availabile( m, & m->Pa_section_lookup_table, 0, va_base );
            ...
            {
                pa = PaSectionLookupTable_use_if_availabile( m, & m->Pa_section_lookup_table, index.s.index1, va_base );
...
YL_BOOL unregister_table( Mapper m, Trunk t )
{
    ...
    FirstPageTable page   = (FirstPageTable)page_mem->data;
...

    {
            ...
            ( (index.s.index0 == end_index0) && ( index.s.index1 < end_index1 ) ) )
        {
            ...
            if ( section_mode && SectionEntry_has_second_level( page->entries[ index.s.index0 ] ) )
            {
                section_mode = 0;
            }
