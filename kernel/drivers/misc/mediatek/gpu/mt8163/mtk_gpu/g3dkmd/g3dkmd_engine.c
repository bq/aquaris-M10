...
static unsigned int _g3dKmdGetToken(G3DKMD_TOKEN token)
{
    unsigned int cnt = 0;
...
static void _g3dKmdReleaseToken(G3DKMD_TOKEN token)
{
    g3dKmdRegWrite(G3DKMD_REG_G3D, token, 0x1, MSK_AREG_G3D_TOKEN0, SFT_AREG_G3D_TOKEN0);
}

static void _g3dKmdDataSync(pG3dExeInst pInstIn);
static void _g3dKmdDataSync(pG3dExeInst pInstIn)
{
    int i;
    pG3dExeInst pInst;
...
unsigned int g3dKmdRegGetReadPtr(int dispatchHwIndex)
{
    ...
    hwreadptr = g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_CQ_DONE_HTBUF_RPTR, MSK_AREG_CQ_DONE_HTBUF_RPTR, SFT_AREG_CQ_DONE_HTBUF_RPTR);
...
unsigned int g3dKmdCheckBufferConsistency(int hwChannel, unsigned int bufferStartIn, unsigned int bufferSizeIn)
{
    ...
    bufferStart = g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_CQ_HTBUF_START_ADDR, MSK_AREG_CQ_HTBUF_START_ADDR, SFT_AREG_CQ_HTBUF_START_ADDR);
    bufferEnd   = g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_CQ_HTBUF_END_ADDR, MSK_AREG_CQ_HTBUF_END_ADDR, SFT_AREG_CQ_HTBUF_END_ADDR);
...
void g3dKmdRegUpdateReadPtr(int hwChannel, unsigned int read_ptr)
{
    ...
    _BYPASS_(BYPASS_RIU);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_DONE_HTBUF_RPTR, read_ptr, MSK_AREG_CQ_DONE_HTBUF_RPTR, SFT_AREG_CQ_DONE_HTBUF_RPTR);
    _RESTORE_();
}
...
void g3dKmdRegUpdateWritePtr(int hwChannel, unsigned int write_ptr)
{
    ...
    _BYPASS_(BYPASS_RIU);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_HTBUF_WPTR, write_ptr, MSK_AREG_CQ_HTBUF_WPTR, SFT_AREG_CQ_HTBUF_WPTR);
    _RESTORE_();
}
#endif
...
unsigned int g3dKmdEngLTCFlush(void)
{
    ...
    // enable LTC-flush
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PA_FLUSH_LTC_SET, 0x1, MSK_AREG_PA_FLUSH_LTC_EN, SFT_AREG_PA_FLUSH_LTC_EN);

    // wait LTC-flush done
    cnt = 0;
    while(cnt++ < TIME_OUT_CNT)
    {
        if (g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_PA_FLUSH_LTC_DONE, MSK_AREG_PA_FLUSH_LTC_DONE, SFT_AREG_PA_FLUSH_LTC_DONE))
        {
            break;
        }
        G3DKMD_UDELAY(1);
    ...
    // disable LTC-flush
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PA_FLUSH_LTC_SET, 0x0, MSK_AREG_PA_FLUSH_LTC_EN, SFT_AREG_PA_FLUSH_LTC_EN);

    // wait LTC-disable done
    cnt = 0;
    while(cnt++ < TIME_OUT_CNT)
    {
        if (g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_PA_FLUSH_LTC_DONE, MSK_AREG_PA_FLUSH_LTC_DONE, SFT_AREG_PA_FLUSH_LTC_DONE) == 0)
        {
            break;
        }
        G3DKMD_UDELAY(1);
...
void g3dKmdRegReset(void)
{
    ...
    // To solve this problem by setting blocking MX transaction to close the input of MX module, then waiting for MX module to finish rest of task.
    ...
    g3dKmdRegWrite(G3DKMD_REG_MX, REG_MX_DBG_DEBUG_CTRL, 1, MSK_MX_DBG_MX_APB_DBG_CK_ON, SFT_MX_DBG_MX_APB_DBG_CK_ON);
...
#endif
    while(cnt++ < TIME_OUT_CNT) {
        if((g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_IRQ_STATUS_RAW, VAL_IRQ_MX_DBG, 0) == VAL_IRQ_MX_DBG) &&
            (g3dKmdRegRead(G3DKMD_REG_MFG, 0x1c, 0x1, 0) == 0x1)) {
            // 4. Clear IRQ
    ...
    if(cnt >= TIME_OUT_CNT) {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_ENGINE, "TIMEOUT mx_debug: 0x%x MFG(0x1c): 0x%x\n", g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_IRQ_STATUS_RAW, 0x400, 10), g3dKmdRegRead(G3DKMD_REG_MFG, 0x1c, 0x1, 0));
        YL_KMD_ASSERT(0);
        return;
    }
    // SW workaround end
...
#ifndef G3D_HW
        _BYPASS_(BYPASS_RIU);
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PA_IDLE_STATUS0, VAL_PA_IDLE_ST0_G3D_ALL_IDLE, VAL_PA_IDLE_ST0_G3D_ALL_IDLE, 0);
    ...
    g3dKmdRegWrite(G3DKMD_REG_UX_DWP, REG_UX_DWP_ERROR_DETECTION, g3dKmdCfgGet(CFG_UXDWP_REG_ERR_DETECT), 0xFFFFFFFF, 0);
    ...
    g3dKmdRegWrite(G3DKMD_REG_MX, REG_MX_DBG_MON_BRK_CTRL, 0x20000000, 0xFFFFFFFF, 0x0);
    // 6. Release forced clock
    g3dKmdRegWrite(G3DKMD_REG_MX, REG_MX_DBG_DEBUG_CTRL, 0, MSK_MX_DBG_MX_APB_DBG_CK_ON, SFT_MX_DBG_MX_APB_DBG_CK_ON);
    ...
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_IRQ_STATUS_MASK, 0, (VAL_IRQ_PA_FRAME_SWAP | VAL_IRQ_PA_FRAME_END | VAL_IRQ_UX_DWP0_RESPONSE | VAL_IRQ_UX_DWP1_RESPONSE), 0);
#endif
    ...
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING0, 0, MSK_AREG_ECLK_CGC0_EN, SFT_AREG_ECLK_CGC0_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING1, 0, MSK_AREG_ECLK_CGC1_EN, SFT_AREG_ECLK_CGC1_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING2, 0, MSK_AREG_ECLK_CGC2_EN, SFT_AREG_ECLK_CGC2_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING3, 0, MSK_AREG_ECLK_CGC3_EN, SFT_AREG_ECLK_CGC3_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING4, 0, MSK_AREG_ECLK_CGC4_EN, SFT_AREG_ECLK_CGC4_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING5, 0, MSK_AREG_ECLK_CGC5_EN, SFT_AREG_ECLK_CGC5_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING6, 0, MSK_AREG_ECLK_CGC6_EN, SFT_AREG_ECLK_CGC6_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING7, 0, MSK_AREG_ECLK_CGC7_EN, SFT_AREG_ECLK_CGC7_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING8, 0, MSK_AREG_ECLK_CGC8_EN, SFT_AREG_ECLK_CGC8_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING9, 0, MSK_AREG_ECLK_CGC9_EN, SFT_AREG_ECLK_CGC9_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING10, 0, MSK_AREG_ECLK_CGC10_EN, SFT_AREG_ECLK_CGC10_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING11, 0, MSK_AREG_ECLK_CGC11_EN, SFT_AREG_ECLK_CGC11_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING12, 0, MSK_AREG_ECLK_CGC12_EN, SFT_AREG_ECLK_CGC12_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING13, 0, MSK_AREG_ECLK_CGC13_EN, SFT_AREG_ECLK_CGC13_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING14, 0, MSK_AREG_ECLK_CGC14_EN, SFT_AREG_ECLK_CGC14_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_CGC_SUB_SETTING15, 0, MSK_AREG_ECLK_CGC15_EN, SFT_AREG_ECLK_CGC15_EN);
    #endif
    
    g3dKmdRegWrite(G3DKMD_REG_G3D, 0x600, 1, 1, 0); // enable +0x80000000 dram address offset for G3D access DRAM like CPU
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_IRQ_STATUS_MASK, 0, VAL_IRQ_G3D_MMCE_FRAME_FLUSH | VAL_IRQ_UX_CMN_RESPONSE, 0); // for qload notify
#endif
    ...
    if (g3dKmdCfgGet(CFG_LTC_ENABLE))
    {
        ...
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_BYPASS_SET, 0x0, MSK_AREG_MX_AXI_INTERLEAVE_EN, SFT_AREG_MX_AXI_INTERLEAVE_EN);
#else // ! G3DKMD_LTC_IN_ORDER_MODE
        // set performance mode
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_BYPASS_SET, 0x1, MSK_AREG_MX_AXI_INTERLEAVE_EN, SFT_AREG_MX_AXI_INTERLEAVE_EN);
    ...
    {   
        // bypass LTC
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_BYPASS_SET, 0x1, MSK_AREG_MX_PSEUDOBYPASS_EN, SFT_AREG_MX_PSEUDOBYPASS_EN);
    }
    ...
    {
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PM_DRAM_START_ADDR, gExecInfo.pm.hwAddr, MSK_AREG_PM_DRAM_START_ADDR, SFT_AREG_PM_DRAM_START_ADDR);
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PM_DRAM_SIZE, gExecInfo.pm.size, 0xFFFFFFFF, 0);
    }
#endif
    ...
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_VX_VTX_SETTING, VX_VW_BUF_MGN_SIZE, MSK_AREG_VX_VW_BUF_MGN, SFT_AREG_VX_VW_BUF_MGN);
    
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_VX_DEFER_SETTING, 0x10080800, 0xFFFFFFFF, 0);
#endif

    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_VX_VTX_SETTING, g3dKmdCfgGet(CFG_VX_BS_HD_TEST), MSK_AREG_VX_BS_HD_TEST, SFT_AREG_VX_BS_HD_TEST);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_VX_VTX_SETTING, g3dKmdCfgGet(CFG_BKJMP_EN), MSK_AREG_VX_BKJMP_EN, SFT_AREG_VX_BKJMP_EN);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_VX_VTX_SETTING, 0x0, MSK_AREG_VX_BS_BOFF_MODE, SFT_AREG_VX_BS_BOFF_MODE);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_CONTEXT_CTRL, g3dKmdCfgGet(CFG_CQ_PMT_MD), MSK_AREG_CQ_PMT_MD, SFT_AREG_CQ_PMT_MD);

    g3dKmdRegWrite(G3DKMD_REG_UX_DWP, REG_UX_DWP_ERROR_DETECTION, 0x0, MSK_UX_DWP_FTZ_RND_ORDER, SFT_UX_DWP_FTZ_RND_ORDER);
...
static INLINE void _qload_set_secure(void)
{
    ...
    if (g3dKmdCfgGet(CFG_SECURE_ENABLE))
    {
        ...
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_CTRL, 0x1, MSK_AREG_CQ_SECURITY, SFT_AREG_CQ_SECURITY);

        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_MX_SECURE0_BASE_ADDR, 0x1, MSK_AREG_MX_SECURE0_ENABLE, SFT_AREG_MX_SECURE0_ENABLE);

        value = G3DKMD_FLOOR(pInst->qbaseBuffer.hwAddr, 0x1000) >> 12;
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_MX_SECURE0_BASE_ADDR, value, MSK_AREG_MX_SECURE0_BASE_ADDR, SFT_AREG_MX_SECURE0_BASE_ADDR);

        value = G3DKMD_CEIL(pInst->qbaseBuffer.hwAddr + pInst->qbaseBufferSize, 0x1000) >> 12;
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_MX_SECURE0_TOP_ADDR, value, MSK_AREG_MX_SECURE0_TOP_ADDR, SFT_AREG_MX_SECURE0_TOP_ADDR);
    }
    else
    {
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_CTRL, 0x0, MSK_AREG_CQ_SECURITY, SFT_AREG_CQ_SECURITY);
    }
}
#else
...
static unsigned int HANG_DEBUG(void)
{
    ...
    if ((++dbg_cnt) % 7 == 0)
    {
        ...
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_QBUF_START_ADDR, 0, MSK_AREG_CQ_QBUF_START_ADDR, SFT_AREG_CQ_QBUF_START_ADDR);
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_QBUF_END_ADDR, 0, MSK_AREG_CQ_QBUF_END_ADDR, SFT_AREG_CQ_QBUF_END_ADDR);
        return G3DKMD_TRUE;
    }
    return G3DKMD_FALSE;
}
...
static INLINE void _qload_set_qload_regs(pG3dExeInst pInst)
{
    ...
    if (!HANG_DEBUG())
    {
        ...
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_QBUF_START_ADDR, pInst->qbaseBuffer.hwAddr, MSK_AREG_CQ_QBUF_START_ADDR, SFT_AREG_CQ_QBUF_START_ADDR);
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_QBUF_END_ADDR, pInst->qbaseBuffer.hwAddr + pInst->qbaseBufferSize, MSK_AREG_CQ_QBUF_END_ADDR, SFT_AREG_CQ_QBUF_END_ADDR);
    }
   
    // trigger qld upd
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_CTRL, 0x1, MSK_AREG_CQ_QLD_UPD, SFT_AREG_CQ_QLD_UPD);

    // enable clk (fire if may triggered by GCE) // for FPGA20188 to refine this option
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_G3D_ENG_FIRE, 0x1, MSK_AREG_G3D_ENG_FIRE, SFT_AREG_G3D_ENG_FIRE);

    // disable qld upd
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_CTRL, 0x0, MSK_AREG_CQ_QLD_UPD, SFT_AREG_CQ_QLD_UPD);

#ifdef PATTERN_DUMP
    if (!g3dKmdIsBypass(BYPASS_RIU))
    {
        g3dKmdRiuWrite(G3DKMD_RIU_G3D, "%04x_%08x\n", 0xfff8, VAL_IRQ_G3D_MMCE_FRAME_FLUSH);
        g3dKmdRiuWrite(G3DKMD_RIU_G3D, "%04x_%08x\n", MOD_OFST(G3D, REG_AREG_IRQ_STATUS_RAW), VAL_IRQ_G3D_MMCE_FRAME_FLUSH);        
    }
    #ifndef G3D_HW
    {
        _DEFINE_BYPASS_
        ...
        _BYPASS_(BYPASS_RIU);
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_IRQ_STATUS_RAW, 0x1, VAL_IRQ_G3D_MMCE_FRAME_FLUSH, 7);
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_IRQ_STATUS_MASKED, 0x1, VAL_IRQ_G3D_MMCE_FRAME_FLUSH, 7);
        _RESTORE_();
...
static INLINE G3DKMD_BOOL _qload_wait_hw_qload_non_polling(void)
{
    ...
    if (!g3dKmdIsBypass(BYPASS_REG))
    {
        ...
        KMDDPF_ENG(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_ENGINE, "QLoad wait queue: going to wait.\n");
        ...
        wqRet = g3dkmd_wait_event_interruptible_timeout(
             /* wait queue */  gExecInfo.s_QLoadWq, 
             /* condition  */  gExecInfo.s_isHwQloadDone,
             /* timeout    */  G3DKMD_QLOAD_WAIT_TIMEOUT_MS);
    ...
    if (wqRet == 0) {  //  timeout
        ...
        KMDDPF_ENG(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_ENGINE, "QLoad wait queue: user process interrupted by singal.\n");
        //tester_custom("qload_test", NULL, "FailInterrupt");
    } else {   // 
        KMDDPF_ENG(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_ENGINE, "QLoad wait queue: released by wq single or condition is true.\n");
...
static INLINE G3DKMD_BOOL _qload_wait_hw_qload_polling(void)
{
    ...
    if( !g3dKmdIsBypass(BYPASS_REG) )
    {
        while (cnt++ < G3DKMD_ENGINE_TIMEOUT_CNT)
        {
            ...
            if (cnt > 1)
            {
                KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_ENGINE, "wait qload finish, cnt %d\n", cnt);
            }
#endif
        }
    ...
    KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_ENGINE, "_qload_wait_hw_qload_polling, IDLE0 0x%x, IDLE1 0x%x, RAW 0x%x\n", 
	g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_PA_IDLE_STATUS0, 0xFFFFFFFF, 0), 
	g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_PA_IDLE_STATUS1, 0xFFFFFFFF, 0),
	g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_IRQ_STATUS_RAW,  0xFFFFFFFF, 0));
...
//    local_bh_enable();
    ...
    // For non-HW, we have to restore by previously saved write pointer (beacuse for non-HW, read pointer equals write pointer)
    ...
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_CONTEXT_CTRL, 1, MSK_AREG_CQ_PMT_MD, SFT_AREG_CQ_PMT_MD);
#endif
...
#if 0 // dump ctx switch info
{
    ...
    printk("[Brady] there are total %d exeInfoCount\n", gExecInfo.exeInfoCount);

    p1 = gExecInfo.exeList[0];
    p2 = gExecInfo.exeList[1];

    printk("[Brady]======= Inst Status==========\n");
    ...
    if (gExecInfo.exeInfoCount > 0)
    {
        ...
        printk("[Brady] p1: nonEmpty %d, isNew %d isPending %d isRunning %d\n", p1nonEmpty, p1isNew, p1isPending, p1isRunning);
    }
    if (gExecInfo.exeInfoCount > 1)
    {
        ...
        printk("[Brady] p2: nonEmpty %d, isNew %d isPending %d isRunnig %d\n", p2nonEmpty, p2isNew, p2isPending, p2isRunning);
    }
    ...
    // update tail pointer
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_HTBUF_WPTR, writePtr, MSK_AREG_CQ_HTBUF_WPTR, SFT_AREG_CQ_HTBUF_WPTR);

#if 0  // debug
    printk("[SEND_COMMAND] wrt_ptr = 0x%x, writePtr\n");
    printk("[SEND_COMMAND] data[0] = 0x%x, writePtr[0]\n");
    printk("[SEND_COMMAND] data[1] = 0x%x, writePtr[1]\n");
    printk("[SEND_COMMAND] data[2] = 0x%x, writePtr[2]\n");
    printk("[SEND_COMMAND] data[3] = 0x%x, writePtr[3]\n");
#endif 
...
void g3dKmdRegRepeatCommand(int hwChannel, pG3dExeInst pInst, unsigned int readPtr)
{
#ifdef YL_HW_DEBUG_DUMP_TEMP
    unsigned int writePtr = pInst->pHtDataForHwRead->htInfo->wtPtr;
    
    // update head pointer
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_CQ_HTBUF_RPTR, readPtr, MSK_AREG_CQ_HTBUF_RPTR, SFT_AREG_CQ_HTBUF_RPTR);
...
void g3dKmdRegRestartHW(int taskID, int mode)
{
    ...
    pG3dTask task;
    unsigned int rptr_bak, rptr;
    int i;
    pG3dExeInst pInst, pExeInst;
    SapphireQreg* qreg;
    ...
    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_ENGINE,"in, taskID %d\n", taskID);

    // 1. backup read pointer, retry the hangup entry again or advance by one entry in order to skip the hangup entry
    rptr_bak = g3dKmdRegGetReadPtr(pInst->hwChannel);

    if (!((rptr_bak >= pInst->pHtDataForHwRead->htInfo->htCommandBaseHw) && 
         (rptr_bak < pInst->pHtDataForHwRead->htInfo->htCommandBaseHw + HT_BUF_SIZE)))
    {
        rptr_bak = pInst->pHtDataForHwRead->htInfo->rdPtr;
    }

    // increment reset count
    ridx = (rptr_bak - pInst->pHtDataForHwRead->htInfo->htCommandBaseHw) / HT_CMD_SIZE;
    task = g3dKmdGetTask(pInst->pHtDataForHwRead->htInfo->htCommand[ridx].command);
    if (task != NULL)
    {
        task->hw_reset_cnt++;
    }
#if 0
    if ((mode == G3DKMD_RECOVERY_SKIP) && (rptr_bak != pInst->pHtDataForHwRead->htInfo->wtPtr))
    {
#ifdef G3D_SWRDBACK_RECOVERY
        g3dKmdRecoveryUpdateSWReadBack(pInst, rptr_bak);
#endif
        rptr_bak += HT_CMD_SIZE;
    }
    
    if(rptr_bak >= pInst->pHtDataForHwRead->htInfo->htCommandBaseHw + HT_BUF_SIZE)
    {
        rptr_bak = pInst->pHtDataForHwRead->htInfo->htCommandBaseHw;
    }
#endif
    #ifdef G3DKMD_SUPPORT_FDQ
    // backup FDQ buf pointer
    g3dKmdFdqBackupBufPtr();
    #endif

    // 2. reset HW, and check if idle
    g3dKmdRegReset();

    // 3. Set all the qbasebuf to skip all of HT contents.
    for(i = 0; i < gExecInfo.exeInfoCount; i++) {
        ...
        if(pExeInst != NULL) {
            ...
            qreg->reg_cq_htbuf_rptr.s.cq_htbuf_rptr = pExeInst->pHtDataForHwRead->htInfo->wtPtr;
            qreg->reg_cq_htbuf_wptr.s.cq_htbuf_wptr = pExeInst->pHtDataForHwRead->htInfo->wtPtr;
            ...
            if(pExeInst->pHtDataForHwRead->htInfo->wtPtr != pExeInst->pHtDataForHwRead->htInfo->rdPtr) {
                rptr = pExeInst->pHtDataForHwRead->htInfo->wtPtr - HT_CMD_SIZE;
                if( rptr < pExeInst->pHtDataForHwRead->htInfo->htCommandBaseHw )
                    rptr = pExeInst->pHtDataForHwRead->htInfo->htCommandBaseHw + HT_BUF_SIZE - HT_CMD_SIZE;
    ...
    #ifdef G3DKMD_SUPPORT_FDQ
    // 5. restore FDQ buffer pointer
    //if (mode == G3DKMD_RECOVERY_SKIP)
    //{
    //    gExecInfo.fdq_rptr = gExecInfo.fdq_wptr; // wptr will write to fdq_wptr_rb
    //}
    
    g3dKmdFdqUpdateBufPtr();
    #endif
    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_ENGINE,"[%s]: out, taskID %d\n", __FUNCTION__, taskID);
}
...
void g3dKmdCacheFlushInvalidate(void)
{
    ...
    if (in_interrupt()) 
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_ENGINE, "Cannot invoke smp_inner_dcache_flush_all() in interrupt/softirq context\n");
        return;
    }
...
void* g3dkmdmalloc(unsigned int size, unsigned int flags, const char * file, const char *func, unsigned int nline)
{
  ...
  KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_ENGINE, "(HW_MEMORY) kmalloc: ptr = 0x%x, size = %d at %s():ln%d in %s\n", p, size, func, nline, file);  
...
void g3dkmdfree(void* p, const char * file, const char *func, unsigned int nline)
{
  ...
  KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_ENGINE, "(HW_MEMORY) kfree: ptr = 0x%x at %s():ln%d in %s\n", p, func, nline, file);  
}
...
void* g3dkmdvmalloc(unsigned int size, const char * file, const char *func, unsigned int nline)
{
  ...
  KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_ENGINE, "(HW_MEMORY) vmalloc: ptr = 0x%x, size = %d at %s():ln%d in %s\n", p, size, func, nline,  file);  
...
void g3dkmdvfree(void* p, const char * file, const char *func, unsigned int nline)
{
  ...
  KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_ENGINE, "(HW_MEMORY) vfree: ptr = 0x%x at %s():ln%d in %s\n", p, func, nline, file);  
}
