...
#define SDPF(...) printk(KERN_INFO __VA_ARGS__) 
//#define SUDPF(...)
...
__DEFINE_SEM_LOCK(memBlockLock);
...
void* openDramFD(void)
{
    ...
    sprintf( filename, "%sp%d_g%03d_dram_buffer_kmd_%05d_%05d.hex", g_control.filePrefix, pid, g_globalCnt, data->frameCnt, data->wpCnt );
...
void* openSaveRegFD(void)
{
    ...
    sprintf( filename, "%sp%d_g%03d_saved_reg_%05d_%05d.hex", g_control.filePrefix, pid, g_globalCnt, data->frameCnt, data->wpCnt );
...
void* getRiuFD(void)
{
	...
	if ( data )
	{
		...
		if ( !riu_fd )
		{
			sprintf( filename, "%sp%d_g%03d_riu_g3d_%05d_%05d.hex", g_control.filePrefix, pid, g_globalCnt, data->frameCnt, data->wpCnt );
	...
	{
		sprintf( filename, "%sp%d_g%03d_riu_g3d_%05d_%05d.hex", g_control.filePrefix, pid, g_globalCnt, 0, 0 );
		#if defined(S_DEBUG_IO)
		SDPF( "[MPD %d]Open %s\n", pid, filename );	
		#endif
		riu_fd = g_pKmdFileIo->fileOpen(filename);
...
#ifndef S_DEBUG_SKIP_DRAM_CONTENT
        g_pKmdFileIo->fPrintf(fd, "%02x%02x%02x%02x_%02x%02x%02x%02x_%02x%02x%02x%02x_%02x%02x%02x%02x // addr=%07x\n", 
            data1[15], data1[14], data1[13], data1[12],
            data1[11], data1[10], data1[9], data1[8],
            data1[7], data1[6], data1[5], data1[4],
...
#if 1 // FPGA
{
    ...
    // dump L1
    CMDumpKMemory( fd, (void*)swAddr, (unsigned int)(uintptr_t)hwAddr, (unsigned int)(uintptr_t)hwAddr, size );
    ...
    list_for_each(iter, &g_KMemBlks)
    {
...
#if 0
        if ( mem->G3DVA == mem->PA )	// this memory block don't have actual G3DVA, dump all block with single PA
        {
            pa = mem->PA;
            #if defined(S_DEBUG_DRAM)
        ...
        else	// this memory block need convert each 4K block to PA
#else
        if ( mem->G3DVA != mem->PA )
#endif
        {
            ...
            while (remain_size > 0)
            {
                ...
                {
                    dump_size = ( remain_size > MMU_SMALL_PAGE_SIZE ? MMU_SMALL_PAGE_SIZE : remain_size );
                }
...
void CMMPDControl(g3dkmd_IOCTL_MPD_Control* cmd)
{
    ...
    case MPD_SET_DUMP_PATH:
        ...
        if ( copy_from_user( (void*) g_control.filePrefix, (const void __user *) cmd->str, cmd->str_len ) == 0 )
        {
            g_control.filePrefix[cmd->str_len] = '\0';
            #ifdef S_DEBUG_FLOW
            SDPF( "[MPD %d]CMMPDControl(MPD_SET_DUMP_PATH, %s)\n", current->tgid, g_control.filePrefix );	
            #endif
        }
        else
        {
    ...
    case MPD_SET_DUMP_APNAME:
        YL_KMD_ASSERT( cmd->str );
        YL_KMD_ASSERT( cmd->str_len < MAX_STR_LEN );
        if ( copy_from_user( (void*) g_control.targetAP, (const void __user *) cmd->str, cmd->str_len ) == 0 )
        {
    ...
    case MPD_QUERY_DUMP_PATH:
        if( copy_to_user ( (void __user *)(uintptr_t)cmd->str,(const void *)g_control.filePrefix, strlen(g_control.filePrefix)+1) == 0 )
        {
...
void CMRiuCommand(int command, int data) 
{
    ...
    int isTargetRange = !g_control.skipNumbers && ( !g_control.isLimitDumpNumber || g_control.dumpNumbers > 0 );
    ...
    switch(command)
    {
        case 0x1F0C:    saved_data[REG_1F0C] = data; break;
        case 0x1F20:    saved_data[REG_1F20] = data; break;
        case 0x1198:    saved_data[REG_1198] = data; break;
        case 0x119C:    saved_data[REG_119C] = data; break;
        case 0x5000:    saved_data[REG_5000] = data; break;
        case 0x5004:    saved_data[REG_5004] = data; break;
        case 0x4180:    saved_data[REG_4180] = data; break;
        case 0x1110:    saved_data[REG_1110] = data; break;
        case 0x1114:    saved_data[REG_1114] = data; break;
    ...
    if ( ( command == REG_AREG_G3D_ENG_FIRE ) && ( data == 1 ) && ( control & MSK_AREG_CQ_HTBUF_WPTR_UPD ) && ( g_control.skipNumbers > 0 ) )
    {
        g_globalCnt++;
	...
	if ( !g_control.isEnableMPD || !isTargetPID || !isTargetRange ) return;
