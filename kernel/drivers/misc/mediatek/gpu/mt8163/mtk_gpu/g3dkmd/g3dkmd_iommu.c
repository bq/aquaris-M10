...
static iommu_regwrite_callback _regCbkFunc = NULL;

static void _g3dKmdIommuRegWrite(unsigned int hw_mod, unsigned int reg, unsigned int value, unsigned int mask, unsigned int sft)
{
    _DEFINE_BYPASS_
...
static unsigned int _g3dKmdIommuRegRead(unsigned int hw_mod, unsigned int reg, unsigned int mask, unsigned int sft)
{
    return g3dKmdRegRead(hw_mod, reg, mask, sft);
}

static void _g3dKmdIommuInvalidTlb(unsigned int mod, unsigned int isInvAll, unsigned int mva_start, unsigned int mva_end)
{
    ...
    {
        _g3dKmdIommuRegWrite(hw_mod, REG_MMU_INVLD_START_A, mva_start, MSK_RANGE_IVD_START_ADDR, 0);
        _g3dKmdIommuRegWrite(hw_mod, REG_MMU_INVLD_END_A, mva_end, MSK_RANGE_IVD_END_ADDR, 0);
        _g3dKmdIommuRegWrite(hw_mod, REG_MMU_INVALIDATE, 0x1, MSK_RANGE_INVLD, SFT_RANGE_INVLD);
    }
    ...
    if (!isInvAll)
    {
        ...
        _BYPASS_(BYPASS_RIU);
        _g3dKmdIommuRegWrite(hw_mod, REG_MMU_CPE_DONE, 0x1, MSK_RANGE_INVAL_DONE, SFT_RANGE_INVAL_DONE);
        _RESTORE_();
    #endif

        do
        {
            value = _g3dKmdIommuRegRead(hw_mod, REG_MMU_CPE_DONE, MSK_RANGE_INVAL_DONE, SFT_RANGE_INVAL_DONE);
        }
        while(value == 0);
        ...
        _g3dKmdIommuRegWrite(hw_mod, REG_MMU_CPE_DONE, 0x0, MSK_RANGE_INVAL_DONE, SFT_RANGE_INVAL_DONE);
    }
#endif
#endif
...
void g3dKmdIommuInit(unsigned int mod, unsigned int page_table_addr)
{
    ...
    _g3dKmdIommuRegWrite(hw_mod, REG_MMU_PT_BASE_ADDR_SEC, page_table_addr, MSK_BASE_ADDR_SEC, 0);
    ...
    _g3dKmdIommuRegWrite(hw_mod, REG_MMU_PRIORITY, (MSK_TABLE_WALK_ULTRA | MSK_AUTO_PF_ULTRA), (MSK_TABLE_WALK_ULTRA | MSK_AUTO_PF_ULTRA), 0);
        
    // enable coherence
    //_g3dKmdIommuRegWrite(hw_mod, REG_MMU_CTRL_REG, 0x1, MSK_COHERENCE_EN, SFT_COHERENCE_EN);

    // disable bypass
    //_g3dKmdIommuRegWrite(hw_mod, REG_MMU_WR_LEN, 0x0, MSK_WRITE_THROTTLING_DIS, SFT_WRITE_THROTTLING_DIS);
...
void g3dKmdIommuBypass(unsigned int mod)
{
    ...
    // bypass
    func->reg_write32_msk_sft(REG_MMU_WR_LEN, 0x1, MSK_WRITE_THROTTLING_DIS, SFT_WRITE_THROTTLING_DIS);
}
#endif
