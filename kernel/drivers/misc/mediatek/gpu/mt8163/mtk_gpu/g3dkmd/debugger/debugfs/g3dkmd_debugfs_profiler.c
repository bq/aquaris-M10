...
void g3dKmd_debugfs_profiler_create(struct dentry *profiler_dir)
{
    ...
    // log_level controler
    debugfs_create_u32("loglevel", (S_IWUGO |S_IRUGO|S_IWUSR), kmemory_dir, _g3dKmdProfilerKMemoryUsageGetLogLevelVariablePointer());

    // ask current status
    _g3dKmdCreateDebugfsFileNode("KMemUsageCurrent", memory_dir, kmemroy_usage_debugfs_seq_show);
#endif
