...
typedef struct _MmceExecuteInfo
{
    MMCE_THD mmceThd[MMCE_MAX_THD_CNT];
} MmceExecuteInfo;

// local variable definition
static MmceExecuteInfo  s_mmceInfo;
...
static unsigned int _g3dKmdMmceReadToken(unsigned token_id)
{
    ...
    value= MMCE_REG_READ32_MSK_SFT(REG_CMDQ_SYNC_TOKEN_VALUE, MSK_TOKEN_VALUE, SFT_TOKEN_VALUE);
...
unsigned int g3dKmdMmceResetThread(unsigned int thd_id)
{
    ...
    {
        if (MMCE_REG_READ32_MSK_SFT(CMDQ_THRx_RESET(thd_id), MSK_THR0_WARM_RST, SFT_THR0_WARM_RST) == 0)
        {
            break;
        }
        G3DKMD_SLEEP(1);
...
void g3dKmdMmceResetCore(void)
{
    ...
    while (cnt < MMCE_TIMEOUT_CNT)
    {
        if (MMCE_REG_READ32_MSK_SFT(REG_CMDQ_CORE_RESET, MSK_CORE_WARM_RST, SFT_CORE_WARM_RST) == 0)
        {
            break;
        }
        G3DKMD_SLEEP(1);
    ...
    if (!g3dKmdIsBypass(BYPASS_RIU))
    {
        ...
        g3dKmdRiuWrite(G3DKMD_RIU_G3D, "%04x_%08x\n", MOD_OFST(MMCE, REG_CMDQ_CORE_RESET), MSK_CORE_WARM_RST);        
    }
#endif
...
{
    MMCE_THD* mmce = (MMCE_THD*) &s_mmceInfo.mmceThd[handle];
...
void g3dKmdMmceResume(MMCE_THD_HANDLE handle, void* mmcq_handle, unsigned int hwChannel)
{
    ...
    MMCE_REG_WRITE32_MSK_SFT(CMDQ_THRx_INSTN_TIMEOUT_CYCLES(mmce->thd_id), 0x1B, MSK_THR0_INSTN_TIMEOUT_CYCLES, SFT_THR0_INSTN_TIMEOUT_CYCLES);

    // enable ISR (TBD, add ISR to do error handling)
    MMCE_REG_WRITE32_MSK_SFT(CMDQ_THRx_IRQ_FLAG_EN(mmce->thd_id), 0x0, MSK_THR0_INSTN_TIMEOUT_FLAG_EN, SFT_THR0_INSTN_TIMEOUT_FLAG_EN);
    MMCE_REG_WRITE32_MSK_SFT(CMDQ_THRx_IRQ_FLAG_EN(mmce->thd_id), 0x0, MSK_THR0_INVALID_INSTN_FLAG_EN, SFT_THR0_INVALID_INSTN_FLAG_EN);
...
void g3dKmdMmceSuspend(MMCE_THD_HANDLE handle, unsigned int hwChannel)
{
    ...
    _BYPASS_(BYPASS_RIU);
    MMCE_REG_WRITE32_MSK_SFT(REG_CMDQ_SYNC_TOKEN_VALUE, MMCQ_TOKEN_VALUE_TRUE, MSK_TOKEN_VALUE, SFT_TOKEN_VALUE);
    _RESTORE_();
#endif
    cnt = 0;
    while (cnt < MMCE_TIMEOUT_CNT)
    ...
    // suspend mmce
    MMCE_REG_WRITE32_MSK_SFT(CMDQ_THRx_SUSPEND(mmce->thd_id), 1, MSK_THR0_SUSPEND, SFT_THR0_SUSPEND);
#ifndef G3D_HW   // this is only to simulate status update after suspend when CMODEL
    _BYPASS_(BYPASS_RIU);
    MMCE_REG_WRITE32_MSK_SFT(CMDQ_THRx_STATUS(mmce->thd_id), 1, MSK_THR0_SUSPENDED, SFT_THR0_SUSPENDED);
    _RESTORE_();
#endif
    cnt = 0;
    while (cnt < MMCE_TIMEOUT_CNT)
