...
static uint64_t g3dKmdAllocPhysical2( G3d_Resource_Type resType, unsigned int size )
{
    ...
    if ((1 << get_order(size))*PAGE_SIZE == size)
    {
        ...
        {
            return (uint64_t)__pa(va);
        }
        else
        {
            if (va != NULL) free_pages((unsigned long)va, get_order(size));
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_MEM,"g3dKmdAllocPhysical2 : wrong memory allocation 0x%p 0x%x\n", va, size);
            return 0;    
        }
    }
    else 
#endif
    {
        return g3dKmdAllocPhysical(size);
    }
}
                           ...
                           unsigned int baseAlign, unsigned int flag)
{
...
#ifdef YL_SECURE
    {
        ...
        if (is_secure)
        {
            memory->resType = (G3d_Resource_Type)((unsigned)memory->resType | (flag << 5));
        }
    }
#endif
    ...
    if (size <= KMALLOC_UPPER_BOUND && expanded_size > KMALLOC_UPPER_BOUND && (size%baseAlign == 0))
    {
       expanded_size = size;
    }
    #endif 
    ...
    {        
        ...
        struct device              *g3ddev  =
            (NULL != g3dpdev)? &(g3dpdev->dev): NULL;

        memory->data0 = dma_alloc_coherent(g3ddev, size, &dma_handle, GFP_KERNEL | __GFP_DMA);
    #else
        memory->data0 = dma_alloc_coherent(NULL, size, &dma_handle, GFP_KERNEL | __GFP_DMA);
    #endif //USING_MISC_DEVICE 

        pa = (uint64_t)dma_handle;
        YL_KMD_ASSERT(((uintptr_t)memory->data0 & (PAGE_SIZE -1)) == 0 && (pa & (PAGE_SIZE -1)) == 0);
        memory->resType = G3D_RES_DMA;
    }
    else if (useMMU)
    {
        pa              =
        memory->data0   = G3DKMDVMALLOC(expanded_size);
    ...
    memory->size        = size;
    memory->data        = (void*)G3DKMD_ALIGN((uintptr_t)memory->data0,
                                (uintptr_t)baseAlign);
    memory->phyAddr     = pa;
    ...
    { 
        ...
        memory->hwAddr0 = fakeAddr_for_64bits | (unsigned int)((uintptr_t)memory->data0 & (PAGE_SIZE -1));
    ...
    if (useMMU)
    {
        ...
        if (useDMA) { gExecInfo.pDMAMemory = memory;}
  #endif
        // only this case, data0 and data will different but hwAddr0 = hwAddr
        memory->hwAddr  =
        memory->hwAddr0 = g3dKmdMmuMap(hMmu, memory->data, expanded_size, MMU_SECTION_DETECTION, flag);
  #if defined(linux) && !defined(linux_user_mode)
    ...
    if (useMMU)
    {
        ...
        if( g3dKmdCfgGet(CFG_DUMP_ENABLE) )
            memory->pa_pair_info = g3dKmdMmuG3dVaToPaFull( hMmu, memory->hwAddr, memory->size );
#endif
#endif
    }
    else
    ...
    if (memory->phyAddr)
    {
        ...
        if (memory->resType == G3D_RES_DMA)
        {
...
#endif
            {
                ...
                dma_free_coherent(g3ddev, memory->size, memory->data, memory->phyAddr);
            #else
                dma_free_coherent(NULL  , memory->size, memory->data, memory->phyAddr);
