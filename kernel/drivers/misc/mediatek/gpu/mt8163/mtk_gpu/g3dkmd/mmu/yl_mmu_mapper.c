...
//Mapper_init_mirror( Mapper m, g3d_va_t g3d_va_base, void * page_space, void * meta_space, void * page_space_mirror, int cache_enable )
{
    FirstPageTable  space = (FirstPageTable)mirror_mem.data;
...
Mapper_init( Mapper m, 
             ...
             const int cache_enable )             ///< The 4k cache
{
    FirstPageTable page_space = (FirstPageTable)page_mem.data;
    ...
    yl_mmu_zeromem( (void *)m, sizeof(MapperRec) );

    m->G3d_va_base  = g3d_va_base;
    m->Page_mem     = page_mem;
    m->Page_space   = page_space;
    m->Meta_space   = (FirstMetaTable)meta_space;
    ...
    else
    {
        ...
        yl_mmu_zeromem( (void *)m->Page_space_lookback, Mapper_get_required_size_page_table() );
    }
#endif 
}

void 
Mapper_uninit( Mapper m )
{
...
{
    FirstPageTable page_space = (FirstPageTable)m->Page_mem.data;
    FirstMetaTable meta_space = m->Meta_space;
...
#if MMU_MIRROR
    {
        FirstPageTable space = (FirstPageTable)m->Mirror_mem.data;
        if (space)
        {
            yl_mmu_zeromem( (void *)space, Mapper_get_required_size_page_table() );
        }
            ...
            const unsigned int flag)
{
...

    {
        Trunk               t                   = NULL;
        Trunk               t2                  = NULL;
        Trunk               first_trunk         = m->Bin_list.first_trunk;
...
// D). Search for just-fit page (around size ~ 2*size) 
        ...
        if ( could_be_section && _Mapper_contain_continuous_section( m, & m->Pa_section_lookup_table, va, occupied_size ) )
        {
            index1_start = PageTable_calc_index1_section_aligned( va );
            index1_tail = (index1_start + occupied_size/MMU_SMALL_SIZE-1) % SMALL_LENGTH;

            t = BinList_scan_for_empty_trunk_section_fit( & m->Bin_list, occupied_size, index1_start, index1_tail );
        ...
        else // t->size > occupied_size
        {
            if ( has_section )
            {
                ...
                unsigned short index1_padding   = PageIndex_tail_padding_for_align( end_idx.s.index1, index1_tail );
...
void 
Mapper_unmap(Mapper m, g3d_va_t g3d_va, size_t size)
{
...

    {
        ...
        if ( trunk->size == MMU_SMALL_SIZE )
        {
            ...
            if ( iter  && (iter <REMAPPER_RCACHE_ITEM_LENGTH))
            {
                ...
                Rcache_remove_by_pa_base( & m->Rcache, SmallPage_pabase_part( pa_base ) );
            }
        }
...
//
//void * 
//Mapper_get_page_space0( Mapper m )
//{
//    return m->Page_space0;
//}

void * 
Mapper_get_page_space( Mapper m )
{
    return m->Page_space;
}

void * 
Mapper_get_meta_space( Mapper m )
{
    return m->Meta_space;
}
...
pa_t 
Mapper_g3d_va_to_pa(Mapper m, g3d_va_t g3d_va)
{
...
#if MMU_MIRROR
pa_t 
Mapper_g3d_va_to_g3d_va(Mapper m, g3d_va_t g3d_va)
{
    YL_MMU_ASSERT( g3d_va >= m->G3d_va_base );
...
#if MMU_LOOKBACK_VA 
va_t 
Mapper_g3d_va_to_va(Mapper m, g3d_va_t g3d_va)
{
...
g3d_va_t
Mapper_map_given_pa(Mapper m, va_t va, pa_t pa, size_t size, YL_BOOL could_be_section, unsigned int flag)
{
    g3d_va_t g3d_va;
    va_t     va2;
...
//    Mapper m;
//
//    m = (Mapper)yl_mmu_alloc( sizeof(MapperRec) );
...
//void 
//Mapper_delete(Mapper* mapper_ref)
//{
//    Mapper m = *mapper_ref;
//
//    // Atomic when possible
