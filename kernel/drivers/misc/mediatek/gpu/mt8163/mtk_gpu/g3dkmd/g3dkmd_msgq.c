...
typedef struct _G3DKMD_MSGQ_INFO
{
...
#define _get_rptr()         ((G3DKMD_MSGQ_INFO*)(_msgQ_buf + _msgQ_rptr))
#define _get_wptr()         ((G3DKMD_MSGQ_INFO*)(_msgQ_buf + _msgQ_wptr))
#define _is_empty()         (_msgQ_rptr == _msgQ_wptr)
#define _req_sz(size)       (_align((sizeof(G3DKMD_MSGQ_INFO) + (size)), G3DKMD_MSGQ_ALIGN))
...
/***** local functions *****/
static G3DKMD_MSGQ_INFO* _g3dKmdMsgQGetEmptyBuf(unsigned int size)
{
    unsigned int empty_size, required_size;

    required_size = _req_sz(size) + G3DKMD_MSGQ_GUARD_BAND;

    // if the last empty size is less than our required size, just mark it as SKIP, in order to prevent "ring-back" case
    if ((_msgQ_rptr < _msgQ_wptr) && ((G3DKMD_MSGQ_SIZE - _msgQ_wptr) < required_size))
    {
        G3DKMD_MSGQ_INFO* pInfo;
    ...
    empty_size = (((_msgQ_rptr <= _msgQ_wptr) ? G3DKMD_MSGQ_SIZE : 0) + _msgQ_rptr) - _msgQ_wptr;
...
G3DKMD_MSGQ_RTN g3dKmdMsgQSend(G3DKMD_MSGQ_TYPE type, unsigned int size, void* buf)
{
    ...
    pInfo->type = type;
    pInfo->size = size;
    pInfo->data = (void*)((unsigned int)pInfo + sizeof(G3DKMD_MSGQ_INFO));
...
#if (defined(linux) && !defined(linux_user_mode))
    if ((uintptr_t)buf < PAGE_OFFSET)
    {   // pointer is user space
...
void g3dKmdMsgQSetReg(unsigned int reg, unsigned int value)
{
    ...
    msg.reg     = reg;
    msg.value   = value;
    while(g3dKmdMsgQSend(G3DKMD_MSGQ_TYPE_SET_REG, sizeof(G3DKMD_MSGQ_SET_REG), &msg) == G3DKMD_MSGQ_RTN_FULL)
    {
        G3DKMD_SLEEP(1);
    }
}
...
void g3dKmdMsgQAllocMem(unsigned int hwAddr, uint64_t phyAddr, unsigned int size)
{
    ...
    msg.phyAddr = phyAddr;
    msg.size    = size;
    while(g3dKmdMsgQSend(G3DKMD_MSGQ_TYPE_ALLOC_MEM, sizeof(G3DKMD_MSGQ_ALLOC_MEM), &msg) == G3DKMD_MSGQ_RTN_FULL)
    {
        G3DKMD_SLEEP(1);
    }
}
...
void g3dKmdMsgQFreeMem(unsigned int hwAddr, unsigned int size)
{
    ...
    msg.hwAddr  = hwAddr;
    msg.size    = size;
    while(g3dKmdMsgQSend(G3DKMD_MSGQ_TYPE_FREE_MEM, sizeof(G3DKMD_MSGQ_FREE_MEM), &msg) == G3DKMD_MSGQ_RTN_FULL)
    {
        G3DKMD_SLEEP(1);
    }
}
