...
{
    { /* 0 */
        {.freq_khz = GPU_DVFS_F480, .volt = GPU_POWER_VGPU_1_25V, .power_mW = 1025, .lower_bound = 40, .upper_bound =  GPAD_IPEM_UPPER_BOUND_MAX},
        {.freq_khz = GPU_DVFS_F380, .volt = GPU_POWER_VGPU_1_15V, .power_mW =  725, .lower_bound =  0, .upper_bound =  40},
    },

    { /* 1 */
        {.freq_khz = GPU_DVFS_F550, .volt = GPU_POWER_VGPU_1_25V, .power_mW = 1150, .lower_bound = 70, .upper_bound = GPAD_IPEM_UPPER_BOUND_MAX},
        {.freq_khz = GPU_DVFS_F480, .volt = GPU_POWER_VGPU_1_25V, .power_mW = 1025, .lower_bound = 30, .upper_bound =  70},
        {.freq_khz = GPU_DVFS_F380, .volt = GPU_POWER_VGPU_1_15V, .power_mW =  725, .lower_bound =  0, .upper_bound =  30},
    },

    { /* 2 */
        {.freq_khz = GPU_DVFS_F380, .volt = GPU_POWER_VGPU_1_15V, .power_mW =  725, .lower_bound =  0, .upper_bound = GPAD_IPEM_UPPER_BOUND_MAX},
    },

    { /* 3 */
        {.freq_khz = GPU_DVFS_F480, .volt = GPU_POWER_VGPU_1_25V, .power_mW = 1025, .lower_bound =  0, .upper_bound = GPAD_IPEM_UPPER_BOUND_MAX},
    },

    { /* 4 */
        {.freq_khz = GPU_DVFS_F550, .volt = GPU_POWER_VGPU_1_25V, .power_mW = 1150, .lower_bound =  0, .upper_bound = GPAD_IPEM_UPPER_BOUND_MAX},
    },
...
unsigned int g_ipem_table_sizes[] = {2, 3, 1, 1, 1, 1};
#if GPAD_IPEM_DEBUG_PROC
static struct proc_dir_entry *g_proc_dir = NULL;
#endif
static bool s_is_command_queue_busy = false;

/// Counters 
static u32 g_debug = 0;
static u32 g_called[GPAD_IPEM_EVENT_LENGTH] = {0}; 
static u32 g_recursive_called[GPAD_IPEM_EVENT_LENGTH] = {0}; 
static u32 g_recursive_called_max[GPAD_IPEM_EVENT_LENGTH] = {0}; 
static u32 g_in_touch = 0;
static atomic_t g_in_performance_mode= ATOMIC_INIT(0);

/// MET
static u32 g_gpuload =   0;
...
static void gpad_ipem_table_init(struct gpad_device *dev)
{
    ...
    switch(efuse_tag) {
    case 0x8: table_id = GPAD_IPEM_TABLE_ID_MT8163A_SD; break;
    case 0x6: table_id = GPAD_IPEM_TABLE_ID_MT8163A_SB; break;
    case 0x5: table_id = GPAD_IPEM_TABLE_ID_MT8163A_SA; break;
    default: table_id = GPAD_IPEM_DEF_TABLE_ID;
    }
...
static void gpad_ipem_tracker_update_top(struct gpad_ipem_info *ipem_info, 
                                         ...
                                         u32 timetotal)
{
    ...
    GPAD_LOG_INFO("[IPEM] %s: callback_prev = %u, callback_next = %u, timestamp_prev = %lu, timestamp_next = %lu, timestamp_off2on = %lu, timebusy_prev = %u, timebusy_next = %u, timetotal_prev = %u, timetotal_next = %u\n", __FUNCTION__, event_prev, event_next, timestamp_prev, timestamp_next, timestamp_off2on, timebusy_prev, timebusy_next, timetotal_prev, timetotal_next); 
}

static void gpad_ipem_tracker_update_middle(struct gpad_ipem_work_info *work_info, 
...
unsigned int gpad_ipem_statistic_gpuload_old(struct gpad_ipem_info *ipem_info, struct gpad_pm_info *pm_info, bool isCalc )
{
    ...
    static unsigned int s_pre_gpuload = 0;
...
static void gpad_ipem_statistic_update_top(struct gpad_ipem_info *ipem_info, 
...
{
    static unsigned long long s_acc_busy_ns_prev  = 0;
    static unsigned long long s_acc_total_ns_prev = 0;
...
static void gpad_ipem_do_work(struct work_struct *work)
{
    ...
    //    if (ipem_info->table_info.table[action].volt > 0) {
            ...
            gpad_pm_change_freq(dev, 
                ipem_info->table_info.table[action].freq_khz * 1000);
...
int gpad_ipem_init(struct gpad_device *dev)
{
    ...
    struct mtk_gpufreq_kmif kmif_ops = {
        .notify_thermal_event   = gpad_ipem_callback_thermal,
        .notify_touch_event     = gpad_ipem_callback_touch,
            .get_loading_stat       = gpad_ipem_met_loading_,
            .get_block_stat         = gpad_ipem_met_block_,
            .get_idle_stat          = gpad_ipem_met_idle_,
            .get_power_loading_stat = gpad_ipem_met_power_loading_
    };
    USE_IPEM_LOCK;
...
static int proc_called_show_(struct seq_file* s, void* v)
{
    ...
    seq_printf(s, "%6d %6d %6d %6d %6d %6d %6d\n", 
                    g_recursive_called_max[0], 
                    g_recursive_called_max[1], 
                    g_recursive_called_max[2], 
                    g_recursive_called_max[3], 
                    g_recursive_called_max[4], 
                    g_recursive_called_max[5], 
                    g_recursive_called_max[6]
              ); 
    return 0;
}
...
static const struct file_operations called_fops = {
    .owner      = THIS_MODULE,
    .write      = proc_called_write_,
    .open       = proc_called_open_,
    .read       = seq_read,
    .llseek     = seq_lseek,
    .release    = single_release,
};
