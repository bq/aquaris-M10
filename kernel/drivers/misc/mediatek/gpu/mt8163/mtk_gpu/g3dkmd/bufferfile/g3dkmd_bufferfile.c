...
#else
#define BF_DPF(type, ...) printk(__VA_ARGS__)
#endif
...
// linux kernel space
// Bufferfile Only Avaible in Linux Kernel mode.
...
__DEFINE_SEM_LOCK(g_bfFileListModifyLock);
__DEFINE_SEM_LOCK(g_bfFileListSearchLock);
...
static Queue_Bufferfile bfQOpened = {NULL, NULL}; // for OPENED files
static Queue_Bufferfile bfQClosed = {NULL, NULL}; // for CLOSED files
static Queue_Bufferfile bfQWriten = {NULL, NULL}; // for WRITEN files
...
// Page management
unsigned long inline _bf_getFreePage(void);
void          inline _bf_freePage(unsigned long addr);
int           inline _bf_getAllocPageCount(void);
//void*         _bf_kmalloc(size_t size, int flags);
void*         _bf_kmalloc(size_t size);
void          inline _bf_kfree(const void *ptr);
int           inline _bf_getKmallocCount(void);


// File Queue
void            _bf_push (Queue_Bufferfile *q, BufferfileFile* bf);
void            _bf_delete (Queue_Bufferfile *q, int status, pid_t pid);
BufferfileFile* _bf_dequeue(Queue_Bufferfile *q, bf_file_fd_t filp, pid_t pid);
BufferfileFile* _bf_find (Queue_Bufferfile *q, bf_file_fd_t filp, pid_t pid);
bool            _bf_isExist(Queue_Bufferfile *q, const char *fn, pid_t pid,
                            int status);
// helper funcion
BufferfileFile* _bf_remove_node (Queue_Bufferfile *q, BufferfileFile *node,
                                 BufferfileFile *pre);
...
void         _bf_deleteFile(BufferfileFile*);


// BF Data Block
BufferfileDataBlock* _bf_createDataBlock(void);
void                 _bf_deleteDBlocks(BufferfileDataBlock* pD);
int                  _bf_writeDBlock(BufferfileFile *pBF, void* data, unsigned int size);
...
void          _bf_deleteUserFileHeaderPage(pid_t pid);
void          _bf_writePidUserFileHeaderPage(void * page, pid_t pid);
pid_t         _bf_getPidUserFileHeaderPage(void * page);
...
void          _bf_writeUserFileHeader (UFileHeader *p, const char* name,
...
// Page management
// ------------------------------------------------------------------------- //
static int _bf_alloc_page_count = 0;
static int _bf_kmalloc_count = 0;

unsigned long inline _bf_getFreePage()
{
...
{
    BufferfileFile * pre = NULL, *n, *nextN;
...
BufferfileFile* _bf_dequeue(Queue_Bufferfile *q, bf_file_fd_t filp, pid_t pid)
{
    BufferfileFile * pre = NULL, *n, *ret = NULL;
...
bool _bf_isExist(Queue_Bufferfile *q, const char *fn, pid_t pid, int status)
{
    BufferfileFile *n, *ret = NULL;
    _bf_sem_lock(LOCK_LEVEL_FILE_SEARCH);
    for(n= q->front; n != NULL; n = n->pNextFile) {
        // ignore filp when -1, pid when 0.
        if(!strcmp(n->name, fn) && (pid == 0 || n->pid == pid) &&
            n->status == status) {
            ret = n;
        }
    }
...
BufferfileFile* _bf_remove_node(Queue_Bufferfile *q, BufferfileFile *n,
                                BufferfileFile *pre)
{
...
bf_file_fd_t bf_fopen(const char *fname, pid_t pid)
{
    BufferfileFile *pBF = NULL;

    if (MAX_BUFFERFILE_NAME -1 <= strlen(fname) )
    {
        BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s] file name (%s) is too long\n",
                __FUNCTION__, fname);
        return 0;
    }
  ...
  //  if yes, return -1
    if (_bf_isExist(&bfQOpened, fname, pid, OPENED)) {
        BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s] file name (%s) is opened!\n",
                __FUNCTION__, fname);
       return 0;
    }

    pBF = (BufferfileFile*) _bf_kmalloc( sizeof(BufferfileFile));
    if(!pBF)
    {
        BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s] kmalloc fail (size %d)\n",
                 __FUNCTION__, sizeof(BufferfileFile) );
        return  0;
    }
    // initialized
...
//#ifdef L_DEBUG
    ...
    {
    BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE, "[bufferfile] Open file: %s(%lu), pid:%d\n", pBF->name, pBF->fd, pid);
        g3dkmd_backtrace();
    }
//#endif
...
void bf_fclose( bf_file_fd_t filp, pid_t pid)
{
    // BufferfileFile *pBF  = _bf_find(&bfQOpened, filp, pid);
    // Remove from OpenFile Queue.
    BufferfileFile *pBF  =  _bf_dequeue(&bfQOpened, filp, pid);
    ...
    if(pBF) {
       ...
       _bf_push(&bfQClosed, pBF);
        ...
        BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[bufferfile] Close file(%lu), pid: %d\n", filp, pid);
...
int bf_fwrite (bf_file_fd_t filp, pid_t pid, void* data, unsigned int size)
{
    ...
    if (filp) {
        ...
        BufferfileFile* currentFile = _bf_find(&bfQOpened, (bf_file_fd_t) filp, 0);
        if (currentFile == NULL) {
            BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s:%d] Could not find BufferfileFile with (filp,tgid):(%u, -)\n", __FUNCTION__, __LINE__, filp);

#ifdef L_DEBUG
    BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE ,"[%s:%d] Could not find BufferfileFile with (filp,tgid):(%lu, -)\n",
            __FUNCTION__, __LINE__, filp);
...
void _bf_deleteFile(BufferfileFile* pBF) {
    BufferfileDataBlock * pD = pBF->front;
#ifdef L_DEBUG
    BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[bufferfile] Delete file: %s(%lu), pid: %d\n", pBF->name, pBF->fd, pBF->pid);
...
BufferfileDataBlock* _bf_createDataBlock(void)
{
    //BufferfileDataBlock *pData = (BufferfileDataBlock*) _bf_kmalloc( sizeof(BufferfileDataBlock), GFP_KERNEL);
    BufferfileDataBlock *pData = (BufferfileDataBlock*) _bf_kmalloc( sizeof(BufferfileDataBlock));
    if(!pData)
    {
        BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s] kmalloc fail (size %d)\n", __FUNCTION__, sizeof(BufferfileDataBlock) );
        return (void*) 0;
    }
...
int _bf_writeDBlock(BufferfileFile *pBF, void* data, unsigned int size)
{
    ...
    int totalWrite = 0;
    BufferfileDataBlock * currentDataBlock;
    ...
    if (pBF->status != OPENED) {
        BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s:%d] File is not opened.(fname: %sfilp:%lu,pid:%d)\n",
            __FUNCTION__, __LINE__, pBF->name, (unsigned long)pBF->fd, pBF->pid);
        return -1;
    }
    ...
    memcpy( (currentDataBlock->data + MAX_BF_DATA_SIZE - currentDataBlock->unusedSize), data, totalWrite);
...
int _bf_createUserFileHeaderPage(pid_t pid)
{
    ...
    for (idx = 0; idx < MAX_USER_HEADER_PAGE; ++idx) {
        ...
        if (!_bf_gUFHPages[idx]) {
            BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s:%d] _bf_gUFHPages[idx] alloc page fail.\n",
                    __FUNCTION__, __LINE__);
            return -1;
        }
        _bf_writePidUserFileHeaderPage(page, pid);
#ifdef L_DEBUG
    BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"get header page[%d]:%lu pid: %d\n", idx, (long unsigned int)page, pid);
#endif //L_DEBUG
        break;
    } // for

    if (!page) {
        BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[%s:%d] _bf_gUFHPages[MAX_USER_HEADER_PAGE(%d)] is full.\n",
               __FUNCTION__, __LINE__, MAX_USER_HEADER_PAGE);
    }
...
void _bf_deleteUserFileHeaderPage(pid_t ipid)
{
    int idx;
    for (idx = 0; idx < MAX_USER_HEADER_PAGE; ++idx) {
...
#ifdef L_DEBUG
                BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[%s:%d] page[%d]: %lu, pid: %d ipid:%d\n", __FUNCTION__, __LINE__,
                       idx, (long unsigned int) page, pid, ipid);
#endif
                if (ipid != 0) { break; }
            }
        }
...
void* _bf_getUserFileHeaderPage(pid_t pid)
{
   ...
   if (idx >= MAX_USER_HEADER_PAGE) return NULL;
...
void _bf_moveClosedFile2WritenList(pid_t pid)
{
    BufferfileFile *fp, *n, *pre, *next; //fQOpened.front;
    Queue_Bufferfile *qc = &bfQClosed, *qw = &bfQWriten;
    int nFile = 0;

    // for each file in Closed file Queue
    fp = qc->front; pre = NULL;
    while(fp != NULL) {
       next = fp->pNextFile;
       if (fp->pid == pid) {
...
#ifdef L_DEBUG
        BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[%s:%d] nFiles: %d\n", __FUNCTION__, __LINE__, nFile);
#endif
}

void _bf_getUserFileHeaderPageInfo(void ** pRoot, int *pPageCount, pid_t pid,
                                   Queue_Bufferfile *q)
{
    BufferfileFile *fd = q->front; //fQOpened.front;
    BufferfileDataBlock *pD;
...
#ifdef L_DEBUG
        BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[%s:%d] fname: %s, acPages: %d\n", __FUNCTION__, __LINE__,
               fd->name, nPage);
#endif
        fd = fd->pNextFile;
    }
...
UFileHeader * _bf_getNthUserFileHeader(void *page, const int n)
{
    const int MAX_USER_HEADER =
              (PAGE_SIZE - sizeof(int) - sizeof(pid_t)) / sizeof(UFileHeader);
    if (n > MAX_USER_HEADER) {
        BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s:%d] bufferfile: nFile(%d)>MAX_USERHEADER(%d)",
               __FUNCTION__, __LINE__, n, MAX_USER_HEADER);
        return NULL;
    }
         ...
         const char *name, const int pOff, const int nPages)
{
    ...
    BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[%s:%d] fname:%s offset:%d flen: %d\n",
       __FUNCTION__, __LINE__, fhead->name, fhead->pageOffset, fhead->fileLen);
#endif
    return ;
}
...
int bf_mmapBufferfile(struct file *filp, struct vm_area_struct *vma, pid_t pid)
{
...
#ifdef L_DEBUG
    BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[%s:%d] bf_mmap = 0x%lx 0x%lx 0x%lx 0x%lx, len: %lu (%lx)\n",
            __FUNCTION__, __LINE__,
             vma->vm_start,
             vma->vm_end,
             vma->vm_pgoff,
...
#endif
    {
        ...
        if (!header) {
            BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s:%d] mmap fail: user file header page alloc fail.\n",
                     __FUNCTION__, __LINE__);
            return -1;
        }
    // map header page.
        if (remapPage2(vma, pageCount, virt_to_page(header))) {
            BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s:%d] mmap fail. header page map fail.\n",
                   __FUNCTION__, __LINE__);
            return -1;
        }
        ++pageCount;
        ...
        BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[%s:%d] bf_mmap: root: 0x%lX, nPage_pre: %d\n",
               __FUNCTION__, __LINE__,  (unsigned long int)root, nPage_pre);
...
// TODO: Need Search Lock
                for (pD = fp->front; pD != NULL;
                        pD = pD->pNextDataBlock) {
                    flen += (PAGE_SIZE - pD->unusedSize);
                    if (remapPage2(vma, pageCount,
                                   virt_to_page(pD->data))) {
                        BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s:%d] mmap fail. pageCount:%d\n",
...
// TODO: Need Search unlock
                ...
                BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[%s:%d] fname:%s pid: %d offset:%d flen: %d\n",
                       __FUNCTION__, __LINE__, fp->name, pid, offset, flen);
            }
        } // for
...
//#ifdef L_DEBUG
        ...
        {
            int i;
            BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"========== List Mapped Files =========\n");
            for( i=1; i<=nFile; ++i)
            {
                ...
                BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,
                    ...
                    __FUNCTION__, __LINE__, pD2->name, pid,
                    pD2->pageOffset, pD2->fileLen);
            }
            BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"======================================\n");
            BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[%s:%d] mmap OK! pageCount:%d\n", __FUNCTION__, __LINE__, pageCount);
        }
//#endif
    }
...
//    _bf_sem_lock(USER_DUMP);
    ...
    *pCount = PAGE_SIZE * nPage;
    ...
    BF_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_BUFFERFILE,"[%s:%d] pCount: %d (%x), npage: %d(%x)\n",
           __FUNCTION__, __LINE__, *pCount, *pCount,
           nPage, nPage);
}
