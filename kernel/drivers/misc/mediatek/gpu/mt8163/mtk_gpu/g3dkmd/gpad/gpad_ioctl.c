...
static long gpad_ioc_pm_read_counters(struct gpad_device *dev, void __user *user_arg)
{
    ...
    for (read_cnt = 0; read_cnt < alloc_cnt; read_cnt++) {
        ...
        if (sample_list) {
            if (unlikely(copy_to_user(&user_out->timestamp_ns, 
                                      &sample_list->timestamp_ns, 
                                      sizeof(sample_list->timestamp_ns)))) {
            ...
            if (unlikely(copy_to_user(&user_out->sample, 
                                      &sample_list->sample, 
                                      sizeof(sample_list->sample)))) {
            ...
            if (unlikely(copy_to_user(&user_out->freq, 
                                      &sample_list->freq, 
                                      sizeof(sample_list->freq)))) {
...
static long gpad_ioc_kmd_query(struct gpad_device *dev, void __user *user_arg)
{
    ...
    if (0 == strcmp("mtune_test", (char *)&(data[0])) && q.buf_len > 4) {
        /* for ut */
        q.buf_len -= 2;
