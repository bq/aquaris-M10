...
void g3dKmdKmifAllocMemoryLocked(unsigned int size, unsigned int align)
{
    ...
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PM_DRAM_SIZE, gExecInfo.pm.size, MSK_AREG_PM_DRAM_SIZE, SFT_AREG_PM_DRAM_SIZE);
}
...
unsigned int g3dKmdKmifMvaToPa(unsigned int mva)
{
    ...
    if (!hMmu)
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_PM, "g3dKmdKmifMvaToPa: can't get mmu handle\n");
        return 0;
    }
...
{
    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_PM, "TODO: g3dKmdKmifChangeDFDDebugMode(): enable: %d\n", enable);
	lock_g3d();    
...
void g3dKmdKmifDFDAllocMemoryLocked(unsigned int size, unsigned int align)
{
    ...
    g3dKmdRegWrite(G3DKMD_REG_MX, REG_MX_DBG_INTERNAL_DUMP, gExecInfo.dfd.hwAddr, MSK_MX_DBG_DBG_DUMP_LDRAM_FLUSH_BASE, 0);
}
