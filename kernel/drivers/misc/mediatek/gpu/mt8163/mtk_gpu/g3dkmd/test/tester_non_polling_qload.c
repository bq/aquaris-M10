...
DECLARE_TEST_CASE(tcase_qload_timeout, "qload_test", "QLOAD_TIMEOUT",
                  ...
                  tester_restore_qload_wait_on_queue,
                  NULL, tester_custom_qload, report, -1);
...
struct myData qSignal = {0}, qTimeout = {0};
...
//printk("[%s] pid %d\n", __FUNCTION__, current->pid);
    ...
    g3dKmdRegWrite(REG_AREG_IRQ_STATUS_MASKED, VAL_IRQ_G3D_MMCE_FRAME_FLUSH,
...
{
    tester_case_t *ptcase = container_of(pdata, struct _tester_case, private_data);
//  printk("test_name: %s\n", ptcase->name);



  // save reg value.
    regFlag = g3dKmdRegRead(REG_AREG_IRQ_STATUS_RAW, VAL_IRQ_G3D_MMCE_FRAME_FLUSH, 0);

  // set reg let the process waitig.
    g3dKmdRegWrite(REG_AREG_IRQ_STATUS_RAW, 0, VAL_IRQ_G3D_MMCE_FRAME_FLUSH, 0);
...
{
    g3dKmdRegWrite(REG_AREG_IRQ_STATUS_RAW, regFlag, VAL_IRQ_G3D_MMCE_FRAME_FLUSH, 0);
}
...
static void report(void *pdata)
{
    tester_case_t *ptcase = container_of(pdata, struct _tester_case, private_data);
    struct myData *mypdata = (struct myData *)ptcase->private_data;
    printk("Testcase Name: %-20s  (%ld/%ld), unexpect: intrpt: %ld, timout: %ld, nrml: %ld unknwn: %ld\n", 
           ptcase->name, ptcase->passCnt, ptcase->testCnt, mypdata->interruptCnt, mypdata->timeoutCnt, 
           mypdata->normalCnt, mypdata->unknownCnt);
}
