...
static Mapper s_mapper = NULL;
static int s_mapper_ref = 0;
#endif
...
Mapper mmuInit( g3d_va_t g3d_va_base )
{
    ...
    if (s_mapper_ref == 0)
    {
...
#endif
        s_mapper = (Mapper)yl_mmu_alloc( Mapper_get_required_size_mapper() );
...
#ifdef G3DKMD_SUPPORT_IOMMU
        if (!allocatePhysicalMemory(&page_mem, Mapper_get_required_size_page_table() + page_ofst, 0x4000, flags))
        {
...
#else
        if (!allocatePhysicalMemory(&page_mem, Mapper_get_required_size_page_table(), 0x4000, flags))
        {
            yl_mmu_free(s_mapper);
            return NULL;
        }
        ...
        if (!allocatePhysicalMemory(&mirror_mem, Mapper_get_required_size_page_table(), 0x4000, 0))
        {
...
#if MMU_MIRROR
        Mapper_init_mirror( s_mapper, g3d_va_base, page_mem, meta_table_space, mirror_mem, 0);
#else
    ...
    return s_mapper;
#else
    Mapper m = (Mapper)yl_mmu_alloc( Mapper_get_required_size_mapper() );
    ...
    page_table_space  = page_mem.data; //(void *)(((unsigned int)page_table_space0 + 0x3FFFUL ) & ( ~ 0x3FFFUL ));
    meta_table_space  = yl_mmu_alloc( Mapper_get_required_size_meta_table() );
...
#endif
    *size   = FIRST_PAGE_OCCUPIED_ALIGNED + ((g3dKmdCfgGet(CFG_MMU_VABASE) / MMU_SECTION_SIZE) * 4);
}
...
void mmuGetVirtualTable(Mapper m, void** swAddr, unsigned int* hwAddr, unsigned int* size)
{
...
unsigned int mmuG3dVaToVa(Mapper m, g3d_va_t g3d_va)
{
