...
{
#if MMU_MIRROR_ROUTE
        return mmuG3dVaToG3dVa((Mapper)m, g3d_va);
#else
        return mmuG3dVaToPa((Mapper)m, g3d_va);
#endif
}
#endif // PATTERN_DUMP
...
static unsigned int _G3dVaToPa(void* m, unsigned int g3d_va)
{
        ...
        return Mapper_g3d_va_to_va((Mapper)m, g3d_va);
#else
        return mmuG3dVaToPa((Mapper)m, g3d_va);
#endif
}
...
void g3dKmdMmuGetTable_ex(void* m, void** swAddr, void** hwAddr, unsigned int* size)
{
    ...
    {
    #if MMU_MIRROR_ROUTE
        mmuGetMirrorTable((Mapper)m, swAddr, hwAddr, size);
    #else
        mmuGetTable((Mapper)m, swAddr, hwAddr, size);
    #endif
    }
    else
#endif
...
unsigned int g3dKmdMmuG3dVaToVa_ex(void* m, unsigned int g3d_va)
{
    ...
    pa = Mapper_g3d_va_to_va((Mapper)m, g3d_va);
...
#endif
{
        ...
        )
    {
        ...
        g3d_va = Mapper_map_given_pa( (Mapper)m, (va_t)va, (pa_t)pa, (size_t)size, could_be_section, flag );
    }
    else
#endif
    {
        g3d_va = Mapper_map((Mapper)m, (va_t)va, (size_t)size, could_be_section, flag );
    }
...
void g3dKmdMmuUnmap_ex( void *m, unsigned int g3d_va, unsigned int size)
{
    ...
    if (g3dKmdCfgGet(CFG_DUMP_ENABLE))
    {
        ...
        if(g3dKmdCfgGet(CFG_PHY_HEAP_ENABLE))
        {
            ...
            if (mmuMapperValid(m))
            {
                Mapper_unmap( (Mapper)m, g3d_va_cast, size );
            }
        }
        else
        {
...
unsigned int g3dKmdAllocHw_ex(uint64_t phyAddr, unsigned int size)
{
    ...
    if ((uint64_t)hwAddr != phyAddr) return 0;
#endif
...
#ifdef PATTERN_DUMP
G3DKMD_APICALL void G3DAPIENTRY g3dKmdMmuGetTable(void* m, void** swAddr, void** hwAddr, unsigned int* size)
{
    __SEM_LOCK(mmuLock);
#if (USE_MMU == MMU_USING_GPU)
...
/*  // method1, parse the PageTable Manually
    ...
    void* pageTbl;
    pageTbl = ((Mapper) m)->Page_mem.data;
...
G3DKMD_APICALL void G3DAPIENTRY g3dKmdMmuPaPairBufInfo(void* m, unsigned int g3d_va, unsigned int size, unsigned int *buffer_size, unsigned int *unit_size, unsigned int *offset_mask)
{
#if ((USE_MMU == MMU_USING_GPU) || (USE_MMU == MMU_USING_M4U))
    {
        unsigned int        index0  = PageTable_index0_from_g3d_va( g3d_va - ((Mapper)m)->G3d_va_base );
        FirstPageTable      page0 = (FirstPageTable) ((Mapper) m)->Page_mem.data;
...
G3DKMD_APICALL void G3DAPIENTRY g3dKmdMmuG3dVaToPaChunk(void* m, unsigned int data, unsigned int size, unsigned int *chunk_pa, unsigned int *chunk_size)
{
...
#elif ((USE_MMU == MMU_USING_M4U) || (USE_MMU == MMU_USING_WCP))
    {
...
#if (USE_MMU == MMU_USING_M4U)
#ifdef YL_SECURE
        // [brady to do] to pass flag infomation to g3dKmdMmuMap_ex
        ...
        if ((flag & G3DKMD_MMU_DMA) != 0)
        {
            ...
            if (gExecInfo.pDMAMemory != NULL && table != NULL)
            {
                ...
                sg_alloc_table(table, (gExecInfo.pDMAMemory->size + PAGE_SIZE-1)/PAGE_SIZE, GFP_KERNEL);
                sg = table->sgl;
                ...
                sg_set_page(sg, page, (gExecInfo.pDMAMemory->size + PAGE_SIZE-1) & ~(PAGE_SIZE-1), 0);

                if (iommu_alloc_mva((m4u_client_t*)m, M4U_PORT_GPU, 0, table, size, prot, 0, &g3d_va))
                {
                    KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_MMU, "kalloc fail\n");
                    g3dkmd_backtrace();
                    YL_MMU_ASSERT(0);
                ...
                KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_MMU, "dma m4u_alloc_mva: 0x%x 0x%x 0x%x 0x%x\n", gExecInfo.pDMAMemory->data, gExecInfo.pDMAMemory->phyAddr, g3d_va, size);
        ...
        {
#ifdef YL_SECURE
            // [brady to do] to pass flag infomation to g3dKmdMmuMap_ex //TODO
#endif // YL_SECURE

            if (iommu_alloc_mva((m4u_client_t*)m, M4U_PORT_GPU, (unsigned long)va, NULL, size, prot, 0, &g3d_va))
            {
                KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_MMU, "kalloc fail\n");
                g3dkmd_backtrace();
                YL_MMU_ASSERT(0);
...
G3DKMD_APICALL unsigned int G3DAPIENTRY g3dKmdMmuRangeSync( void *m, unsigned long va, unsigned int size, unsigned int g3d_va, unsigned int cache_action)
{
    unsigned int ret = 1;
    ...
    if (ret)
    {
        // in K2, shall be if (m4u_cache_sync((m4u_client_t*)m, M4U_PORT_GPU, va, size, g3d_va, sync_type) < 0)
...
// finally, I think the code protected by spinlock can't be scheduled, but some or linux APIs will do that like get_user_page(..)
...
G3DKMD_APICALL int G3DAPIENTRY g3dKmdUserPageGet(unsigned long uaddr, unsigned long size, uintptr_t *ret_pages, unsigned int *ret_nr_pages)
{
    ...
    if ((pages = G3DKMDMALLOC(nr_pages * sizeof(*pages))) == NULL)
    {
        __SEM_UNLOCK(memLock);
        return -ENOMEM;
    }
    ...
    ret = get_user_pages(current, current->mm, uaddr, nr_pages,
      ...
      0, /* don't force */
      pages,  NULL); // when get_user_pages, does we already get_page with gotten pages ?
    up_read(&current->mm->mmap_sem);

    /* Errors and no page mapped should return here */
    if (ret < nr_pages) goto out_unmap;
    ...
    {
        extern pa_t va_to_pa_after_mmap( va_t address );

        pa_t pa = va_to_pa_after_mmap(addr);
        if ((pa >> PAGE_SHIFT) != page_to_pfn(pages[j])) goto out_unmap;
        j++; addr += PAGE_SIZE;
    }
...
#endif
/// 0 if oom
G3DKMD_APICALL uint64_t G3DAPIENTRY
g3dKmdAllocPhysical(unsigned int size )
{
...
G3DKMD_APICALL void G3DAPIENTRY
g3dKmdFreePhysical( uint64_t addr, unsigned int size )
{
#if defined(CMODEL) && (defined(_WIN32)|| (defined(linux) && defined(linux_user_mode))) // linux user space
#ifdef G3DKMD_PHY_HEAP
    if(g3dKmdCfgGet(CFG_PHY_HEAP_ENABLE))
    {
...
void g3dKmdAllocSwRdback(pG3dExecuteInfo execute, unsigned int size)
{
#if defined(linux) && !defined(linux_user_mode)
    if (!allocatePhysicalMemory(&execute->SWRdbackMem, size, G3D_SWRDBACK_SLOT_SIZE, G3DKMD_MMU_DMA | G3DKMD_MMU_ENABLE))
    {
...
void g3dKmdFreeSwRdback(pG3dExecuteInfo execute)
{
#if defined(linux) && !defined(linux_user_mode)
...
#else
G3DKMD_APICALL unsigned int G3DAPIENTRY
g3dKmdRemapPhysical( void* va, unsigned int size )
#endif
{
    /// For pattern & C-Model
#ifdef YL_SECURE
...
G3DKMD_APICALL void G3DAPIENTRY
g3dKmdSetPhysicalStartAddr( unsigned int pa_base )
{
    /// For pattern & C-Model
...
G3DKMD_APICALL unsigned int G3DAPIENTRY g3dKmdMmuG3dVaToVa(void* m, unsigned int g3d_va) // add by Frank  Fu, 2012/12/21 - B
{
    unsigned int pa = 0;
