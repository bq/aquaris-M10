...
void gpad_sdbg_dump_areg(struct seq_file *m, struct gpad_sdbg_info *sdbg_info)
{
    ...
    if (0xFFFFFFFF != value) { // 0xFFFFFFFF is default value
...
void _dump_irq_raw_data(struct seq_file *m, struct gpad_sdbg_info *sdbg_info)
{
    ...
    const char *irq_raw_str[] = {
        "cq preempt finish", "cq resume finish", "pa frame base data queue not empty",
        "pa frame swap", "pa frame end", "pa sync sw", 
        "g3d hang", "g3d mmce frame flush", "g3d mmce frame end",
        "cq error code check", "mx debug", "pwc snap", "DFD internal dump triggered",
        "pm debug mode", "vl negative index happen", "areg apb timeout happen",
        "ux_cmn response", "ux_dwp0 response", "ux_dwp1 response", NULL
    };
    /* dump irq raw data */
...
void gpad_sdbg_dump_common_reg(struct seq_file *m, struct gpad_sdbg_info *sdbg_info)
{
    ...
    const char *input_stage_io_arr[] = {
        "vs0i_rdy", "vs0i_ack", "vs0i2tks_rdy", "tks2vs0i_ack", "vs0i2smm_rdy", "smm2vs0i_ack",
        "vs1i_rdy", "vs1i_ack", "vs1i2tks_rdy", "tks2vs1i_ack", "vs1i2smm_rdy", "smm2vs1i_ack",
        "fsi_rdy", "fsi_ack", "fsi2tks_rdy", "tks2fsi_ack","fsi2smm_rdy", "smm2fsi_ack", NULL
    };

    const char *output_stage_io_arr[] = {
        "ux2vl_vs0_rdy", "vl2ux_vs0_ack", "ux2vl_vs1_rdy", "vl2ux_vs1_ack",
        "ux2vx_rdy", "vx2ux_ack", "ux2cp_rdy", "cp2ux_ack", NULL
    };

    const char *tx_io_arr[] = {
        "ux2tx_rdy", "tx2ux_ack", "ux2tx_idx_rdy", "tx2ux_idx_ack", "ux2tx_pix_rdy", "tx2ux_pix_ack",
        "ux2tx_glbini_rdy", "tx2ux_glbini_ack", "ux2tx_glbclr_rdy", "tx2ux_glbclr_ack", NULL
    };

    const char *ltc_io_arr[] = {
        "ux2mx_cp_rd_req", "mx2ux_cp_rd_ack", "ux2mx_ic_rd_req", "mx2ux_ic_rd_ack",
        "ux2mx_vp_rd_req", "mx2ux_vp_rd_ack", "ux2mx_vp_wr_req", "mx2ux_vp_wr_ack", NULL
    };
