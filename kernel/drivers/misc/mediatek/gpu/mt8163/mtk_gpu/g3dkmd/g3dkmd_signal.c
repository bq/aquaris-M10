...
static void do_work(pid_t pid)
{
    ...
    //printk(KERN_INFO "workqueue pid = %d, task = %p\n", pid, pTask);
   
    if (pTask == NULL || pid == 0) return;
...
int init_sim_keyboard_int(pid_t singal_pid) 
{
    ...
    return request_irq(1,   /* The number of the keyboard IRQ on PCs */
                ...
                IRQF_SHARED, "g3dkmd_keyboard_irq_handler", (void*)singal_pid);
}
...
static unsigned int _ui4Counter = 0;   // Counter. 
...
{    
    ...
    {
        printk(KERN_NOTICE "timer_callback finish\n");
    }
...
{
    printk(KERN_INFO "trigger pid = %d\n", signal_pid);
    do_work(signal_pid);  
}
