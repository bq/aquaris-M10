...
{
    seq_printf(m, "[%s]: trigger [%s]\n", __FUNCTION__, "_g3dkmd_create_exeInst_workqueue");
...
static int exeInsts_debugfs_seq_show(struct seq_file *m, void* v)
{
    ...
    seq_printf(m, "- ExeInsts:\n");
    seq_printf(m, " *: working instance, -: default instance\n");
    seq_printf(m, " ------------------------------------------------------------------\n");
    ...
    for (i = 0, cnt = 0; i < gExecInfo.exeInfoCount; i++)
    {
        ...
        seq_printf(m, "  %c%c %d) %p %s, TaskRefCnt: %d, Type: %s, "
                "Ht(Hw%cSw):(%d, %d) isMigrating:%c(%d, %d) hawChnl: %d "
                "HtSlot(h/s/t): %d/%d hwempty(%c) htInst(hw,sw):va(%p, %p)\n"
                ...
                "\tDfr(va,bk,tb,hd): (%d %d %d %d)\n",
                   ...
                   -1, //g3dKmdGetQueuedHtSlots(G3dKMD_HWCHANNEL_USE_LOCAL_RDPTR, pInst->pHtDataForHwRead->htInfo),
                   -1, //g3dKmdGetQueuedHtSlots(G3dKMD_HWCHANNEL_USE_LOCAL_RDPTR, pInst->pHtDataForSwWrite->htInfo),
...
static int hwStatus_debugfs_seq_show(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "- HW Status:\n");
    seq_printf(m, " ------------------------------------------------------------------\n");
    seq_printf(m, " channel | state   | InstIdx\n");
...
static int tasks_debugfs_seq_show(struct seq_file *m, void *v)
{
    ...
    for (i = 0, cnt = 0; i < gExecInfo.taskInfoCount; i++)
    {
        ...
        seq_printf(m, " id:%u, inst: %u, pid: %u tid: %u, comm: %s, isUrgent: %c, lastFlushStamp: %u, HWStamp: %u, Carry: %u\n", 
