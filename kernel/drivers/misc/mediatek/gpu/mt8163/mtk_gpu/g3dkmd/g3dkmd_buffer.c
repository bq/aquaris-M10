...
int g3dKmdCreateBuffers(g3dExeInst* pInst, G3DKMD_DEV_REG_PARAM* param)
{
    ...
    if (param && (param->usrDfrEnable == G3DKMD_TRUE))
    {
        KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_BUFFERS,
            "Create DFR from user, vary 0x%x(0x%x), binbk 0x%x(0x%x), "
            "bintb 0x%x(0x%x), binhd 0x%x(0x%x)\n",
    ...
    // create from kernel
    {
        ...
        if (!allocatePhysicalMemory(&pInst->defer_vary,  va_size, G3DKMD_DEFER_BASE_ALIGN, flag))
        {
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERS,"allocate vary fail (0x%x)\n", va_size);
            goto fail_handler;
        }

        //twice for buffer0 base1
        if (!allocatePhysicalMemory(&pInst->defer_binBk, bk_size * 2, G3DKMD_DEFER_BASE_ALIGN, flag))
        {
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERS,"allocate binbk fail (0x%x)\n", bk_size);
            goto fail_handler;
        }

        if (!allocatePhysicalMemory(&pInst->defer_binTb, tb_size * 2, G3DKMD_DEFER_BASE_ALIGN, flag))
        {
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERS,"allocate bintb fail (0x%x)\n", tb_size);
            goto fail_handler;
        }

        if (!allocatePhysicalMemory(&pInst->defer_binHd, hd_size * 2, G3DKMD_DEFER_BASE_ALIGN, flag))
        {
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERS,"allocate binhd fail (0x%x)\n", hd_size);
            goto fail_handler;
        }

        KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_BUFFERS,
            "Create DFR from kernel, vary 0x%x(0x%x), binbk 0x%x(0x%x), "
            "bintb 0x%x(0x%x), binhd 0x%x(0x%x)\n",
...
void g3dKmdDestroyBuffers(g3dExeInst* pInst)
{
    ...
    {
        KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_BUFFERS,
            "Destroy DFR from kernel, vary 0x%x(0x%x), binbk 0x%x(0x%x), "
            "bintb 0x%x(0x%x), binhd 0x%x(0x%x)\n",
            pInst->defer_vary.hwAddr,  pInst->defer_vary.size,
            pInst->defer_binBk.hwAddr, pInst->defer_binBk.size,
            pInst->defer_binTb.hwAddr, pInst->defer_binTb.size,
