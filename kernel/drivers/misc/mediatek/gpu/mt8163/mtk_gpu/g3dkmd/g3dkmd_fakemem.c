...
static char * pVideoBaseAddress = NULL;
#endif
...
#ifndef NO_SYSTEM_MEMORY_RESERVED
static HANDLE hBufferMapFile = 0;
#endif
...
#ifdef G3DKMD_PHY_HEAP 
        g_max_pa_addr = _MAX(g_max_pa_addr, g_pa_base+index*VIDEO_PAGE_SIZE + va_offset);
#endif
        return (g_pa_base+index*VIDEO_PAGE_SIZE + va_offset);
    }
    else
    {
        YL_KMD_ASSERT(0);
...
#ifndef NO_SYSTEM_MEMORY_RESERVED
mem_chunk_t* mmAlloc(unsigned int uSize)
#else
...
#endif
{
	...
	uAlignedSize = _aligment(uSize,uAlignment);
#ifndef NO_SYSTEM_MEMORY_RESERVED
	mem_chunk_t* pChunk = (mem_chunk_t*)G3DKMDMALLOC(sizeof(mem_chunk_t));
	if( !pChunk ){
		return NULL;
	}
	memset((void*)pChunk,0,sizeof(mem_chunk_t));
#endif
...
#ifndef NO_SYSTEM_MEMORY_RESERVED
void mmFree(mem_chunk_t* pChunk)
{
	if( pChunk!=NULL )
	{
...
void mmFree(unsigned int uAddr,unsigned int uSize)
{
	...
	if(uAddr < g_pa_base || 
	   uAddr + uSize>g_pa_base+VIDEO_MEMORY_SIZE )
	{
		KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_FAKEMEM,"mmFree Addr out of Range (0x%08x)\n",uAddr);
		return;
	}
...
void mmOpen()
{
    ...
    { 
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_FAKEMEM,"Could not create file mapping object (%d).\n", GetLastError());
        return;
    }
    pVideoBaseAddress = (char *) MapViewOfFile(hBufferMapFile,  
