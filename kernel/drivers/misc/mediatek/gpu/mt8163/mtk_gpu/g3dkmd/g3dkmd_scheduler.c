...
#define CTX_DPF(type, ...) KMDDPF(type, __VA_ARGS__)
    ...
    #define LDPF(fmt,...) printk(MIGRATION_TAG "[%s]" fmt, __FUNCTION__,  __VA_ARGS__)
...
static unsigned int _g3dKmdGetTimeDiff(void)
{
    ...
    time_diff = (cur_tv.tv_sec - s_last_tv.tv_sec) * 1000 + (cur_tv.tv_usec - s_last_tv.tv_usec) / 1000;
...
static void _g3dKmdCheckAndActiveMigratingInsts(void) // must withint g3dlock
{
    ...
    for (idx = 0; idx < gExecInfo.exeInfoCount; idx++)
    {
            ...
            g3dKmdIsHtInfoEmpty(pInst->hwChannel, pInst->pHtDataForHwRead->htInfo))
        {
            ...
            if (pInst->childInstIdx > gExecInfo.exeInfoCount ||
                pInst->childInstIdx < 0)
            {
               KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_DDFR,
                    "pInst(%p)->childInstIdx(%u) > "
                    "(%u)gExecInfo.exeInfoCount\n",
            ...
            G3DKMD_INST_MIGRATION_DISASSOCIATE_AND_WAIT_CLEAN_UP(pInst,
                    pInstChild);
        }
...
cleanup_stage:
        ...
        //
        // ToDo: Bug, when using single_queue setting, the runing queue may
            ...
            pInst->state != G3DEXESTATE_RUNNING)
        {
            ...
            {    // stil some task(s) use it
                pG3dHtInst oldHtInst    = pInst->pHtDataForHwRead;
...
//      Return :
//------------------------------------------------------------------------
static pG3dExeInst _g3dKmdFindSwitchToInstance(unsigned debug_info_enable)
{
    pG3dExeInst pInstTo = NULL, pInst;
    ...
    // find swith-to instance (idle or high-priority or waitqueue, but idle & high-priority has higher priority than waitqueue)
    start_inst  = cur_inst;
    ...
    for (loop = 0; loop < 2; loop++)
    {
        for (i = start_inst; i < end_inst; i++)
        {
            ...
            {
                unsigned char nonEmpty  = !g3dKmdIsHtInfoEmpty(pInst->hwChannel,
...
//------------------------------------------------------------------------
static pG3dExeInst _g3dKmdFindSwitchFromInstance(unsigned char forceSwitch, unsigned debug_info_enable)
{
    int i;
    unsigned char isEmpty, isTimeout;
    pG3dExeInst pInstFrom = NULL, pInst;
    pG3dExeInst pNormal = NULL, pNormalTimeout = NULL, pNormalEmpty = NULL;
    pG3dExeInst pUrgent = NULL, pUrgentTimeout = NULL, pUrgentEmpty = NULL;
    ...
    for (i = 0; i < gExecInfo.exeInfoCount; i++)
    {
        ...
        if (pInst && (pInst->state == G3DEXESTATE_RUNNING))
        {
            ...
            isEmpty     = g3dKmdIsHtInfoEmpty(pInst->hwChannel, pInst->pHtDataForHwRead->htInfo);

            CTX_DPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_SCHDLR,"[%s][%d] forceSwitch %d, isTimeout %d, isEmpty %d, isUrgentPrio %d\n", __FUNCTION__, i, forceSwitch, isTimeout, isEmpty, pInst->isUrgentPrio);
...
#if 0
static INLINE void _g3dKmdDumpRsm(pG3dExeInst pInstTo)
{
    g3dKmdCacheFlushInvalidate();
    printk("resume buffer data: 0x%x\n",((unsigned char*)pInstTo->rsmBuffer.data)[2]);
...
//      Return : 
//------------------------------------------------------------------------
static void _g3dKmdDoContextSwitch(pG3dExeInst pInstFrom, pG3dExeInst pInstTo, int freeHwCh, unsigned debug_info_enable)
{
#ifndef SUPPORT_MPD
    _DEFINE_BYPASS_
#endif
...
{
    pG3dExeInst pInstFrom = NULL, pInstTo = NULL;
    int i, freeHwCh = -1;
    ...
    if ((freeHwCh == G3DKMD_HWCHANNEL_NONE) || (gExecInfo.hwStatus[freeHwCh].state == G3DKMD_HWSTATE_HANG))// no inst from
    {
        //if (debug_info_enable != 0)
        //printk("[ctx switch caller err] cant find hw channel\n");
        CTX_DPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_SCHDLR, "[%s] no Free hwChannel\n", __FUNCTION__);
        return;
    }

    CTX_DPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_SCHDLR,"[%s] instFrom (%d, 0x%x), instTo (%d, 0x%x), freeHwCh %d\n", __FUNCTION__, g3dKmdExeInstGetIdx(pInstFrom), pInstFrom, g3dKmdExeInstGetIdx(pInstTo), pInstTo, freeHwCh);
...
//------------------------------------------------------------------------
// Description : Scheduler function, when G3DKMD_SCHEDULER_FREQ timeout, or manually trigger by g3dKmdTriggerScheduler, it will wake-up and do context switch jobs
//   Parameter : 
//      Return : 
//------------------------------------------------------------------------
#ifdef G3DKMD_SCHEDULER_OS_CTX_SWTICH
    ...
    #endif
{
	...
	if (_work_done == false) {
        ...
        wait_event_interruptible_timeout(gExecInfo.s_SchedWq, gExecInfo.activateSch, msecs_to_jiffies(G3DKMD_SCHEDULER_FREQ));
