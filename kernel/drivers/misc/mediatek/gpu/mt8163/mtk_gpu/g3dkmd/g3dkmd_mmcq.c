...
#define IO_EVENT_ARG_A(io_subsys, val, is_suspendable) (unsigned int)(((io_subsys << SFT_EXT_SUBSYS) & MSK_EXT_SUBSYS) | (val & 0x1FFFF) | ((is_suspendable << SFT_EXT_NON_SUSPENDABLE) & MSK_EXT_NON_SUSPENDABLE))
...
#define MARKER_EVENT_ARG_A(non_suspendable, no_inc_exec)    (((non_suspendable << SFT_EXT_NON_SUSPENDABLE) & MSK_EXT_NON_SUSPENDABLE) | ((no_inc_exec << SFT_MARK_NO_INC_EXEC_CMDS_CNT) & MSK_MARK_NO_INC_EXEC_CMDS_CNT))
...
#define SYNC_ARG_A(val, is_suspendable) (unsigned int)((val & 0x1FFFF) | ((is_suspendable << SFT_EXT_NON_SUSPENDABLE) & MSK_EXT_NON_SUSPENDABLE))
...
MMCQ_INST* _g3dKmdMmcqCreateQueue(void);
void _g3dKmdMmcqFreeQueue(MMCQ_INST* mmcq);
unsigned int _g3dKmdMmcqGetEmptySize(MMCQ_INST* mmcq);
void _g3dKmdMmcqAddCommand(MMCQ_BUF* buf, MMCQ_CMD_OP op, unsigned int argA, unsigned int argB);
void _g3dKmdMmcqWait(MMCQ_HANDLE handle, unsigned int token_id, 
                            MMCQ_TO_UPDATE toUpdate, MMCQ_UPDATE_OP updateOp, unsigned int update_value,
                            MMCQ_TO_WAIT toWait, MMCQ_WAIT_OP waitOp, unsigned int wait_value);
...
MMCQ_INST* _g3dKmdMmcqCreateQueue(void)
{
    ...
    // allocate MMCQ_INST
    mmcq = (MMCQ_INST*) G3DKMDMALLOC(sizeof(MMCQ_INST));
    ...
    memset(mmcq, 0, sizeof(MMCQ_INST));
...
{   // if read_ptr == write_ptr, it means empty
    ...
    {
        buf->read_ptr = (unsigned int*)MMCE_TO_VIR(&(mmcq->inst_buf), g3dKmdMmceGetReadPtr(mmcq->thd_handle));
    }

    buf_size = (buf->base1_vir) ? ((uintptr_t)buf->end1_vir - (uintptr_t)buf->base1_vir) : MAX_MMCQ_SIZE;
...
void _g3dKmdMmcqAddCommand(MMCQ_BUF* buf, MMCQ_CMD_OP op, unsigned int argA, unsigned int argB)
{
    ...
    if ((uintptr_t)buf->write_ptr + MMCQ_INSTRUCTION_SIZE >= (uintptr_t)buf->end1_vir)
    {
        ...
        *buf->write_ptr++   = (MMCQ_CMD_OP_JUMP << SFT_OP_CODE) | ((unsigned int)MMCQ_JUMP_MODE_ABS & MSK_ARG_A);
...
void _g3dKmdMmcqWait(MMCQ_HANDLE handle, unsigned int token_id, 
                            ...
                            MMCQ_TO_WAIT toWait, MMCQ_WAIT_OP waitOp, unsigned int wait_value)
{
    MMCQ_INST* mmcq = (MMCQ_INST*) handle;

    _g3dKmdMmcqAddCommand(&mmcq->inst_buf, MMCQ_CMD_OP_SYNC, SYNC_ARG_A(token_id, mmcq->is_suspendable), 
        SYNC_ARG_B(toUpdate, updateOp, update_value, toWait, waitOp, wait_value));
}
...
{
    MMCQ_INST* mmcq = (MMCQ_INST*) mmcq_handle;
...
// #endif
    ...
    {
        _g3dKmdMmcqAddCommand(buf, MMCQ_CMD_OP_MOVE, SYNC_ARG_A(0, mmcq->is_suspendable), ~mask);
        reg_addr = reg_addr | 0x01;
    }

    _g3dKmdMmcqAddCommand(buf, MMCQ_CMD_OP_WRITE, IO_EVENT_ARG_A(subsys, reg_addr, mmcq->is_suspendable), value);
}
...
void g3dKmdMmcqRead(MMCQ_HANDLE handle, MMCQ_SUBSYS subsys, unsigned int reg_addr)
{
    ...
    _g3dKmdMmcqAddCommand(&mmcq->inst_buf, MMCQ_CMD_OP_READ, IO_EVENT_ARG_A(subsys, reg_addr, mmcq->is_suspendable), 0);
}
...
void g3dKmdMmcqPoll(MMCQ_HANDLE handle, MMCQ_SUBSYS subsys, unsigned int reg_addr, unsigned int value, unsigned int mask)
{
    ...
    _g3dKmdMmcqAddCommand(buf, MMCQ_CMD_OP_POLL, IO_EVENT_ARG_A(subsys, reg_addr, mmcq->is_suspendable), value);
}
...
void g3dKmdMmcqWaitAndClear(MMCQ_HANDLE handle, unsigned int token_id, unsigned int wait_value, unsigned int clear_value)
{
...
void g3dKmdMmcqJumpToNextInst(MMCQ_HANDLE handle)
{
    ...
    nextInst = (unsigned int)((uintptr_t)mmcq->inst_buf.write_ptr - (uintptr_t)mmcq->inst_buf.base0_vir) + mmcq->inst_buf.base0_phy + MMCQ_INSTRUCTION_SIZE;
...
void g3dKmdMmcqFlush(MMCQ_HANDLE handle)
{
    ...
    g3dKmdMmcqWrite(handle, MMCQ_SUBSYS_MMCE, CMDQ_THRx_EXEC_CMDS_CNT(thd_id), MMCE_TO_PHY(buf, buf->write_ptr), MMCQ_DEFAULT_MASK);
...
void g3dKmdMmcqAddMarker(MMCQ_HANDLE handle, MMCQ_MARKER marker)
{
    ...
    switch (marker)
    {
        case MMCQ_MARKER_EOC:
            _g3dKmdMmcqAddCommand(buf, MMCQ_CMD_OP_MARKER, MARKER_EVENT_ARG_A(mmcq->is_suspendable, 0), MARKER_EVENT_ARG_B(0));
            break;

        case MMCQ_MARKER_PREFETCH_EN:
            _g3dKmdMmcqAddCommand(buf, MMCQ_CMD_OP_MARKER, MARKER_EVENT_ARG_A(mmcq->is_suspendable, 1), MARKER_EVENT_ARG_B(1));
            break;
    }
}
...
void g3dKmdMmcqSetJumpTarget(MMCQ_HANDLE handle, unsigned int target)
{
    ...
    for (addr = (unsigned int*)mmcq->inst_buf.base0_vir; addr < (unsigned int*)mmcq->inst_buf.end0_vir; addr += 2)
    {
        if ((addr[1] & MSK_OP_CODE) == (MMCQ_CMD_OP_JUMP << SFT_OP_CODE))
        {
            KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_MMCQ,"jump target changed [0x%x -> 0x%x]\n", addr[0], target);
            addr[0] = target;
            return;
        }
    }
...
void g3dKmdMmcqUpdateBuffAddr(MMCQ_HANDLE handle)
{
    ...
    buf->end0_phy = buf->base1_phy = (unsigned int)((uintptr_t)buf->write_ptr - (uintptr_t)buf->base0_vir) + buf->base0_phy;
}
...
#ifdef YL_MMU_PATTERN_DUMP_V2
    if( gExecInfo.mmu )
        g3dKmdDumpDataToFilePage(gExecInfo.mmu, fp, (unsigned char*)buf->buf_data.data, buf->buf_data.hwAddr ==> invalid?, buf->buf_data.size);
    else
#endif
#ifdef YL_MMU_PATTERN_DUMP_V1
    if( buf->buf_data.pa_pair_info ==> NULL? )
        g3dKmdDumpDataToFileFull(fp, (unsigned char*)buf->buf_data.data, buf->buf_data.pa_pair_info, buf->buf_data.size);
    else
#endif
*/
        g3dKmdDumpDataToFile(fp, (unsigned char*)buf->buf_data.data, buf->buf_data.patPhyAddr, buf->buf_data.size);
}
        ...
        while ( dwData != buf->write_ptr )
        {
            ...
            if ( (op == 0x4) && (type == 0x1) ) //Write & G3D
            {
                if ( useMask )
                {
                    ...
                    orig_value = g3dKmdRegRead( G3DKMD_REG_G3D, (addr & 0xfff), 0xffffffff, 0 );

                    g_pKmdFileIo->fPrintf(fp, "%04x_%08x\n", addr, ((orig_value & mask) | data) );
                }
                else
                {
                    g_pKmdFileIo->fPrintf(fp, "%04x_%08x\n", addr, data);
            ...
            else if ( op == 0x10 ) //Jump
            {
                ...
                //jump
                dwData = (unsigned int*)buf->buf_data.data + ((data - buf->buf_data.patPhyAddr) / 4);
                bFromJump = G3D_TRUE;
                continue;
            }
