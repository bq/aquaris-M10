...
// API for debugfs_fdq
void*        _g3dkmd_getFdqDataPtr(void)  { return gExecInfo.fdq.data;}
unsigned int _g3dkmd_getFdqDataSize(void) { return gExecInfo.fdq_size;}
unsigned int _g3dkmd_getFdqReadIdx(void)  { return gExecInfo.fdq_rptr;}
...
void g3dKmdFdqInit(void)
{
    ...
    if (!allocatePhysicalMemory(&gExecInfo.fdq, size, G3DKMD_FDQ_ALIGN, G3DKMD_MMU_ENABLE | G3DKMD_MMU_SHARE | G3DKMD_MMU_DMA))
    {
        YL_KMD_ASSERT(0);
        return;
    }
...
void g3dKmdFdqBackupBufPtr(void)
{
    ...
    gExecInfo.fdq_wptr = g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_PA_FDQ_HWWPTR, MSK_AREG_PA_FDQ_HWWPTR, SFT_AREG_PA_FDQ_HWWPTR);
...
void g3dKmdFdqUpdateBufPtr(void)
{
    ...
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PA_FDQBUF_BASE, gExecInfo.fdq.hwAddr, MSK_AREG_PA_FDQBUF_BASE, SFT_AREG_PA_FDQBUF_BASE);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PA_FDQBUF_SIZE, gExecInfo.fdq_size, MSK_AREG_PA_FDQBUF_SIZE, SFT_AREG_PA_FDQBUF_SIZE);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PA_FDQ_SWRPTR, gExecInfo.fdq_rptr, MSK_AREG_PA_FDQ_SWRPTR, SFT_AREG_PA_FDQ_SWRPTR);

    // workaround, do it twice, such that we can read the correct REG_AREG_PA_FDQ_HWWPTR that we set
    for (i = 0; i < 2; i++)
    {
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PA_FDQ_HWWPTR_RB, gExecInfo.fdq_wptr, MSK_AREG_PA_FDQ_HWWPTR_RB, SFT_AREG_PA_FDQ_HWWPTR_RB);
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PA_FDQ_HWWPTR_RB, 0x1, MSK_AREG_PA_FDQ_HWWPTR_RB_UPD, SFT_AREG_PA_FDQ_HWWPTR_RB_UPD);
#ifndef G3D_HW
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PA_FDQ_HWWPTR, gExecInfo.fdq_wptr, MSK_AREG_PA_FDQ_HWWPTR, SFT_AREG_PA_FDQ_HWWPTR);
#endif

        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_MCU2G3D_SET, 0x0, MSK_AREG_MCU2G3D_SET, SFT_AREG_MCU2G3D_SET);
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_MCU2G3D_SET, 0x1, MSK_AREG_MCU2G3D_SET, SFT_AREG_MCU2G3D_SET);
        g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_PA_FDQ_HWWPTR_RB, 0x0, MSK_AREG_PA_FDQ_HWWPTR_RB_UPD, SFT_AREG_PA_FDQ_HWWPTR_RB_UPD);
    }
    ...
    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_FDQ, "rptr 0x%x, wptr 0x%x\n", g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_PA_FDQ_SWRPTR, MSK_AREG_PA_FDQ_SWRPTR, SFT_AREG_PA_FDQ_SWRPTR), g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_PA_FDQ_HWWPTR, MSK_AREG_PA_FDQ_HWWPTR, SFT_AREG_PA_FDQ_HWWPTR));
#endif
}
...
int g3dKmdGetAverageFps(void)
{
    ...
    if (lastLogDuration > (unsigned long)g_fpsLogInterval)
    {
         ...
         KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_FDQ, "kernel fps:%d.%04d,dur:%d,fcnt:%d,max:%d,min:%d\n", fps_div, fps_rem,
                lastLogDuration, g_fps_info.logFrameCount, g_fps_info.lastMaxDuration, g_fps_info.lastMinDuration);
...
void g3dKmdFdqReadQ(void)
{
    ...
    pData = (FRM_RTN_DATA*) gExecInfo.fdq.data;
...
#ifdef G3DKMD_SUPPORT_FENCE
        //g3dkmdUpdateTimeline(pData[rIdx].Qid, pData[rIdx].Fid);
...
void g3dKmdFdqWriteQ(int taskID)
{
    ...
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_IRQ_STATUS_RAW, VAL_IRQ_PA_DATA_QUEUE_NOT_EMPTY, VAL_IRQ_PA_DATA_QUEUE_NOT_EMPTY, SFT_AREG_IRQ_RAW);
    g3dKmdRegWrite(G3DKMD_REG_G3D, REG_AREG_IRQ_STATUS_MASKED, VAL_IRQ_PA_DATA_QUEUE_NOT_EMPTY, VAL_IRQ_PA_DATA_QUEUE_NOT_EMPTY, SFT_AREG_IRQ_RAW);

	gExecInfo.fdq_wptr = wIdx;
    cnt++;

    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_FDQ, "r: %d, w: %d, s: %d\n", rIdx, wIdx, size);
