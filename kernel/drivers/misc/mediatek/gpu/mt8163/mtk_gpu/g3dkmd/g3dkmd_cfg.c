...
#define G3DKMD_MMU_VA_BASE  0x40000000  // This value must be 1GB align, in order to fit WCP IOMMU HW using Mindos's MMU implementation
...
void g3dKmdCfgInit(void)
{
    ...
    _g3dkmd_cfg[CFG_DEFER_VARY_SIZE]  = (G3DKMD_MAP_VX_VARY_SIZE > KMALLOC_UPPER_BOUND) ? KMALLOC_UPPER_BOUND : G3DKMD_MAP_VX_VARY_SIZE;
    _g3dkmd_cfg[CFG_DEFER_BINHD_SIZE] = (G3DKMD_MAP_VX_BINHD_SIZE > KMALLOC_UPPER_BOUND/2) ? KMALLOC_UPPER_BOUND/2 : G3DKMD_MAP_VX_BINHD_SIZE;
    _g3dkmd_cfg[CFG_DEFER_BINBK_SIZE] = (G3DKMD_MAP_VX_BINBK_SIZE > KMALLOC_UPPER_BOUND/2) ? KMALLOC_UPPER_BOUND/2 : G3DKMD_MAP_VX_BINBK_SIZE;
    _g3dkmd_cfg[CFG_DEFER_BINTB_SIZE] = (G3DKMD_MAP_VX_BINTB_SIZE > KMALLOC_UPPER_BOUND/2) ? KMALLOC_UPPER_BOUND/2 : G3DKMD_MAP_VX_BINTB_SIZE;
    ...
    YL_KMD_ASSERT(((_g3dkmd_cfg[CFG_DEFER_VARY_SIZE]  & G3DKMD_VARY_SIZE_ALIGN_MSK)   == 0) && (_g3dkmd_cfg[CFG_DEFER_VARY_SIZE]  <= G3DKMD_DEFER_SIZE_MAX)); // 4KB align & max 256MB
    YL_KMD_ASSERT(((_g3dkmd_cfg[CFG_DEFER_BINBK_SIZE] & G3DKMD_BINBK_SIZE_ALIGN_MSK)  == 0) && (_g3dkmd_cfg[CFG_DEFER_BINBK_SIZE] <= G3DKMD_DEFER_SIZE_MAX)); // 128B align & max 256MB
    YL_KMD_ASSERT(((_g3dkmd_cfg[CFG_DEFER_BINTB_SIZE] & G3DKMD_BINTB_SIZE_ALIGN_MSK)  == 0) && (_g3dkmd_cfg[CFG_DEFER_BINTB_SIZE] <= G3DKMD_DEFER_SIZE_MAX)); // 16B align & max 256MB
...
void g3dKmdCfgSet(CFG_ENUM cfg, unsigned int value)
{
    ...
    case CFG_DEFER_VARY_SIZE:
        value = (value  && (value  <= G3DKMD_MAP_VX_VARY_SIZE))  ? value : _g3dkmd_cfg[CFG_DEFER_VARY_SIZE];
        value = G3DKMD_VARY_SIZE_ALIGNED(value);
        break;
        
    case CFG_DEFER_BINHD_SIZE:
        value = (value  && (value  <= G3DKMD_MAP_VX_BINHD_SIZE)) ? value : _g3dkmd_cfg[CFG_DEFER_BINHD_SIZE];
        value = G3DKMD_BINHD_SIZE_ALIGNED(value);
        break;
        
    case CFG_DEFER_BINBK_SIZE:
        value = (value  && (value  <= G3DKMD_MAP_VX_BINBK_SIZE)) ? value : _g3dkmd_cfg[CFG_DEFER_BINBK_SIZE];
        value = G3DKMD_BINBK_SIZE_ALIGNED(value);
        break;

    case CFG_DEFER_BINTB_SIZE:
        value = (value  && (value  <= G3DKMD_MAP_VX_BINTB_SIZE)) ? value : _g3dkmd_cfg[CFG_DEFER_BINTB_SIZE];
        value = G3DKMD_BINTB_SIZE_ALIGNED(value);
        break;

    case CFG_MEM_SHIFT_SIZE:
        /// if too big, set to 0.
        value = (value  <= G3DKMD_MAP_MEM_SHIFT_SIZE) ? value : _g3dkmd_cfg[CFG_MEM_SHIFT_SIZE];
        break;
...
unsigned int g3dKmdCfgGet(CFG_ENUM cfg)
{
    YL_KMD_ASSERT(cfg < CFG_MAX);
