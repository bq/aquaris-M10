...
int _bfio_add_file(unsigned int bffd, void*kffd){
    ...
    if (idx < 0 || idx >= MAX_FILES) {
        BF_DPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_BUFFERFILE,"[%s:%d] (bffd: %u, kffd %p)idx(%d) out of range: (%d-%d)\n",
                __FUNCTION__, __LINE__,
                bffd, kffd,
                idx, 0, MAX_FILES);
        return -1;
    }
    BF_DPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_BUFFERFILE,"[%s:%d] Add(bffd, ffd) at idx(%d) (%u, %p)\n", __FUNCTION__, __LINE__, 
           idx, bffd, kffd);
...
void* g3dKmdFileOpen_bufferfile(const char* path)
{
    ...
    if (!filp) {
        BF_DPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_BUFFERFILE,"[%s:%d] bufferfile open fail.\n", __FUNCTION__, __LINE__);
        return (void*) -1;
    }
...
void g3dKmdVfprintf_bufferfile(void* filp, const char* pszFormat, 
                               va_list args)
{
    if (!filp) {
        return;
    }
    {
        ...
        if (!data) {
            BF_DPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_BUFFERFILE,"[%s] vmalloc fail (size %d)\n",
                     __FUNCTION__, len + 1);
            return;
        }
...
int g3dKmdFileWrite_bufferfile(void* filp, unsigned char* data, 
                               unsigned int size)
{
