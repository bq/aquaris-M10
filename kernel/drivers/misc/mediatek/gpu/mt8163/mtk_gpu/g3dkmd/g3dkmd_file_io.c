...
g3dKmdFileIo g_kmdFileIo_kfile = 
{
...
g3dKmdFileIo g_kmdFileIo_bufferfile = 
{
...
g3dKmdFileIo *g_pKmdFileIo;
static g3dKmdFileIo *g_pKmdFileIo_backup;
...
static struct file* file_open(const char* path)
{
    ...
    if (IS_ERR(filp))
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_FILEIO,"filp_open fail [%s], error code 0x%x\n", path, PTR_ERR(filp));
...
G3Duint *g3dKmd_GetKMDDebugFlagsPtr(void)   { return &g_g3dkmd_debug_flags;   }
G3Duint *g3dKmd_GetKMDDebugLogLevelPtr(void){ return &g_g3dkmd_debug_loglevel;}
...
#endif
#else
    //printk("g3dKmd_closeDebugFile = 0x%x\n", g3dkmdFileHandle);
#ifdef PRINT2FILE
    if(g3dkmdFileHandle)
    {
...
#endif
#else
    //printk output : /var/log/messages
    printk("%s", string);
    //printk("A size = %d, f_pos = 0x%x\n", size, g3dkmdFileHandle->f_pos);
#ifdef PRINT2FILE
    if (g3dkmdFileHandle)
    {
       //int len = 
...
#ifdef ANDROID_PREF_NDEBUG
    if ((G3DKMD_LLVL_MASK & type) > G3DKMD_LLVL_WARNING) //if (type != G3DKMD_LLVL_ERROR && type != G3DKMD_LLVL_WARNING)
    {
        return;
    }
#endif
