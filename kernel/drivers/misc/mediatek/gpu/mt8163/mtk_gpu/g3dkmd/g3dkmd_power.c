...
static DEFINE_TIMER(gpu_early_suspend_timer, _early_suspend_timer_action, EARLY_SUSPEND_TIMER_EXPIRES, EARLY_SUSPEND_TIMER_DATA);
static DEFINE_TIMER(gpu_poweroff_timer, _poweroff_timer_action, POWEROFF_TIMER_EXPIRES, POWEROFF_TIMER_DATA);

static unsigned int _poweron_refcnt = 0;
static unsigned int _poweroff_tocnt = 0;
static unsigned int _store_refcnt = 0;
static unsigned int _freeze_refcnt = 0;
static bool _lateinit_done = false;
...
static bool _pending_work_done(void)
{
    ...
    if (g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_PA_FDQ_HWWPTR, MSK_AREG_PA_FDQ_HWWPTR, SFT_AREG_PA_FDQ_HWWPTR) != gExecInfo.fdq_rptr) {
        KMDDPF(MSG_LEVEL_INFO, "fdq_wptr = 0x%x, fdq_rptr = 0x%x.\n", g3dKmdRegRead(G3DKMD_REG_G3D, REG_AREG_PA_FDQ_HWWPTR, MSK_AREG_PA_FDQ_HWWPTR, SFT_AREG_PA_FDQ_HWWPTR), gExecInfo.fdq_rptr);
...
/******************************************************************************
* Return TURE if all pending work has finished. Otherwise, return FALSE.
******************************************************************************/
