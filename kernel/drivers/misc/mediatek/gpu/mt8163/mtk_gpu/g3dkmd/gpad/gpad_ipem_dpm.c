...
{
    {.name =   "disable", .init_func =   gpad_ipem_dpm_disable_init, .exit_func =   gpad_ipem_dpm_disable_exit, .do_func =   gpad_ipem_dpm_disable_do, .update_func =     gpad_ipem_dpm_disable_update},
    {.name = "immediate", .init_func = gpad_ipem_dpm_immediate_init, .exit_func = gpad_ipem_dpm_immediate_exit, .do_func = gpad_ipem_dpm_immediate_do, .update_func =   gpad_ipem_dpm_immediate_update},
    {.name =   "timeout", .init_func =   gpad_ipem_dpm_timeout_init, .exit_func =   gpad_ipem_dpm_timeout_exit, .do_func =   gpad_ipem_dpm_timeout_do, .update_func =     gpad_ipem_dpm_timeout_update},
    {.name =   "predict", .init_func =   gpad_ipem_dpm_predict_init, .exit_func =   gpad_ipem_dpm_predict_exit, .do_func =   gpad_ipem_dpm_predict_do, .update_func =     gpad_ipem_dpm_predict_update},
    {.name =    "qlearn", .init_func =    gpad_ipem_dpm_qlearn_init, .exit_func =     gpad_ipem_dpm_qlearn_exit, .do_func =      gpad_ipem_dpm_qlearn_do, .update_func =      gpad_ipem_dpm_qlearn_update},
    {.name =     "sarsa", .init_func =     gpad_ipem_dpm_sarsa_init, .exit_func =      gpad_ipem_dpm_sarsa_exit, .do_func =       gpad_ipem_dpm_sarsa_do, .update_func =       gpad_ipem_dpm_sarsa_update},
};
...
static gpad_ipem_action gpad_ipem_dpm_timeout_do(struct gpad_ipem_work_info *work_info)
{
    ...
    condition = wait_event_hrtimeout(queue_info->waitqueue, 
                             gpad_ipem_is_queuing_power_on(),
                             //work_info->event == GPAD_IPEM_EVENT_POWERON, 
                             timeout);
    //LOCK_IPEM();
...
void gpad_ipem_dpm_policy_set(struct gpad_device *dev, unsigned int policy_id)
{
    ...
    if (policy_id != ipem_info->dpm_info.policy_id) {
        ...
        GPAD_LOG_INFO("[IPEM] %s: policy_id = %u, name = %s, init_func = %p, exit_func = %p, do_func = %p, update_func = %p\n", __FUNCTION__, policy_id, ipem_info->dpm_info.policies[policy_id].name, (void *)ipem_info->dpm_info.policies[policy_id].init_func, (void *)ipem_info->dpm_info.policies[policy_id].exit_func, (void *)ipem_info->dpm_info.policies[policy_id].do_func, (void *)ipem_info->dpm_info.policies[policy_id].update_func);
    }
    UNLOCK_IPEM();
}
