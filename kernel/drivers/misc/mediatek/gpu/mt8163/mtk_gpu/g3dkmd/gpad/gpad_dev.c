...
static struct class *gpad_class_s = NULL;    /* GPAD class */
static dev_t         gpad_first_devno_s = 0; /* First number assigned by alloc_chrdev_region. */

struct file_operations  gpad_fops = {
        .owner          = THIS_MODULE,
        .open           = gpad_dev_open,
        .release        = gpad_dev_release,
        .unlocked_ioctl = gpad_dev_ioctl,
        };
...
{
    static int          id = 0; /* only support on device */
     ...
     */
    dev = (struct gpad_device *)kzalloc(sizeof(*dev), GFP_KERNEL);
    if (unlikely(!dev)) {
