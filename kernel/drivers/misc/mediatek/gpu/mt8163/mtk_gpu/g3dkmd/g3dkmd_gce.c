...
void g3dKmdGcePrepareQld(pG3dExeInst pInst)
{
    ...
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_GCE,"[%s] GCE is not enabled, this API should not be called!\n");
        return;
    }
    ...
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_GCE,"g3dKmdMmcqWaitSize error, can't wait enough buffer size (%d)\n", MMCQ_INSTRUCTION_SIZE * 8);
        YL_KMD_ASSERT(0);
        return;
    }
    ...
    //g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_MMCE, REG_CMDQ_SYNC_TOKEN_UPDATE, TOKEN_UPDATE_PAIR(MMCQ_TOKEN_FRAME_FLUSH, MMCQ_TOKEN_VALUE_FALSE), MMCQ_DEFAULT_MASK);
    ...
    if (g3dKmdCfgGet(CFG_SECURE_ENABLE))
    {
        ...
        g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_CQ_SECURE_CTRL, hwCh), 0x1, MSK_AREG_CQ_SECURITY);

        value = 0x1 << SFT_AREG_MX_SECURE0_ENABLE;
        g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_MX_SECURE0_BASE_ADDR, hwCh), value, MSK_AREG_MX_SECURE0_ENABLE);
        
        value = (G3DKMD_FLOOR(pInst->qbaseBuffer.hwAddr, 0x1000) >> 12);
        g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_MX_SECURE0_BASE_ADDR, hwCh), value, MSK_AREG_MX_SECURE0_BASE_ADDR);

        value = G3DKMD_CEIL(pInst->qbaseBuffer.hwAddr + pInst->qbaseBufferSize, 0x1000) >> 12;
        g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_MX_SECURE0_TOP_ADDR, hwCh), value, MSK_AREG_MX_SECURE0_TOP_ADDR);
    }
    else
    {
        g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_CQ_SECURE_CTRL, hwCh), 0x0, MSK_AREG_CQ_SECURITY);
    }
#endif

    // get token
    g3dKmdMmcqPoll(mmcq, MMCQ_SUBSYS_GPU,  G3D_ADDR(REG_AREG_G3D_TOKEN0, hwCh), (0x1 << SFT_AREG_G3D_TOKEN0), MSK_AREG_G3D_TOKEN0);
    
    // set qinfo buffer addr
    g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_CQ_QBUF_START_ADDR, hwCh), pInst->qbaseBuffer.hwAddr, MSK_AREG_CQ_QBUF_START_ADDR);
    g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_CQ_QBUF_END_ADDR, hwCh), (pInst->qbaseBuffer.hwAddr + pInst->qbaseBufferSize), MSK_AREG_CQ_QBUF_END_ADDR);

    // trigger qld upd
    g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_CQ_CTRL, hwCh), (0x1 << SFT_AREG_CQ_QLD_UPD), MSK_AREG_CQ_QLD_UPD);

    // enable clk (fire), auto clear
    g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_G3D_ENG_FIRE, hwCh), (0x1 << SFT_AREG_G3D_ENG_FIRE), MSK_AREG_G3D_ENG_FIRE);

    // disable qld upd
    g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_CQ_CTRL, hwCh), (0x0 << SFT_AREG_CQ_QLD_UPD), MSK_AREG_CQ_QLD_UPD);
  
    // wait frame flush
    g3dKmdMmcqWaitForEvent(mmcq, MMCQ_TOKEN_FRAME_FLUSH, MMCQ_TOKEN_VALUE_TRUE);

    // release token
    g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_G3D_TOKEN0, hwCh), (0x1 << SFT_AREG_G3D_TOKEN0), MSK_AREG_G3D_TOKEN0);
    ...
    #ifndef G3D_HW   // when change thread, we have to restore previous head/ tail pointer, otherwise the h/t pointer will be wrong; in HW case, HW will do it (CMODEL case)
...
unsigned char g3dKmdGceSendCommand(pG3dExeInst pInst)
{
    ...
    #ifdef G3D_HW
    if (!g3dKmdCheckBufferConsistency(hwCh, pInst->pHtDataForHwRead->htBuffer.hwAddr, pInst->pHtDataForHwRead->htBufferSize))
    {
    ...
    // update tail pointer
    g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_CQ_HTBUF_WPTR, hwCh), pInst->pHtDataForHwRead->htInfo->wtPtr, MSK_AREG_CQ_HTBUF_WPTR);

    // trigger tail upd
    g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_CQ_CTRL, hwCh), (0x1 << SFT_AREG_CQ_HTBUF_WPTR_UPD), MSK_AREG_CQ_HTBUF_WPTR_UPD);

    // enable clk (fire)
    g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_G3D_ENG_FIRE, hwCh), (0x1 << SFT_AREG_G3D_ENG_FIRE), MSK_AREG_G3D_ENG_FIRE);

    // disable tail upd
    g3dKmdMmcqWrite(mmcq, MMCQ_SUBSYS_GPU, G3D_ADDR(REG_AREG_CQ_CTRL, hwCh), (0x0 << SFT_AREG_CQ_HTBUF_WPTR_UPD), MSK_AREG_CQ_HTBUF_WPTR_UPD);
...
void g3dKmdGceDoContextSwitch(pG3dExeInst pInstFrom, pG3dExeInst pInstTo, int freeHwCh)
{
    ...
    if (pInstFrom)
    {
        KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_GCE,"suspend task (%d, %p), mmcq_handle %p, thd_handle %d\n", g3dKmdExeInstGetIdx(pInstFrom), pInstFrom, pInstFrom->mmcq_handle, pInstFrom->thd_handle);
    ...
    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_GCE,"resume task (%d, %p), mmcq_handle %p, thd_handle %d\n", inst_idx, pInstTo, pInstTo->mmcq_handle, pInstTo->thd_handle);
    ...
    {
        pInstTo->time_slice = G3DKMD_TIME_SLICE_BASE + pInstTo->prioWeighting* G3DKMD_TIME_SLICE_INC;
...
void g3dKmdGceReleaseMmcq(pG3dExeInst pInst)
{
    ...
    {
        // leon TBD, if the suspend task is currently active. after suspend, should we context switch to another task ?
