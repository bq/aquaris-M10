...
#if GPAD_CHECKING_HW
    static unsigned long long s_cpu_clock = 0;
    static unsigned int s_gpu_timestamp = 0;
    static unsigned int s_last_gpu_ts = 0;
...
static int gpad_pm_alloc_sw_buffer(struct gpad_pm_info *pm_info, unsigned int samples_to_alloc)
{
    ...
    for (idx = samples_to_alloc; idx > 0; --idx) {
        curr = (struct gpad_pm_sample_list *)kzalloc(sizeof(struct gpad_pm_sample_list), GFP_KERNEL);
        if (likely(curr)) {
...
static void gpad_pm_sync_timestamp(struct gpad_pm_info *pm_info)
{
    ...
    if (likely(diff_cpu_ts >= 0)) {
        ...
        /*
         * Error case 1: the CPU timestamp is not increasing, 
         * which means something wrongs in calculation from previous GPU PM samples.
...
void gpad_pm_enable(struct gpad_device *dev)
{
    ...
    if (!IS_PM_ENABLED(dev)) {
        ...
        gpad_hal_write32_mask(0x1f00, 0x0000003f, 0x00000011); /* Reset PM (5/30 version still needs PM_EN while doing reset) */
...
/*
 * FIXME: we shall implement g3dkmd_kQuery() in g3dkmd.
 */
int g3dkmd_kQueryAPI(const char *cmdStr, const unsigned int cmdStrLen, char *buffer, const unsigned int bufferLen, const unsigned int bufferDataLen)
{
    ...
    if (0 == strcmp("perf", cmdStr) && bufferDataLen > 0) {
        value = simple_strtol(buffer, &ptr, 0);
        printk("value = %d\n", value);
        /* call your function
         * Note that if there are invalid string in buffer,
         * value will be zero
         */
        if (value == 1)
