...
int (*ion_mm_heap_register_buf_destroy_callback_in_gpu)(struct ion_buffer *buffer, ion_mm_buf_destroy_callback_t *fn);
...
#else
extern struct ion_device *g_ion_device; // from drivers/staging/android/ion/mtk
extern int ion_mm_heap_register_buf_destroy_callback(struct ion_buffer *buffer, ion_mm_buf_destroy_callback_t *fn);
...
int ion_buffer_destroy_callback(struct ion_buffer* buffer, unsigned int phyAddr);

RADIX_TREE(gIonMapper, GFP_KERNEL);  /* Declare and initialize */

__DEFINE_SEM_LOCK(ionLock);
...
static int ion_radix_tree_insert_safe(struct radix_tree_root *tree, unsigned long key, struct g3d_ion_buffer* item)
{
    int result = 0;
...
{
    static struct ion_client * yoli_client = NULL; 
...
G3DKMD_APICALL int G3DAPIENTRY ion_mirror_create(unsigned long addr, int fd)
{
    ...
    if (ret)
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_ION,"%s : ion_radix_tree_insert_safe error(%d)\n", __func__, ret);
        YL_KMD_ASSERT(0);
    }
...
{
    if (buffer == NULL) return 0; //if 1 means object is removed
...
static int ion_mirror_wait_finish(unsigned long addr)
{
    ...
    do{
        ...
        {
            KMDDPF(G3DKMD_LLVL_NOTICE | G3DKMD_MSG_ION,"%s : g3dKmdLockCheckDone [%d] %u %u %u %u %u\n", 
            __func__, i, data[i].pid, data[i].taskID, data[i].write_stamp, data[i].read_stamp, data[i].pass);
...
int ion_buffer_destroy_callback(struct ion_buffer* buffer, unsigned int phyAddr)
{
    ...
    if (g3d_buffer)
    {
        ...
        {
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_ION,"%s : not match ion_buffer %p %p\n", __func__, buffer, g3d_buffer->ion_buffer);
...
G3DKMD_APICALL int G3DAPIENTRY ion_mirror_stamp_set(unsigned long addr, unsigned int stamp, unsigned readonly, pid_t pid, unsigned int taskID)
{
    struct g3d_ion_buffer* buffer;
    ...
    // unlock dead thread
    if (buffer->lock_now == 1 && find_vpid(buffer->lock_tid) == NULL)
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_ION, "ERROR: ion_mirror_stamp_set, locked thread is dead, [%d,%d]\n", buffer->lock_tid, current->pid);
        buffer->lock_now = 0;
    }
    ...
    // some one already lock it
    if (buffer->lock_now == 1) {
        mutex_unlock(&buffer->lock);
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_ION,"%s : buffer(mva=0x%lx) is locked now (lock tid=%d)\n", __func__, addr, buffer->lock_tid);
        ret = -EAGAIN;
        goto fail;
    }
...
G3DKMD_APICALL int G3DAPIENTRY ion_mirror_lock_update(unsigned long addr, unsigned int num_task, unsigned int readonly, struct ionGrallocLockInfo* data, unsigned int* lock_status)
{
    ...
    *lock_status = LOCK_DONE;
    ...
    if (buffer->lock_now == 1 && find_vpid(buffer->lock_tid) == NULL)
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_ION, "ERROR: ion_lock_update, locked thread is dead, [%d,%d]\n", buffer->lock_tid, current->pid);
        buffer->lock_now = 0;
    }
    ...
    }else
    {
        ...
        {     
            ...
            }else if (node->base.pid == data[i].pid && node->base.taskID == data[i].taskID && i < num_task)
            {
                             ...
                             void* ion_free, void* ion_client_create, void* ion_import_dma_buf)
{
    ...
    ion_client_create_in_gpu = (struct ion_client* (*)(struct ion_device*, const char*))ion_client_create;
    ion_import_dma_buf_in_gpu = (struct ion_handle* (*)(struct ion_client *client, int fd))ion_import_dma_buf;
}
