...
//YL_BOOL PageTable_Initialize( FirstPageTable master, FirstMetaTable metaTable );

void PageTable_Release( FirstPageTable master, FirstMetaTable metaTable );



YL_BOOL Section_register(   Mapper m, unsigned short index0, pa_t pa, Trunk trunk, va_t va, unsigned int flag );
YL_BOOL Small_register(     Mapper m, PageIndexRec index,    pa_t pa, Trunk trunk, va_t va, unsigned int flag );
YL_BOOL Section_unregister( Mapper m, unsigned short index0 );
YL_BOOL Small_unregister(   Mapper m, PageIndexRec index    );




static INLINE YL_BOOL SectionEntry_has_second_level( FirstPageEntryRec entry )	__attribute__((always_inline));

static INLINE YL_BOOL Section_has_second_level( FirstPageTable page0, unsigned short index0 )	__attribute__((always_inline));


/**************** Implementation *****************/
static INLINE size_t
PageTable_get_required_size()
{
...
static INLINE va_t
SmallPage_vabase_part( va_t va )
{
    return (va_t)(va & ~ SMALL_PAGE_OFFSET_MASK);
}

static INLINE pa_t
SmallPage_pabase_part( pa_t pa )
{
    return (pa_t)(pa & ~ SMALL_PAGE_OFFSET_MASK);
}

static INLINE YL_BOOL
SectionEntry_has_second_level( FirstPageEntryRec entry )
{
    return ( entry.Table.type == FIRST_TYPE_TABLE ) ? 1 : 0 ;
}

static INLINE YL_BOOL
Section_has_second_level( FirstPageTable page0, unsigned short index0 )
{
...
static INLINE YL_BOOL
SectionMetaEntry_has_second_level( FirstMetaEntryRec meta_entry )
{
...
PageTable_has_second_level( FirstPageTable page0, unsigned short index0 )
{
    return Section_has_second_level( page0, index0 );
}

static INLINE YL_BOOL
SectionMeta_has_second_level( FirstPageTable table, int index0 )
{
...
static INLINE size_t 
PageTable_calc_size_in_small_blocks( va_t va, size_t size )
{
...
// can't be unsigned short [overflow]
static INLINE size_t 
PageTable_calc_size_in_section_blocks( va_t va, size_t size )
{
    unsigned int offset = SmallPage_offset_part( va );
