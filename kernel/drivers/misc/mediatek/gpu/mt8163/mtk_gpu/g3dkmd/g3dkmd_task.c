...
void g3dKmdInitQbaseBuffer(g3dExeInst* pInst)
{
    ...
    pInfo->reg_cq_htbuf_end_addr.s.cq_htbuf_end_addr            = pInst->pHtDataForHwRead->htBuffer.hwAddr + pInst->pHtDataForHwRead->htBufferSize;
    ...
    pInfo->reg_vx_binhd_base1.s.vx_binhd_base1                  = hd_buffer->hwAddr + ((hdsize + G3DKMD_DEFER_BASE_ALIGN_MSK) & (~G3DKMD_DEFER_BASE_ALIGN_MSK));  // 128B align
    pInfo->reg_vx_bintb_base1.s.vx_bintb_base1                  = tb_buffer->hwAddr + ((tbsize + G3DKMD_DEFER_BASE_ALIGN_MSK) & (~G3DKMD_DEFER_BASE_ALIGN_MSK));  // 128B align
    pInfo->reg_vx_binbk_base1.s.vx_binbk_base1                  = bk_buffer->hwAddr + ((bksize + G3DKMD_DEFER_BASE_ALIGN_MSK) & (~G3DKMD_DEFER_BASE_ALIGN_MSK));  // 128B align
    ...
    pInfo->reg_vx_vary_preboff_msize.s.vx_vary_preboff_msize    = ((va_preboff_size * G3DKMD_PREFULL_THD) / 100) & (~0x3FF);    // multiple of 1KB
    pInfo->reg_vx_tb_preboff_msize.s.vx_tb_preboff_msize        = ((tb_preboff_size * G3DKMD_PREFULL_THD) / 100) & (~0xF);      // multiple of 16B
    pInfo->reg_vx_bk_preboff_msize.s.vx_bk_preboff_msize        = ((bk_preboff_size * G3DKMD_PREFULL_THD) / 100) & (~0x7F);     // multiple of 128B
...
{
    pG3dTask task = (pG3dTask)G3DKMDVMALLOC(sizeof(*task));
    if(task)
    {
        ...
        task->prio = current->prio; // normally, it's 120, non-realtime process, if it's more smaller, priority is higher.
...
/*
    give G3dTask pointer and return the register index in G3dExecuteInfo
*/
int g3dKmdRegisterNewTask(pG3dExecuteInfo execute,pG3dTask task)
{
    pG3dTask *newTaskList = NULL;
    pG3dTask *oldTaskList = NULL;
    int i = 0;
    ...
    {
        KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_TASK, "already reach the Max task count 255\n");
        return -1;
    }
    ...
    {
        execute->taskAllocCount += ((execute->taskAllocCount + 10 >= G3DKMD_MAX_TASK_CNT) ? (G3DKMD_MAX_TASK_CNT - execute->taskAllocCount) : 10);
        newTaskList = (pG3dTask *)G3DKMDVMALLOC(sizeof(pG3dTask) * execute->taskAllocCount);
        if(newTaskList == NULL)
        {
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_TASK, "G3DKMDVMALLOC fail for taskList, taskAllocCount 0x%x\n", execute->taskAllocCount);
            return -1;
        }

        memset(&newTaskList[execute->taskInfoCount], 0, sizeof(pG3dTask) * 10);
        if(execute->taskInfoCount)
        {
            memcpy(newTaskList, execute->taskList, sizeof(pG3dTask) * execute->taskInfoCount);
...
void g3dKmdDestroyTaskList(void)
{
    ...
    memset(&gExecInfo, 0,sizeof(g3dExecuteInfo));
}
...
static pG3dHtInfo _g3dKmdCreateHtInfo(void)
{
    pG3dHtInfo htInfo = (pG3dHtInfo)G3DKMDVMALLOC(sizeof(g3dHtInfo));

    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_TASK, "\n");

    if(htInfo)
    {
        memset(htInfo, 0, sizeof(g3dHtInfo));
    }
...
static void _g3dKmdDestroyHtInfo(pG3dHtInfo ht)
{
    KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_TASK, "\n");
...
void g3dKmdCopyCmd(pG3dHtInfo ht, pG3dHtCommand pSrcCmd)
{
    unsigned int wtIndex = (ht->wtPtr - ht->htCommandBaseHw) / HT_CMD_SIZE;
    pG3dHtCommand pCmd= &ht->htCommand[wtIndex];

    memcpy(pCmd,pSrcCmd,sizeof(g3dHtCommand));
...
//void g3dKmdAddHTCmd(pG3dHtInfo ht, void *head, void *tail, unsigned int task_id)
void g3dKmdAddHTCmd(pG3dExeInst pInst, void *head, void *tail, unsigned int task_id)
{
    pG3dHtInfo ht        = pInst->pHtDataForSwWrite->htInfo;
    unsigned int wtIndex = (ht->wtPtr - ht->htCommandBaseHw) / HT_CMD_SIZE;
    pG3dHtCommand pHtCmd= &ht->htCommand[wtIndex];
    ...
    {
        pG3dTask task = g3dKmdGetTask(task_id);
        if (NULL != task)
        {
        KMDDPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_DDFR | G3DKMD_MSG_TASK,
                "pInst: %p InstIdx:%u state(%d), lastStamp: %u "
                "pHt(sw/hw):(%p, %p) head %p, tail %p, task_id 0x%x, "
                "wtPtr 0x%x rdPtr: 0x%x range(0x%x ~ 0x%x) pHtCmd: %p\n",
            pInst, g3dKmdExeInstGetIdx(pInst),
...
/*
int g3dKmdCaculateIndex(pG3dHtInfo ht, unsigned int wptr, int index)
{
    int ret = index;
    uintptr_t start = (uintptr_t) ht->htCommand;
    unsigned int end   = start + sizeof(g3dHtCommand)*MAX_HT_COMMAND;
    if(start==0)
        return ret;
    if( (wptr <= end) && (wptr >= start) )
    {
        ret = (wptr - start)/sizeof(g3dHtCommand);
...
void g3dKmdTaskAssociateInstIdx(pG3dTask task, const unsigned int instIdx)
{
...
#ifdef G3DKMD_ENABLE_TASK_EXEINST_MIGRATION
    pG3dExeInst pInst = g3dKmdExeInstGetInst(task->inst_idx);
#endif
...
static void _g3dKmdFreeHtInst(pG3dHtInst pHtInst)
{
    if (!pHtInst)
    {
        return;
...
static void _g3dKmdInitHtInst(pG3dHtInst pHtInst)
{
   pHtInst->isUsed                   = G3DKMD_FALSE;
   pHtInst->htInfo->htCommand        = (pG3dHtCommand)pHtInst->htBuffer.data;
...
static void _g3dKmdReleaseHtInst(pG3dHtInst pHtInst)
{
    if (!pHtInst)
    {
...
static G3DKMD_BOOL _g3dKmdCreateHtInst(pG3dHtInst pHtInst, const int vG3DKMD_BUF_BASE_ALIGN, const int vG3DKMD_BUF_SIZE_ALIGN)
{
    if (!pHtInst) { return G3DKMD_FALSE; }
    if (NULL == pHtInst->htInfo)
    {
        unsigned int flags;

        pHtInst->htBufferSize = G3DKMD_ALIGN(HT_BUF_SIZE, vG3DKMD_BUF_SIZE_ALIGN);
        if (NULL == (pHtInst->htInfo = _g3dKmdCreateHtInfo()))
        {
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_TASK,"[%s] _g3dKmdCreateHtInfo fail\n", __FUNCTION__);
            return G3DKMD_FALSE;
        }
...
static pG3dHtInst _getAllocFreeHtInstance(pG3dExeInst pInst)
{
    int idx;
    pG3dHtInst pRetHtInst = NULL;
...
static pG3dExeInst _g3dKmdExeInstCreate(G3DKMD_DEV_REG_PARAM* param, const g3dExeState state_in)
{
  ...
  #define G3DKMD_BUF_SIZE_ALIGN   16
    ...
    pInst = (g3dExeInst*)G3DKMDVMALLOC(sizeof(g3dExeInst));
    if (!pInst)
    {
    ...
    for (i = 0; i < G3DKMD_MAX_HT_INSTANCE; ++i)
    {
        ...
        if (G3DKMD_FALSE == _g3dKmdCreateHtInst( &pInst->htInstance[i],
                    G3DKMD_BUF_BASE_ALIGN, G3DKMD_BUF_SIZE_ALIGN))
        {
            goto ERR_EXIT;
        }
    }
    // get free htInstance
    {
        pG3dHtInst pHtInst      = _getAllocFreeHtInstance(pInst);
...
#else
    pInst->prioWeighting = ((139 - current->prio) * 100 / 140) + 1; // range of current->prio is 0~139 (lower means higher priority), we just remap to 1~100 (higher has higher priority)
...
ERR_EXIT:
    {   // free Ht
        ...
        for (i = 0; i < G3DKMD_MAX_HT_INSTANCE; ++i)
        {
            pG3dHtInst pHtInst = &(pInst->htInstance[i]);
            if (pHtInst)
            {
...
static void _g3dKmdExeInstRelease(pG3dExeInst pInst)
{
    int i;
...
static unsigned int _g3dKmdExeInstRegister(pG3dExeInst pInst)
{
    int i = 0;
    ...
    {
        pG3dExeInst *newInst = NULL;
        gExecInfo.exeAllocCount += 10;
        newInst = (pG3dExeInst *)G3DKMDVMALLOC(sizeof(pG3dExeInst) * gExecInfo.exeAllocCount);
        ...
        memset(&newInst[gExecInfo.exeInfoCount], 0, sizeof(pG3dExeInst) * 10);
        if(gExecInfo.exeInfoCount)
        {
            memcpy(newInst, gExecInfo.exeList, sizeof(pG3dExeInst) * gExecInfo.exeInfoCount);
...
static void _g3dKmdExeInstIncRef(unsigned int instIdx)
{
    ...
    KMDDPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_DDFR | G3DKMD_MSG_TASK," inst[%u]->ref: %u -> %u (%p)\n",
...
void g3dKmdExeInstInit(void)
{
    ...
    if (size)
    {
        for (i = 0; i < size; i++)
        {
            ...
            _g3dKmdExeInstIncRef(instIdx); // add ref, such that these instances won't be freed when AP release
        }
    }
    else
    {
...
typedef struct _G3dKmdExeInstAllocWork
{
...
} G3dKmdExeInstAllocWork, *pG3dKmdExeInstAllocWork;


typedef struct _G3dKmdExeInstReleaseWork
{
    struct work_struct work;
    int                instIdx;
} G3dKmdExeInstReleaseWork, *pG3dKmdExeInstReleaseWork;



static void _g3dkmd_create_exeInst_workqueue(struct work_struct *work)
{
    pG3dKmdExeInstAllocWork my_work  = (pG3dKmdExeInstAllocWork) work;
    ...
    // check if the request migration work can be done or not.
    {
        ...
        if (sysinfo.freeram < G3DKMD_DDFR_SYSRAM_REJECT_THRLHLD)
        {
            KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_DDFR | G3DKMD_MSG_TASK,
                "Free System memory(%u) < thrlhld (%u)"
                "Stop migrating Task(%d).\n",
                sysinfo.freeram, G3DKMD_DDFR_SYSRAM_REJECT_THRLHLD,
                taskID);
            return ;
        }
    }
    ...
    lock_g3d();
    {
        ...
        {
            KMDDPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_DDFR | G3DKMD_MSG_TASK,
                "Some other task migration is going now. "
                "Stop migrating Task(%d).\n", taskID);
            unlock_g3d();
            return;
        }
        ...
        {
            KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_TASK,
                "Get task(%d)->tryAllocMemFailCnt (%u) > MAX(%u). "
                "Stop migration task(%d).\n",
        ...
        {
            KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_TASK,
                "The Inst(%d)->tryEnlargeFailCnt(%u) > MAX(%u). "
                "Stop migration task(%d).\n",
        ...
        {
            KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_TASK,
                "The Inst(%d)%p Idx(child,parent): %d,%d of task(%d) is "
                "migrating. Stop migration task!!!\n",
...
#define MIN(a,b) ((a)<(b))?(a):(b)
                ...
                param->dfrVarySize < G3DKMD_DDFR_VA_MAX)
            {
                ...
                //param->dfrVarySize = MAX(param->dfrVarySize + G3DKMD_BOFF_VA_INC_SIZE, G3DKMD_DDFR_VA_MAX)
                ...
                param->dfrBinbkSize < G3DKMD_DDFR_BK_MAX)
            {
                ...
                //param->dfrBinbkSize = MAX(param->dfrBinbkSize + G3DKMD_BOFF_BK_INC_SIZE, G3DKMD_DDFR_BK_MAX)
                ...
                param->dfrBintbSize < G3DKMD_DDFR_TB_MAX)
            {
                ...
                //param->dfrBintbSize = MAX(param->dfrBintbSize + G3DKMD_BOFF_TB_INC_SIZE, G3DKMD_DDFR_TB_MAX)
                ...
                param->dfrBinhdSize < G3DKMD_DDFR_HD_MAX)
            {
                ...
                //param->dfrBinhdSize = MAX(param->dfrBinhdSize + G3DKMD_BOFF_HD_INC_SIZE, G3DKMD_DDFR_HD_MAX)
        ...
        KMDDPF(G3DKMD_LLVL_INFO | G3DKMD_MSG_TASK,
            "(taskID: %d, isMigratingAllTasks: %c, "
            "boff(va,bk,tb,hd): (%u, %u, %u, %u)\n",
            taskID, isMigratingAllTasks?'Y':'N',
            boff_va_ovf_cnt, boff_bk_ovf_cnt, boff_tb_ovf_cnt,
            boff_hd_ovf_cnt);
    }
    ...
    // do the heavy works which require no locker
    {
        ...
        if (EXE_INST_NONE == newInstIdx)
        {
            ...
            lock_g3d();
            {
                pG3dTask pTask = g3dKmdGetTask(taskID);
                if (NULL == pTask)
                {
    ...
    lock_g3d();
    {
        ...
        {
            KMDDPF(G3DKMD_LLVL_ERROR | G3DKMD_MSG_TASK,
                "One of oldInst(%d)/newInst(%d)/pTask(%d) is NULL. "
                "Stop migration task(%d).\n",
 ...
 /// bug when not two_queue setting.
        ...
        /// 2. if any task in oldInst, "not move all"
            if (oldInst->refCnt > 0)
            {
            //   2-2. move oldInst->pHtDataForSwWrite to backHt   : so that the new cmds will go to backHt.
    ...
    const unsigned int boff_tb_ovf_cnt, const unsigned int boff_hd_ovf_cnt)
{
        ...
        boff_hd_ovf_cnt <= G3DKMD_BOFF_HD_THRLHLD)
    {
        KMDDPF(G3DKMD_LLVL_DEBUG | G3DKMD_MSG_TASK,
            "(taskID: %d, isMigratingAllTasks: %c, "
            "boff(va,bk,tb,hd): (%u, %u, %u, %u)"
            "REJECT due to too few boff.\n",
            taskID, isMigratingAllTasks?'Y':'N',
            boff_va_ovf_cnt, boff_bk_ovf_cnt, boff_tb_ovf_cnt, boff_hd_ovf_cnt);
        return ;
    }

    pWork = (pG3dKmdExeInstAllocWork)G3DKMDMALLOC_ATOMIC(
                sizeof(G3dKmdExeInstAllocWork)); // free by work.
...
{
    pG3dKmdExeInstReleaseWork my_work;
    int                       instIdx;
    ...
    my_work = (pG3dKmdExeInstReleaseWork) work;
...
{
    pG3dKmdExeInstReleaseWork pWork = (pG3dKmdExeInstReleaseWork)G3DKMDMALLOC(sizeof(G3dKmdExeInstReleaseWork)); // free by work.

    if (!pWork)
    {
        KMDDPF(G3DKMD_LLVL_WARNING | G3DKMD_MSG_TASK,
            "========= ERROR: NOT Expected ====== "
            "(instIdx:%d) release is requested, but alloc struc work fail.\n");
