...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_MON_BRK_STAR_FRM    0x0000ffff // [default:16'bx][perf:] monitor and break enable start sub frame count
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_MON_BRK_VP_EN       0x01000000 // [default:1'b0][perf:] enable mx_frame_id/mx_debug_flag compare at vp phase
#define SFT_MX_DBG_MON_BRK_VP_EN       24
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_MON_BRK_PP_EN       0x02000000 // [default:1'b0][perf:] enable mx_frame_id/mx_debug_flag compare at pp phase
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_BRK_RELEASE         0x10000000 // [default:1'b0][perf:] release previously matched break condition
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_CMD_RDATA_SEL       0x00000070 // [default:3'b0][perf:] read command return data selection in read burst
                                                  // 0: 1st 128-bit data
                                                  // 1: 2nd 128-bit data
                                                  // 2: 3rd 128-bit data
                                                  // 3: 4th 128-bit data
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_DBG_DUMP_LDRAM_FLUSH_EN 0x00000002 // [default:1'b0][perf:] internal dump flush turn-on when engine flush & dbg_dump_ldram_flush=1
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_PA2MX_CU_CLR        0x04000000 // [default:1'b0][perf:] debug mode pa2mx_cu_clr trigger (write 1 then write 0 to trigger)
#define SFT_MX_DBG_PA2MX_CU_CLR        26
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_MX_L3_FLUSH         0x08000000 // [default:1'b0][perf:] debug mode mx_l3_flush trigger (write 1 then write 0 to trigger)
#define SFT_MX_DBG_MX_L3_FLUSH         27
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_MX_L2_FLUSH         0x10000000 // [default:1'b0][perf:] debug mode mx_l2_flush trigger (write 1 then write 0 to trigger)
#define SFT_MX_DBG_MX_L2_FLUSH         28
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_MX_INVALIDATE       0x20000000 // [default:1'b0][perf:] debug mode mx_invalidate trigger (write 1 then write 0 to trigger)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_HW_SUBFRAME_MATCH   0x00001000 // [default:1'b0][perf:] monitor/break hw subframe count match status
#define SFT_MX_DBG_HW_SUBFRAME_MATCH   12
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_MX_DBG_SW_FRAME_ID_MATCH   0x00002000 // [default:1'b1][perf:] monitor/break sw frame_id/debug flag match status
    ...
    union //0x0180,  
    {
        ...
        struct 
        {
            unsigned int dbg_dump_ldram_enable : 1 ; // internal dump enable
                                                    // default: 1'b0
            unsigned int dbg_dump_ldram_flush_en : 1 ; // internal dump flush turn-on when engine flush & dbg_dump_ldram_flush=1
                                                    // default: 1'b0
    ...
    union //0x0ff0,  
    {
        ...
        struct 
        {
            ...
            unsigned int rsv_dbg_ctrl         : 1 ; // Reserved debug control
                                                    // default: 1'b0
            unsigned int pa2mx_cu_clr         : 1 ; // debug mode pa2mx_cu_clr trigger (write 1 then write 0 to trigger)
                                                    // default: 1'b0
            unsigned int mx_l3_flush          : 1 ; // debug mode mx_l3_flush trigger (write 1 then write 0 to trigger)
                                                    // default: 1'b0
            unsigned int mx_l2_flush          : 1 ; // debug mode mx_l2_flush trigger (write 1 then write 0 to trigger)
                                                    // default: 1'b0
            unsigned int mx_invalidate        : 1 ; // debug mode mx_invalidate trigger (write 1 then write 0 to trigger)
                                                    // default: 1'b0
    ...
    union //0x0ff4,  
    {
        ...
        struct 
        {
            ...
            unsigned int apb_cmd_busy         : 1 ; // apb command busy status
                                                    // default: 1'b0
            unsigned int hw_subframe_match    : 1 ; // monitor/break hw subframe count match status
                                                    // default: 1'b0
            unsigned int sw_frame_id_match    : 1 ; // monitor/break sw frame_id/debug flag match status
                                                    // default: 1'b1
        } s;
    }reg_mx_idle;
