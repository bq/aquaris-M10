...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_HT_FM_HEAD                 0xffffffff // [default:32'b0][perf:] the start address of frame global buffer , 16bytes alignment, (tail - head)must <=8M byte
                                                  // head tail context
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_HT_FM_TAIL                 0xffffffff // [default:32'b0][perf:] the end address of frame global buffer, 16 bytes alignment 
                                                  // head tail context
...
typedef struct
{
    ...
    union //0x0008, CQ 
    {
        ...
        struct 
        {
            unsigned int fm_head              : 32; // the start address of frame global buffer , 16bytes alignment, (tail - head)must <=8M byte
                                                    // head tail context
                                                    // default: 32'b0
        } s;
    }reg_fm_head;
    ...
    union //0x000c, CQ 
    {
        ...
        struct 
        {
            unsigned int fm_tail              : 32; // the end address of frame global buffer, 16 bytes alignment 
                                                    // head tail context
                                                    // default: 32'b0
        } s;
    }reg_fm_tail;
