...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_SYSCALL_WP0_INFO    0x00000fff // [default:][perf:] syscall wp0 info (active is in syscall_wait state)
#define SFT_UX_DWP_SYSCALL_WP0_INFO    0
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_SYSCALL_WP2_INFO    0x0fff0000 // [default:][perf:] syscall wp2 info (active is in syscall_wait state)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_SYSCALL_BIT0_INFO_ID 0x00ffffff // [default:][perf:] syscall bit0 id info (indicate more syscall information, cid or instruction sys_id)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_SYSCALL_BIT1_INFO_ID 0x00ffffff // [default:][perf:] syscall bit1 id info (indicate more syscall information, cid or instruction sys_id)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_SYSCALL_BIT2_INFO_ID 0x00ffffff // [default:][perf:] syscall bit2 id info (indicate more syscall information, cid or instruction sys_id)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_FTZ_RND_ORDER       0x00010000 // [default:1'b0][perf:] vp alu FTZ/RND operation order ([0]: RND->FTZ(for INTEL) [1]: FTZ->RND(for OpenCL & ARM) )
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_WP0_INTP_TYPE       0x0000000f // [default:][perf:] wave pool 0 interrupt type {subnormal,nan,underflow,overflow}
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_WP2_INTP_TYPE       0x000f0000 // [default:][perf:] wave pool 2 interrupt type {subnormal,nan,underflow,overflow}
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_ADVDBG_EN           0x00000001 // [default:1'b0][perf:] advanced debug enable (stop in specific pc condition)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_ADVDBG_PC_NEXT      0x00000004 // [default:1'b0][perf:] advanced debug stop for next pc value by one pulse (only valid after stop condition)
#define SFT_UX_DWP_ADVDBG_PC_NEXT      2
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_ADVDBG_PC_TYPE      0x00000010 // [default:1'b0][perf:] advanced debug pc type (0: pc 1: issued pc number)
#define SFT_UX_DWP_ADVDBG_PC_TYPE      4
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_DEBUG_EN            0x00000080 // [default:1'b0][perf:] Debug Enable for data latch (read/dump eu_status_data should active this bit)
#define SFT_UX_DWP_DEBUG_EN            7
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_ADVDBG_PC_VAL       0xffffff00 // [default:24'd0][perf:] advanced debug pc value (stop when cur_pc==reg_pc(pc_type=0)/cur_pc>=reg_pc(pc_type=1))
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_WAVE_INFO_EN        0x00000001 // [default:1'b0][perf:] wave info wave-pool enable (some debug counter would be active)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_VRF_DBG_WPSEL       0x00000030 // [default:2'd0][perf:] vprf debug wave pool select (0:1st/2:2nd)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_VRF_DBG_SRAMSEL     0x0f000000 // [default:4'd0][perf:] vprf debug sram select ([0~3]bk0 [4~7]bk1 [8~11]bk2 [12~15]bk3)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_SMM_DBG_DUMP_MD     0x0000001c // [default:3'd0][perf:] smm debug dump mode (0: private segment, 1: group segment, 2: private base of wave, 3: group base of wave, 4: share memory raw data dump)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_EU_STATUS_ID_A      0x0000000f // [default:4'd0][perf:] eu status id (0: sfu 1:ifb 2:vs0r 3:vs1r 4:fsr 8:vrf_wp0 9:vrf_wp2 10:vppipe_wp0 11:vppipe_wp2)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_EU_STATUS_ID_B      0x0000000f // [default:4'd0][perf:] eu status id (0: sfu 1:ifb 2:vs0r 3:vs1r 4:fsr 8:vrf_wp0 9:vrf_wp2 10:vppipe_wp0 11:vppipe_wp2)
...
/*--------------------------------------------------------------------------------------------------------*/
#define MSK_UX_DWP_IDLE_STATUS         0x000000ff // [default:8'hff][perf:] {wp0_vpu,wp0_lsm,vp1_vpu,wp1_lsm,sfu,vs0r,vs1r,fsr}
...
typedef struct
{
    ...
    union //0x0004,  
    {
        ...
        struct 
        {
            unsigned int syscall_bit0_info_id : 24; // syscall bit0 id info (indicate more syscall information, cid or instruction sys_id)
        } s;
    }reg_syscall_info_1;
    ...
    union //0x0008,  
    {
        ...
        struct 
        {
            unsigned int syscall_bit1_info_id : 24; // syscall bit1 id info (indicate more syscall information, cid or instruction sys_id)
        } s;
    }reg_syscall_info_2;
    ...
    union //0x000c,  
    {
        ...
        struct 
        {
            unsigned int syscall_bit2_info_id : 24; // syscall bit2 id info (indicate more syscall information, cid or instruction sys_id)
        } s;
    }reg_syscall_info_3;
    ...
    union //0x0014,  
    {
        ...
        struct 
        {
            ...
            unsigned int ftz_rnd_order        : 1 ; // vp alu FTZ/RND operation order ([0]: RND->FTZ(for INTEL) [1]: FTZ->RND(for OpenCL & ARM) )
                                                    // default: 1'b0
        } s;
    }reg_error_detection;
    ...
    union //0x0018,  
    {
        ...
        struct 
        {
            unsigned int wp0_intp_type        : 4 ; // wave pool 0 interrupt type {subnormal,nan,underflow,overflow}
            ...
            unsigned int wp2_intp_type        : 4 ; // wave pool 2 interrupt type {subnormal,nan,underflow,overflow}
    ...
    union //0x0020,  
    {
        ...
        struct 
        {
            ...
            unsigned int advdbg_clear         : 1 ; // advanced debug clear stop status
                                                    // default: 1'b0
            unsigned int advdbg_pc_next       : 1 ; // advanced debug stop for next pc value by one pulse (only valid after stop condition)
                                                    // default: 1'b0
            ...
            unsigned int debug_en             : 1 ; // Debug Enable for data latch (read/dump eu_status_data should active this bit)
                                                    // default: 1'b0
            unsigned int advdbg_pc_val        : 24; // advanced debug pc value (stop when cur_pc==reg_pc(pc_type=0)/cur_pc>=reg_pc(pc_type=1))
                                                    // default: 24'd0
        } s;
    }reg_adv_debug;
    ...
    union //0x0024,  
    {
        ...
        struct 
        {
            unsigned int wave_info_en         : 1 ; // wave info wave-pool enable (some debug counter would be active)
                                                    // default: 1'b0
    ...
    union //0x0028,  
    {
        ...
        struct 
        {
            ...
            unsigned int vrf_dbg_vpadr        : 8 ; // vprf debug vp address
                                                    // default: 8'd0
            unsigned int vrf_dbg_sramsel      : 4 ; // vprf debug sram select ([0~3]bk0 [4~7]bk1 [8~11]bk2 [12~15]bk3)
                                                    // default: 4'd0
    ...
    union //0x0034,  
    {
        ...
        struct 
        {
            ...
            unsigned int smm_dbg_fire         : 1 ; // smm debug fire (1T pulse)
                                                    // default: 1'b0
            unsigned int smm_dbg_dump_md      : 3 ; // smm debug dump mode (0: private segment, 1: group segment, 2: private base of wave, 3: group base of wave, 4: share memory raw data dump)
                                                    // default: 3'd0
    ...
    union //0x0040,  
    {
        ...
        struct 
        {
            unsigned int eu_status_id_a       : 4 ; // eu status id (0: sfu 1:ifb 2:vs0r 3:vs1r 4:fsr 8:vrf_wp0 9:vrf_wp2 10:vppipe_wp0 11:vppipe_wp2)
                                                    // default: 4'd0
    ...
    union //0x0048,  
    {
        ...
        struct 
        {
            unsigned int eu_status_id_b       : 4 ; // eu status id (0: sfu 1:ifb 2:vs0r 3:vs1r 4:fsr 8:vrf_wp0 9:vrf_wp2 10:vppipe_wp0 11:vppipe_wp2)
                                                    // default: 4'd0
