...
#define FIELD_OF_UINT64(name)    uint64_t name __attribute__((aligned(8)))
...
typedef struct _g3dkmd_context
{
    ...
    //out:
    int dispatchIndex;
    int reserved;                  // padding for 64 bits alignment
}g3dkmd_context;
 ...
 * Provided by userspace as an argument to the ioctl G3D_IOCTL_ION_MIRROR_SET_ROTATE_INFO and G3D_IOCTL_ION_MIRROR_GET_ROTATE_INFO
 */
...
#define  G3D_IOCTL_FENCE_STATUS            _IOWR(G3D_IOCTL_MAGIC, 35, g3dkmd_FenceCmd)
...
// for auto recovery
#define  G3D_IOCTL_HT_READ_WRITE_EQUAL     _IOWR(G3D_IOCTL_MAGIC, 52, G3DKMD_BOOL)
