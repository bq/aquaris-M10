...
#define ATRACE_FUNC_G3D_BEGIN() atrace_begin(ATRACE_TAG_GRAPHICS, __FUNCTION__)
...
#define G3DKMD_ACTION_QUERY_ION_MIRROR       5
...
#define ASSERT2(_Expression, _Msg) do { (void)( (!!(_Expression)) || (_wassert(L##_Msg, _CRT_WIDE(__FILE__), __LINE__), 0) ); } while(0)
...
#define ASSERT(X)   do{ assert(X); }while(0)
#endif
#define ASSERT2(_Expression, _Msg) do { if (!(_Expression)) ALOGE("assert msg = %s\n", _Msg); assert(_Expression); } while(0);
#else // linux
#define ASSERT(X)  assert(X)
#define ASSERT2(_Expression, _Msg) do { if (!(_Expression)) printf("assert msg = %s\n", _Msg); assert(_Expression); } while(0);
#define ASSERT_MSG(_Expression, fmt, ...) do { if (!(_Expression)) { printf("ASSERT: "); printf(fmt, __VA_ARGS__); assert(_Expression);} } while(0);
#endif
#else
...
#endif
#ifdef ANDROID
#define ASSERT2_EX(_Expression, _Msg)   do { if (!(_Expression)) {ALOGE("assert msg = %s\n", _Msg); if (g3dGbl.config.assertMode == 2) {assert(_Expression);}} } while(0);
#else
#define ASSERT2_EX(_Expression, _Msg)   do { if (!(_Expression)) {printf("assert msg = %s\n", _Msg); if (g3dGbl.config.assertMode == 2) {assert(_Expression);}} } while(0);
#endif
