 ...
 *  UFO_INFO
 *
 *  This struct should be 64bit aligned. Due to stuct UFO_INFO will pass between
...
typedef struct _UFO_INFO_
{
    ...
    */
    ...
    } ufo_lc_mem __attribute__((aligned(8))); // force 64-bit aligned.
...
((((range_addr + range_size - (range_addr & 0xFFFFC000) + (UFO_LEN_SURFACE_SIZE_ALIGN - 1)) / UFO_LEN_SURFACE_SIZE_ALIGN) + (UFO_LEN_SURFACE_SIZE_PADDING - 1)) & (~(UFO_LEN_SURFACE_SIZE_PADDING - 1))) + 128

#else
#define _yl_ufo_calc_length_buffer_size(range_addr, range_size) \
(((range_addr + range_size - (range_addr & 0xFFFFC000) + (UFO_LEN_SURFACE_SIZE_ALIGN - 1)) / UFO_LEN_SURFACE_SIZE_ALIGN) + (UFO_LEN_SURFACE_SIZE_PADDING - 1)) & (~(UFO_LEN_SURFACE_SIZE_PADDING - 1))
