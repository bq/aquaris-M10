...
#pragma warning(disable:4005)  // malloc redefinition
#include <stdlib.h>
#include <crtdbg.h>
#endif
...
/*-- memory allocation related macros --*/
    ...
    #define G3D_MALLOC(x)       g3dMAlloc3(x, __FILE__, __FUNCTION__ , __LINE__)
    #define G3D_CALLOC(x, y)    g3dCAlloc3(x, y, __FILE__, __FUNCTION__, __LINE__)
    #define G3D_FREE(x)         g3dFree3(x, __FILE__, __FUNCTION__, __LINE__)
    #define G3D_REALLOC(x, y)   g3dReAlloc3(x, y, __FILE__, __FUNCTION__, __LINE__)
    #define G3D_STRDUP(x)       g3dStrDup3(x, __FILE__, __FUNCTION__, __LINE__)
...
// We fix the path for the priorty :1.PE  2.HW  3.SW                        [2013.10.28]
...
//#define YL_SAME_CONSTBUF_DATA_WITH_SAME_ADDR      //not allocate memory from ringbuffer if the constant buffer is the same as previous one
...
/*
  ...
  driver may be able to do the patch by forcing lreg_vp0_vs0in_att_num to 1 when driver finds lreg_vp0_vs0in_att_num = 0 happens
 */
...
#if !defined(SEMU) && !defined(SEMU64) && !defined(QEMUL64)
 ...
 * and other(interger render target) cannot. The driver would 
 * split two lists, one for blendable render targets, one for 
 * unblendable reder target. [2015.3.28]
 */
#define YL_PATCH_HW_MRT_BLEND_INTERGER
#endif
...
#define G3D_PRIVATE_MEMORY_SLOT        3        // if optin is enabled, it means we will use dynamic flow to adjust private memory size
#define G3D_PRIVATE_MEMORY_SIZE        (32*16*G3D_NUM_SHADER_LANES*G3D_NUM_SHADER_WAVES) // 32 items x 16 bytes (vec4) x 64pixels x 24 (threads) = 0.75M
...
/* It's for rendering transfer path. Will not change for depth_stencil binding, but will stil add render_target binding */
...
#define G3D_CMD_END_LIST_FLUSH              (1<<0)  // For list-base use (every list should have this flag, and for the last list of the frame, it should also include G3D_CMD_END_FRAME_END)
#define G3D_CMD_END_FRAME_END               (1<<1)  // The flag means frame boundary (both for swap-buffer frame or sub-frame)
#define G3D_CMD_END_FRAME_SWAP              (1<<2)  // The flag is needed for swap-buffer frame
#define G3D_CMD_END_WAIT_PREV_FRAME_READY   (1<<3)  // The flag is for CL and defer mode, G3D will wait for previous frame done first, then process current frame
...
/* yuv buffer layout */
/* sync Renderbuffer & Texture buffer stride size */
#define YL_YUV_SYNC_RB_TX_STRIDE
#define YL_YUV_OUT_STRIDE 32
#define YL_YUV_YV12_Y_STRIDE 32
#define YL_YUV_YV12_UV_STRIDE 16
/* yv12 height stride: 1 or YL_ROTATE_WH_ALIGN */
#define YL_YUV_YV12_HEIGHT_STRIDE 1
/* yuyv height stride: 1 or YL_ROTATE_WH_ALIGN */
...
/* Patch conformance test :ES31-CTS.texture_buffer.texture_buffer_texture_buffer_range.
 * When receiving the not supported extension enum, we do not return GL_INVALID_ENUM.
 * Wait for conformance patch.
 */
#define YL_PATCH_TEXTURE_BUFFER
...
#define YL_PATCH_DRAW_INDIRECT_BUFFER_SIZE 20    //MAX of DispatchCompute: 12(3*4), DrawArrayIndirect: 16(4*4), DrawElementIndirect: 20(5*4)
