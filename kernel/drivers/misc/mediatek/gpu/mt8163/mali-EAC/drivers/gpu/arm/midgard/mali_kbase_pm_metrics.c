...
static u32 mtk_min_loading = 20;
static u32 mtk_max_loading = 40;
...
static enum hrtimer_restart dvfs_callback(struct hrtimer *timer)
{
	...
	//call platform dvfs action
	kbase_platform_dvfs_action(metrics->kbdev, action);
...
enum kbase_pm_dvfs_action kbase_pm_get_dvfs_action(struct kbase_device *kbdev)
{
	...
	if (utilisation < mtk_min_loading)
		action = KBASE_PM_DVFS_CLOCK_DOWN;
	else if (utilisation > mtk_max_loading)
		action = KBASE_PM_DVFS_CLOCK_UP;
	else
		action = KBASE_PM_DVFS_NOP;
...
void kbase_pm_metrics_set_threshold(u32 min_loading, u32 max_loading) {
	mtk_min_loading = min_loading;
	mtk_max_loading = max_loading;	
}
KBASE_EXPORT_TEST_API(kbase_pm_metrics_set_threshold)

void kbase_pm_metrics_get_threshold( u32 * min_loading, u32 * max_loading) {
	*min_loading = mtk_min_loading;
	*max_loading = mtk_max_loading;	
}
KBASE_EXPORT_TEST_API(kbase_pm_metrics_get_threshold)

void kbase_pm_metrics_set_timer(struct kbase_device *kbdev, u32 time) {
	unsigned long flags;
	spin_lock_irqsave(&kbdev->pm.metrics.lock, flags);
	kbdev->pm.platform_dvfs_frequency = time;
	spin_unlock_irqrestore(&kbdev->pm.metrics.lock, flags);
}
KBASE_EXPORT_TEST_API(kbase_pm_metrics_set_timer)

u32 kbase_pm_metrics_get_timer(struct kbase_device *kbdev) {
	unsigned long flags;
	u32 time;
	spin_lock_irqsave(&kbdev->pm.metrics.lock, flags);
	time = kbdev->pm.platform_dvfs_frequency;
	spin_unlock_irqrestore(&kbdev->pm.metrics.lock, flags);
	return time;
}
KBASE_EXPORT_TEST_API(kbase_pm_metrics_get_timer)
