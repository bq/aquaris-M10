...
#define PERI_NFI_CLK_SOURCE_SEL ((volatile P_U32)(mtk_infra_base+0x098))
...
u32 micron_pairpage_mapping(u32 page, bool high_to_low)
{
	...
	//UBI PEB size is 4M, page size is 16K when open combo nand
	//but UBI gets page number by offset in PEB / real page
	//so, page value may be 256 ~ 511. we should patch for this case
	u32 temp_page = page;
	if(temp_page > 255)
		page -= 256;
#endif
...
unsigned long nand_virt_to_phys_add(unsigned long va)
{
    ...
    {
        printk(KERN_ERR "[nand_virt_to_phys_add] ERROR, va=0x%lx, pgd invalid! \n", va);
        return 0;
    }
    ...
    {
        printk(KERN_ERR "[nand_virt_to_phys_add] ERROR, va=0x%lx, pmd invalid! \n", va);
        return 0;
    }
    ...
    printk(KERN_ERR "[nand_virt_to_phys_add] ERROR va=0x%lx, pte invalid! \n", va);
    return 0;
}
...
static bool mtk_nand_check_bch_error(struct mtd_info *mtd, u8 * pDataBuf,u8 * spareBuf,u32 u4SecIndex, u32 u4PageAddr, u32* bitmap)
{
    ...
    u32 u4ErrorNumDebug0,u4ErrorNumDebug1, i, u4ErrNum, j;
    u32 timeout = 0xFFFF;
    u32 correct_count = 0;
 ...
 for (j = 0; j <= u4SecIndex; ++j)
{
    ...
    } else
    {
	    ...
	    for(i = 0 ; i < u4ErrNum ; i++)
	    {
	        ...
	        {
	            pDataBuf[host->hw->nand_sec_size*j + u4ErrByteLoc] ^= (1<<(err_pos&0x7));
	        }
	        else
	        {
...
static bool mtk_nand_ready_for_read(struct nand_chip *nand, u32 u4RowAddr, u32 u4ColAddr, u16 sec_num, bool full, u8 * buf)
{
    ...
    if (full)
    {
        ...
        if (!phys)
        {
            printk(KERN_ERR "[mtk_nand_ready_for_read]convert virt addr (%lx) to phys add (%x)fail!!!", (unsigned long) buf, phys);
            return false;
        } else
        {
...
static bool mtk_nand_ready_for_write(struct nand_chip *nand, u32 u4RowAddr, u32 col_addr, bool full, u8 * buf)
{
    ...
    if (full)
    {
        ...
        if (!phys)
        {
            printk(KERN_ERR "[mt65xx_nand_ready_for_write]convert virt addr (%lx) to phys add fail!!!", (unsigned long) buf);
            return false;
        } else
        {
            DRV_WriteReg32(NFI_STRADDR_REG32, phys);
...
static bool mtk_nand_dma_read_data(struct mtd_info *mtd, u8 * buf, u32 length)
{
    ...
    do_gettimeofday(&stimer);
#endif
    //printk("[xl] dma read buf in 0x%lx\n", (unsigned long)buf);
    //sg_init_one(&sg, buf, length);
    //printk("[xl] dma read buf out 0x%lx\n", (unsigned long)buf);
...
static bool mtk_nand_dma_write_data(struct mtd_info *mtd, u8 * pDataBuf, u32 u4Size)
{
    ...
    do_gettimeofday(&stimer);
#endif
    //printk("[xl] dma write buf in 0x%lx\n", (unsigned long)pDataBuf);
    //sg_init_one(&sg, pDataBuf, u4Size);
    //printk("[xl] dma write buf out 0x%lx\n", (unsigned long)pDataBuf);
    //dma_map_sg(&(mtd->dev), &sg, 1, dir);
    NFI_CLN_REG16(NFI_CNFG_REG16, CNFG_BYTE_RW);
    DRV_Reg16(NFI_INTR_REG16);
...
int mtk_nand_exec_read_page(struct mtd_info *mtd, u32 u4RowAddr, u32 u4PageSize, u8 * pPageBuf, u8 * pFDMBuf)
{
    ...
    if (((unsigned long) pPageBuf % 16) && local_buffer_16_align)
    {
		...
		//printk("[xl] read buf (1) 0x%lx\n",(unsigned long)buf);
    } else
    {
    	  if(virt_addr_valid (pPageBuf)==0)
    	  { // It should be allocated by vmalloc
    	  	buf = local_buffer_16_align;
			//printk("[xl] read buf (2) 0x%lx\n",(unsigned long)buf);
    	  }
    	  else
    	  {    	  	
        	buf = pPageBuf;
			//printk("[xl] read buf (3) 0x%lx\n",(unsigned long)buf);
        }
    }
    ...
    if (buf == local_buffer_16_align)
	{
		...
		//printk("[xl] mtk_nand_exec_read_page memcpy 0x%x 0x%x\n", pPageBuf[0],buf[0]);
	}
	//else
		//printk("[xl] mtk_nand_exec_read_page no memcpy 0x%x 0x%x\n", pPageBuf[0],buf[0]);
	if(bRet != ERR_RTN_SUCCESS)
	{
...
//printk("[xl] mtk_nand_read_page buf 0x%lx\n", (unsigned long)buf);
...
static void NFI_GPIO_SET_FIELD(unsigned long reg, U32 field, U32 val) 
{
...
__attribute__((aligned(64))) static u8 temp_buffer_xl[LPAGE + LSPARE];
__attribute__((aligned(64))) static u8 temp_buffer_xl_rd[LPAGE + LSPARE];
...
int mtk_nand_unit_test(struct nand_chip *nand_chip, struct mtd_info *mtd)
{
    ...
    printk("[P] %x\n", p);
printk("[xiaolei] bias = 0x%x", *(volatile u32*)(GPIO_BASE+0xE20));
printk("[xiaolei] ACC = 0x%x", *(volatile u32*)(NFI_BASE+0x00C));
    ...
    for(m=0;m<32;m++)
        MSG(INIT,"[5]0x%x %x %x %x\n", *((int *)temp_buffer_xl_rd+m*4), *((int *)temp_buffer_xl_rd+1+m*4), *((int *)temp_buffer_xl_rd+2+m*4), *((int *)temp_buffer_xl_rd+3+m*4));
	...
	//for(m = 0; m < 32; m++)
        //MSG(INIT,"[1]0x%x %x %x %x\n", *((int *)local_buffer+m*4), *((int *)local_buffer+1+m*4), *((int *)local_buffer+2+m*4), *((int *)local_buffer+3+m*4));
        mtk_nand_erase(mtd, j*p);
        memset(temp_buffer_xl_rd, 0x00, 16384);
        if(mtk_nand_read_page(mtd, nand_chip, temp_buffer_xl_rd, j*p))
            printk("Read page 0x%x fail!\n", j*p);
        MSG(INIT,"[2]0x%x %x %x %x\n", *(int *)temp_buffer_xl_rd, *((int *)temp_buffer_xl_rd+1), *((int *)temp_buffer_xl_rd+2), *((int *)temp_buffer_xl_rd+3));
        ...
        for (k = 0; k < p; k++)
        {
			...
			for(m = 0; m < 32; m++)
            MSG(INIT,"[1]0x%x %x %x %x\n", *((int *)temp_buffer_xl+m*4), *((int *)temp_buffer_xl+1+m*4), *((int *)temp_buffer_xl+2+m*4), *((int *)temp_buffer_xl+3+m*4));
        
            if(mtk_nand_write_page(mtd, nand_chip,0, 0, temp_buffer_xl/*(u8 *)patternbuff*/,0, j*p+k, 0, 0))
        ...
        //#endif
            ...
            if(memcmp(temp_buffer_xl/*(u8 *)patternbuff*/, temp_buffer_xl_rd, 512/*g_page_size*/))
            {
...
static int mtk_nand_probe(struct platform_device *pdev)
{
	...
	if (mtk_gpio_node == NULL) {
		...
		MSG(INIT, "of_iomap for gpio base @ 0x%p\n", mtk_gpio_base);
	}
	...
	if (mtk_efuse_node == NULL) {
		...
		MSG(INIT, "of_iomap for efuse base @ 0x%p\n", mtk_efuse_base);
	}
	...
	if (mtk_infra_node == NULL) {
		...
		MSG(INIT, "of_iomap for infra base @ 0x%p\n", mtk_infra_base);
	}
    //dt modify
    #endif
	...
	BUG_ON(!hw);
	of_property_read_u32(pdev->dev.of_node, "nfi_bus_width",     (u32 *)&hw->nfi_bus_width);
	of_property_read_u32(pdev->dev.of_node, "nfi_access_timing", (u32 *)&hw->nfi_access_timing);
	of_property_read_u32(pdev->dev.of_node, "nfi_cs_num",        (u32 *)&hw->nfi_cs_num);
	of_property_read_u32(pdev->dev.of_node, "nand_sec_size",     (u32 *)&hw->nand_sec_size);
	of_property_read_u32(pdev->dev.of_node, "nand_sec_shift",    (u32 *)&hw->nand_sec_shift);
	of_property_read_u32(pdev->dev.of_node, "nand_ecc_size",     (u32 *)&hw->nand_ecc_size);
	of_property_read_u32(pdev->dev.of_node, "nand_ecc_bytes",    (u32 *)&hw->nand_ecc_bytes);
	of_property_read_u32(pdev->dev.of_node, "nand_ecc_mode",     (u32 *)&hw->nand_ecc_mode);
	#else
	hw = (struct mtk_nand_host_hw *)pdev->dev.platform_data;
    BUG_ON(!hw);
	...
	if(dma_set_mask(&pdev->dev,DMA_BIT_MASK(32)))
	{
		...
		printk("[NAND] set dma mask fail\n");
	}
	else
		printk("[NAND] set dma mask ok\n");
...
//	struct nand_chip *chip = mtd->priv;
	...
	//int val = -1;   // for test
        ...
        if(host->saved_para.suspend_flag==1)
        {
			...
			#endif
			udelay(200);
		    MSG(INIT, "[NFI] delay 200us for power on reset flow!\n");
			nand_enable_clock();
