...
static INT_32 wlanProbe(PVOID pvData)
{
	...
	do {
...
#if CFG_SUPPORT_CFG_FILE
		{
			...
			u4ConfigReadLen = 0;
			DBGLOG(INIT, TRACE, ("CFG_FILE: Read File...\n"));
			if (pucConfigBuf) {
				if (kalReadToFile("/data/misc/wifi.cfg",
						pucConfigBuf, WLAN_CFG_FILE_BUF_SIZE, &u4ConfigReadLen) == 0) {
					DBGLOG(INIT, TRACE, ("CFG_FILE: Read /data/misc/wifi.cfg\n"));

				} else if (kalReadToFile("/data/misc/wifi/wifi.cfg",
						pucConfigBuf, WLAN_CFG_FILE_BUF_SIZE, &u4ConfigReadLen) == 0) {
					DBGLOG(INIT, TRACE, ("CFG_FILE: Read /data/misc/wifi/wifi.cfg\n"));
				} else if (kalReadToFile("/etc/firmware/wifi.cfg",
						pucConfigBuf, WLAN_CFG_FILE_BUF_SIZE, &u4ConfigReadLen) == 0) {
					DBGLOG(INIT, TRACE, ("CFG_FILE: Read /etc/firmware/wifi.cfg\n"));
				}
