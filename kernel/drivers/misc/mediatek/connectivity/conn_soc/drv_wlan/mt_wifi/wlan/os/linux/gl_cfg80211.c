...
int mtk_cfg80211_testmode_hs20_cmd(IN struct wiphy *wiphy, IN void *data, IN int len)
{
	...
	DBGLOG(INIT, TRACE, ("[%s] Cmd Type (%d)\n", __func__, prParams->CmdType));
	...
	if (prParams) {
		...
		case HS20_CMD_ID_SET_BSSID_POOL:
			DBGLOG(INIT, TRACE, ("fgBssidPoolIsEnable=%d, ucNumBssidPool=%d\n", 
			...
			break;
		default:
			DBGLOG(INIT, TRACE, ("[%s] Unknown Cmd Type (%d)\n", __func__, prParams->CmdType));
...
#if 0
	DBGLOG(INIT, TRACE, ("poorlink set param valid(%d)rssi(%d)linkspeed(%d)\n",
...
int mtk_cfg80211_vendor_get_capabilities(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	DBGLOG(INIT, TRACE, ("%s for vendor command \r\n", __func__));
	...
	if (!skb) {
		DBGLOG(INIT, ERROR, ("%s allocate skb failed:%x\n", __func__, i4Status));
		return -ENOMEM;
	}
...
int mtk_cfg80211_vendor_set_config(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	if (!prWifiScanCmd) {
		DBGLOG(INIT, ERROR, ("Can not alloc memory for PARAM_WIFI_GSCAN_CMD_PARAMS\n"));
		return -ENOMEM;
	}

	DBGLOG(INIT, INFO, ("%s for vendor command: data_len=%d \r\n", __func__, data_len));
	...
	for (k = GSCAN_ATTRIBUTE_NUM_BUCKETS; k <= GSCAN_ATTRIBUTE_REPORT_EVENTS; k++) {
		if (attr[k]) {
			...
			case GSCAN_ATTRIBUTE_NUM_BUCKETS:
				...
				len_basic += NLA_ALIGN(attr[k]->nla_len);
				DBGLOG(INIT, TRACE, ("attr=0x%x, num_buckets=%d nla_len=%d, \r\n",
	...
	DBGLOG(INIT, TRACE, ("+++basic attribute size=%d pbucket=%p\r\n", len_basic, pbucket));
	...
	for (i = 0; i < prWifiScanCmd->num_buckets; i++) {
		...
		for (k = GSCAN_ATTRIBUTE_NUM_BUCKETS; k <= GSCAN_ATTRIBUTE_REPORT_EVENTS; k++) {
			if (attr[k]) {
				...
				case GSCAN_ATTRIBUTE_BUCKET_NUM_CHANNELS:
					...
					len_bucket += NLA_ALIGN(attr[k]->nla_len);
					DBGLOG(INIT, TRACE, ("bucket%d: attr=0x%x, num_channels=%d nla_len = %d, \r\n",
		...
		DBGLOG(INIT, TRACE, ("+++pure bucket size=%d pbucket=%p \r\n", len_bucket, pbucket));
	...
	DBGLOG(INIT, TRACE, ("base_period=%d, num_buckets=%d, bucket0: %d %d %d %d",
	...
	DBGLOG(INIT, TRACE, ("num_channels=%d, channel0=%d, channel1=%d; num_channels=%d, channel0=%d, channel1=%d",
...
int mtk_cfg80211_vendor_set_scan_config(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	DBGLOG(INIT, TRACE, ("attr=0x%x, attr2=0x%x ", *(UINT_32 *) attr[GSCAN_ATTRIBUTE_NUM_AP_PER_SCAN],
	       *(UINT_32 *) attr[GSCAN_ATTRIBUTE_REPORT_THRESHOLD]));

	DBGLOG(INIT, TRACE, ("max_ap_per_scan=%d, report_threshold=%d num_scans=%d \r\n",
					       ...
					       const void *data, int data_len)
{
	...
	DBGLOG(INIT, INFO, ("%s for vendor command: data_len=%d \r\n", __func__, data_len));
	for (i = 0; i < 6; i++)
		DBGLOG(INIT, LOUD, ("0x%x 0x%x 0x%x 0x%x \r\n",
	...
	for (k = GSCAN_ATTRIBUTE_RSSI_SAMPLE_SIZE; k <= GSCAN_ATTRIBUTE_SIGNIFICANT_CHANGE_FLUSH; k++) {
		if (attr[k]) {
			...
			case GSCAN_ATTRIBUTE_NUM_AP:
				...
				len_basic += NLA_ALIGN(attr[k]->nla_len);
				DBGLOG(INIT, TRACE, ("attr=0x%x, num_ap=%d nla_len=%d, \r\n",
	...
	DBGLOG(INIT, TRACE, ("+++basic attribute size=%d flush=%d\r\n", len_basic, flush));
	...
	for (i = 0; i < rWifiChangeCmd.num_ap; i++) {
		...
		if (((i + 1) % 4 == 0) || (i == rWifiChangeCmd.num_ap - 1))
			DBGLOG(INIT, TRACE, ("ap[%d], len_aplist=%d\n", i, len_aplist));
		else
			DBGLOG(INIT, TRACE, ("ap[%d], len_aplist=%d \t", i, len_aplist));
...
int mtk_cfg80211_vendor_enable_scan(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	if ((data == NULL) || !data_len)
		goto nla_put_failure;
	DBGLOG(INIT, INFO, ("%s for vendor command: data_len=%d, data=0x%x 0x%x\r\n",
						 ...
						 const void *data, int data_len)
{
	...
	DBGLOG(INIT, INFO, ("gFullScanResultsEn=%d, \r\n", gFullScanResultsEn));
...
int mtk_cfg80211_vendor_get_scan_results(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	if ((data == NULL) || !data_len)
		goto nla_put_failure;
	DBGLOG(INIT, TRACE, ("%s for vendor command: data_len=%d \r\n", __func__, data_len));
	for (i = 0; i < 2; i++)
		DBGLOG(INIT, LOUD, ("0x%x 0x%x 0x%x 0x%x \r\n", *((UINT_32 *) data + i * 4),
	...
	DBGLOG(INIT, TRACE, ("number=%d, flush=%d \r\n", get_num, flush));
	...
	if (g_GetResultsBufferedCnt > 0) {
		...
		if (i == MAX_BUFFERED_GSCN_RESULTS)
			DBGLOG(INIT, TRACE, ("all buffered results are invalid, unexpected case \r\n"));
		DBGLOG(INIT, TRACE, ("BufferedResultReportIndex[%d] i = %d real_num[%d] get_num[%d] \r\n",
...
int mtk_cfg80211_vendor_get_channel_list(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	DBGLOG(INIT, INFO, ("get channel list: band=%d \r\n", band));
...
int mtk_cfg80211_vendor_llstats_get_info(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	DBGLOG(INIT, INFO, ("%s for vendor command \r\n", __func__));
...
int mtk_cfg80211_vendor_event_complete_scan(struct wiphy *wiphy, struct wireless_dev *wdev, WIFI_SCAN_EVENT complete)
{
	...
	if (!skb) {
		DBGLOG(INIT, ERROR, ("%s allocate skb failed\n", __func__));
		return -ENOMEM;
	}
...
int mtk_cfg80211_vendor_event_scan_results_avaliable(struct wiphy *wiphy, struct wireless_dev *wdev, UINT_32 num)
{
	...
	DBGLOG(INIT, INFO, ("%s for vendor command %d  \r\n", __func__, num));
