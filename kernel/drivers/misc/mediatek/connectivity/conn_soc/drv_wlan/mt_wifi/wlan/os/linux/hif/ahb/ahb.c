...
VOID glGetChipInfo(GLUE_INFO_T *GlueInfo, UINT_8 *pucChipBuf)
{
	...
	HifInfo = &GlueInfo->rHifInfo;
	DBGLOG(INIT, INFO, ("glGetChipInfo ChipID = 0x%x\n", HifInfo->ChipID));
...
INT_32 glBusSetIrq(PVOID pvData, PVOID pfnIsr, PVOID pvCookie)
{
	...
	if (of_property_read_u32_array(node, "interrupts", irq_info, ARRAY_SIZE(irq_info))) {
		...
		DBGLOG(INIT, INFO, ("WIFI-OF: get interrupt flag(0x%x)\n", irq_flags));
	}
...
kalDevPortRead(IN GLUE_INFO_T *GlueInfo, IN UINT_16 Port, IN UINT_16 Size, OUT PUINT_8 Buf, IN UINT_16 MaxBufSize)
{
	...
	if ((HifInfo->fgDmaEnable == TRUE) && (HifInfo->DmaOps != NULL)
		...
		if (RtnVal <= 0) {
			while (1)
				DBGLOG(INIT, ERROR, ("fatal error1! reset DMA!\n"));
		}
		HifInfo->HifDmaWaitFlg = 0;
#else
		...
		do {
			...
			if (time_before(jiffies, PollTimeout)) {
				 ...
				 */
				DBGLOG(INIT, INFO, ("DMA LoopCnt > 100000... (%lu %lu)\n", jiffies, PollTimeout));
/* LabelErr: */
...
/* UCHAR AeeBuffer[100]; */
				...
				if ((RegValChip != 0) && (RegValLP == 0)) {
					...
					DBGLOG(INIT, WARN, ("<WLANRXERR><vend_samp.lin> 0x%x 0x%x 0x%x\n",
		...
		do {
			if (LoopCnt++ > 100000) {
				/* TODO: impossible! reset DMA */
				DBGLOG(INIT, ERROR, ("fatal error2! reset DMA!\n"));
				break;
			}
...
/* UCHAR AeeBuffer[100]; */
					...
					if ((RegValChip != 0) && (RegValLP == 0)) {
						/* HIF clock & CONNSYS is powered on but own fails */
						/* for the case we just printk kernel error message */
						DBGLOG(INIT, WARN, ("<WLANTXERR><vend_samp.lin> 0x%x 0x%x 0x%x\n",
						       *(volatile unsigned int *)0xF0000024,
						       (UINT_32) RegValChip, (UINT_32) RegValLP));
						return TRUE;
					}
