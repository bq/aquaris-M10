	...
	if (*f_pos > 0) {
...
#endif
		{
			...
			if (max_num > gCoexBuf1.availSize)
				...
				DBGLOG(INIT, TRACE, ("round to %d bytes due to local buffer size limitation\n", max_num));
...
INT_32 procInitProcfs(struct net_device *prDev, char *pucDevName)
{
	...
	if (init_net.proc_net == (struct proc_dir_entry *)NULL) {
		DBGLOG(INIT, WARN, ("init proc fs fail: proc_net == NULL\n"));
		return -ENOENT;
	}
	...
	if (prEntry == NULL) {
		DBGLOG(INIT, WARN, ("Unable to create /proc entry\n\r"));
		return -1;
	}
