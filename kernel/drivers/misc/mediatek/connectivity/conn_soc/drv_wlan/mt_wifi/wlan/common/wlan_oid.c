...
wlanoidSetP2pMode(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	ASSERT(prAdapter);
	ASSERT(pu4SetInfoLen);
	DBGLOG(INIT, INFO, ("wlanoidSetP2pMode init status=0x%x\n", status));
...
#endif
	DBGLOG(INIT, INFO, ("wlanoidSetP2pMode return status=0x%x\n", status));
	return status;
		     ...
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	for (i = 0; i < prCmdGscnParam->num_buckets; i++) {
		...
		DBGLOG(INIT, INFO, ("prCmdGscnParam->buckets[%d] has channel: ", i));
		for (j = 0; j < prCmdGscnParam->buckets[i].num_channels; j++)
			DBGLOG(INIT, INFO, (" %d,  ", prCmdGscnParam->buckets[i].channels[j].channel));
		DBGLOG(INIT, INFO, ("\n"));
	}
#endif
