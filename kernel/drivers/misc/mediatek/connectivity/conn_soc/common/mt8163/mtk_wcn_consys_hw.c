...
INT32 mtk_wcn_consys_hw_reg_ctrl(UINT32 on,UINT32 co_clock_en)
{
	...
	if(on)
	{
		...
		/*12.poll CONNSYS CHIP ID until 8163 is returned		0x18070008	32'h8163 */
		while (retry-- > 0)
		{
			consysHwChipId = CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_CHIP_ID_OFFSET);
			if ((consysHwChipId == 0x6752) ||
				(consysHwChipId == 0x6582) ||
				(consysHwChipId == 0x6572) ||
				(consysHwChipId == 0x6571) ||
