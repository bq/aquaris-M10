...
static void __clear_emi_mpu_vio(void)
{
    ...
    ret = KREE_TeeServiceCall(emi_session, TZCMD_EMI_CLR,\
                            TZ_ParamTypes4(TZPT_VALUE_OUTPUT, TZPT_VALUE_OUTPUT, TZPT_VALUE_OUTPUT, TZPT_VALUE_OUTPUT),\
                            param);
    if (ret != TZ_RESULT_SUCCESS)
    {
       printk("%s error: %s\n", __FUNCTION__, TZ_GetErrorString(ret));
...
static irqreturn_t mpu_violation_irq(int irq, void *dev_id)
{
    ...
    pr_err("EMI MPU violation handler@ap.\n");
...
#else /* CONFIG_MTK_IN_HOUSE_TEE_SUPPORT */
    ...
    dbg_s = emi_reg_read(EMI_MPUS);
    dbg_t = emi_reg_read(EMI_MPUT);
    ...
    switch (domain_ID) {
    case 0:
        dbg_pqry = emi_reg_read(EMI_MPUP);
    break;
    case 1:
        dbg_pqry = emi_reg_read(EMI_MPUQ);
    break;
    case 2:
        dbg_pqry = emi_reg_read(EMI_MPUR);
    break;
    case 3:
        dbg_pqry = emi_reg_read(EMI_MPUY);
    break;
    case 4:
        dbg_pqry = emi_reg_read(EMI_MPUP2);
    break;
    case 5:
        dbg_pqry = emi_reg_read(EMI_MPUQ2);
    break;
    case 6:
        dbg_pqry = emi_reg_read(EMI_MPUR2);
    break;
    case 7:
        dbg_pqry = emi_reg_read(EMI_MPUY2);
    break;
    default:
        dbg_pqry = 0;
    break;
...
#if 0
    /* only for test EMI register R/W */
    pr_err("EMI REG: CONA = 0x%x.\n", emi_reg_read(EMI_CONA));
    pr_err("EMI REG: MPUM = 0x%x.\n", emi_reg_read(EMI_MPUM));
    pr_err("EMI REG: Write 0x55 to MPUM\n");
    emi_reg_write(0x55, EMI_MPUM);
    pr_err("EMI REG: MPUM = 0x%x.\n", emi_reg_read(EMI_MPUM));
    pr_err("EMI REG: Write 0x0 to MPUM\n");
    emi_reg_write(0x0, EMI_MPUM);
    pr_err("EMI REG: MPUM = 0x%x.\n", emi_reg_read(EMI_MPUM));
#endif    
    return 0;
}
