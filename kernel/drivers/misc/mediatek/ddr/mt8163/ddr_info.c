...
struct ddr_info_driver ddr_info_driver = {
	.driver = {
		   .name = "ddr_info",
...
static int ddr_reg_of_iomap(struct ddr_reg_base *reg_base)
{
	...
	if (node) {
		reg_base->ddrphy1_base = reg_base->ddrphy0_base = of_iomap(node, 0);
    ...
    if (node) {
		reg_base->dramc1_base = reg_base->dramc0_base = of_iomap(node, 0);
	...
	if (node) {
		reg_base->dramc1_nao_base = reg_base->dramc0_nao_base = of_iomap(node, 0);
		ddr_info_warn("DRAMC_NAO ADDRESS %p,\n", reg_base->dramc0_nao_base);
	} else {
		ddr_info_warn("can't find DRAMC0_NA compatible node\n");
		return -1;
	}
#else
...
static ssize_t ddr_drv_info_show(struct device_driver *driver, char *buf)
{
	...
	ddr_info_dbg("ddr_drv_info_show, buf:0x%llx, dramc:0x%llx\n", (u64)buf, (u64)ddr_info_driver.reg_base.dramc0_base);
...
static ssize_t ddr_drv_show(struct device_driver *driver, char *buf)
{
	...
	ddr_info_dbg("ddr_drv_show, buf:0x%llx, dramc:0x%llx\n", (u64)buf, (u64)ddr_info_driver.reg_base.dramc0_base);
	...
	ddr_info_dbg("ddr_drv_show, read done\n");
...
static int __init ddr_info_init(void)
{
	...
	ddr_info_dbg("ddr_info_init, phy:0x%llx, ao:0x%llx, nao:0x%llx\n", (u64)ddr_info_driver.reg_base.ddrphy0_base,
		(u64)ddr_info_driver.reg_base.dramc0_base,
		(u64)ddr_info_driver.reg_base.dramc0_nao_base);
