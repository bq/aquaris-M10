...
static void external_display_debug_output(unsigned long param)
{
...
static external_display_read_reg(READ_REG_VALUE reg_val)
{
    hdmi_read_reg(reg_val);
}

static external_display_write_reg(hdmi_device_write reg_val)
{
    ...
    switch (cmd)
    {
		...
		case MTK_HDMI_AUDIO_SETTING:
		 {
			if (copy_from_user(&audio_para, (void __user *)arg, sizeof(audio_para))) {
				...
				external_display_set_audio_param(&audio_para);
			}
			break;
		}
#endif
	   ...
	   case MTK_HDMI_COLOR_DEEP:
       {
           if (copy_from_user(&data_info, (void __user *)arg, sizeof(data_info))) {
               ...
               r = -EFAULT;
                ...
                external_display_set_deepcolor(data_info.u4Data1 & 0xFF,data_info.u4Data2 & 0xFF);
           }
       ...
       {           
               ...
               r = -EFAULT;
                ...
                hdmi_install_hdcpkey((UINT8*)&key);
           }
           break;
       }
        default:
...
{
    printk("[EXTD]%s\n", __func__);
	printk(">> mtk_ext_disp_mgr_init\n");
#if 0
    if (platform_device_register(&external_display_device))
    {
        printk("[EXTD]failed to register hdmitx device\n");
        return -1;
    }
#endif
    ...
    {
        printk("[EXTD]failed to register hdmitx driver\n");
        return -1;
    }
