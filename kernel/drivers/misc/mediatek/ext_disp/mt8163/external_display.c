...
static int _convert_disp_input_to_ovl(OVL_CONFIG_STRUCT *dst, ext_disp_input_config* src)
{
	if(src && dst)
	{
		...
		if (src->buff_source == DISP_BUFFER_ALPHA) {
			dst->source = OVL_LAYER_SOURCE_RESERVED;  /*dim layer, constant alpha*/
		} else if (src->buff_source == DISP_BUFFER_ION || src->buff_source == DISP_BUFFER_MVA) {
			dst->source = OVL_LAYER_SOURCE_MEM;  /*from memory*/
		} else {
			EXT_DISP_ERR("unknow source = %d", src->buff_source);
