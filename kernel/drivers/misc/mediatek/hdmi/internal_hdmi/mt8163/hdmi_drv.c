...
const char *hdmi_use_module_name_spy(HDMI_REF_MODULE_ENUM module)
{
	switch (module) {
	case AP_CCIF0:
		return "mediatek,APMIXED";	/* TVD//PLL */
	case TOPCK_GEN:
...
int hdmi_internal_power_on(void)
{
    ...
    HDMI_DRV_FUNC();
    //upmu_set_vproc_vosel(63);
	//upmu_set_vproc_vosel_on(63);
	...
	vWriteHdmiSYSMsk(HDMI_SYS_CFG110,DPI1_PIXEL_CLK_EN|HDMI_PIXEL_CLK_EN|HDMI_PLL_CLK_EN, DPI1_PIXEL_CLK|HDMI_PIXEL_CLK|HDMI_PLL_CLK); 
...
void hdmi_internal_power_off(void)
{
    ...
    #endif
	vWriteHdmiSYSMsk(HDMI_SYS_CFG110,DPI1_PIXEL_CLK_DISABLE|HDMI_PIXEL_CLK_DISABLE|HDMI_PLL_CLK_DISABLE, DPI1_PIXEL_CLK|HDMI_PIXEL_CLK|HDMI_PLL_CLK); 
...
void hdmi_read(unsigned long u2Reg, unsigned int *p4Data)
{
	...
	case 0x1401c000:
		internal_hdmi_read(hdmi_ref_reg[DPI1_REG] + u2Reg - 0x1401d000, p4Data);
		break;
...
void hdmi_write(unsigned long u2Reg, unsigned int u4Data)
{
	...
	case 0x1401c000:
		internal_hdmi_write(hdmi_ref_reg[DPI1_REG] + u2Reg - 0x1401d000, u4Data);
		break;

	default:
		break;
	}
    printk("Reg write= 0x%08lx, data = 0x%08x\n", u2Reg, u4Data);
}
