...
void OV5670Ofilm_otpupdate()
{
	...
	OV5670ofilm_read_all(0x7021+3*(valueGroup-1),0x7020+3*(valueGroup),tempWB);
       ...
       kal_uint16 rg = (tempWB[0]<<2) +( (tempWB[2]&0xc0)>>6);
	kal_uint16 bg = (tempWB[1]<<2) +( (tempWB[2]&0x30)>>4);
	
	kal_uint16 RG_Ratio_Typical = 0x147;
	kal_uint16 BG_Ratio_Typical = 0x163;
