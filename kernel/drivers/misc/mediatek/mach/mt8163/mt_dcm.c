...
#define unlikely(a)  (a)
#define BUG() (__asm__ __volatile__ ("dsb\n\t"))
#else
#endif /* #if !defined(__KERNEL__) */
...
int dcm_armcore_pll_clkdiv(int pll, int div)
{
	...
	reg_write(INFRACFG_TOP_CKMUXSEL, aor(reg_read(INFRACFG_TOP_CKMUXSEL), ~(3 << 0), 0 << 0));

	reg_write(TOPCKGEN_CLK_MISC_CFG_0,
		  aor(reg_read(TOPCKGEN_CLK_MISC_CFG_0), ~(0x03 << 4), mux << 4));

	reg_write(INFRACFG_TOP_CKMUXSEL, aor(reg_read(INFRACFG_TOP_CKMUXSEL), ~(3 << 0), pll << 0));
	reg_write(INFRACFG_TOP_CKDIV1, aor(reg_read(INFRACFG_TOP_CKDIV1), ~(0x1f << 0), div << 0));
...
int dcm_mcusys(ENUM_MCUSYS_DCM on)
{
	if (on == MCUSYS_DCM_OFF) {
		...
		MCUSYS_SMC_WRITE(MCUCFG_L2C_SRAM_CTRL,
				 aor(reg_read(MCUCFG_L2C_SRAM_CTRL), ~(1 << 0), 0));
		/* bus_fabric_dcm_ctrl */
		...
		/* MCUSYS bus DCM */
		MCUSYS_SMC_WRITE(MCUCFG_CCI_CLK_CTRL,
				 aor(reg_read(MCUCFG_CCI_CLK_CTRL), ~(0x1 << 8), (1 << 8)));
		/* L2C SRAM DCM */
		MCUSYS_SMC_WRITE(MCUCFG_L2C_SRAM_CTRL,
				 aor(reg_read(MCUCFG_L2C_SRAM_CTRL), ~(1 << 0), (1 << 0)));
		/* bus_fabric_dcm_ctrl */
		MCUSYS_SMC_WRITE(MCUCFG_BUS_FABRIC_DCM_CTRL,
...
int dcm_emi_dbc(ENUM_EMI_DCM mode, int cnt)
{
	if (mode == EMI_DCM_1PLL_MODE) {
		reg_write(INFRACFG_DFS_MEM_DCM_CTRL,
			  aor(reg_read(INFRACFG_DFS_MEM_DCM_CTRL), ~(0x1f << 9), cnt));
	} else {
		reg_write(INFRACFG_MEM_DCM_CTRL,
			  aor(reg_read(INFRACFG_MEM_DCM_CTRL), ~(0x1f << 9), cnt));
	}
...
#define REG_DUMP(addr)	\
	dcm_info("%-30s(0x%08lx): 0x%08lx\n", #addr, addr, (unsigned long)reg_read(addr))
...
static int dcm_cpu_notify(struct notifier_block *self, unsigned long action, void *hcpu)
{
...
static ssize_t dcm_debug_mon_store(struct kobject *kobj, struct kobj_attribute *attr,
...
void debug_mon(int major, int minor, int fqmtr_div)
{
	...
	if (major & CKGEN_CK) {
		...
		reg_write(TOP_MISC_TEST_MODE_CFG,
			  aor(reg_read(TOP_MISC_TEST_MODE_CFG), 0xfc00ffff, 0x18c0000));
		...
		/* fqmtr divider */
		fqmtr_div--;
		reg_write(CLK_MISC_CFG_0,
			  aor(reg_read(CLK_MISC_CFG_0), ~((0x0ff << 24)), fqmtr_div << 24));
	...
	if (major & ABIST_CLK) {
		...
		if (AD_ARMCA7PLL_754M_CORE_CK) {
			...
			fqmtr_div >>= 4;
			reg_write(CLK_MISC_CFG_0,
				  aor(reg_read(CLK_MISC_CFG_0), ~((0x0ff << 16)), (arm_k1 << 16)));
		}
		reg_write(CLK_MISC_CFG_0,
			  aor(reg_read(CLK_MISC_CFG_0), ~((0x0ff << 24)), (fqmtr_div << 24)));
