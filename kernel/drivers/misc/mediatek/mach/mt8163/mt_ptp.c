...
#else
		if ((det->ctrl_id != PTP_CTRL_GPUSYS) || !ptp_dis_gpu)
#endif
...
static void ptp_init_det(struct ptp_det *det, struct ptp_devinfo *devinfo)
{
	...
	case PTP_DET_MCUSYS:
		...
		det->VMIN	= PTP_VOLT_TO_PMIC_VAL(105000); // override default setting
		break;
...
#ifndef CONFIG_MTK_INHOUSE_GPU
			if ((ctrl->det_id != PTP_DET_GPUSYS) && (det->ctrl_id != PTP_CTRL_GPUSYS)) {
#else
			if (((ctrl->det_id != PTP_DET_GPUSYS) && (det->ctrl_id != PTP_CTRL_GPUSYS)) || !ptp_dis_gpu) {
#endif
...
//	atomic_set(&ptp_init01_cnt, 0);
	for_each_ctrl(ctrl) {
#ifndef CONFIG_MTK_INHOUSE_GPU
		if (ctrl->det_id != PTP_DET_GPUSYS)
#else
		if ((ctrl->det_id != PTP_DET_GPUSYS) || !ptp_dis_gpu)
#endif
			ptp_init_ctrl(ctrl);
	}
