...
int kdCISModulePowerOn(CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx, char *currSensorName, BOOL On, char* mode_name)
{
    ...
    if (On) {
       ...
       if ((pinSetIdx == 0 && (0 == strcmp(SENSOR_DRVNAME_GC2355_MIPI_RAW, currSensorName)))
					||(pinSetIdx == 1 && (0 == strcmp(SENSOR_DRVNAME_GC0310_MIPI_YUV, currSensorName)))) {
			mdelay(1);
		...
		*/
        ...
        else if( (pinSetIdx == 0 && (0 == strcmp(SENSOR_DRVNAME_GC2355_MIPI_RAW, currSensorName)))
					||(pinSetIdx ==1 && (0 == strcmp(SENSOR_DRVNAME_GC0310_YUV, currSensorName)))) {
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
    						if (mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])) {PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n"); }
    						if (mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])) {PK_DBG("[CAMERA LENS] set gpio mode failed!! \n"); }
    						if (mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_DIR_OUT)) {PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n"); }
