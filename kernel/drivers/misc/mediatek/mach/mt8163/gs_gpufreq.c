...
static s32 g_fake_vgpu_sel_default_called = 0;
static s32 g_fake_vgpu_disable_called = 0;
...
static s64 g_set_load_call_count = 0;
static s64 g_set_index_call_count = 0;
static s64 g_touch_boost_call_count = 0;
static s64 g_thermal_call_count = 0;
static s64 g_low_battery_volt_call_count = 0;
static s64 g_low_battery_volume_call_count = 0;

/*****************************************************************************
 * Fake external values
 *****************************************************************************/
static u32 g_fake_freq_to_thermal = 0;
static u32 g_fake_volt_to_thermal = 0;
...
static u32      g_load              = 0; /* Current GPU loading. */
static u32      g_limited_power_mW  = 0; /* Max power allowed, by thermal module. */
static u32      g_bottom_freq       = 0; /* Lowest frequest expected */
...
/*
static gpufreq_power_limit_notify g_power_limit_notify = NULL;
#ifdef MT_GPUFREQ_INPUT_BOOST
static gpufreq_input_boost_notify g_input_boost_notify = NULL;
#endif
...
* internal function
********************/
static void copy_row_(struct mtk_gpufreq_info *out,
					  struct mtk_gpufreq_info const *in)
{
...
/* will call kzalloc() */
/* num must > 0 */
static s32 alloc_table_(struct freq_table **out,
						u32 const num)
{
...
static s32 init_table_(struct freq_table *out,
					   struct mtk_gpufreq_info const *freqs,
					   u32 const num)
{
	s32 i = 0;
...
static void swap_2_table_(struct freq_table *new_table,
						  struct freq_table *new_backup,
						  struct freq_table **old,
...
static void get_row_by_load_(u32 const load,
							 struct mtk_gpufreq_info *out_row)
{
	s32 i = 0;
	u32 input_index   = 0;
	u32 current_index = 0;
	...
	max_index = calc_max_index_(thermal_switch, thermal_max_index,
								volt_switch,    volt_max_index,
								volume_switch,  volume_max_index);
...
/* will set current_index */
static void get_row_by_index_(u32 index,
							  struct mtk_gpufreq_info *out_row)
{
	u32 size          = 0;
	bool thermal_switch   = 0;
...
/* return limit_index  */
static u32 find_thermal_index_(struct freq_table const* table,
							   u32 const limited_power)
{
	int i = 0;
	u32 index = 0;
...
/* voltage upper bound */
u32 find_bottom_index_(struct freq_table const * table,
					   u32 const bottom_freq)
{
	u32 i = 0;
	struct mtk_gpufreq_info *rows = table->rows;
...
u32 find_ptpod_disable_index_(struct freq_table const * table,
							  u32 const freq)
{
	s32 i = 0;
	struct mtk_gpufreq_info *rows = table->rows;
...
{
#if GS_GPUFREQ_PATHB_MTCMOS
	enable_clock(MT_CG_G3D_MX, "GPUFREQ");
	enable_clock(MT_CG_G3D_TX, "GPUFREQ");
	enable_clock(MT_CG_G3D_DW, "GPUFREQ");
	enable_clock(MT_CG_G3D_FF, "GPUFREQ");
	enable_clock(MT_CG_DISP0_SMI_COMMON, "GPUFREQ");
...
{
#if GS_GPUFREQ_PATHB_MTCMOS
	disable_clock(MT_CG_DISP0_SMI_COMMON, "GPUFREQ");
	disable_clock(MT_CG_G3D_FF, "GPUFREQ");
	disable_clock(MT_CG_G3D_DW, "GPUFREQ");
	disable_clock(MT_CG_G3D_TX, "GPUFREQ");
	disable_clock(MT_CG_G3D_MX, "GPUFREQ");
...
unsigned int mt_gpufreq_update_volt(unsigned int pmic_volt[], unsigned int array_size)
{
	...
	if ((error&1) > 0) {
		print_error("[GPUFREQ] update volt : input size (%d) is different from current size (%d.\n", array_size, num_rows);
	}
...
void gs_gpufreq_get_cur_indices(u32 *bottom_index,
								u32 *limit_index,
								u32 *current_index)
{
...
s32 gs_gpufreq_get_state(void)  /* bool */
{
...
static void gs_gpufreq_clock_switch_(unsigned int freq_new)
{
...
#if defined(GS_GPUFREQ_BRING_UP) && !GS_GPUFREQ_PATHB_FREQ
	{
		...
		print_debug("@%s: freq_new = %d, mmpll_n_info = 0x%x\n", __func__, freq_new, mmpll_n_info);

		gs_writel(MMPLL_CON1, mmpll_n_info_chg | mmpll_pdiv | mmpll_n_info); /*set MMPLL_N_INFO_CHG, MMPLL_PDIV, MMPLL_N_INFO */
...
static ssize_t proc_debug_write_(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	print_error("[GPUFREQ][PROC][ERROR] debug write : should be 0 or 1\n");

	return count;
}

static ssize_t proc_power_switch_write_(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	s32 power_switch = 0;

	if (sscanf(buffer, "%d", &power_switch) == 1) {
		if (power_switch > 0) {
			gs_gpufreq_vgpu_sel_default();
			return count;
		} else {
	...
	print_error("[GPUFREQ][PROC][ERROR] debug write : should be -1 or 1\n");
 ...
 * set GPU DVFS stauts
 **********************/
static ssize_t proc_dvfs_state_write_(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	s32 enabled = 0;
...
static ssize_t proc_limited_power_write_(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	u32 power = 0;
	...
	print_error("[GPUFREQ][PROC][ERROR] limited power write : input maximum limited power\n");

	return count;
}

static ssize_t proc_bottom_freq_write_(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	u32 bottom_freq = 0;

	if (sscanf(buffer, "%u", &bottom_freq) == 1) {
		gs_gpufreq_bottom_speed(bottom_freq);
		return count;
	}

	print_error("[GPUFREQ][PROC][ERROR] bottom freq write : input bottom freq.\n");
...
static ssize_t proc_index_write_(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	print_error("[GPUFREQ][PROC][ERROR] index write : value\n");

	return count;
}


static ssize_t proc_freq_volt_write_(struct file *file, const char *buffer, size_t count, loff_t *data)
{
...
static ssize_t proc_ptpod_write_(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	print_error("[GPUFREQ][PROC][ERROR] ptpod write\n");

	return count;
}

static ssize_t proc_input_switch_write_(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(buffer, "%u %u %u %u", &touch_boost, &thermal, &low_battery_volt, &low_battery_volume) == 4) {
 ...
 **********************/
static ssize_t proc_mtcmos_state_write_(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	s32 enabled = 0;
...
{
	if (g_debug)
		seq_printf(s, "gpufreq debug enabled\n");
	else
		seq_printf(s, "gpufreq debug disabled\n");
...
static int proc_power_switch_show_(struct seq_file* s, void* v)
{
	...
	if (power_switch > 0) {
		seq_printf(s, "power_switch enabled\n");
	} else {
		seq_printf(s, "power_switch disabled\n");
	}
...
static int proc_dvfs_state_show_(struct seq_file* s, void* v)
{
	...
	if (dvfs_enable_count > 0) {
		seq_printf(s, "GPU DVFS enabled\n");
	} else {
		seq_printf(s, "GPU DVFS disabled\n");
	}
...
static int proc_var_dump_show_(struct seq_file* s, void* v)
{
	...
	seq_printf(s, "#### Callback register ####\n");
...
static int proc_ptpod_show_(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "usage : 0:disable, 1:enable 2:restore 10~11:fake value\n");
...
static int proc_input_switch_show_(struct seq_file* s, void* v)
{
	...
	seq_printf(s, "touch boost / thermal / low battery volt / low battery volume =\n %d %d %d %d\n",
			   touch_boost,
			   thermal,
			   low_battery_volt,
			   low_battery_volume);
...
static int proc_mtcmos_state_show_(struct seq_file* s, void* v)
{
	...
	if (mtcmos_enable_count > 0) {
		seq_printf(s, "GPU MTCMOS enabled\n");
	} else {
		seq_printf(s, "GPU MTCMOS disabled\n");
	}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Mindos Cheng");
MODULE_DESCRIPTION("GPUFREQ");
