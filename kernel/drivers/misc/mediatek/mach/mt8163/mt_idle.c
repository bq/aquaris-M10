...
extern bool clkmgr_idle_can_enter(unsigned int *condition_mask,
			unsigned int *block_mask, enum idle_mode mode);
...
static int idle_switch[NR_TYPES] = {
	DPIDLE_EN,		/* dpidle switch */
	SOIDLE_EN,		/* soidle switch */
	0,			/* mcidle switch */
	0,			/* slidle switch */
	1,			/* rgidle switch */
};
...
void soidle_after_wfi(int cpu)
{
	...
	soidle_cnt[cpu]++;
#if SODI_DVT_APxGPT
	/* pr_notice("soidle_cnt:%d",soidle_cnt[cpu]); */
#endif
}
...
#elif (MCDI_DVT_IPI)
/* localtimer_set_next_event(130000000); */
/* pr_notice("delay local timer next event"); */
#endif
}
...
static void go_to_rgidle(int cpu)
{
...
static void empty_function(void *info)
{
	...
	mdelay(1);
	pr_notice("core %x ipi received, WFI count %d, core IPI command count: %d\n",
			...
			struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < nr_cpu_ids; i++) {
		p += sprintf(p,
			...
			i, soidle_cnt[i], i, dpidle_cnt[i], i, mcidle_cnt[i],
