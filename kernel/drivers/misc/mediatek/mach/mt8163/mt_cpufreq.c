...
#define cpufreq_warn(fmt, args...)      \
    pr_warning(TAG, "[WARNING]"fmt, ##args)
...
struct mt_cpu_dvfs_ops {
    ...
    unsigned int (*get_cur_volt)(struct mt_cpu_dvfs *p);             /* return volt (mV * 1000) */
    int (*set_cur_volt)(struct mt_cpu_dvfs *p, unsigned int volt);   /* set volt (mv * 1000), return 0 (success), -1 (fail) */
};
...
static unsigned int get_cur_volt_extbuck(struct mt_cpu_dvfs *p)
{
	...
	ret_volt = sym827_buck_get_voltage();
...
static int set_cur_volt_extbuck(struct mt_cpu_dvfs *p, unsigned int volt) /* volt: vproc (mv*1000) */
{
	...
	sym827_buck_set_voltage(volt + SYM827_PRECISION - 1);
...
#define DORMANT_MODE_VOLT   80000
        ...
        cpufreq_frequency_table_target(policy, id_to_cpu_dvfs(id)->freq_tbl_for_cpufreq, policy->cur, CPUFREQ_RELATION_L, &opp_idx);
