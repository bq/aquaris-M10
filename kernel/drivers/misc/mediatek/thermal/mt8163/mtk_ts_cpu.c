...
static int GPU_TS_MCU1_T=0,GPU_TS_MCU2_T=0;
static int SOC_TS_MCU1_T=0,SOC_TS_MCU2_T=0,SOC_TS_MCU3_T=0;

static int CPU_TS_MCU1_R=0,CPU_TS_MCU2_R=0;
static int GPU_TS_MCU1_R=0,GPU_TS_MCU2_R=0;
static int SOC_TS_MCU1_R=0,SOC_TS_MCU2_R=0,SOC_TS_MCU3_R=0;
...
static void set_adaptive_cpu_power_limit(unsigned int limit)
{
    ...
    if (prv_adp_cpu_pwr_lim != adaptive_cpu_power_limit)
    {
        tscpu_printk("set_adaptive_cpu_power_limit %d, T=%d,%d,%d,%d,%d,%d\n", (final_limit != 0x7FFFFFFF) ? final_limit : 0,
                     CPU_TS_MCU1_T, CPU_TS_MCU2_T, GPU_TS_MCU2_T, SOC_TS_MCU1_T, SOC_TS_MCU2_T, SOC_TS_MCU3_T);
...
//    temp2 = DRV_Reg32(0xF0206188);
	...
	tscpu_printk("[calibration] devinfo idx7=0x%x, idx8=0x%x, idx9=0x%x\n", temp1, temp0,temp2);
...
static void read_each_bank_TS(thermal_bank_name bank_num)
{
	...
	switch(bank_num){
	    ...
	    case THERMAL_BANK1:
			...
			GPU_TS_MCU2_T  = read_tc_raw_and_temp((volatile u32 *)TEMPMSR1,THERMAL_SENSOR2,&GPU_TS_MCU2_R);
	        break;
	    case THERMAL_BANK2:
            //Bank2 : SOC (TS_MCU3)
			SOC_TS_MCU1_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR0,THERMAL_SENSOR1,&SOC_TS_MCU1_R);
...
#ifdef CONFIG_BATTERY_THERMAL_CHECK_POWER_ON
       //printk("xkpeng cpu temperature = %d\n",curr_temp);
       if(curr_temp >= 95000){
...
static int _adaptive_power(long prev_temp, long curr_temp, unsigned int gpu_loading)
{
	...
	if (cl_dev_adp_cpu_state_active == 1)
	{
		...
		tscpu_printk("get %s Tp %d, Tc %d, Pt %d\n", __func__, prev_temp, curr_temp, total_power);
		return P_adaptive(total_power, gpu_loading);
	}
	else
	{
		...
		{
			triggered = 0;
			tscpu_printk("stop %s Tp %d, Tc %d, Pt %d\n", __func__, prev_temp, curr_temp, total_power);
...
//	int bank3_T;
    ...
    for(;;)
	{
	    ...
	    if ((temp_tc_mid_trip > -275000) && (max_temp >= (temp_tc_mid_trip-5000))) // trip_temp[1] should be shutdown point...
        {
            ...
            mt_cpufreq_thermal_protect(300); /*min CPU*/
