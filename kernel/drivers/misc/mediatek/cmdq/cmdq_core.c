...
int cmdqCorePrintStatusSeq(struct seq_file *m, void *v)
{
	...
	if (gCmdqFirstError.cmdqCount > 0) {
		...
		seq_printf(m,
			   "kernel time:[%5llu.%06lu], UTC time:[%04ld-%02d-%02d %02d:%02d:%02d.%06ld],"
			   " Pid: %d, Name: %s\n",
			   saveTimeSec, rem_nsec / 1000, (nowTM.tm_year + 1900), (nowTM.tm_mon + 1),
			   nowTM.tm_mday, nowTM.tm_hour, nowTM.tm_min, nowTM.tm_sec,
			   gCmdqFirstError.savetv.tv_usec, gCmdqFirstError.callerPid,
...
#undef DECLARE_CMDQ_EVENT
#define DECLARE_CMDQ_EVENT(name, val) \
	CMDQ_REG_SET32(CMDQ_SYNC_TOKEN_UPD, (CMDQ_SYNC_TOKEN_MAX & cmdq_core_get_event_value(name)));
#include "cmdq_event.h"
#undef DECLARE_CMDQ_EVENT
...
DEBUG_STATIC int32_t cmdq_core_copy_buffer_impl(void *dst, void *src, const uint32_t size,
						       ...
						       cmdqCommandStruct *pCommandDesc)
{
	...
	const uint32_t reservedAppendBlockInstrSize = 1 * CMDQ_INST_SIZE;
	...
	if (pTask->regCount) {
		...
		for (i = 0; i < pTask->regCount; ++i) {
			...
			subsysCode = cmdq_subsys_from_phys_addr(physAddr);
			if (CMDQ_SPECIAL_SUBSYS_ADDR == subsysCode) {
									     ...
									     const bool isSecure)
{
	...
	do {
		...
		if (preferSecurePath == isSecure) {
			...
			CMDQ_MSG
			    ("THREAD: is sec(%d, eng:0x%llx) as first waiting task(0x%p, eng:0x%llx),"
			     " start thread dispatch.\n",
			     isSecure, engineFlag, pFirstWaitingTask,
						     ...
						     const bool isSecure)
{
	...
	do {
		...
		thread = cmdq_core_get_thread_ID_from_scenario_and_secure(scenario, isSecure);
	...
	do {
		...
		if (cmdq_core_get_pc(pTask, thread, insts)) {
			...
			case CMDQ_CODE_WFE:
				...
				module = cmdq_core_module_from_event_id(eventENUM);
				break;
			case CMDQ_CODE_READ:
			case CMDQ_CODE_MOVE:
			case CMDQ_CODE_JUMP:
		...
		case CMDQ_CODE_MOVE:
			if (1 & (pCommand[1] >> 23)) {
				if (CMDQ_SPECIAL_SUBSYS_ADDR ==
				    cmdq_subsys_from_phys_addr(pCommand[0])) {
...
DEBUG_STATIC int32_t cmdq_core_insert_task_from_thread_array_by_cookie(TaskStruct *pTask,
								       ThreadStruct *pThread,
...
DEBUG_STATIC int32_t cmdq_core_handle_wait_task_result_secure_impl(TaskStruct *pTask,
								   int32_t thread,
								   const int32_t waitQ)
{
	int32_t i;
...
DEBUG_STATIC int32_t cmdq_core_handle_wait_task_result_impl(TaskStruct *pTask, int32_t thread,
							    const int32_t waitQ)
{
	int32_t status;
	int32_t index;
...
DEBUG_STATIC int32_t cmdq_core_exec_task_async_secure_impl(TaskStruct *pTask, int32_t thread)
{
	...
	CMDQ_MSG
	    ("-->EXEC: task 0x%p on thread %d begin, VABase: 0x%p, MVABase: %pa, Size: %d,"
	     " bufferSize: %d, scenario:%d, flag:0x%llx\n",
...
DEBUG_STATIC int32_t cmdq_core_exec_task_async_impl(TaskStruct *pTask, int32_t thread)
{
	...
	int threadPrio = 0;
	uint32_t EndAddr;
	/* for non-loop command, we shift END before JUMP */
	int32_t shiftEnd = CMDQ_INST_SIZE;
	...
	if (pThread->taskCount <= 0) {
		...
		/* due to we do not suspend HW thread, not set instruction cycles, too */
		/*CMDQ_REG_SET32(CMDQ_THR_INST_CYCLES(thread),
		   cmdq_core_get_task_timeout_cycle(pThread)); */
		...
		/* PC = END - 8, Wait is executed */
		/* PC = END - 0, EOC is executed */
		if ((CMDQ_AREG_TO_PHYS(CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread))) ==
			...
			if (loop < 0) {
				...
				CMDQ_AEE("CMDQ", "Invalid task count(%d) in thread %d for reorder,"
				...
				for (prev = (cookie % CMDQ_MAX_TASK_IN_THREAD); loop > 0; loop--) {
					...
					while ((NULL == pPrev) && (loop > 0)) {
						CMDQ_LOG
						    ("pPrev is NULL, prev:%d, loop:%d, index:%d\n",
						     prev, loop, index);
						prev = prev - 1;
						if (prev < 0) {
							prev = CMDQ_MAX_TASK_IN_THREAD - 1;
					...
					if (NULL != pPrev) {
						if (loop > 1) {
							if (pPrev->priority < pTask->priority) {
								CMDQ_LOG
								    ("Switch prev(%d, 0x%p) and current(%d, 0x%p)"
								     " order\n", prev, pPrev, index,
								     pTask);
								...
								CMDQ_VERBOSE
								    ("EXEC: modify jump to %pa, line:%d\n",
								     &(pPrev->MVABase), __LINE__);

								if (pLast == pTask) {
									CMDQ_LOG
									    ("update pLast from 0x%p to 0x%p\n",
									     pTask, pPrev);
									pLast = pPrev;
								}
							} else {
							...
							CMDQ_MSG
							    ("Set current(%d) order for the new task, line:%d\n",
							     index, __LINE__);
...
int32_t cmdqCoreAutoReleaseTask(TaskStruct *pTask)
{
	...
	if (CMDQ_INVALID_THREAD != pTask->thread) {
		...
		threadNo =
		    cmdq_core_get_thread_ID_from_scenario_and_secure(pTask->scenario, isSecure);
...
int32_t cmdq_core_save_first_dump(const char *string, ...)
{
	...
	if (logLen > 1) {
		if ((gCmdqFirstError.cmdqCount + logLen) < CMDQ_MAX_FIRSTERROR) {
			char *pBuffer = gCmdqFirstError.cmdqString + gCmdqFirstError.cmdqCount;
			gCmdqFirstError.cmdqCount += vsnprintf(pBuffer, logLen, string, argptr);
		} else {
			gCmdqFirstError.flag = false;
