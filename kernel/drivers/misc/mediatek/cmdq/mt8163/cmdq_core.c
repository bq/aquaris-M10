									     ...
									     const bool isSecure)
{
	...
	do {
		...
		CMDQ_LOG("THREAD: is not the first waiting task(%p), yield.\n", pFirstWaitingTask);
		status = -EFAULT;
	} while (0);
						     ...
						     bool forceLog, const bool isSecure)
{
	...
	do {
		...
		if (CMDQ_INVALID_THREAD != thread &&
			...
			if (forceLog) {
				CMDQ_ERR("THREAD: thread %d task count = %d full\n",
...
void cmdq_core_dump_instructions(uint64_t *pInstrBuffer, uint32_t bufferSize)
{
	...
	while (pBuffer <= (uint64_t *) ((uint8_t *) (pInstrBuffer) + bufferSize)) {
		...
		CMDQ_MSG("index:%05d,INST:0x%016llx    ----->      %s\n", i, *pBuffer, textBuffer);
		pBuffer++;
	}
}
		...
		if ((CMDQ_AREG_TO_PHYS(CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread))) ==
			...
			for (index = (cookie % CMDQ_MAX_TASK_IN_THREAD); loop > 0;
				...
				while ((NULL == pPrev) && (count > 0)) {
					CMDQ_LOG
					    ("pPrev is NULL, prev:%d, loop:%d, index:%d, count:%d\n",
