...
int ddp_path_init()
{
    ...
    sel_in_map[4].reg = DISP_REG_CONFIG_DPI1_SEL_IN; // WDMA_SEL
            ...
            if(module_list[i] == sel_in_map[j].id)
            {
                ...
                for(k = 0 ; k < 4 ; k++)
                {
                    ...
                    if(sel_in_map[j].id_bit_map[k] == module_list[step])
                    {
                        ...
                        {
                            DISP_REG_SET_FIELD(handle, DPI0_SEL_IN_FLD_DPI0_SEL_IN, sel_in_map[j].reg, (kal_uint16)k);
                        }
                        else if (module_list[i] == DISP_MODULE_DPI1)
                        {
                            DISP_REG_SET_FIELD(handle, DPI0_SEL_IN_FLD_DPI1_SEL_IN, sel_in_map[j].reg, (kal_uint16)k);
                        }
                        else
                        {
