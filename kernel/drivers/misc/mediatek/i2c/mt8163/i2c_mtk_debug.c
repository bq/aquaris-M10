			  ...
			  size_t count)
{
	...
	scanf_ret = sscanf
	    (buf, "%d %x %d %d %d %s", &bus_id, &address, &operation, &wr_number, &rd_number,
	     data_buffer);
	if (scanf_ret) {
		printk(KERN_ALERT "bus_id:%d,address:%x,operation:0x%x\n",
		...
		if ((address != 0) && (operation <= 2)) {
			...
			if (operation == 1) {
				if ((length >> 1) != wr_number)
					printk("Error lenth of data number = %d,length = %d\n",
					       wr_number, length >> 1);
			...
			if (ret >= 0) {
				...
				if (operation == 2) {
					...
					printk("Actual return Value:%d %s\n", ret, data_buffer);
				} else if (operation == 0) {
					hex2string(vir_addr_rd, tmpbuffer, rd_number);
					sprintf(data_buffer, "1 %s", tmpbuffer);
					printk("Actual return Value:%d %s\n", ret, data_buffer);
...
static int i2c_common_probe(struct platform_device *pdev)
{
	...
	printk("i2c_common device probe ret = %d\n", ret);
	return ret;
}
