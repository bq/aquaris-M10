...
static long __mt_sched_addaffinity(struct task_struct *p, const struct cpumask *new_mask)
{
        ...
        printk(KERN_ALERT "MT_SCHED: %s pid[%d]\n", __FUNCTION__, p->pid);
#endif
	struct mt_task *new;
	struct mt_task *tmp, *tmp2;
	unsigned long irq_flags;
...
#if MT_SCHED_AFFININTY_DEBUG
		printk(KERN_ALERT "MT_SCHED: %s [IOCTL_SETAFFINITY]\n", __FUNCTION__);
#endif
...
#if MT_SCHED_AFFININTY_DEBUG
		printk(KERN_ALERT "MT_SCHED: %s [IOCTL_GETAFFINITY]\n", __FUNCTION__);
#endif
...
#if MT_SCHED_AFFININTY_DEBUG
	        printk(KERN_ALERT "MT_SCHED: %s [IOCTL_EXITAFFINITY]\n", __FUNCTION__);
#endif
