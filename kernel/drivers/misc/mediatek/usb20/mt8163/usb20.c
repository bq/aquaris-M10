...
static void mt_usb_enable(struct musb *musb)
{
    ...
    if (!mtk_usb_power) {
        if (down_interruptible(&power_clock_lock))
            printk("[MUSB]USB20 %s: busy, Couldn't get power_clock_lock\n", __func__);
...
void musb_sync_with_bat(struct musb *musb,int usb_state)
{
	...
	printk("[Flow][USB20]%s:%d musb_sync_with_bat call her, state=%d\n", __FUNCTION__, __LINE__,usb_state);
...
static int usb_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("****************before bank 0x00*************************\n");
	printk("0x00~0x0F 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x \n",
	...
	printk("[MUSB]addr: 0x05, value: %x\n", USB_PHY_Read_Register8(0x05));
	printk("[MUSB]addr: 0x18, value: %x\n", USB_PHY_Read_Register8(0x18));
	printk("*****************after **********************************\n");	
	...
	printk("[MUSB]addr: 0xFF, value: %x\n", USB_PHY_Read_Register8(0xFF));
	printk("[MUSB]addr: 0x05, value: %x\n", USB_PHY_Read_Register8(0x05));
	printk("[USB]addr: 0x18, value: %x\n", USB_PHY_Read_Register8(0x18));
	printk("****************before bank 0x10*************************\n");
	...
	printk("[MUSB]addr: 0x6A, value: %x\n", USB_PHY_Read_Register8(0x6A));
	printk("[MUSB]addr: 0x68, value: %x\n", USB_PHY_Read_Register8(0x68));
    printk("[MUSB]addr: 0x6C, value: %x\n", USB_PHY_Read_Register8(0x6C));
	printk("[MUSB]addr: 0x6D, value: %x\n", USB_PHY_Read_Register8(0x6D));
