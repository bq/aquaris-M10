...
kal_uint32 PMIC_IMM_RequestAuxadcChannel(upmu_adc_chl_list_enum dwChannel)
{
	...
	kal_uint32 ret = 0;
		...
		default:
		        pr_info("[AUXADC] Invalid channel value(%d)\n", dwChannel);
	...
	*/
	...
	do
	{
		...
		while( pmic_is_auxadc_ready(channel_num, chip_num, user_num) != 1 ) {
			...
			{
			pr_info("[IMM_GetOneChannelValue_PMIC] (%d) Time out!\n", dwChannel);
			break;
			}            
		}
	...
	case AUX_ADCVIN0_GPS:
		...
		break;
	case AUX_CH0_6311:
		//pr_info("[AUX_CH0_6311]");
		r_val_temp = 1;           
		adc_result = (adc_result_temp*r_val_temp*VOLTAGE_FULL_RANGE_6311)/ADC_PRECISE_6311;
		break;
	case AUX_CH1_6311:
		//pr_info("[AUX_CH1_6311]");
		r_val_temp = 1;       
		adc_result = (adc_result_temp*r_val_temp*VOLTAGE_FULL_RANGE_6311)/ADC_PRECISE_6311;
		break;
	default:
	    pr_info("[AUXADC] Invalid channel value(%d,%d)\n", dwChannel, trimd);
	    wake_unlock(&pmicAuxadc_irq_lock);
	    return -1;
	}
	if (Enable_BATDRV_LOG == 2) {
		pr_info("outputdata=%d, transfer data=%d, r_val=%d.\n", 
