...
{
    pr_debug("[show_fan5405_access] 0x%x\n", g_reg_value_fan5405);
...
static ssize_t store_fan5405_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_debug("[store_fan5405_access] \n");
    
    if(buf != NULL && size != 0)
    {
        pr_debug("[store_fan5405_access] buf is %s and size is %zu \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);
        
        if(size > 3)
        {        
            reg_value = simple_strtoul((pvalue+1),NULL,16);        
            pr_debug("[store_fan5405_access] write fan5405 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        {    
            ...
            pr_debug("[store_fan5405_access] read fan5405 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_fan5405);
            pr_debug("[store_fan5405_access] Please use \"cat fan5405_access\" to get value\r\n");
        }        
    }    
    return size;
}
...
{    
    ...
    pr_debug("[fan5405_init] init start\n");
    
    i2c_register_board_info(FAN5405_BUSNUM, &i2c_fan5405, 1);

    if(i2c_add_driver(&fan5405_driver)!=0)
    {
        pr_debug("[fan5405_init] failed to register fan5405 i2c driver.\n");
    }
    else
    {
        pr_debug("[fan5405_init] Success to register fan5405 i2c driver.\n");
    }

    // fan5405 user space access interface
    ret = platform_device_register(&fan5405_user_space_device);
    if (ret) {
        pr_debug("****[fan5405_init] Unable to device register(%d)\n", ret);
        return ret;
    }    
    ret = platform_driver_register(&fan5405_user_space_driver);
    if (ret) {
        pr_debug("****[fan5405_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
