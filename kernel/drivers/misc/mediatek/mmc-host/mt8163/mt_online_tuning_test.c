static int mt_ot_stress_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 *error, u8 rw)
{
    ...
    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Enter %s\n", __func__);
    ...
    for (i=0; i<2000; i++) {
		...
		if (CRCStatus_CMD != goldenCRC) {
		    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]CMD CRC error, CRC status: 0x%x\n", CRCStatus_CMD);
		    *error = 1;
		}
		...
		if (cmd->resp[0] != goldenCRC || 
		    ...
		    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]DAT CRC error\n");
		    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]DAT0 CRC status: 0x%x\n", cmd->resp[0]);
		    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]DAT1 CRC status: 0x%x\n", cmd->resp[1]);
		    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]DAT2 CRC status: 0x%x\n", cmd->resp[2]);
		    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]DAT3 CRC status: 0x%x\n", cmd->resp[3]);
		    *error = 1;
		}
    }
...
static int mt_ot_tmo_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 *error, u8 rw)
{
	...
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] Enter mt_ot_tmo_test\n", rw);
	if(ot_do_command(host, rawcmd, rawarg, &intsts))
	{
		*error = 1;
		//return -1;
	...
	if(ACMDTMO)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] ACMD TMO\n", rw);
		*error = 1;
	}
	sdr_get_field(MSDC_INT, MSDC_INT_DATTMO, DATTMO);	// Get DAT TMO flag
	if(DATTMO)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] DAT TMO\n", rw);
		*error = 1;
	}
	...
	if(ACMDTMO | DATTMO)
	{
		...
		if(ACMD53Done == 1)
		{
			MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] AUTOCMD53_DONE status error, AUTOCMD53_DONE should NOT be set to 1 when TMO happened\n", rw);
			*error = 1;
		}
	}

	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] Leave mt_ot_tmo_test\n", rw);
...
static int mt_ot_gearRegister_test(struct msdc_host *host, u8 rw, u32 eco_ver, u8 *error)
{
	...
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Enter mt_ot_otRegister_test\n");
    ...
    for(cmdrdly = 0; cmdrdly < 32; cmdrdly++)
    {
		...
		if(cmdrdly < 32)
		{
			...
			if(tmpdly != cmdrdly)
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] PAD_CMD_RXDLY write fail, write = %d, read = %d\n", rw, cmdrdly, tmpdly);
				*error = 1;
			}
		}
		else
		{
			sdr_get_field(MSDC_PAD_TUNE, MSDC_PAD_TUNE_CMDRDLY, tmpdly);	// Get cmd delay
			MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmdrdly = 32, PAD_CMD_RXDLY value = %d\n", rw, tmpdly);
		}
    }
	...
	for(; dat0rddly < 32; dat0rddly++)
	{
		...
		if(dat0rddly < 32)
		{
			...
			if(tmpdly != rxdly0)
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] DAT_RD_DLY0 write fail, write = %d, read = %d, cmddly = %d\n", rw, rxdly0, tmpdly, cmdrdly);
				*error = 1;
			}
		}
		else
		{
			tmpdly = sdr_read32(MSDC_DAT_RDDLY0);			// Read dat delay
			MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] dat0rddly = 32, DAT_RD_DLY0 value = %d\n", rw, tmpdly);
		}
	}
	...
	for(; dat1rddly < 32; dat1rddly++)
	{
		...
		{
			tmpdly = sdr_read32(MSDC_DAT_RDDLY0);			// Read dat delay
			MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] dat1rddly = 32, DAT_RD_DLY0 value = %d\n", rw, tmpdly);
		}
	}
	...
	for(; dat2rddly < 32; dat2rddly++)
	{
		...
		{
			tmpdly = sdr_read32(MSDC_DAT_RDDLY0);			// Read dat delay
			MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] dat2rddly = 32, DAT_RD_DLY0 value = %d\n", rw, tmpdly);
		}
	}
	...
	for(; dat3rddly < 32; dat3rddly++)
	{
		...
		{
			tmpdly = sdr_read32(MSDC_DAT_RDDLY0);			// Read dat delay
			MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] dat3rddly = 32, DAT_RD_DLY0 value = %d\n", rw, tmpdly);
		}
	}
	...
	for(gear_window_size = 0; gear_window_size < 16; gear_window_size++)
	{
		...
		{
			MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] SDIO_TUNE_WIND write fail, write = %d, read = %d, cmddly = %d, datdly = %d\n", rw, gear_window_size, tmpdly, cmdrdly, rxdly0);
			*error = 1;
		}
	}

	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Leave mt_ot_otRegister_test\n");
...
static int mt_ot_outbound_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 i_rw, u32 eco_ver, u8 *error)
{
	...
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Enter mt_ot_outbound_test\n");
	if(cmd == NULL)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]host->cmd is NULL\n");
		*error = 1;
		return -1;
	}
	...
	for(i=0; i<20; i++)
	{
		...
		if(ot_do_command(host, rawcmd, rawarg, &intsts))
		{
		    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n", i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i], dat3rddly[i], gear_window_size);
			*error = 1;
			return -1;
		}
		...
		if(cmdrdly[i] > 31 || dat0rddly[i] > 31 || dat1rddly[i] > 31 || dat2rddly[i] > 31 || dat3rddly[i] > 31)
		{
			if(GearOutBound == 0)
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x, GearOutBound = %d\n", intsts, GearOutBound);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] GEAR_OUT_BOUND status error, GEAR_OUT_BOUND should be set to 1 when gear is out of bound\n", i_rw);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n", i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i], dat3rddly[i], gear_window_size);
				*error = 1;
			}
		}
				...
				dat3rddly[i] + gear_window_size > 31 || dat3rddly[i] < gear_window_size )
		{
			...
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x, GearOutBound = %d\n", intsts, GearOutBound);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] GEAR_OUT_BOUND status error, GEAR_OUT_BOUND should be set to 1 when gear +/- gear window is out of bound\n", i_rw);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n", i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i], dat3rddly[i], gear_window_size);
				*error = 1;
			}
		}
		else
		{
			...
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x, GearOutBound = %d\n", intsts, GearOutBound);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] GEAR_OUT_BOUND status error, GEAR_OUT_BOUND should be set to 0 when gear is in bound\n", i_rw);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n", i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i], dat3rddly[i], gear_window_size);
				*error = 1;
			}
		}
	...
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Leave mt_ot_outbound_test\n");
...
static int mt_ot_crcerr_acmd53_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 rw, u32 eco_ver, u8 *error)
{
	...
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Enter %s\n", __func__);
	//MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]goldenCRC = 0x%x\n", goldenCRC);
	...
	for (i=0; i<20; i++) {
		...
		//MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x\n", intsts);
		...
		//MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]MASK_ACMD53_CRC_ERR_INTR = 0x%x, ACMD53_FAIL_ONE_SHOT = 0x%x\n", temp0, temp1);
		...
		//MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]D_DLYLINE_SEL = 0x%x, R_D_SMPL_SEL = 0x%x, W_D_SMPL_SEL = 0x%x\n", temp0, temp1, temp2);
		...
		if(CRCStatus_CMD != goldenCRC)
		{
			...
			if(CRCErr == 0)
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] SD_AUTOCMD_RESP_CRCERR status error, SD_AUTOCMD_RESP_CRCERR should be set to 1 when CRC error happened\n", rw);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n", rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x\n", intsts);
				*error = 1;
			}
			#endif
			if(ACMD53Err == 0)
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] AUTOCMD53_FAIL status error, AUTOCMD53_FAIL should be set to 1 when CRC error happened\n", rw);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n", rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x\n", intsts);
				*error = 1;
			}
		...
		{
			//MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] AUTOCMD53 CMD CRC is 0\n", rw);
			//MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmd crc status = %d, gear window size = %d, cmddly = %d, datdly = %d\n", rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
			...
			if(CRCErr)
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] SD_AUTOCMD_RESP_CRCERR status error, SD_AUTOCMD_RESP_CRCERR should be set to 0 when N0 CRC error\n", rw);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n", rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x\n", intsts);
				*error = 1;
			}
			if(ACMD53Err)
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] AUTOCMD53_FAIL status error, AUTOCMD53_FAIL should be set to 0 when No CRC error\n", rw);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n", rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x\n", intsts);
				*error = 1;
			}
		   ...
		   cmd->resp[3] != goldenCRC )
		{
			...
			if(CRCErr == 0)
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] SD_DATA_CRCERR status error, SD_DATA_CRCERR should be set to 1 when CRC error happened\n", rw);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] dat0 crc status = 0x%x, dat1 crc status = 0x%x, dat2 crc status = 0x%x, dat3 crc status = 0x%x\n", rw, cmd->resp[0], cmd->resp[1], cmd->resp[2], cmd->resp[3]);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] gear window size = %d, cmddly = %d, datdly = %d\n", rw, gear_window_size, cmdrdly[i], rxdly0);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x\n", intsts);
				*error = 1;
			}
			if(ACMD53Err == 0)
		...
		{
			//MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] AUTOCMD53 DAT CRC is 0\n", rw);
			//MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] cmd crc status = %d, gear window size = %d, cmddly = %d, datdly = %d\n", rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
			//sdr_get_field(MSDC_INT, MSDC_INT_DATCRCERR, CRCErr);	// Get dat crc err
			//sdr_get_field(MSDC_INT, MSDC_INT_ACMD53_FAIL, ACMD53Err);	// Get ACMD53 error flag
			CRCErr = (intsts & MSDC_INT_DATCRCERR) >> 15;	// Get dat crc err
			...
			if(CRCErr)
			{
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] SD_DATA_CRCERR status error, SD_DATA_CRCERR should be set to 0 when No CRC error\n", rw);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] dat0 crc status = 0x%x, dat1 crc status = 0x%x, dat2 crc status = 0x%x, dat3 crc status = 0x%x\n", rw, cmd->resp[0], cmd->resp[1], cmd->resp[2], cmd->resp[3]);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] gear window size = %d, cmddly = %d, datdly = %d\n", rw, gear_window_size, cmdrdly[i], rxdly0);
				MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x\n", intsts);
				*error = 1;
		...
		if(ACMD53Done == 0)
		{
			MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] AUTOCMD53_DONE status error, AUTOCMD53_DONE should be set to 1 at the end of training blocks\n", rw);
			MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%d] gear window size = %d, cmddly = %d, datdly = %d\n", rw, gear_window_size, cmdrdly[i], rxdly0);
			MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]intsts = 0x%x\n", intsts);
			*error = 1;
		}
	...
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Leave %s\n", __func__);
...
static int mt_ot_composite_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 *error,  char *xferMode, u8 rw, u32 eco_ver)
{
	...
	if(nRet || *error)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] TMO test(%d) fail\n", xferMode, rw);
		goto result;
	}

	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] TMO test(%d) pass\n", xferMode, rw);
	...
	if(nRet || *error)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] gear register test fail\n", xferMode);
		goto result;
	}

	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] gear register test pass\n", xferMode);
#endif
#if 1
	/*======Gear out bound test======*/
	...
	if(nRet || *error)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] gear out bound test(%d) fail\n", xferMode, rw);
		goto result;
	}

	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] gear out bound test(%d) pass\n", xferMode, rw);
#endif
#if 1
	...
	if(nRet || *error)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] CRC Err and ACMD53 test(%d) fail\n", xferMode, rw);
		goto result;
	}

	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] CRC Err and ACMD53 test(%d) pass\n", xferMode, rw);
#endif
#if 1
	...
	if(nRet || *error)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] stress test(%d) fail\n", xferMode, rw);
		goto result;
	}

	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] stress test(%d) pass\n", xferMode, rw);
#endif
result:
	if(*error == 0)
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] Online tuning composite test(%d) Pass!!!\n", xferMode, rw);
	else
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] Online tuning composite test(%d) Fail!!!\n", xferMode, rw);
...
int mt_msdc_online_tuning_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 rw)
{
	...
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Enter online tuning test \n");
    ...
    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]default cmd delay = %d\n", default_cmdrdly);
    
    default_rxdly0 = sdr_read32(MSDC_DAT_RDDLY0);
    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]default DAT read delay line 0 = %x\n", default_rxdly0);
	...
	if(nRet || error)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]PIO test(%d) fail\n", rw);
		goto result;
	}

	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]PIO test pass\n");
	...
	if(nRet || error)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Basic DMA test(%d) fail\n", rw);
		goto result;
	}

	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Basic DMA test pass\n");
#endif
#if 1
	/*======Descriptor DMA test======*/
	msdc_dma_on();  /* enable DMA mode first!! */
	...
	if(nRet || error)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Descriptor DMA test(%d) fail\n", rw);
		goto result;
	}
	
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Descriptor DMA test pass\n");
#endif
#if 0
	/*======Enhanced DMA test======*/
	msdc_dma_on();  /* enable DMA mode first!! */
	...
	if(nRet || error)
	{
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Enhanced DMA test(%s) fail\n", rw == 0 ? "Read" : "Write");
		goto result;
	}
	
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Enhanced DMA test pass\n");
#endif	
...
result:
	if(error == 0)
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Online tuning test Pass!!!\n");
	else
		MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Online tuning test Fail!!!\n");
	...
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]Leave online tuning test \n");
...
static int msdc_ottest_proc_write(struct file *file, const char *buf, size_t count, loff_t *data)
{
	...
	MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT]ottest Write %s\n", mt_ot_cmd_buf);
	
	sscanf(mt_ot_cmd_buf, "%x", &id);

    host = mtk_msdc_host[id];
    
    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] Start Online Tuning DVT test \n", __func__);
    mt_msdc_online_tuning_test(host, 0, 0, 0);
    MSDC_DBG_PRINT(MSDC_TUNING, "[SDIO_DVT][%s] Finish Online Tuning DVT test \n", __func__);
...
int msdc_ottest_proc_init(void)
{
    ...
    if(prEntry)
    {
       MSDC_DBG_PRINT(MSDC_TUNING, "[%s]: successfully create /proc/msdc_ottest\n", __func__);
    }else{
       MSDC_DBG_PRINT(MSDC_TUNING, "[%s]: failed to create /proc/msdc_ottest\n", __func__);
    }
