...
int icm30628_kick_m0_interrupt(void)
{
	...
	do {		
		if(cnt > 0){
			printk("retry icm30628_kick_m0_interrupt() cnt=%d, length = %d, LP = %d\n", cnt, fifo_length, st->is_lp_enabled);
			msleep(10);
		}
	...
	if(fifo_length > 0){
		printk("icm30628_kick_m0_interrupt() error\n");
...
int icm30628_configure_fifo(void)
{
	...
	temp[0] = 0x00;
	ret |= invensense_reg_write(st, GARNET_FIFO_WM_INT_EN_B0, 1, temp);
	if (UNLIKELY(ret < 0)) {
		INV_ERR;
		return ret;
		}
	...
	temp[0] = 0xF8;
	ret |= invensense_reg_write(st, GARNET_FIFO_1_WM_B0, 1, temp);
	if (UNLIKELY(ret < 0)) {
		INV_ERR;
		return ret;
	}
	...
	temp[0] = 0xE1;
	ret |= invensense_reg_write(st, GARNET_FIFO_0_WM_B0, 1, temp);
	if (UNLIKELY(ret < 0)) {
		INV_ERR;
		return ret;
	}
...
int invensense_fifo_write(struct icm30628_state_t * st, int length, u8 * data)
{
	...
	// Read one more time. This is a workaround just to increase the delay to
	// satisfy the 10 usec delay needed between clearing LP_EN and writing FIFO.
	invensense_get_fifo_status(st, GARNET_FIFO_ID_CMDIN, &fifo_length);
	...
	switch(id){
		...
		case INV_PROXIMITY:
			...
			if(st->icm30628_status.is_sensor_in_batchmode[invensense_get_sensor_num(id)] == true
				...
				ps_report_interrupt_data(st->icm30628_status.proximity_current_data[0]);	
			}
			break;
		case INV_STEP_COUNTER:
				...
				case INV_CMD_LOAD:
					...
					if(what == FIFOPROTOCOL_LOAD_WHAT_CHECK){
						...
						if(who == FIFOPROTOCOL_LOAD_WHO_DMP3_FW){
							...
							st->dmp3_checked = true; // for testing
