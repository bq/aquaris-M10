...
irqreturn_t mt6605_dev_irq_handler(int irq, void *data);    /* NFC IRQ handler */
...
static int mt6605_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (node) {
		...
		pr_debug("<<<<<<mt6605>>>>>> nfc_irq=%d\n",nfc_irq);
		nfc_irq = 292;
		pr_debug("<<<<<<mt6605>>>>>> mt_gpio_to_irq=%d\n",mt_gpio_to_irq(26));
		*/
		nfc_irq = mt_gpio_to_irq(26);/*fix irq_of_parse_and_map return error irq num; GPIO26/EINT4*/
        client->irq = nfc_irq;
        /*mt_eint_set_polarity(nfc_irq, IRQF_TRIGGER_HIGH); */
        ret = request_irq(nfc_irq, mt6605_dev_irq_handler, IRQF_TRIGGER_HIGH, "IRQ_NFC-eint", NULL);
        ...
        if(ret) {
            ...
            pr_debug("%s : set EINT finished, nfc_irq=%d", __func__, nfc_irq);
            nfc_irq_count++;
	...
	if ((cmd & 0xFFFF) == 0xFE00) {
        ...
        if (node) {
			...
			*/
			nfc_irq = mt_gpio_to_irq(26);/*fix irq_of_parse_and_map return error irq num; GPIO26/EINT4*/
            //client->irq = nfc_irq;
			//mt_eint_set_polarity(4, IRQF_TRIGGER_HIGH);     
            ret = request_irq(nfc_irq, mt6605_dev_irq_handler, IRQF_TRIGGER_HIGH, "IRQ_NFC-eint", NULL);
    
            if( ret ) {
                pr_err("%s : EINT IRQ LINE NOT AVAILABLE\n", __func__);
            } else {
                pr_debug("%s : set EINT finished, nfc_irq=%d. \n", __func__, nfc_irq);
                nfc_irq_count++;
                mt6605_disable_irq(nfc_irq);                    
            }
        } else {
...
{ 
    ...
    for (retry = 0; retry < 10; retry++) {
        ...
        if (ret == send_bytes) {
            pr_debug("inform_nfc_vsim_change, send to mt6605 OK. retry %d. \n", retry);
            break;
        } else {
            pr_debug("inform_nfc_vsim_change, send to mt6605 fail. retry %d, ret %d. \n", retry, ret);
        }    
        /* sleep 2ms */
        msleep(2);
    }
