...
static int dt_scan_memory(unsigned long node, const char *uname, int depth, void *data)
{
	...
	spm_suspend_flag &= ~(SPM_DRAM_RANK1_ADDR_SEL0|SPM_DRAM_RANK1_ADDR_SEL1|SPM_DRAM_RANK1_ADDR_SEL2);
...
static wake_reason_t spm_output_wake_reason(struct wake_status *wakesta, struct pcm_desc *pcmdesc)
{
    ...
    #endif
    spm_crit2("mempll mode = %d, suspend dormant state = %d, md32_flag = 0x%x, md32_flag2 = %d\n", mt_get_clk_mem_sel(), spm_dormant_sta, md32_flag, md32_flag2);
