...
#define SPK_WARM_UP_TIME        (100) //unit is ms
    ...
    if (enable)
    {
        ...
        AW8736_MODE3;
#elif defined(CONFIG_AW8738_SPEAKER_AMP)
	AW8738_MODE5;
#else
...
#define TPA6130A_I2C_BUSNUM  2
#define I2C_ID_NAME "tpa6130a"
#define TPA6130A_ADDR 0x60
 ...
 * GLobal Variable
 *****************************************************************************/
static struct i2c_board_info __initdata tpa6130a_board_info = {I2C_BOARD_INFO(I2C_ID_NAME, TPA6130A_ADDR)};
...
static int tpa6130a_remove(struct i2c_client *client);
/*****************************************************************************
 * Data Structure
 *****************************************************************************/
...
static const struct i2c_device_id tpa6130a_id[] = {
	{ I2C_ID_NAME, 0 },
	{ }
};

static struct i2c_driver tpa6130a_iic_driver = {
	.id_table	= tpa6130a_id,
	.probe		= tpa6130a_probe,
	.remove		= tpa6130a_remove,
	.driver		= {
		.owner	= THIS_MODULE,
		.name	= "tpa6130a",
	},
...
static int tpa6130a_read_bytes(u8 addr, u8 *data)
{
	...
	if (ret < 0) {
		printk("tpa6130a read data fail !!\n");
		return ret;
	}
...
static int tpa6130a_remove(struct i2c_client *client)
{
	printk( "tpa6130a_remove\n");
	tpa6130a_i2c_client = NULL;
...
static int __init tpa6130a_iic_init(void)
{
   ...
   i2c_register_board_info(TPA6130A_I2C_BUSNUM, &tpa6130a_board_info, 1);
   printk( "tpa6130a_iic_init2\n");
   i2c_add_driver(&tpa6130a_iic_driver);
   printk( "tpa6130a_iic_init success\n");
   return 0;
}
...
#define HP_WARM_UP_TIME        (25) //unit is ms
    ...
    if (enable)
    {
        ...
        mt_set_gpio_out(GPIO_AUD_EXTHP_EN_PIN, GPIO_OUT_ZERO); // low disable
        udelay(1000);
        mt_set_gpio_dir(GPIO_AUD_EXTHP_EN_PIN, GPIO_DIR_OUT); // output

#ifdef CONFIG_TPA6130A_HP_AMP
        mt_set_gpio_out(GPIO_AUD_EXTHP_EN_PIN, GPIO_OUT_ONE); // high enable
        mdelay(10);
    ...
    else
    {
        ...
        //mt_set_gpio_mode(GPIO_AUD_EXTHP_EN_PIN, GPIO_MODE_00);
        mt_set_gpio_dir(GPIO_AUD_EXTHP_EN_PIN, GPIO_DIR_OUT); // output
        mt_set_gpio_out(GPIO_AUD_EXTHP_EN_PIN, GPIO_OUT_ZERO); // low disbale
        udelay(500);
