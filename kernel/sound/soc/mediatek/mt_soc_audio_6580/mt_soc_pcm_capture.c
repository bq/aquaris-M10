				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	PRINTK_AUD_UL1("mtk_capture_pcm_copy finish0, read_count:%lx, read_size:%lx, u4DataRemained:%x,
		u4DMAReadIdx:0x%x, u4WriteIdx:%x \r\n", (unsigned int)read_count, (unsigned int)read_size,
	...
	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		...
		PRINTK_AUD_UL1("mtk_capture_pcm_copy finish1, copy size:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x,
			u4DataRemained:%x \r\n", (unsigned int)read_size, Vul_Block->u4DMAReadIdx,
		...
		PRINTK_AUD_UL1("mtk_capture_pcm_copy finish3, copy size_2:%x, u4DMAReadIdx:0x%x,
			u4WriteIdx:0x%x u4DataRemained:%x \r\n", size_2, Vul_Block->u4DMAReadIdx,
			Vul_Block->u4WriteIdx, Vul_Block->u4DataRemained);
	}
